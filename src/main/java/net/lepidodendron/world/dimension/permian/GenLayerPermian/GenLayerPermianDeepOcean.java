...
public class GenLayerPermianDeepOcean extends GenLayer
{
    ...
    public GenLayerPermianDeepOcean(long seed, GenLayer genLayer)
    {
        super(seed);
        ...
        for (int i1 = 0; i1 < p_151626_4_; ++i1)
        {
            for (int j1 = 0; j1 < p_151626_3_; ++j1)
            {
                ...
                if (k1 == PERMIAN_OCEAN_SHORE_ID)
                {
                    ...
                    boolean flag = (
                        (
                        (l1 == PERMIAN_OCEAN_ID || l1 == PERMIAN_OCEAN_SHORE_ID)
                        && (i2 == PERMIAN_OCEAN_ID || i2 == PERMIAN_OCEAN_SHORE_ID)
                        && (j2 == PERMIAN_OCEAN_ID || j2 == PERMIAN_OCEAN_SHORE_ID)
                        && (k2 == PERMIAN_OCEAN_ID || k2 == PERMIAN_OCEAN_SHORE_ID)
                        )
                        || (l1 == PERMIAN_MOUNTAINS_ID
                            || i2 == PERMIAN_MOUNTAINS_ID
                            || j2 == PERMIAN_MOUNTAINS_ID
                            || k2 == PERMIAN_MOUNTAINS_ID
                        )
                    );
                    if (flag)
                    {
