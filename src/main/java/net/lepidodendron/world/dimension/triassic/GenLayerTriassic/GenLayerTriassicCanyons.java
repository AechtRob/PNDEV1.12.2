...
public class GenLayerTriassicCanyons extends GenLayer
{
    ...
    public GenLayerTriassicCanyons(long seed, GenLayer genLayer)
    {
        super(seed);
        ...
        for (int i = 0; i < areaHeight; ++i)
        {
            for (int j = 0; j < areaWidth; ++j)
            {
                ...
                if (isOcean(k))
                {
                    ...
                    {
                        aint1[j + i * areaWidth] = TRIASSIC_DESERT_CANYONS_ID;
                    }
                    else
                    {
