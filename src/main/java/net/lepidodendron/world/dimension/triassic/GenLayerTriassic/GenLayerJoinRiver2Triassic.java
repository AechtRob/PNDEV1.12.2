...
public class GenLayerJoinRiver2Triassic extends GenLayer
{
    ...
    public GenLayerJoinRiver2Triassic(long seed, GenLayer genLayer)
    {
        super(seed);
        ...
        for (int i = 0; i < areaHeight; ++i)
        {
            for (int j = 0; j < areaWidth; ++j)
            {
                ...
                if (!isRiver(k))
                {
                    ...
                    if (
                        (isRiver(l1) && isRiver(k2))
                        || (isRiver(j3) && isRiver(i4))
                    )
                    {
    ...
    private boolean isRiver(int biomeID) {
        if (biomeID == TRIASSIC_RIVER_ID || biomeID == TRIASSIC_RIVERBANK_ID) {
            return true;
        }
        return false;
    }
