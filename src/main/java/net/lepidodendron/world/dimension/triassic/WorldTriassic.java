...
package net.lepidodendron.world.dimension.triassic;
...
public class WorldTriassic extends ElementsLepidodendronMod.ModElement {
	public static int DIMID = LepidodendronConfig.dimTriassic;
	public static final boolean NETHER_TYPE = false;
	@GameRegistry.ObjectHolder("lepidodendron:triassic_portal")
	public static final BlockCustomPortal portal = null;
	public static DimensionType dtype;
	public WorldTriassic(ElementsLepidodendronMod instance) {
		super(instance, 1590);
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		if (DimensionManager.isDimensionRegistered(DIMID)) {
			...
			System.err.println("Prehistoric Nature loading error! Dimension ID for the Triassic dimension is already registered by another mod: " + DIMID);
			return;
		}
		dtype = DimensionType.register("triassic", "_triassic", DIMID, WorldProviderMod.class, false);
	...
	public static class WorldProviderMod extends WorldProvider {
		...
		public IChunkGenerator createChunkGenerator() {
			return new ChunkProviderTriassic(this.world, this.world.getSeed() - DIMID);
		}
		...
		public boolean doesXZShowFog(int par1, int par2) {
			...
			//	if ((biomeTriassic.getBiomeType() == EnumBiomeTypeTriassic.Wetlands
			//		|| biomeTriassic.getBiomeType() == EnumBiomeTypeTriassic.Glossopteris)
			//		&& world.isRaining()) {
			//		return true;
			//	}
			//}
					...
					for (int l7 = 1; l7 < 3; ++l7) {
						for (int k8 = -1; k8 < 3; ++k8) {
							...
							this.world.setBlockState(new BlockPos(k9, k10+1, k11),
									flag
											? BlockDicroidiumFPlanks.block.getDefaultState().getBlock().getDefaultState()
				...
				for (int l8 = 0; l8 < 4; ++l8) {
					for (int l9 = -1; l9 < 4; ++l9) {
						...
						this.world.setBlockState(new BlockPos(l10, l11+1, k12),
								flag1 ? BlockDicroidiumFPlanks.block.getDefaultState().getBlock().getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockCustomPortal extends BlockPortal {
		public BlockCustomPortal() {
			setHardness(-1.0F);
			setTranslationKey("pf_triassic_portal");
			setRegistryName("triassic_portal");
			setLightLevel(0F);
		}
			...
			if (!worldIn.isRemote && !entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn instanceof EntityPlayerMP) {
				...
				if (thePlayer.timeUntilPortal > 0) {
					...
					if (thePlayer.dimension == DIMID) {
						...
						ModTriggers.ENTER_TRIASSIC.trigger((EntityPlayerMP)thePlayer);
					}
				} else {
					thePlayer.timeUntilPortal = 10;
					ReflectionHelper.setPrivateValue(EntityPlayerMP.class, thePlayer, true, "invulnerableDimensionChange", "field_184851_cj");
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
							|| this.world.getBlockState(blockpos.down()).getBlock() != BlockDicroidiumFPlanks.block.getDefaultState().getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == BlockDicroidiumFPlanks.block.getDefaultState().getBlock() ? i : 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != BlockDicroidiumFPlanks.block.getDefaultState().getBlock()) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height))
							.getBlock() != BlockDicroidiumFPlanks.block.getDefaultState().getBlock()) {
						this.height = 0;
						break;
					}
				}
