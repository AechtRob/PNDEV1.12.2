    ...
    public boolean generate(World worldIn, Random rand, BlockPos position, boolean needsWater, int minHeight, int maxHeight)
    {
        ...
        for (int i = 0; i < 18; ++i) {
            ...
            if (!needsWater) {
                ...
                if (blockpos.getY() >= Functions.getAdjustedSeaLevel(worldIn, blockpos) - 4 && worldIn.isAirBlock(blockpos) && (!worldIn.provider.isNether() || blockpos.getY() < 254) && BlockCycadeoideaSapling.block.canPlaceBlockAt(worldIn, blockpos)
