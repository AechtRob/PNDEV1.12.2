...
public class WorldGenSingleSponge extends WorldGenerator
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position, int minWaterDepth, int maxWaterDepth, int minY, int maxY, @Nullable Block spongeIn)
    {
        ...
        for (int i = 0; i < 25; ++i) {
            ...
            if (canAttachToSide(BlockBlueSponge.block, worldIn, blockpos, EnumFacing.UP)) {
                int type = rand.nextInt(10);
                IBlockState state = BlockBlueSponge.block.getDefaultState();
                if (spongeIn != null) {
                    state = spongeIn.getDefaultState();
                }
                else {
                    switch (type) {
                        case 0:
