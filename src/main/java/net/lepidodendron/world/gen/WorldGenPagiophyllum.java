...
public class WorldGenPagiophyllum extends WorldGenerator
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        for (int i = 0; i < 64; ++i)
        {
            ...
            if (blockpos.getY() >= worldIn.getSeaLevel()-4 && worldIn.isAirBlock(blockpos) && (!worldIn.provider.isNether() || blockpos.getY() < 254) && BlockPagiophyllumWater.block.canPlaceBlockAt(worldIn, blockpos))
            {
                worldIn.setBlockState(blockpos, BlockPagiophyllumWater.block.getDefaultState(), 2);
                flag = true;
            }
        }
