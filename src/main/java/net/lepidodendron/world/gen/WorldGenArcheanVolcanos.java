...
public class WorldGenArcheanVolcanos extends WorldGenerator
{

    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        position = ChunkGenSpawner.getTopSolidBlock(position, worldIn);
        ...
        if (position.getY() <= 4)
        {
            ...
            int coneRadius = rand.nextInt(55) + 85;
                                        ...
                                        if (Math.random() < lavaBlockChance) {
                                            if (Math.random() > 0.1) {
                                                if (rand.nextInt(29) == 0) {
                                                    ...
                                                    Functions.setBlockStateAndCheckForDoublePlant(worldIn,bp, BlockSandBlack.block.getDefaultState(), 2);
                                                }
                                            }
                                            else {
                                                if (rand.nextInt(5) == 0) {
                    ...
                    if ((Math.pow((int) xct, 2) + Math.pow((int) zct, 2)) <= Math.pow((int) radiusLake, 2)) {
                        if (!worldIn.isAirBlock(posLake.add(xct, yct + 29, zct))) {
                            ...
                            if (rand.nextInt(240) == 0) {
                                EntityVolcano volcanoEntity = new EntityVolcano(worldIn, posLake.add(xct, yct + 30, zct));
