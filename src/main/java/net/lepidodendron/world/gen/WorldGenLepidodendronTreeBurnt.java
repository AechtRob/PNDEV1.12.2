...
public class WorldGenLepidodendronTreeBurnt extends WorldGenAbstractTree
{

    public WorldGenLepidodendronTreeBurnt(boolean notify)
    {
        super(notify);
    }
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256)
        {
            ...
            else
            {
                ...
                if (position.getY() >= worldIn.getSeaLevel()-4 && isSoil && position.getY() < worldIn.getHeight() - i - 1)
                {
					...
					ProcedureWorldGenLepidodendronBurnt.executeProcedure($_dependencies);
                    return true;
                }
                else
                {
