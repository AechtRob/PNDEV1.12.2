...
public class WorldGenSaurauiaTree extends WorldGenAbstractTree
{

    public WorldGenSaurauiaTree(boolean notify)
    {
        super(notify);
    }
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256)
        {
            ...
            else
            {
                ...
                if (position.getY() >= Functions.getAdjustedSeaLevel(worldIn, position)-4 && isSoil && position.getY() < worldIn.getHeight() - i - 1)
                {
                    ...
                    ProcedureWorldGenSaurauia.executeProcedure(worldIn, position, TreeHeight);
                    return true;
                }
                else
                {
