...
public class WorldGenSauroSkelly extends WorldGenerator
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position, int maxY)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            {
                Functions.setBlockStateAndCheckForDoublePlant(worldIn,blockpos, BlockSauroSkeleton.block.getDefaultState(), 2);
                TileEntity te = worldIn.getTileEntity(blockpos);
                if (te != null) {
                    if (te instanceof BlockSauroSkeleton.TileEntityCustom) {
                        BlockSauroSkeleton.TileEntityCustom ribcage = (BlockSauroSkeleton.TileEntityCustom) te;
                        ribcage.getTileData().setInteger("rotation", rand.nextInt(24) * 15);
                        ribcage.getTileData().setInteger("var", rand.nextInt(2));
                    }
                }
                flag = true;
            }
