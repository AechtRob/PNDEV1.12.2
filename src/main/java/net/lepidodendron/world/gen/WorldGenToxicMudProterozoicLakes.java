...
public class WorldGenToxicMudProterozoicLakes extends WorldGenerator
    //Used for the Caustic Quagmire biome:
{
    private final Block block;

    public WorldGenToxicMudProterozoicLakes(Block blockIn)
    {
        this.block = blockIn;
    }
                        ...
                        if (aboolean[(l1 * 16 + i3) * 8 + i4])
                        {
                            ...
                            if ((this.block.getDefaultState().getMaterial() == Material.WATER && i4 < 4)
                                ...
                                if (worldIn.getBlockState(position.add(l1, i4-2, i3)).getBlockFaceShape(
                                    ...
                                    if (rand.nextInt(6) == 0) {
                                        Functions.setBlockStateAndCheckForDoublePlant(worldIn,position.add(l1, i4-2, i3), Blocks.STONE.getDefaultState());
                                    }
                                    else {
                            ...
                            if ((worldIn.getBlockState(blockpos).getMaterial() == Material.GROUND || worldIn.getBlockState(blockpos).getMaterial() == Material.ROCK) && worldIn.getLightFor(EnumSkyBlock.SKY, position.add(i2, j4, j3)) > 0)
                            {
                                ...
                                if (this.block.getDefaultState().getMaterial() == Material.WATER) {
                                    if (rand.nextInt(5) == 0) {
                                        ...
                                        Functions.setBlockStateAndCheckForDoublePlant(worldIn,blockpos, Blocks.STONE.getDefaultState(), 2);
                                    }
                                }
                            }
                        }
