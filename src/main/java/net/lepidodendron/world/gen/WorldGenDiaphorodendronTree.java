...
public class WorldGenDiaphorodendronTree extends WorldGenAbstractTree
{

    public WorldGenDiaphorodendronTree(boolean notify)
    {
        super(notify);
    }
                ...
                if (position.getY() >= Functions.getAdjustedSeaLevel(worldIn, position)-4 && isSoil && position.getY() < worldIn.getHeight() - i - 1)
                {
					...
					if (position.getY() > (Functions.getAdjustedSeaLevel(worldIn, position)+15)) {
                        ...
                        if (rand.nextInt(12) == 0) {
                            ProcedureWorldGenDiaphorodendronYoung.executeProcedure($_dependencies);
                        }
                        else {
