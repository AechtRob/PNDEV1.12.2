...
public class WorldGenSingleStaticInWaterSideways extends WorldGenerator
{
    ...
    public boolean generate(@Nullable IBlockState staticBlock, World worldIn, Random rand, BlockPos position, int minWaterDepth, int maxWaterDepth, int minY, int maxY)
    {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            if (canAttachToSide(staticBlock.getBlock(), worldIn, blockpos, EnumFacing.byHorizontalIndex(facer))) {
                if (staticBlock.getPropertyKeys().contains(FACING)) {
                    Functions.setBlockStateAndCheckForDoublePlant(worldIn, blockpos, staticBlock.withProperty(FACING, EnumFacing.byHorizontalIndex(facer)), 2);
