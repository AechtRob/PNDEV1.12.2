...
public class WorldGenWaterSideDriedMud extends WorldGenerator
{
        ...
        for (int i = 0; i < 256; ++i)
        {
            ...
            if ((!worldIn.provider.isNether() || blockpos.getY() < 254)
				...
				&& (
                    ...
                    ||
                    (rand.nextInt(4) == 0 &&
                    (((worldIn.getBlockState(blockpos.north(2))).getMaterial() == Material.WATER)
                    || ((worldIn.getBlockState(blockpos.south(2))).getMaterial() == Material.WATER)
                    || ((worldIn.getBlockState(blockpos.east(2))).getMaterial() == Material.WATER)
                    || ((worldIn.getBlockState(blockpos.west(2))).getMaterial() == Material.WATER)))
                )
            )
