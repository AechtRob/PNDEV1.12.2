...
public class WorldGenSingleRugoseCoralSideways extends WorldGenerator
{
    ...
    public boolean generate( World worldIn, Random rand, BlockPos position, int minWaterDepth, int maxWaterDepth, int minY, int maxY, int specificVariant)
    {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            if (canAttachToSide(BlockRugosa1.block, worldIn, blockpos, EnumFacing.byHorizontalIndex(facer))) {
                int type = rand.nextInt(9);
                if (specificVariant >= 1 && specificVariant <= 5) {
                    type = specificVariant - 1;
                }
                switch (type) {
                    case 0: default:
                        Functions.setBlockStateAndCheckForDoublePlant(worldIn, blockpos, BlockRugosa1.block.getDefaultState().withProperty(FACING, EnumFacing.byHorizontalIndex(facer)), 2);
                        break;

                    case 1:
                        Functions.setBlockStateAndCheckForDoublePlant(worldIn, blockpos, BlockRugosa2.block.getDefaultState().withProperty(FACING, EnumFacing.byHorizontalIndex(facer)), 2);
                        break;

                    case 2:
                        Functions.setBlockStateAndCheckForDoublePlant(worldIn, blockpos, BlockRugosa3.block.getDefaultState().withProperty(FACING, EnumFacing.byHorizontalIndex(facer)), 2);
                        break;

                    case 3:
                        Functions.setBlockStateAndCheckForDoublePlant(worldIn, blockpos, BlockRugosa4.block.getDefaultState().withProperty(FACING, EnumFacing.byHorizontalIndex(facer)), 2);
                        break;

                    case 4:
                        Functions.setBlockStateAndCheckForDoublePlant(worldIn, blockpos, BlockRugosa5.block.getDefaultState().withProperty(FACING, EnumFacing.byHorizontalIndex(facer)), 2);
                        break;
