...
public class WorldGenDicroidiumHTree extends WorldGenAbstractTree
{

    public WorldGenDicroidiumHTree(boolean notify)
    {
        super(notify);
    }
                ...
                if (position.getY() >= Functions.getAdjustedSeaLevel(worldIn, position)-4 && isSoil && position.getY() < worldIn.getHeight() - i - 1)
                {
                    ...
                    for (int ii = 0; ii < 12; ++ii) {
                        ...
                        for (int iii = 0; iii < 5; ++iii) {
                            if (worldIn.getBlockState(blockpos.add(1,iii,0)).getBlock() == BlockDicroidiumHLog.block) {
                                isClear = false;
                            }
                            if (worldIn.getBlockState(blockpos.add(-1,iii,0)).getBlock() == BlockDicroidiumHLog.block) {
                                isClear = false;
                            }
                            if (worldIn.getBlockState(blockpos.add(0,iii,1)).getBlock() == BlockDicroidiumHLog.block) {
                                isClear = false;
                            }
                            if (worldIn.getBlockState(blockpos.add(0,iii,-1)).getBlock() == BlockDicroidiumHLog.block) {
                                isClear = false;
