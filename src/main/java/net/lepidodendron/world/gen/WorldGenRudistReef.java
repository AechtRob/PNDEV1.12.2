...
public class WorldGenRudistReef extends WorldGenerator
{
    //private final Block block;
    private final int startRadius;

    public WorldGenRudistReef()
    {
        super(false);
        //this.block = blockIn;
            ...
            label50:
            {
                ...
                for (int i = 0; i1 >= 0 && i < 3; ++i)
                {
                    ...
                    for (BlockPos blockpos : BlockPos.getAllInBox(position.add(-j, -k, -l), position.add(j, k, l)))
                    {
                        ...
                        {
                        	Block blockIn = BlockRudistReef.block;
                            int dir = rand.nextInt(4);
                            Functions.setBlockStateAndCheckForDoublePlant(worldIn,blockpos, blockIn.getDefaultState().withProperty(BlockRudistReef.FACING, EnumFacing.byHorizontalIndex(dir)), 4);
                        }
                    }
