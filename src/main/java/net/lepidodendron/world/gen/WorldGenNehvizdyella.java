...
public class WorldGenNehvizdyella extends WorldGenAbstractTree
{

    public WorldGenNehvizdyella(boolean notify)
    {
        super(notify);
    }
            ...
            else
            {
				...
				boolean isSoil = (worldIn.isAirBlock(position) && 
					...
					(((worldIn.getBlockState(position.down(2))).getMaterial() == Material.SAND) 
                        ...
                        || ((worldIn.getBlockState(position.down(2))).getMaterial() == Material.GRASS))
					);
				if (isSoil) {
				...
				if (!isSoil) {
					...
					(((worldIn.getBlockState(position.down(3))).getMaterial() == Material.SAND) 
                        ...
                        || ((worldIn.getBlockState(position.down(3))).getMaterial() == Material.GRASS))
					) {
                ...
                if (position.getY() >= worldIn.getSeaLevel()-4 && isSoil && waterCriteria && position.getY() < worldIn.getHeight() - i - 1)
                {
