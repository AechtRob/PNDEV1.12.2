...
public class GlassSpongeReefGenerator extends WorldGenerator
{
	private Block reef;
    private IBlockState state;

    public GlassSpongeReefGenerator(Block reefIn)
    {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
		...
		int[] dimCheck = LepidodendronConfigPlants.dimGlassSpongeReef;
		if (shouldGenerateInDimension(dimID, dimCheck))
			dimensionCriteria = true;

		Biome biome = worldIn.getBiome(position);
		if (biome.getRegistryName().toString().equalsIgnoreCase("lepidodendron:jurassic_ocean")
		...
		for (int i = 0; i < 32; ++i)
		{
			...
			if (this.reef.canPlaceBlockAt(worldIn, new BlockPos(j, k, l))
				...
				)
				{
					...
					if (ii == 0) {
						this.state = this.state.withProperty(BlockGlassSpongeReef.FACING, EnumFacing.NORTH);
					}
					if (ii == 1) {
						this.state = this.state.withProperty(BlockGlassSpongeReef.FACING, EnumFacing.EAST);
					}
					if (ii == 2) {
						this.state = this.state.withProperty(BlockGlassSpongeReef.FACING, EnumFacing.SOUTH);
					}
					if (ii == 3) {
						this.state = this.state.withProperty(BlockGlassSpongeReef.FACING, EnumFacing.WEST);
					}
