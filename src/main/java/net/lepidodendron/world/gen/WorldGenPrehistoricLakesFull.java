...
public class WorldGenPrehistoricLakesFull extends WorldGenerator
{
    private final Block block;

    public WorldGenPrehistoricLakesFull(Block blockIn)
    {
        this.block = blockIn;
    }
                ...
                for (int l2 = 0; l2 < 16; ++l2)
                {
                    for (int k = 0; k < 8; ++k)
                    {
                        ...
                        if (flag)
                        {
                            ...
                            if (k >= 5 && material.isLiquid())
                            {
                                return false;
                            }

                            if (k < 5 && !material.isSolid() && worldIn.getBlockState(position.add(k1, k, l2)).getBlock() != this.block)
                            {
                                return false;
                            }
                        }
            ...
            if (this.block.getDefaultState().getMaterial() == Material.WATER)
            {
                for (int k2 = 0; k2 < 16; ++k2)
                {
                    for (int l3 = 0; l3 < 16; ++l3)
                    {
                        ...
                        if (worldIn.canBlockFreezeWater(position.add(k2, 5, l3)))
                        {
                            ...
                            Functions.setBlockStateAndCheckForDoublePlant(worldIn,position.add(k2, 5, l3), Blocks.ICE.getDefaultState(), 16); //Forge
                        }
                    }
                }
            }
