		...
		for (int i = 0; i < 32; ++i)
		{
			...
			if (this.reef.canPlaceBlockAt(worldIn, new BlockPos(j, k, l))
				...
				)
				{
					...
					if (ii == 0) {
						this.state = this.state.withProperty(BlockArchaeocyatha.FACING, EnumFacing.NORTH);
					}
					if (ii == 1) {
						this.state = this.state.withProperty(BlockArchaeocyatha.FACING, EnumFacing.EAST);
					}
					if (ii == 2) {
						this.state = this.state.withProperty(BlockArchaeocyatha.FACING, EnumFacing.SOUTH);
					}
					if (ii == 3) {
						this.state = this.state.withProperty(BlockArchaeocyatha.FACING, EnumFacing.WEST);
					}
