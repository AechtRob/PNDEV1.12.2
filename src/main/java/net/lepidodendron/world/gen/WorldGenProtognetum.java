...
public class WorldGenProtognetum extends WorldGenerator
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position, boolean needsWater, int minHeight)
    {
        ...
        for (int i = 0; i < k; ++i)
        {
            ...
            if (!needsWater) {
                {
                    Functions.setBlockStateAndCheckForDoublePlant(worldIn,blockpos, BlockProtognetum.block.getDefaultState(), 2);
                    BlockProtognetum.block.onBlockAdded(worldIn, blockpos, BlockProtognetum.block.getDefaultState());
                    flag = true;
                }
            }
            else {
