...
public class WorldGenLiquidambarTree extends WorldGenAbstractTree
{

    public WorldGenLiquidambarTree(boolean notify)
    {
        super(notify);
    }
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256)
        {
            ...
            else
            {
                ...
                if (position.getY() >= Functions.getAdjustedSeaLevel(worldIn, position)-4 && isSoil && position.getY() < worldIn.getHeight() - i - 1)
                {
                    ...
                    ProcedureWorldGenLiquidambar.executeProcedure(worldIn, position, TreeHeight, false);
                    return true;
                }
                else
                {
