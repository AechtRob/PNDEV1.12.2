...
public class WorldGenCretaceousReefCoral extends WorldGenerator
{
    //private final Block block;
    private final int startRadius;

    public WorldGenCretaceousReefCoral()
    {
        super(false);
        //this.block = blockIn;
            ...
            label50:
            {
                ...
                for (int i = 0; i1 >= 0 && i < 3; ++i)
                {
                    ...
                    for (BlockPos blockpos : BlockPos.getAllInBox(position.add(-j, -k, -l), position.add(j, k, l)))
                    {
                        ...
                        {
                        	Block blockIn = BlockCoral.block;
                            int ii = rand.nextInt(4);

                            Functions.setBlockStateAndCheckForDoublePlant(worldIn,blockpos, blockIn.getDefaultState().withProperty(BlockCoral.FACING, EnumFacing.byHorizontalIndex(ii)), 4);
                        }
                    }
