...
public class WorldGenAnemone extends WorldGenerator
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position, boolean needsWater)
    {
        ...
        for (int i = 0; i < k; ++i)
        {
            ...
            if (blockpos.getY() >= Functions.getAdjustedSeaLevel(worldIn, blockpos)-4 && worldIn.isAirBlock(blockpos) && (!worldIn.provider.isNether() || blockpos.getY() < 254) && BlockAnemone.block.canPlaceBlockAt(worldIn, blockpos)
            )
            if (!needsWater) {
                {
