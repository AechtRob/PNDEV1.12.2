...
public class WorldGenAshesSurface extends WorldGenerator
{
    //private final Block block;
    private final int startRadius;

    public WorldGenAshesSurface()
    {
        super(false);
        //this.block = blockIn;
                ...
                for (int i = 0; i1 >= 0 && i < 2; ++i)
                {
                    ...
                    for (BlockPos blockpos : BlockPos.getAllInBox(position.add(-j, -k, -l), position.add(j, k, l)))
                    {
                        if (blockpos.distanceSq(position) <= (double)(f * f))
                        {
                        	...
                        	if (Math.random() > 0.65) {
                        		blockIn = BlockVolcanicAsh.block;
                        	}
                        	if (Math.random() > 0.85) {
                        		blockIn = BlockVolcanicAshLight.block;
                        	}
                        	if (Math.random() > 0.85) {
