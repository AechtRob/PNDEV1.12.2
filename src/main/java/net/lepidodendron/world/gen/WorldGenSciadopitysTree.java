...
public class WorldGenSciadopitysTree extends WorldGenAbstractTree
{

    public WorldGenSciadopitysTree(boolean notify)
    {
        super(notify);
    }
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256)
        {
            ...
            else
            {
                ...
                if (position.getY() >= Functions.getAdjustedSeaLevel(worldIn, position)-4 && isSoil && position.getY() < worldIn.getHeight() - i - 1)
                {
					...
					if (!(worldIn.getBiome(position).getRegistryName().toString().equalsIgnoreCase("lepidodendron:jurassic_rough_hills") && position.getY() > 100)) {
