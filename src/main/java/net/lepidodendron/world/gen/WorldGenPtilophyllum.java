    ...
    public boolean generate(World worldIn, Random rand, BlockPos position, boolean needsWater, int minHeight, int maxHeight)
    {
        ...
        for (int i = 0; i < 24; ++i) {
            ...
            if (!needsWater) {
                ...
                if (blockpos.getY() >= worldIn.getSeaLevel() - 4 && worldIn.isAirBlock(blockpos) && (!worldIn.provider.isNether() || blockpos.getY() < 254) && BlockPtilophyllumSapling.block.canPlaceBlockAt(worldIn, blockpos)
