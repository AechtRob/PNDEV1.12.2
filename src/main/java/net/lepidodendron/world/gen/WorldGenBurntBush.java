...
import net.lepidodendron.procedure.ProcedureWorldGenBurntBush;
...
public class WorldGenBurntBush extends WorldGenAbstractTree
{

    public WorldGenBurntBush(boolean notify)
    {
        super(notify);
    }
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256)
        {
            ...
            else
            {
                ...
                if (position.getY() >= worldIn.getSeaLevel()-4 && isSoil && position.getY() < worldIn.getHeight() - i - 1)
                {
					...
					ProcedureWorldGenBurntBush.executeProcedure($_dependencies);
                    return true;
                }
                else
                {
