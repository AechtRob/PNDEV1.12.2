...
public class WorldGenSphenophyllalesShrubby extends WorldGenerator
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position, boolean needsWater)
    {
        ...
        for (int i = 0; i < 25; ++i)
        {
            ...
            if (!needsWater) {
                ...
                if (blockpos.getY() >= Functions.getAdjustedSeaLevel(worldIn, blockpos) - 4 && worldIn.isAirBlock(blockpos) && (!worldIn.provider.isNether() || blockpos.getY() < 254) && BlockSphenophyllales2.block.canPlaceBlockAt(worldIn, blockpos)) {
                    Functions.setBlockStateAndCheckForDoublePlant(worldIn, blockpos, BlockSphenophyllales2.block.getDefaultState(), 2);
                    flag = true;
                }
            }
            else {
