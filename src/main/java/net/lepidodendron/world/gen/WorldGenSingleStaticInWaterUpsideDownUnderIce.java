...
public class WorldGenSingleStaticInWaterUpsideDownUnderIce extends WorldGenerator
{
    public final PropertyDirection FACING = BlockDirectional.FACING;

    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        return generate(null, worldIn, rand, position, 1, 0);
    }

    public boolean generate(@Nullable IBlockState staticBlock, World worldIn, Random rand, BlockPos position, int minY, int maxY)
    {
        ...
        for (int i = 0; i < 25; ++i) {
            ...
            if (canAttachToSide(staticBlock.getBlock(), worldIn, blockpos, EnumFacing.DOWN)) {
                if (staticBlock.getPropertyKeys().contains(FACING)) {
                    Functions.setBlockStateAndCheckForDoublePlant(worldIn, blockpos, staticBlock.withProperty(FACING, EnumFacing.DOWN), 2);
                } else {
    ...
    public boolean canAttachToSide(Block block, World worldIn, BlockPos pos, EnumFacing facing) {
        ...
        boolean flag1 = ((attachBlock.getMaterial() == Material.ICE)
                || (attachBlock.getMaterial() == Material.PACKED_ICE)
