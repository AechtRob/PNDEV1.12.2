...
public class WorldGenRockPilesBolide extends WorldGenerator
{
    //private final Block block;
    private final int startRadius;

    public WorldGenRockPilesBolide()
    {
        super(false);
        //this.block = blockIn;
                    ...
                    for (BlockPos blockpos : BlockPos.getAllInBox(position.add(-j, -k, -l), position.add(j, k, l)))
                    {
                        if (worldIn.isBlockLoaded(blockpos)) {
                            if (blockpos.distanceSq(position) <= (double) (f * f)) {
                                Block blockIn = BlockLavaCobbleMossy.block;
                                int ii = rand.nextInt(100) + 1;
                                if (ii >= 75) {
                                    blockIn = BlockLavaRock.block;
                                }
                                else if (ii >= 65) {
                                    blockIn = BlockMeteorite.block;
                                }
                                else if (ii >= 35) {
