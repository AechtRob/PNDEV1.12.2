            ...
            && (!worldIn.provider.isNether() || blockpos.getY() < 254) && BlockSelaginella.block.canPlaceBlockAt(worldIn, blockpos))
            {
            	...
            	if (orientation == 0) { //North
            		if (BlockSelaginella.block.canPlaceBlockOnSide(worldIn, blockpos, EnumFacing.NORTH)) {
	                	Functions.setBlockStateAndCheckForDoublePlant(worldIn,blockpos, BlockSelaginella.block.getDefaultState().withProperty(FACING, EnumFacing.NORTH), 2);
	                	flag = true;
            		}
            	}
            	else {
            		if (orientation == 1) { //South
	            		if (BlockSelaginella.block.canPlaceBlockOnSide(worldIn, blockpos, EnumFacing.SOUTH)) {
		                	Functions.setBlockStateAndCheckForDoublePlant(worldIn,blockpos, BlockSelaginella.block.getDefaultState().withProperty(FACING, EnumFacing.SOUTH), 2);
		                	flag = true;
	            		}
	            	}
	            	else {
	            		if (orientation == 2) { //West
		            		if (BlockSelaginella.block.canPlaceBlockOnSide(worldIn, blockpos, EnumFacing.WEST)) {
			                	Functions.setBlockStateAndCheckForDoublePlant(worldIn,blockpos, BlockSelaginella.block.getDefaultState().withProperty(FACING, EnumFacing.WEST), 2);
			                	flag = true;
		            		}
		            	}
		            	else {
		            		if (orientation == 3) { //East
			            		if (BlockSelaginella.block.canPlaceBlockOnSide(worldIn, blockpos, EnumFacing.EAST)) {
				                	Functions.setBlockStateAndCheckForDoublePlant(worldIn,blockpos, BlockSelaginella.block.getDefaultState().withProperty(FACING, EnumFacing.EAST), 2);
				                	flag = true;
			            		}
			            	}
			            	else {
			            		if (orientation == 4) { //Up
				            		if (BlockSelaginella.block.canPlaceBlockOnSide(worldIn, blockpos, EnumFacing.UP)) {
					                	Functions.setBlockStateAndCheckForDoublePlant(worldIn,blockpos, BlockSelaginella.block.getDefaultState().withProperty(FACING, EnumFacing.UP), 2);
				            		...
				            		if (orientation == 5) { //Down
					            		if (BlockSelaginella.block.canPlaceBlockOnSide(worldIn, blockpos, EnumFacing.DOWN)) {
						                	Functions.setBlockStateAndCheckForDoublePlant(worldIn,blockpos, BlockSelaginella.block.getDefaultState().withProperty(FACING, EnumFacing.DOWN), 2);
						                	flag = true;
					            		}
					            	}
				            	}
