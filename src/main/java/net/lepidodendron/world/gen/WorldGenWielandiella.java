...
public class WorldGenWielandiella extends WorldGenerator
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position, int minHeight, int maxHeight)
    {
        ...
        for (int i = 0; i < 16; ++i)
        {
            ...
            if (blockpos.getY() >= Functions.getAdjustedSeaLevel(worldIn, blockpos)-4 && worldIn.isAirBlock(blockpos) && (!worldIn.provider.isNether() || blockpos.getY() < 254) && BlockWielandiella.block.canPlaceBlockAt(worldIn, blockpos) &&
                (
