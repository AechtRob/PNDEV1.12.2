...
public class WorldGenRockPilesPaleoproterozoic extends WorldGenerator
{
    //private final Block block;
    private final int startRadius;

    public WorldGenRockPilesPaleoproterozoic()
    {
        super(false);
        //this.block = blockIn;
            ...
            label50:
            {
                ...
                for (int i = 0; i1 >= 0 && i < 3; ++i)
                {
                    ...
                    for (BlockPos blockpos : BlockPos.getAllInBox(position.add(-j, -k, -l), position.add(j, k, l)))
                    {
                        ...
                        {
                        	Block blockIn = Blocks.STONE.getStateFromMeta(1).getBlock();
                        	if (Math.random() > 0.65) {
