...
public class StructureSpawnThucydia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnThucydia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimThucydia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genThucydia && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genThucydiaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genThucydiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 48000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierThucydia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightThucydia;
				int minheight = LepidodendronConfigPlants.minheightThucydia;
