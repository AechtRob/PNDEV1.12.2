...
public class StructureSpawnSwampHorsetail extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSwampHorsetailBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSwampHorsetailOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 50000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSwampHorsetail;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSwampHorsetail;
				int minheight = LepidodendronConfigPlants.minheightSwampHorsetail;
				...
				int z = spawnTo.getZ();
				{
					...
					if (canSurviveAt(world, spawnTo)) {
						Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSwampHorsetailWater.block.getDefaultState(), 3);
					}
					else { //It must be the land version:
						Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSwampHorsetailLand.block.getDefaultState(), 3);
					}
				}
