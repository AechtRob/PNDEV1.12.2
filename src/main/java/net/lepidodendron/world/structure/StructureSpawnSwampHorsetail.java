...
public class StructureSpawnSwampHorsetail extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSwampHorsetail(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimSwampHorsetail))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genSwampHorsetail && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genSwampHorsetailBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfig.genSwampHorsetailOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 50000;
		double GenMultiplier = LepidodendronConfig.multiplierSwampHorsetail;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightSwampHorsetail;
				int minheight = LepidodendronConfig.minheightSwampHorsetail;
				...
				if ((!canSurviveAt(world, new BlockPos(i, j + 1, k)))
					&& (!BlockSwampHorsetailLand.BlockCustom.canSurviveAt(world, new BlockPos(i, j + 1, k))))
					continue;
				...
				int z = spawnTo.getZ();
				{
					...
					//System.err.println("Trying to spawn SwampHorsetail: " + i + " " + (j+1) + " " + k);
					if (canSurviveAt(world, spawnTo)) {
						world.setBlockState(spawnTo, BlockSwampHorsetailWater.block.getDefaultState(), 3);
					}
					else { //It must be the land version:
						world.setBlockState(spawnTo, BlockSwampHorsetailLand.block.getDefaultState(), 3);
					}
				}
