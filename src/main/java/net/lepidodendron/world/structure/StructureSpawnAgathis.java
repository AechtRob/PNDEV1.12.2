...
public class StructureSpawnAgathis extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimAgathis))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genAgathis && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genAgathisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genAgathisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 10000;
		double GenMultiplier = LepidodendronConfig.multiplierAgathis;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightAgathis;
				int minheight = LepidodendronConfig.minheightAgathis;
