...
public class StructureSpawnScolecopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnScolecopteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimScolecopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genScolecopteris && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genScolecopterisBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genScolecopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 29000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierScolecopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightScolecopteris;
				int minheight = LepidodendronConfigPlants.minheightScolecopteris;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockScolecopteris.block.getDefaultState(), 3);
			}
		}
	}
