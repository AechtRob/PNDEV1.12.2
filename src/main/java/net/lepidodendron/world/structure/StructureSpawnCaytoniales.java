...
public class StructureSpawnCaytoniales extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnCaytoniales(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int z = spawnTo.getZ();
				{
					...
					if (Math.random() >= 0.5) {
						ProcedureWorldGenCaytoniales.executeProcedure($_dependencies);
					}
					else {
						ProcedureWorldGenCaytoniales2.executeProcedure($_dependencies);
					}
				}
