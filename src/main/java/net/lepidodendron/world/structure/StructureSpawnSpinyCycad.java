...
import net.lepidodendron.procedure.ProcedureWorldGenSpinyCycad;
...
public class StructureSpawnSpinyCycad extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSpinyCycad(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimSpinyCycad))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genSpinyCycad && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genSpinyCycadBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfig.genSpinyCycadOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfig.multiplierSpinyCycad;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				while ((xct <= 1) && (!proximityCriteria)) {
					...
					while ((yct <= 2) && (!proximityCriteria)) {
						...
						while ((zct <= 1) && (!proximityCriteria)) {
							if ((world.getBlockState(new BlockPos(i + xct, j + yct, k + zct))).getBlock() == BlockSpinyCycadLog.block) {
				...
				int maxheight = LepidodendronConfig.maxheightSpinyCycad;
				int minheight = LepidodendronConfig.minheightSpinyCycad;
				...
				
				{
					...
					ProcedureWorldGenSpinyCycad.executeProcedure($_dependencies);
				}
