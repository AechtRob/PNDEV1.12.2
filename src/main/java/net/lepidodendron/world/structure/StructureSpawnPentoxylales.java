...
public class StructureSpawnPentoxylales extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnPentoxylales(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPentoxylales))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPentoxylales && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genPentoxylalesBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPentoxylalesOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 11000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierPentoxylales;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightPentoxylales;
				int minheight = LepidodendronConfigPlants.minheightPentoxylales;
