...
public class StructureSpawnGlossopterisDuocaudata extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnGlossopterisDuocaudata(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimGlossopterisDuocaudata))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genGlossopterisDuocaudata && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genGlossopterisDuocaudataBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genGlossopterisDuocaudataOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 25000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierGlossopterisDuocaudata;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightGlossopterisDuocaudata;
				int minheight = LepidodendronConfigPlants.minheightGlossopterisDuocaudata;
