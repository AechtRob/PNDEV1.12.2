...
public class StructureSpawnTsuga extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnTsuga(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimTsuga))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genTsuga && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genTsugaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genTsugaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 14000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierTsuga;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightTsuga;
				int minheight = LepidodendronConfigPlants.minheightTsuga;
