...
public class StructureSpawnFlabellopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnFlabellopteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimFlabellopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genFlabellopteris && !LepidodendronConfig.genAllPlants)
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genFlabellopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierFlabellopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightFlabellopteris;
				int minheight = LepidodendronConfigPlants.minheightFlabellopteris;
				...
				if (Math.random() < 0.7) {
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockFlabellopteris.block.getDefaultState(), 3);
				}
				else {
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockFlabellopterisSpore.block.getDefaultState(), 3);
				}
			}
		}
	}
