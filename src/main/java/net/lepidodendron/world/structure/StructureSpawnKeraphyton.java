...
public class StructureSpawnKeraphyton extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnKeraphyton(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimKeraphyton))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genKeraphyton && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genKeraphytonBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genKeraphytonOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 12000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierKeraphyton;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightKeraphyton;
				int minheight = LepidodendronConfigPlants.minheightKeraphyton;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockKeraphyton.block.getDefaultState(), 3);
				BlockKeraphyton.block.onBlockAdded(world, spawnTo, BlockKeraphyton.block.getDefaultState());
