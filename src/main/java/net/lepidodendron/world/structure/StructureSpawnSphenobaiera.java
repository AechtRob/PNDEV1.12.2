...
public class StructureSpawnSphenobaiera extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimSphenobaiera))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genSphenobaiera && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genSphenobaieraBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genSphenobaieraOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 12500;
		double GenMultiplier = LepidodendronConfig.multiplierSphenobaiera;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightSphenobaiera;
				int minheight = LepidodendronConfig.minheightSphenobaiera;
