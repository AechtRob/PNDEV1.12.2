...
public class StructureSpawnSphenobaiera extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSphenobaiera(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSphenobaiera))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSphenobaiera && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSphenobaieraBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSphenobaieraOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 12500;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSphenobaiera;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSphenobaiera;
				int minheight = LepidodendronConfigPlants.minheightSphenobaiera;
