...
public class StructureSpawnNypa extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnNypa(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genNypaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genNypaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 100000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierNypa;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightNypa;
				int minheight = LepidodendronConfigPlants.minheightNypa;
