...
public class StructureSpawnArthropitys extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnArthropitys(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimArthropitys))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genArthropitys && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genArthropitysBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genArthropitysOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 33000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierArthropitys;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightArthropitys;
				int minheight = LepidodendronConfigPlants.minheightArthropitys;
