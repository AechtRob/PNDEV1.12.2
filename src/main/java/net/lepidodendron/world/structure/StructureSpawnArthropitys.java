...
public class StructureSpawnArthropitys extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimArthropitys))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genArthropitys && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genArthropitysBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genArthropitysOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 33000;
		double GenMultiplier = LepidodendronConfig.multiplierArthropitys;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightArthropitys;
				int minheight = LepidodendronConfig.minheightArthropitys;
