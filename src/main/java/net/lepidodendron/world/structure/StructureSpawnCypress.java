...
public class StructureSpawnCypress extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnCypress(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genCypressBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genCypressOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 18000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierCypress;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightCypress;
				int minheight = LepidodendronConfigPlants.minheightCypress;
