...
public class StructureSpawnCinnamonFern extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnCinnamonFern(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimCinnamonFern))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genCinnamonFern && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genCinnamonFernBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfig.genCinnamonFernOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 18000;
		double GenMultiplier = LepidodendronConfig.multiplierCinnamonFern;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightCinnamonFern;
				int minheight = LepidodendronConfig.minheightCinnamonFern;
				...
				world.setBlockState(spawnTo, BlockCinnamonFern.block.getDefaultState(), 3);
			}
		}
	}
