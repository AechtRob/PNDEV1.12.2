...
public class StructureSpawnCinnamonFern extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genCinnamonFernBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genCinnamonFernOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 18000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierCinnamonFern;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightCinnamonFern;
				int minheight = LepidodendronConfigPlants.minheightCinnamonFern;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockCinnamonFern.block.getDefaultState(), 3);
			}
		}
	}
