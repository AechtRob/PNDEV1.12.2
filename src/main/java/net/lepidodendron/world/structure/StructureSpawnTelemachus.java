...
public class StructureSpawnTelemachus extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnTelemachus(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimTelemachus))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genTelemachus && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genTelemachusBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genTelemachusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierTelemachus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightTelemachus;
				int minheight = LepidodendronConfigPlants.minheightTelemachus;
