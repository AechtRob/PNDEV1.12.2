...
public class StructureSpawnKomlopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnKomlopteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimKomlopteris))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genKomlopterisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genKomlopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 8000;
		double GenMultiplier = LepidodendronConfig.multiplierKomlopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightKomlopteris;
				int minheight = LepidodendronConfig.minheightKomlopteris;
