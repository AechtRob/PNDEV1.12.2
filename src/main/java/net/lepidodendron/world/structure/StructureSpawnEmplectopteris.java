...
public class StructureSpawnEmplectopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnEmplectopteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimEmplectopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genEmplectopteris && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genEmplectopterisBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genEmplectopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 18200;
		double GenMultiplier = LepidodendronConfigPlants.multiplierEmplectopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				//This can also generate over one-block deep water:
				if ((world.getBlockState(new BlockPos(i, j, k))).getMaterial() == Material.WATER) {
					if (
						...
						|| ((world.getBlockState(new BlockPos(i, j-1, k))).getMaterial() == Material.ROCK)
				...
				while ((xct <= 1) && (!proximityCriteria)) {
					...
					while ((yct <= 2) && (!proximityCriteria)) {
						...
						while ((zct <= 1) && (!proximityCriteria)) {
							if ((world.getBlockState(new BlockPos(i + xct, j + yct, k + zct))).getBlock() == BlockEmplectopterisLog.block) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightEmplectopteris;
				int minheight = LepidodendronConfigPlants.minheightEmplectopteris;
				...
				//System.err.println("Spawn at " + x + " " + y + " " + z);
				{
