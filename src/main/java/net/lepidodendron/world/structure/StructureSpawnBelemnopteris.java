...
public class StructureSpawnBelemnopteris extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimBelemnopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genBelemnopteris && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genBelemnopterisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genBelemnopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 33000;
		double GenMultiplier = LepidodendronConfig.multiplierBelemnopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightBelemnopteris;
				int minheight = LepidodendronConfig.minheightBelemnopteris;
