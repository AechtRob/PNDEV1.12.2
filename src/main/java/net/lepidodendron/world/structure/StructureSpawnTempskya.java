...
import net.lepidodendron.procedure.ProcedureWorldGenTempskya;
...
public class StructureSpawnTempskya extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnTempskya(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimTempskya))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genTempskya && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genTempskyaBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genTempskyaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 26000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierTempskya;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightTempskya;
				int minheight = LepidodendronConfigPlants.minheightTempskya;
...

				{
					...
					ProcedureWorldGenTempskya.executeProcedure($_dependencies);
				}
			}
		}
	}
