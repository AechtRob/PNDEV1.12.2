...
import net.lepidodendron.procedure.ProcedureWorldGenBunya;
...
public class StructureSpawnBunya extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnBunya(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimAraucariaBidwilli))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genAraucariaBidwilli && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genAraucariaBidwilliBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genAraucariaBidwilliOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfig.multiplierAraucariaBidwilli;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightAraucariaBidwilli;
				int minheight = LepidodendronConfig.minheightAraucariaBidwilli;
...

				{
					...
					ProcedureWorldGenBunya.executeProcedure($_dependencies);
				}
