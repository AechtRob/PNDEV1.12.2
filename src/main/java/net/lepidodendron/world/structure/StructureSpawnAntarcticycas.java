...
import net.lepidodendron.procedure.ProcedureWorldGenAntarcticycas;
...
public class StructureSpawnAntarcticycas extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnAntarcticycas(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimAntarcticycas))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genAntarcticycas && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genAntarcticycasBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genAntarcticycasOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierAntarcticycas;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightAntarcticycas;
				int minheight = LepidodendronConfigPlants.minheightAntarcticycas;
				...
				int z = spawnTo.getZ();
				{
					...
					ProcedureWorldGenAntarcticycas.executeProcedure($_dependencies);
				}
