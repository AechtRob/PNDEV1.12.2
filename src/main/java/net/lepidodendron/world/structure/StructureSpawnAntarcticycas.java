...
public class StructureSpawnAntarcticycas extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimAntarcticycas))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genAntarcticycas && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genAntarcticycasBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genAntarcticycasOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfig.multiplierAntarcticycas;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightAntarcticycas;
				int minheight = LepidodendronConfig.minheightAntarcticycas;
