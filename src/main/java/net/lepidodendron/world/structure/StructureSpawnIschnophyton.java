...
public class StructureSpawnIschnophyton extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnIschnophyton(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimIschnophyton))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genIschnophyton && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genIschnophytonBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genIschnophytonOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 70000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierIschnophyton;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightIschnophyton;
				int minheight = LepidodendronConfigPlants.minheightIschnophyton;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockIschnophyton.block.getDefaultState(), 3);
				BlockIschnophyton.block.onBlockAdded(world, spawnTo, BlockIschnophyton.block.getDefaultState());
