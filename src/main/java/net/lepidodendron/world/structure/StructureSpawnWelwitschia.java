...
public class StructureSpawnWelwitschia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnWelwitschia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimWelwitschia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genWelwitschia && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genWelwitschiaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genWelwitschiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 13000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierWelwitschia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightWelwitschia;
				int minheight = LepidodendronConfigPlants.minheightWelwitschia;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockWelwitschia.block.getDefaultState(), 3);
			}
		}
	}
