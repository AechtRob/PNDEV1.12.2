...
public class StructureSpawnSphenophyllales2 extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSphenophyllales2(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSphenophyllales2))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSphenophyllales2 && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSphenophyllales2BlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSphenophyllales2OverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSphenophyllales2;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSphenophyllales2;
				int minheight = LepidodendronConfigPlants.minheightSphenophyllales2;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSphenophyllales2.block.getDefaultState(), 3);
			}
		}
	}
