...
public class StructureSpawnCladophlebis extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnCladophlebis(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimCladophlebis))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genCladophlebis && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genCladophlebisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genCladophlebisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 70000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierCladophlebis;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightCladophlebis;
				int minheight = LepidodendronConfigPlants.minheightCladophlebis;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockCladophlebis.block.getDefaultState(), 3);
				BlockCladophlebis.block.onBlockAdded(world, spawnTo, BlockCladophlebis.block.getDefaultState());
