...
public class StructureSpawnDicroidiumE extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnDicroidiumE(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimDicroidiumElongatum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genDicroidiumElongatum && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genDicroidiumElongatumBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genDicroidiumElongatumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 12000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierDicroidiumElongatum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightDicroidiumElongatum;
				int minheight = LepidodendronConfigPlants.minheightDicroidiumElongatum;
