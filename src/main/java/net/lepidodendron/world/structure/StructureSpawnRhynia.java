...
public class StructureSpawnRhynia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnRhynia(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimRhynia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genRhynia && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genRhyniaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genRhyniaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 37000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierRhynia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightRhynia;
				int minheight = LepidodendronConfigPlants.minheightRhynia;
				...
				if ((!canSurviveAt(world, new BlockPos(i, j + 1, k)))
					&& (!BlockRhyniaLand.BlockCustom.canSurviveAt(world, new BlockPos(i, j + 1, k))))
					continue;
