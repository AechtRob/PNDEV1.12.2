...
public class StructureSpawnSycamore extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSycamore(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimSycamore))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genSycamore && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genSycamoreBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genSycamoreOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfig.multiplierSycamore;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightSycamore;
				int minheight = LepidodendronConfig.minheightSycamore;
				...
				int z = spawnTo.getZ();
				{
					...
					int TreeHeight = 15 + random.nextInt(15);
					if (TreeHeight < 22 && Math.random() > 0.5) {
						TreeHeight = 17 + random.nextInt(17);
					}
					if (TreeHeight > 25) {
						ProcedureWorldGenSycamore.executeProcedure(world, spawnTo, TreeHeight,2);
					}
					else {
						ProcedureWorldGenSycamore.executeProcedure(world, spawnTo, TreeHeight,1);
					}
				}
