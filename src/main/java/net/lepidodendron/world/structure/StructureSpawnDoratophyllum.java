...
public class StructureSpawnDoratophyllum extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnDoratophyllum(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimDoratophyllum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genDoratophyllum && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genDoratophyllumBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genDoratophyllumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 12000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierDoratophyllum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightDoratophyllum;
				int minheight = LepidodendronConfigPlants.minheightDoratophyllum;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockDoratophyllum.block.getDefaultState(), 3);
				BlockDoratophyllum.block.onBlockAdded(world, spawnTo, BlockDoratophyllum.block.getDefaultState());
			}
		}
	}
