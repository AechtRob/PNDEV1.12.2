...
public class StructureSpawnPitys extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimPitys))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genPitys && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genPitysBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genPitysOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfig.multiplierPitys;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightPitys;
				int minheight = LepidodendronConfig.minheightPitys;
