...
public class StructureSpawnPitys extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnPitys(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPitys))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPitys && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genPitysBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPitysOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierPitys;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightPitys;
				int minheight = LepidodendronConfigPlants.minheightPitys;
