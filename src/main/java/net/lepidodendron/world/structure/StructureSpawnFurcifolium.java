...
public class StructureSpawnFurcifolium extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnFurcifolium(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimFurcifolium))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genFurcifolium && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genFurcifoliumBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genFurcifoliumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 15500;
		double GenMultiplier = LepidodendronConfigPlants.multiplierFurcifolium;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightFurcifolium;
				int minheight = LepidodendronConfigPlants.minheightFurcifolium;
				...
				int z = spawnTo.getZ();
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockFurcifolium.block.getDefaultState());
				}
			}
		}
	}
