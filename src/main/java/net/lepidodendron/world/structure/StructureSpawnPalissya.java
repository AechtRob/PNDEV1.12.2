...
public class StructureSpawnPalissya extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnPalissya(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPalissya))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPalissya && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genPalissyaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPalissyaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierPalissya;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightPalissya;
				int minheight = LepidodendronConfigPlants.minheightPalissya;
				...
				int z = spawnTo.getZ();
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockPalissya.block.getDefaultState());
					BlockPalissya.block.onBlockAdded(world, spawnTo, BlockPalissya.block.getDefaultState());
				}
