...
public class StructureSpawnWoodHorsetail extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genWoodHorsetailBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genWoodHorsetailOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierWoodHorsetail;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightWoodHorsetail;
				int minheight = LepidodendronConfigPlants.minheightWoodHorsetail;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockWoodHorsetail.block.getDefaultState(), 3);
			}
		}
	}
