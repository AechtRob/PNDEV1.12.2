...
public class StructureSpawnSanfordiacaulis extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSanfordiacaulis(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSanfordiacaulis))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSanfordiacaulis && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSanfordiacaulisBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSanfordiacaulisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 28000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSanfordiacaulis;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSanfordiacaulis;
				int minheight = LepidodendronConfigPlants.minheightSanfordiacaulis;
