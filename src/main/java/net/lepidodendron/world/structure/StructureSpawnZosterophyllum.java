...
public class StructureSpawnZosterophyllum extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnZosterophyllum(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimZosterophyllum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genZosterophyllum && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genZosterophyllumBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genZosterophyllumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 20000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierZosterophyllum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightZosterophyllum;
				int minheight = LepidodendronConfigPlants.minheightZosterophyllum;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockZosterophyllum.block.getDefaultState(), 3);
				if ((Math.random() > 0.7)) {
					if ((Math.random() > 0.7)) {
						Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockZosterophyllumSpore.block.getDefaultState(), 3);
					}
	        	}
			}
		}
