...
public class StructureSpawnZosterophyllum extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimZosterophyllum))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genZosterophyllum && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genZosterophyllumBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfig.genZosterophyllumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 20000;
		double GenMultiplier = LepidodendronConfig.multiplierZosterophyllum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightZosterophyllum;
				int minheight = LepidodendronConfig.minheightZosterophyllum;
