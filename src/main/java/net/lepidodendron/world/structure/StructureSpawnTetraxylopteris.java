...
public class StructureSpawnTetraxylopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnTetraxylopteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimTetraxylopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genTetraxylopteris && !LepidodendronConfig.genAllPlants)
		...
		if (matchBiome(biome, LepidodendronConfig.genTetraxylopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfig.multiplierTetraxylopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightTetraxylopteris;
				int minheight = LepidodendronConfig.minheightTetraxylopteris;
