...
public class StructureSpawnTetraxylopteris extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimTetraxylopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genTetraxylopteris && !LepidodendronConfig.genAllPlants)
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genTetraxylopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierTetraxylopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightTetraxylopteris;
				int minheight = LepidodendronConfigPlants.minheightTetraxylopteris;
