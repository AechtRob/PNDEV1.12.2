...
import net.lepidodendron.procedure.ProcedureWorldGenWollemi;
...
public class StructureSpawnWollemi extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnWollemi(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimWollemi))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genWollemi && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genWollemiBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genWollemiOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 40000;
		double GenMultiplier = LepidodendronConfig.multiplierWollemi;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightWollemi;
				int minheight = LepidodendronConfig.minheightWollemi;
				...
				int z = spawnTo.getZ();
				{
					...
					ProcedureWorldGenWollemi.executeProcedure($_dependencies);
				}
