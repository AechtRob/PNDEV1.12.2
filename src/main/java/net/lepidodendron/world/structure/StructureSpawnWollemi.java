...
public class StructureSpawnWollemi extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genWollemiBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genWollemiOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 40000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierWollemi;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightWollemi;
				int minheight = LepidodendronConfigPlants.minheightWollemi;
