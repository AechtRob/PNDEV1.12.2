...
public class StructureSpawnThinnfeldia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnThinnfeldia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimThinnfeldia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genThinnfeldia && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genThinnfeldiaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genThinnfeldiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 8100;
		double GenMultiplier = LepidodendronConfigPlants.multiplierThinnfeldia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightThinnfeldia;
				int minheight = LepidodendronConfigPlants.minheightThinnfeldia;
