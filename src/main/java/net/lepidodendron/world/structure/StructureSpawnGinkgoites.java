...
public class StructureSpawnGinkgoites extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnGinkgoites(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimGinkgoites))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genGinkgoites && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genGinkgoitesBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genGinkgoitesOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 16500;
		double GenMultiplier = LepidodendronConfigPlants.multiplierGinkgoites;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightGinkgoites;
				int minheight = LepidodendronConfigPlants.minheightGinkgoites;
