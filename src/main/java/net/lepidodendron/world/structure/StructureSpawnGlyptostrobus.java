...
public class StructureSpawnGlyptostrobus extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnGlyptostrobus(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimGlyptostrobus))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genGlyptostrobus && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genGlyptostrobusBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genGlyptostrobusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 18500;
		double GenMultiplier = LepidodendronConfigPlants.multiplierGlyptostrobus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightGlyptostrobus;
				int minheight = LepidodendronConfigPlants.minheightGlyptostrobus;
