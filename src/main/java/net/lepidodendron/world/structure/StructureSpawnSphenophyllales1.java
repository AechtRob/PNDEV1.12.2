...
public class StructureSpawnSphenophyllales1 extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSphenophyllales1(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSphenophyllales1))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSphenophyllales1 && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSphenophyllales1BlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSphenophyllales1OverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSphenophyllales1;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSphenophyllales1;
				int minheight = LepidodendronConfigPlants.minheightSphenophyllales1;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSphenophyllales1.block.getDefaultState(), 3);
			}
		}
	}
