...
public class StructureSpawnBjuvia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnBjuvia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
				...
				while ((xct <= 1) && (!proximityCriteria)) {
					...
					while ((yct <= 2) && (!proximityCriteria)) {
						...
						while ((zct <= 1) && (!proximityCriteria)) {
							if ((world.getBlockState(new BlockPos(i + xct, j + yct, k + zct))).getBlock() == BlockBjuviaLog.block) {
