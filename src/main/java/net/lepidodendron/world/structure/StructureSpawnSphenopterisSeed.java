...
import net.lepidodendron.block.BlockSphenopterisSeedTop;
...
public class StructureSpawnSphenopterisSeed extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSphenopterisSeed(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSphenopterisSeed))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSphenopterisSeed && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSphenopterisSeedBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSphenopterisSeedOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 28000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSphenopterisSeed;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSphenopterisSeed;
				int minheight = LepidodendronConfigPlants.minheightSphenopterisSeed;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSphenopterisSeed.block.getDefaultState(), 3);
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.up(), BlockSphenopterisSeedTop.block.getDefaultState(), 3);
			}
		}
	}
