...
public class StructureSpawnElatocladus extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnElatocladus(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimElatocladus))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genElatocladus && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genElatocladusBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genElatocladusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 18000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierElatocladus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightElatocladus;
				int minheight = LepidodendronConfigPlants.minheightElatocladus;
