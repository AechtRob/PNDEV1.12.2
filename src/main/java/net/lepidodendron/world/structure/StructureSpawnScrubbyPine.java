...
public class StructureSpawnScrubbyPine extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnScrubbyPine(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genScrubbyPineBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genScrubbyPineOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 30000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierScrubbyPine;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightScrubbyPine;
				int minheight = LepidodendronConfigPlants.minheightScrubbyPine;
