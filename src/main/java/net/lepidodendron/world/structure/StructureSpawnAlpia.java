...
import net.lepidodendron.procedure.ProcedureWorldGenAlpia;
...
public class StructureSpawnAlpia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnAlpia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimAlpia))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genAlpiaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genAlpiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 10000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierAlpia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightAlpia;
				int minheight = LepidodendronConfigPlants.minheightAlpia;
...

				{
					...
					ProcedureWorldGenAlpia.executeProcedure($_dependencies);
				}
