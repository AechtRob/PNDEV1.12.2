...
public class StructureSpawnDicroidium extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnDicroidium(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimDicroidiumZuberi))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genDicroidiumZuberi && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genDioonBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genDicroidiumZuberiOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 20000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierDicroidiumZuberi;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightDicroidiumZuberi;
				int minheight = LepidodendronConfigPlants.minheightDicroidiumZuberi;
				...
				if (Math.random() > 0.5) {
					if (Math.random() > 0.3) {
						ProcedureTreeLog.executeProcedure((int) i, (int) j + 1, (int) k, world, BlockDicroidiumLeavesPlaceable.block, EnumFacing.UP);
						ProcedureTreeLog.executeProcedure((int) i, (int) j + 2, (int) k, world, BlockDicroidiumLeavesTop.block, EnumFacing.UP);
						}
					else {
						ProcedureTreeLog.executeProcedure((int) i, (int) j + 1, (int) k, world, BlockDicroidiumLeavesSmallPlaceable.block, EnumFacing.UP);
					}
				}
				else {
