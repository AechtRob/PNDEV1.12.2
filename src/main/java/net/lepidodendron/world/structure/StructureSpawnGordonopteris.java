...
public class StructureSpawnGordonopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnGordonopteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimGordonopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genGordonopteris && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genGordonopterisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genGordonopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierGordonopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightGordonopteris;
				int minheight = LepidodendronConfigPlants.minheightGordonopteris;
