...
public class StructureSpawnPterophyllum extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnPterophyllum(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPterophyllum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPterophyllum && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genPterophyllumBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPterophyllumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 20000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierPterophyllum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				while ((xct <= 3) && (!proximityCriteria)) {
					...
					while ((yct <= 2) && (!proximityCriteria)) {
						...
						while ((zct <= 3) && (!proximityCriteria)) {
							if ((world.getBlockState(new BlockPos(i + xct, j + yct, k + zct))).getBlock() == BlockPterophyllumLog.block) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightPterophyllum;
				int minheight = LepidodendronConfigPlants.minheightPterophyllum;
