...
public class StructureSpawnCzekanowskia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnCzekanowskia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimCzekanowskia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genCzekanowskia && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genCzekanowskiaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genCzekanowskiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 50000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierCzekanowskia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightCzekanowskia;
				int minheight = LepidodendronConfigPlants.minheightCzekanowskia;
