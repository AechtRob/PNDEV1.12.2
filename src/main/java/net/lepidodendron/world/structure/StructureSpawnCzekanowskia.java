...
public class StructureSpawnCzekanowskia extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimCzekanowskia))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genCzekanowskia && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genCzekanowskiaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genCzekanowskiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 50000;
		double GenMultiplier = LepidodendronConfig.multiplierCzekanowskia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightCzekanowskia;
				int minheight = LepidodendronConfig.minheightCzekanowskia;
