...
public class StructureSpawnAnemone extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnAnemone(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimAnemoneFlower))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genAnemoneFlower && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genAnemoneFlowerBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genAnemoneFlowerOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 50000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierAnemoneFlower;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightAnemoneFlower;
				int minheight = LepidodendronConfigPlants.minheightAnemoneFlower;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockAnemone.block.getDefaultState(), 3);
			}
		}
	}
