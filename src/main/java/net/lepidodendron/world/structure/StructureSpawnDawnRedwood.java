...
public class StructureSpawnDawnRedwood extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnDawnRedwood(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimDawnRedwood))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genDawnRedwood && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genDawnRedwoodBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genDawnRedwoodOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfig.multiplierDawnRedwood;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightDawnRedwood;
				int minheight = LepidodendronConfig.minheightDawnRedwood;
