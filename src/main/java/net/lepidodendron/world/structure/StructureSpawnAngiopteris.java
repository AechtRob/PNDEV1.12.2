...
public class StructureSpawnAngiopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnAngiopteris(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimAngiopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genAngiopteris && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genAngiopterisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genAngiopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 8000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierAngiopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightAngiopteris;
				int minheight = LepidodendronConfigPlants.minheightAngiopteris;
				...
				int z = spawnTo.getZ();
				{
					...
					if ((world.canSeeSky(new BlockPos((int) x, (int) y, (int) z)))) {			
						...
						if (BlockAngiopteris.block.canPlaceBlockAt(world, new BlockPos((int) x, (int) y, (int) z))) {
							Functions.setBlockStateAndCheckForDoublePlant(world,new BlockPos((int) x, (int) y, (int) z), BlockAngiopteris.block.getDefaultState(), 2);
							BlockAngiopteris.block.onBlockAdded(world, new BlockPos((int) x, (int) y, (int) z), BlockAngiopteris.block.getDefaultState());
						}
					}
				}
