...
public class StructureSpawnDollyphyton extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimDollyphyton))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genDollyphyton && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genDollyphytonBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genDollyphytonOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 20000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierDollyphyton;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightDollyphyton;
				int minheight = LepidodendronConfigPlants.minheightDollyphyton;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockDollyphyton.block.getDefaultState(), 3);
			}
		}
	}
