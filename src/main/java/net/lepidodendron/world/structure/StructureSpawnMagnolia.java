...
import net.lepidodendron.procedure.ProcedureWorldGenMagnoliaSpaceChecker;
...
public class StructureSpawnMagnolia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnMagnolia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimMagnolia))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genMagnolia && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genMagnoliaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genMagnoliaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfig.multiplierMagnolia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightMagnolia;
				int minheight = LepidodendronConfig.minheightMagnolia;
				...
				int z = spawnTo.getZ();
				{
					...
					ProcedureWorldGenMagnoliaSpaceChecker.executeProcedure($_dependencies);
				}
