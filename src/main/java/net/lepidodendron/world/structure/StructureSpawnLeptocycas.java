...
public class StructureSpawnLeptocycas extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnLeptocycas(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimLeptocycas))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genLeptocycas && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genLeptocycasBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genLeptocycasOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 30000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierLeptocycas;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				while ((xct <= 1) && (!proximityCriteria)) {
					...
					while ((yct <= 2) && (!proximityCriteria)) {
						...
						while ((zct <= 1) && (!proximityCriteria)) {
							if ((world.getBlockState(new BlockPos(i + xct, j + yct, k + zct))).getBlock() == BlockLeptocycasLog.block) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightLeptocycas;
				int minheight = LepidodendronConfigPlants.minheightLeptocycas;
				...
				if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genLeptocycasBlacklistBiomes))) {
					...
					if (BiomeDictionary.hasType(biome, BiomeDictionary.Type.JUNGLE))
						biomeCriteria = true;
					if ((BiomeDictionary.hasType(biome, BiomeDictionary.Type.RIVER)) && Math.random() > 0.8)
						biomeCriteria = true;
					if (BiomeDictionary.hasType(biome, BiomeDictionary.Type.LUSH))
						biomeCriteria = true;
					if (BiomeDictionary.hasType(biome, BiomeDictionary.Type.SNOWY))
