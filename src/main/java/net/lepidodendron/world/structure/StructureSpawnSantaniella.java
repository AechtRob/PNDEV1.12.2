...
public class StructureSpawnSantaniella extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSantaniella(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSantaniella))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSantaniella && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSantaniellaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSantaniellaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 100000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSantaniella;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSantaniella;
				int minheight = LepidodendronConfigPlants.minheightSantaniella;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSantaniella.block.getDefaultState(), 3);
			}
		}
	}
