...
public class StructureSpawnClubmoss extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnClubmoss(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genClubmossBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genClubmossOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 38000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierClubmoss;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightClubmoss;
				int minheight = LepidodendronConfigPlants.minheightClubmoss;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockClubmoss.block.getDefaultState(), 3);
			}
		}
	}
