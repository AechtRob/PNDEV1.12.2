...
import net.lepidodendron.procedure.ProcedureWorldGenNothofagus;
...
public class StructureSpawnNothofagus extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnNothofagus(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimNothofagus))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genNothofagus && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genNothofagusBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genNothofagusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfig.multiplierNothofagus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightNothofagus;
				int minheight = LepidodendronConfig.minheightNothofagus;
				...
				int z = spawnTo.getZ();
				{
					...
					ProcedureWorldGenNothofagus.executeProcedure($_dependencies);
				}
