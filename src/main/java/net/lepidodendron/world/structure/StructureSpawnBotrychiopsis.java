...
public class StructureSpawnBotrychiopsis extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnBotrychiopsis(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimBotrychiopsis))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genBotrychiopsis && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genBotrychiopsisBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genBotrychiopsisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierBotrychiopsis;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightBotrychiopsis;
				int minheight = LepidodendronConfigPlants.minheightBotrychiopsis;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockBotrychiopsis.block.getDefaultState(), 3);
			}
		}
	}
