...
public class StructureSpawnDechellyia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnDechellyia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimDechellyia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genDechellyia && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genDechellyiaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genDechellyiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 48000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierDechellyia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightDechellyia;
				int minheight = LepidodendronConfigPlants.minheightDechellyia;
