...
public class StructureSpawnAlliopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnAlliopteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimAlliopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genAlliopteris && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genAlliopterisBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genAlliopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 30000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierAlliopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightAlliopteris;
				int minheight = LepidodendronConfigPlants.minheightAlliopteris;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockAlliopteris.block.getDefaultState(), 3);
			}
		}
	}
