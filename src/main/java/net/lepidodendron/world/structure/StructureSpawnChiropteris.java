...
public class StructureSpawnChiropteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnChiropteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimChiropteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genChiropteris && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genChiropterisBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genChiropterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 32500;
		double GenMultiplier = LepidodendronConfigPlants.multiplierChiropteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightChiropteris;
				int minheight = LepidodendronConfigPlants.minheightChiropteris;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockChiropteris.block.getDefaultState(), 3);
			}
		}
	}
