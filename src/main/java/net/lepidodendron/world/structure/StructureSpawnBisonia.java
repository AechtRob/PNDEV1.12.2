...
public class StructureSpawnBisonia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnBisonia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimBisonia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genBisonia && (!LepidodendronConfig.genAllPlants))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genBisoniaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genBisoniaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierBisonia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightBisonia;
				int minheight = LepidodendronConfigPlants.minheightBisonia;
				...
				int z = spawnTo.getZ();
				{
					...
					BlockBisoniaSapling.BlockCustom.growBisonia(world, spawnTo, world.rand);
				}
