...
public class StructureSpawnCunninghamia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnCunninghamia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genCunninghamiaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genCunninghamiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 11000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierCunninghamia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightCunninghamia;
				int minheight = LepidodendronConfigPlants.minheightCunninghamia;
