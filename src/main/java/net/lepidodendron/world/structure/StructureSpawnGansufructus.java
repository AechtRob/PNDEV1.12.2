...
public class StructureSpawnGansufructus extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnGansufructus(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimGansufructus))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genGansufructus && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genGansufructusBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genGansufructusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 100000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierGansufructus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightGansufructus;
				int minheight = LepidodendronConfigPlants.minheightGansufructus;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockGansufructus.block.getDefaultState(), 3);
			}
		}
	}
