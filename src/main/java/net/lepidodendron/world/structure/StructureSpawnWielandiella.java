...
import net.lepidodendron.block.BlockWielandiellaTop;
...
public class StructureSpawnWielandiella extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnWielandiella(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimWielandiella))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genWielandiella && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genWielandiellaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genWielandiellaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 20000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierWielandiella;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightWielandiella;
				int minheight = LepidodendronConfigPlants.minheightWielandiella;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockWielandiella.block.getDefaultState(), 3);
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.up(), BlockWielandiellaTop.block.getDefaultState(), 3);
			}
		}
	}
