...
public class StructureSpawnMahogany extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnMahogany(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimMahogany))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genMahogany && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genMahoganyBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genMahoganyOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierMahogany;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightMahogany;
				int minheight = LepidodendronConfigPlants.minheightMahogany;
				...
				int z = spawnTo.getZ();
				{
					...
					int BareTrunk = 7 + world.rand.nextInt(5);

					for (int log = 0; log<BareTrunk; log++) {
						world.setBlockState(spawnTo.up(log), BlockMahoganyLog.block.getDefaultState().withProperty(BlockMahoganyLog.BlockCustom.FACING, EnumFacing.NORTH), 3);
					}
					...
					ProcedureWorldGenMahogany.executeProcedure(world, spawnTo.up(BareTrunk), TreeHeight);
				}
