...
public class StructureSpawnMonanthesia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnMonanthesia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimMonanthesia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genMonanthesia && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genMonanthesiaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genMonanthesiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 17000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierMonanthesia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int yct;
				int zct;
				while ((xct <= 3) && (!proximityCriteria)) {
					yct = 0;
					while ((yct <= 2) && (!proximityCriteria)) {
						zct = -3;
						while ((zct <= 3) && (!proximityCriteria)) {
							if ((world.getBlockState(new BlockPos(i + xct, j + yct, k + zct))).getBlock() == BlockMonanthesiaLog.block) {
								if (Math.abs(xct) + Math.abs(zct) < 5) proximityCriteria = true;
							}
							zct = zct + 1;
						}
						yct = yct + 1;
				...
				int maxheight = LepidodendronConfigPlants.maxheightMonanthesia;
				int minheight = LepidodendronConfigPlants.minheightMonanthesia;
