...
public class StructureSpawnChloranthus extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnChloranthus(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimChloranthus))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genChloranthus && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genChloranthusBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genChloranthusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 48000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierChloranthus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightChloranthus;
				int minheight = LepidodendronConfigPlants.minheightChloranthus;
