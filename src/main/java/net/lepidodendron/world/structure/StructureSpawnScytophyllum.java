...
import net.lepidodendron.block.BlockScytophyllumTop;
...
public class StructureSpawnScytophyllum extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnScytophyllum(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimScytophyllum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genScytophyllum && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genScytophyllumBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genScytophyllumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 18000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierScytophyllum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightScytophyllum;
				int minheight = LepidodendronConfigPlants.minheightScytophyllum;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockScytophyllum.block.getDefaultState(), 3);
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.up(), BlockScytophyllumTop.block.getDefaultState(), 3);
			}
		}
	}
