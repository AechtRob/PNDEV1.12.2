...
public class StructureSpawnBrachyphyllum extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnBrachyphyllum(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimBrachyphyllum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genBrachyphyllum && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genBrachyphyllumBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genBrachyphyllumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 6000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierBrachyphyllum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightBrachyphyllum;
				int minheight = LepidodendronConfigPlants.minheightBrachyphyllum;
