...
public class StructureSpawnBrachyphyllum extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimBrachyphyllum))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genBrachyphyllum && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genBrachyphyllumBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genBrachyphyllumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 6000;
		double GenMultiplier = LepidodendronConfig.multiplierBrachyphyllum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightBrachyphyllum;
				int minheight = LepidodendronConfig.minheightBrachyphyllum;
