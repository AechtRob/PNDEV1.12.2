...
public class StructureSpawnWaterHorsetail extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnWaterHorsetail(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimWaterHorsetail))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genWaterHorsetail && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genWaterHorsetailBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfig.genWaterHorsetailOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 50000;
		double GenMultiplier = LepidodendronConfig.multiplierWaterHorsetail;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightWaterHorsetail;
				int minheight = LepidodendronConfig.minheightWaterHorsetail;
				...
				int z = spawnTo.getZ();
				{
					...
					world.setBlockState(spawnTo, BlockWaterHorsetail.block.getDefaultState(), 3);
