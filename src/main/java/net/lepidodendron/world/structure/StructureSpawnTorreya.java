...
import net.lepidodendron.procedure.ProcedureWorldGenTorreya;
...
public class StructureSpawnTorreya extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnTorreya(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimTorreya))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genTorreya && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genTorreyaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genTorreyaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 12000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierTorreya;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightTorreya;
				int minheight = LepidodendronConfigPlants.minheightTorreya;
				...
				int z = spawnTo.getZ();
				{
					...
					ProcedureWorldGenTorreya.executeProcedure($_dependencies);
				}
