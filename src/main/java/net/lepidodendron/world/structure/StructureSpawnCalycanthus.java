...
public class StructureSpawnCalycanthus extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnCalycanthus(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimCalycanthus))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genCalycanthus && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genCalycanthusBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genCalycanthusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 16000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierCalycanthus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightCalycanthus;
				int minheight = LepidodendronConfigPlants.minheightCalycanthus;
