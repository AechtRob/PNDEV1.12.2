...
import net.lepidodendron.procedure.ProcedureWorldGenHymenaea;
...
public class StructureSpawnHymenaea extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnHymenaea(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimHymenaea))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genHymenaea && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genHymenaeaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genHymenaeaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 20000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierHymenaea;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				if ((world.getBlockState(new BlockPos(i, j, k))).getMaterial() == Material.GROUND)
					blockCriteria = true;
				if (((world.getBlockState(new BlockPos(i, j, k))).getMaterial() == Material.SAND) && (!BiomeDictionary.hasType(biome, BiomeDictionary.Type.MESA)))
					blockCriteria = true;
				if (!blockCriteria)
					continue;
		
				int maxheight = LepidodendronConfigPlants.maxheightHymenaea;
				int minheight = LepidodendronConfigPlants.minheightHymenaea;
...

				{
					...
					ProcedureWorldGenHymenaea.executeProcedure($_dependencies);
				}
