...
public class StructureSpawnIsoetes extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genIsoetesBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genIsoetesOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
					
		int GenChance = 50000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierIsoetes;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightIsoetes;
				int minheight = LepidodendronConfigPlants.minheightIsoetes;
				...
				int z = spawnTo.getZ();
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockIsoetes.block.getDefaultState(), 3);
