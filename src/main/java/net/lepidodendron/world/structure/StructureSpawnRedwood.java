...
public class StructureSpawnRedwood extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnRedwood(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!LepidodendronConfigPlants.genRedwood && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				boolean SaplingSpawn = false;
				{
					...
					ProcedureChooseRedWoodTypeWorldGen.executeProcedure($_dependencies);
				}
