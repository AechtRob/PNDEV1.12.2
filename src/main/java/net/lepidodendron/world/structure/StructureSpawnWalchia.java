...
public class StructureSpawnWalchia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnWalchia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimWalchia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genWalchia && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genWalchiaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genWalchiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierWalchia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightWalchia;
				int minheight = LepidodendronConfigPlants.minheightWalchia;
