...
public class StructureSpawnCedar extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnCedar(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimCedar))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genCedar && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genCedarBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genCedarOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 17500;
		double GenMultiplier = LepidodendronConfigPlants.multiplierCedar;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightCedar;
				int minheight = LepidodendronConfigPlants.minheightCedar;
