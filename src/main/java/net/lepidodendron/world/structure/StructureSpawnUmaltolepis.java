...
public class StructureSpawnUmaltolepis extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnUmaltolepis(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimUmaltolepis))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genUmaltolepis && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genUmaltolepisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genUmaltolepisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 80000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierUmaltolepis;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightUmaltolepis;
				int minheight = LepidodendronConfigPlants.minheightUmaltolepis;
				...
				int z = spawnTo.getZ();
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockUmaltolepis.block.getDefaultState(), 3);
				}
			}
		}
	}
