...
public class StructureSpawnWachtleria extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnWachtleria(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimWachtleria))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genWachtleria && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genWachtleriaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genWachtleriaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 32000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierWachtleria;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightWachtleria;
				int minheight = LepidodendronConfigPlants.minheightWachtleria;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockWachtleria.block.getDefaultState(), 3);
			}
		}
	}
