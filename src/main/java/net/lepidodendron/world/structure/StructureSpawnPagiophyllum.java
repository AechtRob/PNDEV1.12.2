...
public class StructureSpawnPagiophyllum extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnPagiophyllum(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPagiophyllum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPagiophyllum && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genPagiophyllumBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPagiophyllumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 28000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierPagiophyllum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightPagiophyllum;
				int minheight = LepidodendronConfigPlants.minheightPagiophyllum;
				...
				if ((!canSurviveAt(world, new BlockPos(i, j + 1, k)))
					&& (!BlockPagiophyllumLand.BlockCustom.canSurviveAt(world, new BlockPos(i, j + 1, k))))
					continue;
				...
				int z = spawnTo.getZ();
				{
					...
					if (canSurviveAt(world, spawnTo)) {
						world.setBlockState(spawnTo, BlockPagiophyllumWater.block.getDefaultState(), 3);
						BlockPagiophyllumWater.block.onBlockAdded(world, spawnTo, BlockPagiophyllumWater.block.getDefaultState());
					}
					else { //It must be the land version:
						world.setBlockState(spawnTo, BlockPagiophyllumLand.block.getDefaultState(), 3);
						BlockPagiophyllumLand.block.onBlockAdded(world, spawnTo, BlockPagiophyllumLand.block.getDefaultState());
					}
				}
