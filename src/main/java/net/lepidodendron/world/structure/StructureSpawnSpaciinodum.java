...
public class StructureSpawnSpaciinodum extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSpaciinodum(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSpaciinodum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSpaciinodum && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSpaciinodumBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSpaciinodumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 10000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSpaciinodum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSpaciinodum;
				int minheight = LepidodendronConfigPlants.minheightSpaciinodum;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSpaciinodum.block.getDefaultState(), 3);
			}
		}
	}
