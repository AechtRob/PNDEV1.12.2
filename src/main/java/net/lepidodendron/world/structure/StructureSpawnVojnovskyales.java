...
public class StructureSpawnVojnovskyales extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnVojnovskyales(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimVojnovskyales))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genVojnovskyales && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genVojnovskyalesBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genVojnovskyalesOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 80000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierVojnovskyales;
		...
		//System.err.println("GenChance: " + GenChance);
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				//System.err.println("Trying: " + i + " " + j + " " + k);
				...
				int maxheight = LepidodendronConfigPlants.maxheightVojnovskyales;
				int minheight = LepidodendronConfigPlants.minheightVojnovskyales;
				...
				BlockPos spawnTo = new BlockPos(i, j + 1, k);
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockVojnovskyales.block.getDefaultState());
				}
