...
public class StructureSpawnJianchangia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnJianchangia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimJianchangia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genJianchangia && (!LepidodendronConfig.genAllPlants))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genJianchangiaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genJianchangiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 50000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierJianchangia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightJianchangia;
				int minheight = LepidodendronConfigPlants.minheightJianchangia;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockJianchangia.block.getDefaultState(), 3);
			}
		}
	}
