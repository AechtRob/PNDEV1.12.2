...
public class StructureSpawnArlenea extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnArlenea(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimArlenea))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genArlenea && (!LepidodendronConfig.genAllPlants))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genArleneaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genArleneaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 50000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierArlenea;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightArlenea;
				int minheight = LepidodendronConfigPlants.minheightArlenea;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockArlenea.block.getDefaultState(), 3);
			}
		}
	}
