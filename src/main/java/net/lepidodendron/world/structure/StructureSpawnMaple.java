...
public class StructureSpawnMaple extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnMaple(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimMaple))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genMaple && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genMapleBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genMapleOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 25000;
		double GenMultiplier = LepidodendronConfig.multiplierMaple;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightMaple;
				int minheight = LepidodendronConfig.minheightMaple;
				...
				int z = spawnTo.getZ();
				{
					...
					int TreeHeight = 6 + random.nextInt(12);
					if (TreeHeight > 8 && Math.random() > 0.5) {
						TreeHeight = TreeHeight = 5 + random.nextInt(6);
					}
