...
public class StructureSpawnMaple extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genMapleBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genMapleOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 25000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierMaple;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightMaple;
				int minheight = LepidodendronConfigPlants.minheightMaple;
