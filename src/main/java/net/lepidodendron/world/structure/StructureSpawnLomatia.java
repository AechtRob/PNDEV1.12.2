...
public class StructureSpawnLomatia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnLomatia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimLomatia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genLomatia && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genLomatiaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genLomatiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 11000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierLomatia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightLomatia;
				int minheight = LepidodendronConfigPlants.minheightLomatia;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockLomatia.block.getDefaultState(), 3);
				BlockLomatia.block.onBlockAdded(world, spawnTo, BlockLomatia.block.getDefaultState());
			}
		}
	}
