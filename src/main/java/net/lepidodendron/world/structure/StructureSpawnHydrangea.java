...
public class StructureSpawnHydrangea extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnHydrangea(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimHydrangea))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genHydrangea && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genHydrangeaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genHydrangeaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 18000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierHydrangea;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightHydrangea;
				int minheight = LepidodendronConfigPlants.minheightHydrangea;
					...
					if ((world.canSeeSky(new BlockPos((int) x, (int) y, (int) z)))) {			
						...
						if (block.canBeReplacedByLeaves(world.getBlockState(new BlockPos((int) x, (int) (y), (int) z)), world, new BlockPos((int) x, (int) (y), (int) z))) {
							...
							if (block.canBeReplacedByLeaves(world.getBlockState(new BlockPos((int) x, (int) (y + 1), (int) z)), world, new BlockPos((int) x, (int) (y + 1), (int) z))) {
								Functions.setBlockStateAndCheckForDoublePlant(world,new BlockPos((int) x, (int) (y), (int) z), BlockHydrangea.block.getDefaultState(), 3);
							}
						}
					}
				}
