...
public class StructureSpawnGangamopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnGangamopteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimGangamopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genGangamopteris && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genGangamopterisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genGangamopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 25000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierGangamopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightGangamopteris;
				int minheight = LepidodendronConfigPlants.minheightGangamopteris;
				...
				int z = spawnTo.getZ();
				{
					...
					ProcedureWorldGenGangamopteris.executeProcedure($_dependencies);
				}
