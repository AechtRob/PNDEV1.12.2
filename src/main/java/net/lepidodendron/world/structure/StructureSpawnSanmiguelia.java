...
public class StructureSpawnSanmiguelia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSanmiguelia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSanmiguelia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSanmiguelia && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSanmigueliaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSanmigueliaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 70000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSanmiguelia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSanmiguelia;
				int minheight = LepidodendronConfigPlants.minheightSanmiguelia;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSanmiguelia.block.getDefaultState(), 3);
				BlockSanmiguelia.block.onBlockAdded(world, spawnTo, BlockSanmiguelia.block.getDefaultState());
