...
public class StructureSpawnSkaaripteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSkaaripteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSkaaripteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSkaaripteris && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSkaaripterisBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSkaaripterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 32500;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSkaaripteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSkaaripteris;
				int minheight = LepidodendronConfigPlants.minheightSkaaripteris;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSkaaripteris.block.getDefaultState(), 3);
			}
		}
	}
