...
public class StructureSpawnTodites extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnTodites(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimTodites))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genTodites && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genToditesBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genToditesOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierTodites;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightTodites;
				int minheight = LepidodendronConfigPlants.minheightTodites;
