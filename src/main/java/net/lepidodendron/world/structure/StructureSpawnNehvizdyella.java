...
public class StructureSpawnNehvizdyella extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnNehvizdyella(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimNehvizdyella))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genNehvizdyella && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genNehvizdyellaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genNehvizdyellaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 20000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierNehvizdyella;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightNehvizdyella;
				int minheight = LepidodendronConfigPlants.minheightNehvizdyella;
				...
				if (!shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSphenophyllales))
					vines = false;
				//if (!LepidodendronConfigPlants.genSphenophyllalesNehvizdyella && !LepidodendronConfig.genAllPlants)
				//	vines = false;
				...
				if (!shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimCallistophytales))
					vines2 = false;
				//if (!LepidodendronConfigPlants.genCallistophytalesNehvizdyella && !LepidodendronConfig.genAllPlants)
				//	vines2 = false;
				...
				int z = spawnTo.getZ();
				{
					...
					ProcedureWorldGenNehvizdyella.executeProcedure($_dependencies);
				}
