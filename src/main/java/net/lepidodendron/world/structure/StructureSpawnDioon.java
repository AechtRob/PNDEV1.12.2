...
public class StructureSpawnDioon extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnDioon(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimDioon))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genDioon && !LepidodendronConfig.genAllPlants)
		...
		if (matchBiome(biome, LepidodendronConfig.genDioonOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 12000;
		double GenMultiplier = LepidodendronConfig.multiplierDioon;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				while ((xct <= 1) && (!proximityCriteria)) {
					...
					while ((yct <= 2) && (!proximityCriteria)) {
						...
						while ((zct <= 1) && (!proximityCriteria)) {
							if ((world.getBlockState(new BlockPos(i + xct, j + yct, k + zct))).getBlock() == BlockDioonLog.block) {
				...
				int maxheight = LepidodendronConfig.maxheightDioon;
				int minheight = LepidodendronConfig.minheightDioon;
