...
public class StructureSpawnDioon extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genDioonOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 12000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierDioon;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightDioon;
				int minheight = LepidodendronConfigPlants.minheightDioon;
