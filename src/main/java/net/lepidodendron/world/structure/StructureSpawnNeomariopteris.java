...
public class StructureSpawnNeomariopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnNeomariopteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimNeomariopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genNeomariopteris && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genNeomariopterisBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genNeomariopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierNeomariopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightNeomariopteris;
				int minheight = LepidodendronConfigPlants.minheightNeomariopteris;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockNeomariopteris.block.getDefaultState(), 3);
			}
		}
	}
