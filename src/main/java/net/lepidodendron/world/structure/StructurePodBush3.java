...
public class StructurePodBush3 extends ElementsLepidodendronMod.ModElement {
	public StructurePodBush3(ElementsLepidodendronMod instance) {
		super(instance, 44);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimPodocarp))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genPodocarpBush && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genPodocarpBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genPodocarpOverrideBiomes))
			biomeCriteria = true;
		...
		int GenChance = 17000;
		double GenMultiplier = LepidodendronConfig.multiplierPodocarp;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightPodocarp;
				int minheight = LepidodendronConfig.minheightPodocarp;
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("lepidodendron", "podbush3"));
				if (template == null)
					return;
