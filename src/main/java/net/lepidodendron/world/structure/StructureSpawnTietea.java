...
import net.lepidodendron.procedure.ProcedureWorldGenTietea;
...
public class StructureSpawnTietea extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnTietea(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimTietea))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genTietea && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genTieteaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genTieteaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 20000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierTietea;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightTietea;
				int minheight = LepidodendronConfigPlants.minheightTietea;
				...
				//System.err.println("Spawn at " + x + " " + y + " " + z);
				{
					...
					ProcedureWorldGenTietea.executeProcedure($_dependencies);
				}
