...
public class StructureSpawnGlossophyllum extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnGlossophyllum(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimGlossophyllum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genGlossophyllum && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genGlossophyllumBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genGlossophyllumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierGlossophyllum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightGlossophyllum;
				int minheight = LepidodendronConfigPlants.minheightGlossophyllum;
