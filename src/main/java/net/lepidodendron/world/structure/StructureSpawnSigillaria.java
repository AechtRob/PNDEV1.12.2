...
public class StructureSpawnSigillaria extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSigillaria(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSigillaria))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSigillaria && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSigillariaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSigillariaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSigillaria;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSigillaria;
				int minheight = LepidodendronConfigPlants.minheightSigillaria;
