...
public class StructureSpawnLaurus extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnLaurus(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimLaurus))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genLaurus && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genLaurusBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genLaurusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 18000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierLaurus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightLaurus;
				int minheight = LepidodendronConfigPlants.minheightLaurus;
				...
				int z = spawnTo.getZ();
				{
					...
					int TreeHeight = 3 + world.rand.nextInt(7);
