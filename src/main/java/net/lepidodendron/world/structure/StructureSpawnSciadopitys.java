...
public class StructureSpawnSciadopitys extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSciadopitys(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimSciadopitys))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genSciadopitys && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genSciadopitysBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genSciadopitysOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 14000;
		double GenMultiplier = LepidodendronConfig.multiplierSciadopitys;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightSciadopitys;
				int minheight = LepidodendronConfig.minheightSciadopitys;
