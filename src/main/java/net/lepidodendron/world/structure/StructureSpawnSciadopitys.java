...
public class StructureSpawnSciadopitys extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSciadopitysBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSciadopitysOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 14000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSciadopitys;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSciadopitys;
				int minheight = LepidodendronConfigPlants.minheightSciadopitys;
