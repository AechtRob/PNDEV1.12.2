...
public class StructureSpawnArchaefructus extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnArchaefructus(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimArchaefructus))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genArchaefructus && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genArchaefructusBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genArchaefructusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 30000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierArchaefructus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightArchaefructus;
				int minheight = LepidodendronConfigPlants.minheightArchaefructus;
