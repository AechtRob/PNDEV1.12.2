...
public class StructureSpawnErdtmanithecales extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnErdtmanithecales(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimErdtmanithecales))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genErdtmanithecales && (!LepidodendronConfig.genAllPlants))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genErdtmanithecalesBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genErdtmanithecalesOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 50000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierErdtmanithecales;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightErdtmanithecales;
				int minheight = LepidodendronConfigPlants.minheightErdtmanithecales;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockErdtmanithecales.block.getDefaultState(), 3);
			}
		}
	}
