...
public class StructureSpawnThuja extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnThuja(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimThuja))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genThuja && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genThujaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genThujaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierThuja;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightThuja;
				int minheight = LepidodendronConfigPlants.minheightThuja;
