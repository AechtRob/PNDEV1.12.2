...
public class StructureSpawnAridPine extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnAridPine(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimAridPine))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genAridPine && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genAridPineBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genAridPineOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 6000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierAridPine;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightAridPine;
				int minheight = LepidodendronConfigPlants.minheightAridPine;
