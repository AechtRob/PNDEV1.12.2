...
public class StructureSpawnPertica extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnPertica(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimPertica))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genPertica && !LepidodendronConfig.genAllPlants)
		...
		if (matchBiome(biome, LepidodendronConfig.genPerticaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfig.multiplierPertica;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightPertica;
				int minheight = LepidodendronConfig.minheightPertica;
