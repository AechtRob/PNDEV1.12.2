...
public class StructureSpawnWelwitschiophyllum extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnWelwitschiophyllum(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimWelwitschiophyllum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genWelwitschiophyllum && (!LepidodendronConfig.genAllPlants))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genWelwitschiophyllumBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genWelwitschiophyllumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 13000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierWelwitschiophyllum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightWelwitschiophyllum;
				int minheight = LepidodendronConfigPlants.minheightWelwitschiophyllum;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockWelwitschiophyllum.block.getDefaultState(), 3);
			}
		}
	}
