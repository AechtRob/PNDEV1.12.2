...
public class StructureSpawnSphenopterisFern extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSphenopterisFern(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSphenopterisFern))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSphenopterisFern && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSphenopterisFernBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSphenopterisFernOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 28000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSphenopterisFern;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSphenopterisFern;
				int minheight = LepidodendronConfigPlants.minheightSphenopterisFern;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSphenopterisFern.block.getDefaultState(), 3);
			}
		}
	}
