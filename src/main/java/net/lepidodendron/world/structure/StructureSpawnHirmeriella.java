...
public class StructureSpawnHirmeriella extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnHirmeriella(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimHirmeriella))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genHirmeriella && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genHirmeriellaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genHirmeriellaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 6000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierHirmeriella;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightHirmeriella;
				int minheight = LepidodendronConfigPlants.minheightHirmeriella;
