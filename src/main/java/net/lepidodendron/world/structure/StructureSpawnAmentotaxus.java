...
public class StructureSpawnAmentotaxus extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnAmentotaxus(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genAmentotaxusBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genAmentotaxusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 12000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierAmentotaxus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightAmentotaxus;
				int minheight = LepidodendronConfigPlants.minheightAmentotaxus;
