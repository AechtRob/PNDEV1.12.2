...
public class StructureSpawnAraucarites extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnAraucarites(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimAraucarites))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genAraucarites && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genAraucaritesBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genAraucaritesOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 11000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierAraucarites;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightAraucarites;
				int minheight = LepidodendronConfigPlants.minheightAraucarites;
