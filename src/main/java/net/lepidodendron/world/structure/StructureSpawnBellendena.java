...
import net.lepidodendron.block.BlockBellendenaSeeds;
...
public class StructureSpawnBellendena extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnBellendena(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimBellendena))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genBellendena && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genBellendenaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genBellendenaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 11000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierBellendena;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightBellendena;
				int minheight = LepidodendronConfigPlants.minheightBellendena;
				...
				if (random.nextInt(3 ) == 0) {
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockBellendena.block.getDefaultState(), 3);
				}
				else if (random.nextInt(2) == 0) {
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockBellendenaSeeds.block.getDefaultState(), 3);
				}
				else {
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockBellendena.block.getDefaultState().withProperty(BlockBellendena.BlockCustom.FLOWERS, true), 3);
				}
			}
		}
	}
