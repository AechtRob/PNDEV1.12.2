...
public class StructureSpawnRufloria extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnRufloria(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimRufloria))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genRufloria && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genRufloriaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genRufloriaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierRufloria;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightRufloria;
				int minheight = LepidodendronConfigPlants.minheightRufloria;
