...
public class StructureSpawnUtrechtia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnUtrechtia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimUtrechtia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genUtrechtia && !LepidodendronConfig.genAllPlants)
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genUtrechtiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 20000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierUtrechtia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightUtrechtia;
				int minheight = LepidodendronConfigPlants.minheightUtrechtia;
				...
				if (Math.random() > 0.5) {
					if (Math.random() > 0.3) {
						ProcedureTreeLog.executeProcedure((int) i, (int) j + 1, (int) k, world, BlockUtrechtiaLeavesPlaceable.block, EnumFacing.UP);
						ProcedureTreeLog.executeProcedure((int) i, (int) j + 2, (int) k, world, BlockUtrechtiaLeavesTop.block, EnumFacing.UP);
						}
					else {
						ProcedureTreeLog.executeProcedure((int) i, (int) j + 1, (int) k, world, BlockUtrechtiaLeavesSmallPlaceable.block, EnumFacing.UP);
					}
				}
				else {
