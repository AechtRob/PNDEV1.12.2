...
public class StructureSpawnValmeyerodendron extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimValmeyerodendron))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genValmeyerodendron && !LepidodendronConfig.genAllPlants)
		...
		if (matchBiome(biome, LepidodendronConfig.genValmeyerodendronOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfig.multiplierValmeyerodendron;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightValmeyerodendron;
				int minheight = LepidodendronConfig.minheightValmeyerodendron;
