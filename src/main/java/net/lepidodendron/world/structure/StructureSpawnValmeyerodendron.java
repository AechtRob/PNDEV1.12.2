...
public class StructureSpawnValmeyerodendron extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnValmeyerodendron(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimValmeyerodendron))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genValmeyerodendron && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genValmeyerodendronBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genValmeyerodendronOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierValmeyerodendron;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightValmeyerodendron;
				int minheight = LepidodendronConfigPlants.minheightValmeyerodendron;
				...
				//System.err.println("Spawn at " + x + " " + y + " " + z);
				{
					...
					ProcedureWorldGenValmeyerodendron.executeProcedure($_dependencies);
				}
