...
public class StructureSpawnDrewria extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnDrewria(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimDrewria))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genDrewria && (!LepidodendronConfig.genAllPlants))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genDrewriaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genDrewriaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 50000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierDrewria;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightDrewria;
				int minheight = LepidodendronConfigPlants.minheightDrewria;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockDrewria.block.getDefaultState(), 3);
			}
		}
	}
