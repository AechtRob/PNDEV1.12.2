...
public class StructureSpawnPachypteris extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPachypteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPachypteris && !LepidodendronConfig.genAllPlants)
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPachypterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				*/
				...
				int maxheight = LepidodendronConfigPlants.maxheightPachypteris;
				int minheight = LepidodendronConfigPlants.minheightPachypteris;
