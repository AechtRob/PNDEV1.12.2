...
public class StructureSpawnOnychiopsis extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnOnychiopsis(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimOnychiopsis))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genOnychiopsis && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genOnychiopsisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genOnychiopsisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 70000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierOnychiopsis;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightOnychiopsis;
				int minheight = LepidodendronConfigPlants.minheightOnychiopsis;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockOnychiopsis.block.getDefaultState(), 3);
				BlockOnychiopsis.block.onBlockAdded(world, spawnTo, BlockOnychiopsis.block.getDefaultState());
