...
public class StructureSpawnTwiggyCypress extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnTwiggyCypress(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimTwiggyCypress))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genTwiggyCypress && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genTwiggyCypressBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genTwiggyCypressOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 30000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierTwiggyCypress;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightTwiggyCypress;
				int minheight = LepidodendronConfigPlants.minheightTwiggyCypress;
