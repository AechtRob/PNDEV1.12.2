...
public class StructureSpawnPodozamites extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimPodozamites))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genPodozamites && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genPodozamitesBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genPodozamitesOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 17500;
		double GenMultiplier = LepidodendronConfig.multiplierPodozamites;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightPodozamites;
				int minheight = LepidodendronConfig.minheightPodozamites;
