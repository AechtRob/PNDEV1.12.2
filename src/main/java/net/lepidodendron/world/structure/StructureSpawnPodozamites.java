...
public class StructureSpawnPodozamites extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnPodozamites(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPodozamites))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPodozamites && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genPodozamitesBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPodozamitesOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 17500;
		double GenMultiplier = LepidodendronConfigPlants.multiplierPodozamites;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				//Allow to spawn IN water up to one block deep ONLY:
				...
				int maxheight = LepidodendronConfigPlants.maxheightPodozamites;
				int minheight = LepidodendronConfigPlants.minheightPodozamites;
				...
				int y = spawnTo.getY();
				int z = spawnTo.getZ();
				//System.err.println("Spawned Podozamites: " + x + " " + y + " " + z);
				{
