...
public class StructureSpawnShrubbyCycad extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnShrubbyCycad(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimShrubbyCycad))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genShrubbyCycad && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genShrubbyCycadBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genShrubbyCycadOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 10000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierShrubbyCycad;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightShrubbyCycad;
				int minheight = LepidodendronConfigPlants.minheightShrubbyCycad;
