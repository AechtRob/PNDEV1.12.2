...
public class StructureSpawnPietzschia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnPietzschia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPietzschia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPietzschia && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genPietzschiaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPietzschiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 12000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierPietzschia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightPietzschia;
				int minheight = LepidodendronConfigPlants.minheightPietzschia;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockPietzschia.block.getDefaultState(), 3);
				BlockPietzschia.block.onBlockAdded(world, spawnTo, BlockPietzschia.block.getDefaultState());
