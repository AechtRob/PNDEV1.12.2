...
public class StructureSpawnQuasistrobus extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimQuasistrobus))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genQuasistrobus && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genQuasistrobusBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genQuasistrobusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 25000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierQuasistrobus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightQuasistrobus;
				int minheight = LepidodendronConfigPlants.minheightQuasistrobus;
