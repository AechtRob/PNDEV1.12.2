...
import net.lepidodendron.block.BlockNupharLeaves;
import net.lepidodendron.block.BlockNupharStem;
...
public class StructureSpawnNuphar extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnNuphar(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimNuphar))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genNuphar && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genNupharBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genNupharOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierNuphar;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightNuphar;
				int minheight = LepidodendronConfigPlants.minheightNuphar;
				...
				int z = spawnTo.getZ();
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockNuphar.block.getDefaultState(), 3);
			    	...
			    	if (isWaterBlock(world, spawnTo.north())) {
				    	while (colWater < 6 && isWaterBlock(world, spawnTo.north().up(colWater))) {
				    		...
				    		{
				    			Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.north().up(colWater), BlockNupharStem.block.getDefaultState(), 3);
				    		}
				    		colWater = colWater + 1;
				    	}
				    	if (canLeavesPlaceAt(world, spawnTo.north().up(colWater))) {
				    		Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.north().up(colWater), BlockNupharLeaves.block.getDefaultState(), 3);
				    	}
			    	}
			    	...
			    	if (isWaterBlock(world, spawnTo.south())) {
				    	while (colWater < 6 && isWaterBlock(world, spawnTo.south().up(colWater))) {
				    		...
				    		{
				    			Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.south().up(colWater), BlockNupharStem.block.getDefaultState(), 3);
				    		}
				    		colWater = colWater + 1;
				    	}
				    	if (canLeavesPlaceAt(world, spawnTo.south().up(colWater))) {
				    		Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.south().up(colWater), BlockNupharLeaves.block.getDefaultState(), 3);
				    	}
			    	}
			    	...
			    	if (isWaterBlock(world, spawnTo.east())) {
				    	while (colWater < 6 && isWaterBlock(world, spawnTo.east().up(colWater))) {
				    		...
				    		{
				    			Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.east().up(colWater), BlockNupharStem.block.getDefaultState(), 3);
				    		}
				    		colWater = colWater + 1;
				    	}
				    	if (canLeavesPlaceAt(world, spawnTo.east().up(colWater))) {
				    		Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.east().up(colWater), BlockNupharLeaves.block.getDefaultState(), 3);
				    	}
			    	}
			    	...
			    	if (isWaterBlock(world, spawnTo.west())) {
				    	while (colWater < 6 && isWaterBlock(world, spawnTo.west().up(colWater))) {
				    		...
				    		{
				    			Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.west().up(colWater), BlockNupharStem.block.getDefaultState(), 3);
				    		}
				    		colWater = colWater + 1;
				    	}
				    	if (canLeavesPlaceAt(world, spawnTo.west().up(colWater))) {
				    		Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.west().up(colWater), BlockNupharLeaves.block.getDefaultState(), 3);
				    	}
			    	}
