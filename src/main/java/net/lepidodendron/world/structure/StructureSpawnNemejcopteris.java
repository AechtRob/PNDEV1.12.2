...
public class StructureSpawnNemejcopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnNemejcopteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimNemejcopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genNemejcopteris && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genNemejcopterisBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genNemejcopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 28000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierNemejcopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightNemejcopteris;
				int minheight = LepidodendronConfigPlants.minheightNemejcopteris;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockNemejcopteris.block.getDefaultState(), 3);
			}
		}
	}
