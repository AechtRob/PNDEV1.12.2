...
public class StructureSpawnSahnioxylon extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimSahnioxylon))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genSahnioxylon && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genSahnioxylonBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genSahnioxylonOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 75000;
		double GenMultiplier = LepidodendronConfig.multiplierSahnioxylon;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightSahnioxylon;
				int minheight = LepidodendronConfig.minheightSahnioxylon;
