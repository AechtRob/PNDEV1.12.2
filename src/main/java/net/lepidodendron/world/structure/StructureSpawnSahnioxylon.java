...
public class StructureSpawnSahnioxylon extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSahnioxylon(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSahnioxylon))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSahnioxylon && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSahnioxylonBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSahnioxylonOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 75000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSahnioxylon;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSahnioxylon;
				int minheight = LepidodendronConfigPlants.minheightSahnioxylon;
				...
				int z = spawnTo.getZ();
				{
					...
					ProcedureWorldGenSahnioxylon.executeProcedure($_dependencies);
				}
			}
		}
	}
