...
public class StructureSpawnTodea extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnTodea(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimTodea))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genTodea && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genTodeaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genTodeaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 19000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierTodea;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightTodea;
				int minheight = LepidodendronConfigPlants.minheightTodea;
				...
				int z = spawnTo.getZ();
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockTodea.block.getDefaultState());
					BlockTodea.block.onBlockAdded(world, spawnTo, BlockTodea.block.getDefaultState());
