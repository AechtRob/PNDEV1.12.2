...
public class StructureSpawnTicoa extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnTicoa(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimTicoa))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genTicoa && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genTicoaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genTicoaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierTicoa;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightTicoa;
				int minheight = LepidodendronConfigPlants.minheightTicoa;
