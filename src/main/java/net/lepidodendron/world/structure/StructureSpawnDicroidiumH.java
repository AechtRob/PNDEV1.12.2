...
public class StructureSpawnDicroidiumH extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnDicroidiumH(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimDicroidiumHughesii))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genDicroidiumHughesii && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genDicroidiumHughesiiBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genDicroidiumHughesiiOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 32000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierDicroidiumHughesii;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightDicroidiumHughesii;
				int minheight = LepidodendronConfigPlants.minheightDicroidiumHughesii;
