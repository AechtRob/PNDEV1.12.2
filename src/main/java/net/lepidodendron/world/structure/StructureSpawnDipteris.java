...
public class StructureSpawnDipteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnDipteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimDipteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genDipteris && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genDipterisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genDipterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 16000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierDipteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightDipteris;
				int minheight = LepidodendronConfigPlants.minheightDipteris;
...

				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockDipteris.block.getDefaultState().withProperty(BlockDirectional.FACING, facing), 3);
