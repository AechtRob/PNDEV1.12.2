...
public class StructureSpawnFurcula extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimFurcula))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genFurcula && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genFurculaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genFurculaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 23000;
		double GenMultiplier = LepidodendronConfig.multiplierFurcula;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightFurcula;
				int minheight = LepidodendronConfig.minheightFurcula;
