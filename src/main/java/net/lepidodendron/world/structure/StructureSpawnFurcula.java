...
public class StructureSpawnFurcula extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnFurcula(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimFurcula))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genFurcula && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genFurculaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genFurculaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 23000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierFurcula;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightFurcula;
				int minheight = LepidodendronConfigPlants.minheightFurcula;
