...
public class StructureSpawnDrimys extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnDrimys(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimDrimys))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genDrimys && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genDrimysBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genDrimysOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 19000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierDrimys;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightDrimys;
				int minheight = LepidodendronConfigPlants.minheightDrimys;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockDrimys.block.getDefaultState(), 3);
				BlockDrimys.block.onBlockAdded(world, spawnTo, BlockDrimys.block.getDefaultState());
			}
		}
	}
