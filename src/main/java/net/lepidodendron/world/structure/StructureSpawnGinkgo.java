...
public class StructureSpawnGinkgo extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genGinkgoBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genGinkgoOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 17500;
		double GenMultiplier = LepidodendronConfigPlants.multiplierGinkgo;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightGinkgo;
				int minheight = LepidodendronConfigPlants.minheightGinkgo;
