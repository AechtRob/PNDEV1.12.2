...
public class StructureSpawnGinkgo extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnGinkgo(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimGinkgo))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genGinkgo && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genGinkgoBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genGinkgoOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 17500;
		double GenMultiplier = LepidodendronConfig.multiplierGinkgo;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightGinkgo;
				int minheight = LepidodendronConfig.minheightGinkgo;
