...
public class StructureSpawnCordaites extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimCordaites))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genCordaites && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genCordaitesBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genCordaitesOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 20000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierCordaites;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightCordaites;
				int minheight = LepidodendronConfigPlants.minheightCordaites;
				...
				if (!shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSphenophyllales))
					vines = false;
				if (!LepidodendronConfigPlants.genSphenophyllalesCordaites && !LepidodendronConfig.genAllPlants)
					vines = false;
				...
				if (!shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimCallistophytales))
					vines2 = false;
				if (!LepidodendronConfigPlants.genCallistophytalesCordaites && !LepidodendronConfig.genAllPlants)
					vines2 = false;
