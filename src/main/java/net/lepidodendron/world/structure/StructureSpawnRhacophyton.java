...
public class StructureSpawnRhacophyton extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimRhacophyton))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genRhacophyton && !LepidodendronConfig.genAllPlants)
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genRhacophytonOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierRhacophyton;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightRhacophyton;
				int minheight = LepidodendronConfigPlants.minheightRhacophyton;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockRhacophyton.block.getDefaultState(), 3);
			}
		}
	}
