...
public class StructureSpawnRhacophyton extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnRhacophyton(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimRhacophyton))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genRhacophyton && !LepidodendronConfig.genAllPlants)
		...
		if (matchBiome(biome, LepidodendronConfig.genRhacophytonOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 35000;
		double GenMultiplier = LepidodendronConfig.multiplierRhacophyton;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightRhacophyton;
				int minheight = LepidodendronConfig.minheightRhacophyton;
				...
				world.setBlockState(spawnTo, BlockRhacophyton.block.getDefaultState(), 3);
			}
		}
	}
