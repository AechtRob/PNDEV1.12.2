...
public class StructureSpawnSphenopteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSphenopteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimSphenopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genSphenopteris && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genSphenopterisBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfig.genSphenopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 28000;
		double GenMultiplier = LepidodendronConfig.multiplierSphenopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightSphenopteris;
				int minheight = LepidodendronConfig.minheightSphenopteris;
				...
				world.setBlockState(spawnTo, BlockSphenopteris.block.getDefaultState(), 3);
			}
		}
	}
