...
public class StructureSpawnSphenopteris extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSphenopteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSphenopteris && !LepidodendronConfig.genAllPlants)
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSphenopterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 28000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSphenopteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSphenopteris;
				int minheight = LepidodendronConfigPlants.minheightSphenopteris;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSphenopteris.block.getDefaultState(), 3);
			}
		}
	}
