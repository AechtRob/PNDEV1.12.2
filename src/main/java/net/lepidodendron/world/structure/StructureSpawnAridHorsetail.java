...
public class StructureSpawnAridHorsetail extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genAridHorsetailBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genAridHorsetailOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 45000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierAridHorsetail;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightAridHorsetail;
				int minheight = LepidodendronConfigPlants.minheightAridHorsetail;
				...
				int z = spawnTo.getZ();
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockAridHorsetail.block.getDefaultState());
				}
			}
		}
	}
