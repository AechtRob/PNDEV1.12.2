...
public class StructureSpawnAridHorsetail extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnAridHorsetail(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimAridHorsetail))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genAridHorsetail && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genAridHorsetailBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genAridHorsetailOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 45000;
		double GenMultiplier = LepidodendronConfig.multiplierAridHorsetail;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightAridHorsetail;
				int minheight = LepidodendronConfig.minheightAridHorsetail;
				...
				int z = spawnTo.getZ();
				{
					...
					world.setBlockState(spawnTo, BlockAridHorsetail.block.getDefaultState());
				}
			}
		}
	}
