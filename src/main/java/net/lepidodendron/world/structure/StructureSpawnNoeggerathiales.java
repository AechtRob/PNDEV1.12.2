...
public class StructureSpawnNoeggerathiales extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnNoeggerathiales(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				if ((world.getBlockState(new BlockPos(i, j, k))).getMaterial() == Material.SAND)
					blockCriteria = true;//Check we are not next to another plant - the connected textures will spoil the look!
				...
				while ((xct <= 1) && (!proximityCriteria)) {
					...
					while ((yct <= 2) && (!proximityCriteria)) {
						...
						while ((zct <= 1) && (!proximityCriteria)) {
							if ((world.getBlockState(new BlockPos(i + xct, j + yct, k + zct))).getBlock() == BlockNoeggerathialesLog.block) {
