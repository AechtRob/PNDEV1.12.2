...
public class StructureSpawnPseudoctenis extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnPseudoctenis(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPseudoctenis))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPseudoctenis && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genPseudoctenisBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPseudoctenisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 16000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierPseudoctenis;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightPseudoctenis;
				int minheight = LepidodendronConfigPlants.minheightPseudoctenis;
