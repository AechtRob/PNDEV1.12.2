...
public class StructureSpawnButtercup extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnButtercup(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimButtercup))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genButtercup && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genButtercupBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genButtercupOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 100000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierButtercup;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightButtercup;
				int minheight = LepidodendronConfigPlants.minheightButtercup;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockButtercup.block.getDefaultState(), 3);
			}
		}
	}
