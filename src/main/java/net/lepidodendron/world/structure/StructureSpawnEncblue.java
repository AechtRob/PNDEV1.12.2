...
import net.lepidodendron.procedure.ProcedureWorldGenEncblue;
...
public class StructureSpawnEncblue extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnEncblue(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimBlueCycad))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genBlueCycad && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genBlueCycadBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfig.genBlueCycadOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfig.multiplierBlueCycad;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				while ((xct <= 1) && (!proximityCriteria)) {
					...
					while ((yct <= 2) && (!proximityCriteria)) {
						...
						while ((zct <= 1) && (!proximityCriteria)) {
							if ((world.getBlockState(new BlockPos(i + xct, j + yct, k + zct))).getBlock() == BlockEncblueLog.block) {
				...
				int maxheight = LepidodendronConfig.maxheightBlueCycad;
				int minheight = LepidodendronConfig.minheightBlueCycad;
				...
				
				{
					...
					ProcedureWorldGenEncblue.executeProcedure($_dependencies);
				}
