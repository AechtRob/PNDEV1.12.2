...
public class StructureSpawnLepidophloios extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnLepidophloios(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimLepidophloios))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genLepidophloios && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genLepidophloiosBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genLepidophloiosOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 40000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierLepidophloios;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightLepidophloios;
				int minheight = LepidodendronConfigPlants.minheightLepidophloios;
				...
				if (!shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSphenophyllales))
					vines = false;
				if (!LepidodendronConfigPlants.genSphenophyllalesLepidophloios && !LepidodendronConfig.genAllPlants)
					vines = false;
				...
				if (!shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimCallistophytales))
					vines2 = false;
				if (!LepidodendronConfigPlants.genCallistophytalesLepidophloios && !LepidodendronConfig.genAllPlants)
					vines2 = false;
