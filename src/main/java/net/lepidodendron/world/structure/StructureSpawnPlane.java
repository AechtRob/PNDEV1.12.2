...
public class StructureSpawnPlane extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnPlane(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPlane))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPlane && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genPlaneBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPlaneOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierPlane;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightPlane;
				int minheight = LepidodendronConfigPlants.minheightPlane;
				...
				int z = spawnTo.getZ();
				{
					...
					int TreeHeight = 20 + random.nextInt(18);
					if (TreeHeight > 30 && Math.random() > 0.5) {
						TreeHeight = 10 + random.nextInt(18);
					}
					ProcedureWorldGenPlane.executeProcedure(world, spawnTo, TreeHeight);
				}
