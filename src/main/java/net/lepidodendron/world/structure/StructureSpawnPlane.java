...
public class StructureSpawnPlane extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimPlane))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genPlane && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genPlaneBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genPlaneOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfig.multiplierPlane;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightPlane;
				int minheight = LepidodendronConfig.minheightPlane;
