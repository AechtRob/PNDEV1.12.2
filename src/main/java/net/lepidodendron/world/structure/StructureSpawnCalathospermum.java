...
public class StructureSpawnCalathospermum extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnCalathospermum(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimCalathospermum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genCalathospermum && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genCalathospermumBlacklistBiomes))) {
			biomeCriteria = true;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genCalathospermumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 18000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierCalathospermum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightCalathospermum;
				int minheight = LepidodendronConfigPlants.minheightCalathospermum;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockCalathospermum.block.getDefaultState(), 3);
			}
		}
	}
