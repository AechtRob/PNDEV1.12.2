...
public class StructureSpawnZamites extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnZamites(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimZamites))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genZamites && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genZamitesBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genZamitesOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 70000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierZamites;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightZamites;
				int minheight = LepidodendronConfigPlants.minheightZamites;
