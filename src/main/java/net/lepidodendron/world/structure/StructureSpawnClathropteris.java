...
public class StructureSpawnClathropteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnClathropteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimClathropteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genClathropteris && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genClathropterisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genClathropterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 70000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierClathropteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightClathropteris;
				int minheight = LepidodendronConfigPlants.minheightClathropteris;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockClathropteris.block.getDefaultState(), 3);
				BlockClathropteris.block.onBlockAdded(world, spawnTo, BlockClathropteris.block.getDefaultState());
