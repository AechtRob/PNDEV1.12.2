...
import net.lepidodendron.block.BlockMesodescoleaCone;
...
public class StructureSpawnMesodescolea extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnMesodescolea(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimMesodescolea))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genMesodescolea && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genMesodescoleaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genMesodescoleaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 16000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierMesodescolea;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				if ((world.getBlockState(new BlockPos(i, j, k))).getMaterial() == Material.GROUND)
					blockCriteria = true;
				if ((world.getBlockState(new BlockPos(i, j, k))).getMaterial() == Material.CLAY)
					blockCriteria = true;
					
				if (!blockCriteria)
					continue;
		
				int maxheight = LepidodendronConfigPlants.maxheightMesodescolea;
				int minheight = LepidodendronConfigPlants.minheightMesodescolea;
				...
				
				{
					...
					if (ii == 0) {
						Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockMesodescoleaCone.block.getDefaultState());
					}
					else {
						Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockMesodescolea.block.getDefaultState());
					}
				}
