...
public class StructureSpawnLophosoria extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnLophosoria(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimLophosoria))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genLophosoria && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genLophosoriaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genLophosoriaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 10000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierLophosoria;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightLophosoria;
				int minheight = LepidodendronConfigPlants.minheightLophosoria;
