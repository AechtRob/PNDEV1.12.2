...
public class StructureSpawnOrtiseia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnOrtiseia(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimOrtiseia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genOrtiseia && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genOrtiseiaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genOrtiseiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 80000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierOrtiseia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightOrtiseia;
				int minheight = LepidodendronConfigPlants.minheightOrtiseia;
				...
				BlockPos spawnTo = new BlockPos(i, j + 1, k);
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockOrtiseia.block.getDefaultState());
				}
