...
public class StructureSpawnIlicium extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnIlicium(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimIlicium))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genIlicium && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genIliciumBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genIliciumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 26000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierIlicium;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightIlicium;
				int minheight = LepidodendronConfigPlants.minheightIlicium;
