...
public class StructureSpawnArchaeanthus extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnArchaeanthus(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimArchaeanthus))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genArchaeanthus && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genArchaeanthusBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genArchaeanthusOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 10000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierArchaeanthus;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightArchaeanthus;
				int minheight = LepidodendronConfigPlants.minheightArchaeanthus;
