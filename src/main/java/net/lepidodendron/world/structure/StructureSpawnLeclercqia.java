...
public class StructureSpawnLeclercqia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnLeclercqia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimLeclercqia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genLeclercqia && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genLeclercqiaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genLeclercqiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 30000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierLeclercqia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightLeclercqia;
				int minheight = LepidodendronConfigPlants.minheightLeclercqia;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockLeclercqia.block.getDefaultState(), 3);
			}
		}
	}
