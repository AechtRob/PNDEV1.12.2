...
public class StructureSpawnChestnut extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnChestnut(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimChestnut))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genChestnut && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genChestnutBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genChestnutOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 16000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierChestnut;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightChestnut;
				int minheight = LepidodendronConfigPlants.minheightChestnut;
				...
				int z = spawnTo.getZ();
				{
					...
					int TreeHeight = 10 + world.rand.nextInt(10);
					if (TreeHeight < 22 && Math.random() > 0.5) {
						TreeHeight = 15 + world.rand.nextInt(15);
					}
					if (TreeHeight > 22) {
						ProcedureWorldGenChestnut.executeProcedure(world, spawnTo, TreeHeight,2);
					}
					else {
						ProcedureWorldGenChestnut.executeProcedure(world, spawnTo, TreeHeight,1);
					}
				}
