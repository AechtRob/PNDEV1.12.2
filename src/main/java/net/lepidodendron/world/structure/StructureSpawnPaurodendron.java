...
public class StructureSpawnPaurodendron extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnPaurodendron(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPaurodendron))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPaurodendron && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genPaurodendronBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPaurodendronOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 33000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierPaurodendron;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightPaurodendron;
				int minheight = LepidodendronConfigPlants.minheightPaurodendron;
				...
				int z = spawnTo.getZ();
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockPaurodendron.block.getDefaultState());
				}
