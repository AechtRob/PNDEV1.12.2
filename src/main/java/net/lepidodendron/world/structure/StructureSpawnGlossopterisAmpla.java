...
public class StructureSpawnGlossopterisAmpla extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnGlossopterisAmpla(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimGlossopterisAmpla))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genGlossopterisAmpla && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genGlossopterisAmplaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genGlossopterisAmplaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 25000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierGlossopterisAmpla;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightGlossopterisAmpla;
				int minheight = LepidodendronConfigPlants.minheightGlossopterisAmpla;
