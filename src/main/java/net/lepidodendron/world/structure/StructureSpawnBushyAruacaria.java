...
public class StructureSpawnBushyAruacaria extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnBushyAruacaria(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimBushyAraucaria))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genBushyAraucaria && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genBushyAraucariaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genBushyAraucariaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfig.multiplierBushyAraucaria;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightBushyAraucaria;
				int minheight = LepidodendronConfig.minheightBushyAraucaria;
