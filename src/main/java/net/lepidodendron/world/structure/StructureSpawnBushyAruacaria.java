...
public class StructureSpawnBushyAruacaria extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genBushyAraucariaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genBushyAraucariaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierBushyAraucaria;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightBushyAraucaria;
				int minheight = LepidodendronConfigPlants.minheightBushyAraucaria;
