...
public class StructureSpawnYew extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnYew(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimYew))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genYew && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genYewBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genYewOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 12000;
		double GenMultiplier = LepidodendronConfig.multiplierYew;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightYew;
				int minheight = LepidodendronConfig.minheightYew;
				...
				int z = spawnTo.getZ();
				{
					...
					ProcedureWorldGenYew.executeProcedure($_dependencies);
				}
