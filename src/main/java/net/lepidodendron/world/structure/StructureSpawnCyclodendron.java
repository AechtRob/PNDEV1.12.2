...
public class StructureSpawnCyclodendron extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnCyclodendron(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimCyclodendron))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genCyclodendron && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genCyclodendronBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genCyclodendronOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 23000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierCyclodendron;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightCyclodendron;
				int minheight = LepidodendronConfigPlants.minheightCyclodendron;
				...
				int z = spawnTo.getZ();
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockCyclodendron.block.getDefaultState());
				}
