...
public class StructureSpawnCyclodendron extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimCyclodendron))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genCyclodendron && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genCyclodendronBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genCyclodendronOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 23000;
		double GenMultiplier = LepidodendronConfig.multiplierCyclodendron;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightCyclodendron;
				int minheight = LepidodendronConfig.minheightCyclodendron;
