...
public class StructureSpawnBeech extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnBeech(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimBeech))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genBeech && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genBeechBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genBeechOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 22000;
		double GenMultiplier = LepidodendronConfig.multiplierBeech;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightBeech;
				int minheight = LepidodendronConfig.minheightBeech;
				...
				int z = spawnTo.getZ();
				{
					...
					int TreeHeight = 18 + random.nextInt(12);
					if (TreeHeight < 25 && Math.random() > 0.5) {
						TreeHeight = 22 + random.nextInt(18);
					}
					ProcedureWorldGenBeech.executeProcedure(world, spawnTo, TreeHeight);
				}
