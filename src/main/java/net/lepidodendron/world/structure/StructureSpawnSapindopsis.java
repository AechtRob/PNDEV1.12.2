...
public class StructureSpawnSapindopsis extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnSapindopsis(ElementsLepidodendronMod instance) {
		super(instance, 49);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimSapindopsis))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genSapindopsis && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genSapindopsisBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genSapindopsisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 80000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierSapindopsis;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightSapindopsis;
				int minheight = LepidodendronConfigPlants.minheightSapindopsis;
				...
				BlockPos spawnTo = new BlockPos(i, j + 1, k);
				{
					...
					Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockSapindopsis.block.getDefaultState());
				}
