...
import net.lepidodendron.procedure.ProcedureWorldGenHironoia;
...
public class StructureSpawnHironoia extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnHironoia(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimHironoia))
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genHironoiaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genHironoiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 15000;
		double GenMultiplier = LepidodendronConfig.multiplierHironoia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightHironoia;
				int minheight = LepidodendronConfig.minheightHironoia;
...

				{
					...
					ProcedureWorldGenHironoia.executeProcedure($_dependencies);
				}
