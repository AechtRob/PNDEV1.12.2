...
public class StructureSpawnMacroneuropteris extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnMacroneuropteris(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimMacroneuropteris))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genMacroneuropteris && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genMacroneuropterisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genMacroneuropterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 60000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierMacroneuropteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				//Allow ONLY to spawn in shallow water:
				...
				else if (((world.getBlockState(new BlockPos(i, j, k))).getMaterial() == Material.WATER)
				...
				int maxheight = LepidodendronConfigPlants.maxheightMacroneuropteris;
				int minheight = LepidodendronConfigPlants.minheightMacroneuropteris;
