...
public class StructureSpawnMacroneuropteris extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfig.dimMacroneuropteris))
			dimensionCriteria = true;
		if (!LepidodendronConfig.genMacroneuropteris && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfig.genMacroneuropterisBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfig.genMacroneuropterisOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
			
		int GenChance = 60000;
		double GenMultiplier = LepidodendronConfig.multiplierMacroneuropteris;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfig.maxheightMacroneuropteris;
				int minheight = LepidodendronConfig.minheightMacroneuropteris;
