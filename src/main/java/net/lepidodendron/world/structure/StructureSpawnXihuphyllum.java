...
public class StructureSpawnXihuphyllum extends ElementsLepidodendronMod.ModElement {
	public StructureSpawnXihuphyllum(ElementsLepidodendronMod instance) {
		super(instance, 48);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimXihuphyllum))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genXihuphyllum && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genXihuphyllumBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genXihuphyllumOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 70000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierXihuphyllum;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightXihuphyllum;
				int minheight = LepidodendronConfigPlants.minheightXihuphyllum;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockXihuphyllum.block.getDefaultState(), 3);
				BlockXihuphyllum.block.onBlockAdded(world, spawnTo, BlockXihuphyllum.block.getDefaultState());
