...
public class StructureSpawnLycopia extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimLycopia))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genLycopia && !LepidodendronConfig.genAllPlants)
			dimensionCriteria = false;
		if (!dimensionCriteria)
			return;
		boolean biomeCriteria = false;
		Biome biome = world.getBiome(new BlockPos(i2, world.getSeaLevel(), k2));
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genLycopiaBlacklistBiomes))) {
			biomeCriteria = false;
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genLycopiaOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;

		int GenChance = 30000;
		double GenMultiplier = LepidodendronConfigPlants.multiplierLycopia;
		...
		if ((random.nextInt(1000000) + 1) <= GenChance) {
			...
			for (int a = 0; a < count; a++) {
				...
				int maxheight = LepidodendronConfigPlants.maxheightLycopia;
				int minheight = LepidodendronConfigPlants.minheightLycopia;
				...
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo, BlockLycopia.block.getDefaultState(), 3);
				Functions.setBlockStateAndCheckForDoublePlant(world,spawnTo.up(), BlockLycopiaTop.block.getDefaultState(), 3);
			}
		}
	}
