	...
	public void onUseBottle(PlayerInteractEvent.RightClickItem event) {
		...
		else
		{
			if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK)
			{
				...
				if (worldIn.getBlockState(blockpos).getMaterial() == MaterialResin.RESIN)
				{
					if (worldIn.getBlockState(blockpos).getBlock() == (new FluidStack(FluidRegistry.getFluid("pn_resin"), 1000)).getFluid().getBlock()) {
						event.getEntityPlayer().swingArm(event.getHand());
						worldIn.playSound(playerIn, playerIn.posX, playerIn.posY, playerIn.posZ, SoundEvents.ITEM_BOTTLE_FILL, SoundCategory.NEUTRAL, 1.0F, 1.0F);
				...
				if (worldIn.getBlockState(blockpos).getMaterial() == MaterialTar.TAR)
				{
					if (worldIn.getBlockState(blockpos).getBlock() == (new FluidStack(FluidRegistry.getFluid("pn_tar"), 1000)).getFluid().getBlock()) {
						event.getEntityPlayer().swingArm(event.getHand());
						worldIn.playSound(playerIn, playerIn.posX, playerIn.posY, playerIn.posZ, SoundEvents.ITEM_BOTTLE_FILL, SoundCategory.NEUTRAL, 1.0F, 1.0F);
						this.turnBottleIntoItem(itemstack, playerIn, new ItemStack(ItemBottleOfTar.block, 1));
						event.setCanceled(true);
						return;
					}
				}
