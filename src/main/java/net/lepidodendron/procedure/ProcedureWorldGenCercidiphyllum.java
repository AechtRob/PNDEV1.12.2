...
import net.lepidodendron.block.BlockCercidiphyllumLeaves;
import net.lepidodendron.block.BlockCercidiphyllumLeavesAutumn;
...
public class ProcedureWorldGenCercidiphyllum extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenCercidiphyllum(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    private static int heightLimit;
   	private static int heightLimitLimit = 12;
    private static double heightAttenuation = 0.618D;
    private static double leafDensity = 0.8D; //1.0
    private static int leafDistanceLimit = 4;
    private static List<ProcedureWorldGenCercidiphyllum.FoliageCoordinates> foliageCoords;
    ...
    static void generateLeafNodeList()
    {
        ...
        foliageCoords = Lists.<ProcedureWorldGenCercidiphyllum.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenCercidiphyllum.FoliageCoordinates(basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            foliageCoords.add(new ProcedureWorldGenCercidiphyllum.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    {
        for (ProcedureWorldGenCercidiphyllum.FoliageCoordinates procedureworldgentrochodendroides$foliagecoordinates : foliageCoords)
        {
            generateLeafNode(procedureworldgentrochodendroides$foliagecoordinates, isAutumn);
        }
    }
    ...
    static void generateLeafNode(BlockPos pos, boolean isAutumn)
    {
        ...
        {
            //crosSection(pos.up(i), leafSize(i), BlockCercidiphyllumLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
            crosSection(pos.up(i), leafSize(i), isAutumn ? BlockCercidiphyllumLeavesAutumn.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)) : BlockCercidiphyllumLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    static void generateTrunk()
    {
        ...
        Block block = BlockTrochodendroidesLog.block;
    ...
    static void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            Functions.setBlockStateAndCheckForDoublePlant(world,blockpos1, BlockTrochodendroidesLog.block.getDefaultState().withProperty(FACING,blocklog$enumaxis), 3);
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenCercidiphyllum.FoliageCoordinates procedureworldgentrochodendroides$foliagecoordinates : foliageCoords)
        {
            int i = procedureworldgentrochodendroides$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(basePos.getX(), i, basePos.getZ());

			//System.err.println("Branch height? " + i);

            if (!blockpos.equals(procedureworldgentrochodendroides$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
            	//System.err.println("limb called " + blockpos.getX() + " " + blockpos.getY() + " " + blockpos.getZ());
                limb(blockpos, procedureworldgentrochodendroides$foliagecoordinates, BlockTrochodendroidesLog.block);
            }
        }
    } 
