...
import net.lepidodendron.block.BlockAphanantheLeaves;
import net.lepidodendron.block.BlockAphanantheLog;
...
public class ProcedureWorldGenAphananthe extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenAphananthe(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    private static int heightLimit;
   	private static int heightLimitLimit = 9;
    private static double heightAttenuation = 0.798D;
    private static double leafDensity = 0.40D;
    private static int leafDistanceLimit = 3;
    private static List<ProcedureWorldGenAphananthe.FoliageCoordinates> foliageCoords;
    private static double branchSlope = 0.781D;
    private static double scaleWidth = 0.27D;
    ...
    static void generateLeafNodeList()
    {
        ...
        foliageCoords = Lists.<ProcedureWorldGenAphananthe.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenAphananthe.FoliageCoordinates(basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            foliageCoords.add(new ProcedureWorldGenAphananthe.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    {
        for (ProcedureWorldGenAphananthe.FoliageCoordinates procedureworldgenapple$foliagecoordinates : foliageCoords)
        {
    ...
    static void generateLeafNode(BlockPos pos)
    {
        ...
        {
            //crosSection(pos.up(i), leafSize(i), BlockAphanantheLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
            crosSection(pos.up(i), leafSize(i), BlockAphanantheLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    static void generateTrunk()
    {
        ...
        Block block = BlockAphanantheLog.block;
    ...
    static void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            Functions.setBlockStateAndCheckForDoublePlant(world,blockpos1, BlockAphanantheLog.block.getDefaultState().withProperty(FACING,blocklog$enumaxis), 3);
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenAphananthe.FoliageCoordinates procedureworldgenapple$foliagecoordinates : foliageCoords)
        {
            ...
            if (!blockpos.equals(procedureworldgenapple$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
                ...
                limb(blockpos, procedureworldgenapple$foliagecoordinates, BlockAphanantheLog.block);
            }
        }
    } 
