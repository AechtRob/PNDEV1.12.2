...
import net.lepidodendron.block.BlockAppleLeaves;
...
public class ProcedureWorldGenApple extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenApple(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    private static List<ProcedureWorldGenApple.FoliageCoordinates> foliageCoords;
    ...
    static void generateLeafNodeList()
    {
        ...
        foliageCoords = Lists.<ProcedureWorldGenApple.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenApple.FoliageCoordinates(basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            foliageCoords.add(new ProcedureWorldGenApple.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    {
        for (ProcedureWorldGenApple.FoliageCoordinates procedureworldgenapple$foliagecoordinates : foliageCoords)
        {
    ...
    static void generateLeafNode(BlockPos pos)
    {
        ...
        {
            //crosSection(pos.up(i), leafSize(i), BlockAppleLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
            crosSection(pos.up(i), leafSize(i), BlockAppleLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    static void generateTrunk()
    {
        ...
        Block block = BlockAppleLog.block;
    ...
    static void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            Functions.setBlockStateAndCheckForDoublePlant(world,blockpos1, BlockAppleLog.block.getDefaultState().withProperty(FACING,blocklog$enumaxis), 3);
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenApple.FoliageCoordinates procedureworldgenapple$foliagecoordinates : foliageCoords)
        {
            ...
            if (!blockpos.equals(procedureworldgenapple$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
                ...
                limb(blockpos, procedureworldgenapple$foliagecoordinates, BlockAppleLog.block);
            }
        }
    } 
