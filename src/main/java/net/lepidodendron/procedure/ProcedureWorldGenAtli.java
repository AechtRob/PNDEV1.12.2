...
public class ProcedureWorldGenAtli extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenAtli(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
	...
	public static void executeProcedure(int x, int y, int z, World world) {
		...
		//System.err.println("running WorldGenAtli!");
		//	return;
		//}
		//if (dependencies.get("y") == null) {
		//	System.err.println("Failed to load dependency y for procedure WorldGenAtli!");
		//	return;
		//}
		//if (dependencies.get("z") == null) {
		//	System.err.println("Failed to load dependency z for procedure WorldGenAtli!");
		//	return;
		//}
		//if (dependencies.get("world") == null) {
		//	System.err.println("Failed to load dependency world for procedure WorldGenAtli!");
		//	return;
		//}
		...
		{//Check each axis to try to work out where to grow:
			...
			//System.err.println("running WorldGenAtli: " + x + " " + y + " " + z );
		...
		if (FaceN || FaceS || FaceE || FaceW)
		{
			...
			//Trunk:
			TrunkHeight = 5 + (int) Math.round(Math.random() * 10);
			counter = 0;
	...
	static void PlaceSegment(EnumFacing facing, int counter, int TrunkHeight, World world, BlockPos pos) {
		if (counter >= TrunkHeight) {
			if ((canPlace(pos, world))
					&& world.getBlockState(pos.down()).getBlock() == BlockAtliStem.block) {
				PlaceBlock(pos, world, BlockAtliShoot.block, EnumFacing.UP);
			}
				...
				if (Math.random() > 0.5) {
					...
					if (facing == EnumFacing.NORTH) {
						if (
							...
							PlaceBlock(pos, world, BlockAtliStemSE.block, facing);
							PlaceBlock(pos.west(), world, BlockAtliStemNW.block, facing);
							...
							if (
								...
								PlaceBlock(pos.west().south(), world, BlockAtliStemNW.block, facing);
								//And apply recursion for the next block:
								counter = counter + 1;
								Bendplaced = true;
						...
						if (facing == EnumFacing.SOUTH) {
							if (
								...
								//We can place the bend:
								PlaceBlock(pos, world, BlockAtliStemSE.block, facing);
								PlaceBlock(pos.east(), world, BlockAtliStemNW.block, facing);
								//And apply recursion for the next block:
								counter = counter + 1;
								Bendplaced = true;
								...
								if (
									...
									PlaceBlock(pos.east().north(), world, BlockAtliStemNW.block, facing);
									//And apply recursion for the next block:
									counter = counter + 1;
									Bendplaced = true;
							...
							if (facing == EnumFacing.EAST) {
								if (
									...
									//We can place the bend:
									PlaceBlock(pos, world, BlockAtliStemSE.block, facing);
									PlaceBlock(pos.south(), world, BlockAtliStemNW.block, facing);
									//And apply recursion for the next block:
									counter = counter + 1;
									Bendplaced = true;
									...
									if (
										...
										//We can place the bend round the corner::
										PlaceBlock(pos, world, BlockAtliStemSW.block, facing);
										//And swap to the new axis now:
										facing = EnumFacing.SOUTH;
										PlaceBlock(pos.south().west(), world, BlockAtliStemNE.block, facing);
										//And apply recursion for the next block:
										counter = counter + 1;
										Bendplaced = true;
								...
								if (
									...
									if (
										...
										//And swap to the new axis now:
										facing = EnumFacing.SOUTH;
										PlaceBlock(pos.south().east(), world, BlockAtliStemNW.block, facing);
										//And apply recursion for the next block:
										counter = counter + 1;
										Bendplaced = true;
					...
					if (facing == EnumFacing.NORTH) {
						if (
							...
							//We can place the bend:									
							PlaceBlock(pos, world, BlockAtliStemSW.block, facing);
							PlaceBlock(pos.east(), world, BlockAtliStemNE.block, facing);
							...
							if (
								...
								//And swap to the new axis now:
								facing = EnumFacing.EAST;
								PlaceBlock(pos.east().south(), world, BlockAtliStemNE.block, facing);
								//And apply recurion for the next block:
								counter = counter + 1;
								Bendplaced = true;
						...
						if (facing == EnumFacing.SOUTH) {
							if (
								...
								//We can place the bend:									
								PlaceBlock(pos, world, BlockAtliStemSW.block, facing);
								PlaceBlock(pos.west(), world, BlockAtliStemNE.block, facing);
								//And apply recurion for the next block:
								counter = counter + 1;
								Bendplaced = true;
								PlaceSegment(facing, counter, TrunkHeight, world, pos.west().up());
								...
								if (
									...
									//And swap to the new axis now:
									facing = EnumFacing.WEST;
									PlaceBlock(pos.west().north(), world, BlockAtliStemNE.block, facing);
									//And apply recurion for the next block:
									counter = counter + 1;
									Bendplaced = true;
							...
							if (facing == EnumFacing.EAST) {
								if (
									...
									//We can place the bend:									
									PlaceBlock(pos, world, BlockAtliStemSW.block, facing);
									PlaceBlock(pos.south(), world, BlockAtliStemNE.block, facing);
									//And apply recurion for the next block:
									counter = counter + 1;
									Bendplaced = true;
									PlaceSegment(facing, counter, TrunkHeight, world, pos.south().up());
								...
								if (
									...
									//We can place the bend:									
									PlaceBlock(pos, world, BlockAtliStemSW.block, facing);
									PlaceBlock(pos.north(), world, BlockAtliStemNE.block, facing);
									//And apply recurion for the next block:
									counter = counter + 1;
									Bendplaced = true;
									...
									if (
										...
										PlaceBlock(pos.north().east(), world, BlockAtliStemNE.block, facing);
										//And apply recurion for the next block:
										counter = counter + 1;
										Bendplaced = true;
			...
			if (!Bendplaced) {
				...
				if (facing == EnumFacing.NORTH) {
					if ((world.getBlockState(pos.south().up()).getBlockFaceShape(world, pos.south().up(), facing) == BlockFaceShape.SOLID)
						...
						PlaceBlock(pos, world, BlockAtliStem.block, facing);
						//if ((Math.random() > 0.82) && (counter >= 4) && (counter < (TrunkHeight - 1))) {
						//		PlaceBlock(pos.north(), world, BlockAtliShoot.block, facing);
						//}
						counter = counter + 1;
						if (counter >= TrunkHeight) {
							PlaceBlock(pos.up(), world, BlockAtliShoot.block, EnumFacing.UP);
						}
						else {
							if (world.rand.nextInt(4) == 0) {
								...
								if (world.getBlockState(pos.south().east()).getBlockFaceShape(world, pos.south().east(), EnumFacing.NORTH) == BlockFaceShape.SOLID
									...
									PlaceBlock(pos.east(), world, BlockAtliStemNE.block, facing);
									PlaceBlock(pos.east().up(), world, BlockAtliStem.block, facing);
									...
									PlaceBlock(pos.east().south(), world, BlockAtliStemNE.block, EnumFacing.EAST);
									PlaceBlock(pos.east().south().up(), world, BlockAtliStem.block, EnumFacing.EAST);
							...
							if (world.rand.nextInt(4) == 0) {
								...
								if (world.getBlockState(pos.south().west()).getBlockFaceShape(world, pos.south().west(), EnumFacing.NORTH) == BlockFaceShape.SOLID
									...
									PlaceBlock(pos.west(), world, BlockAtliStemNW.block, facing);
									PlaceBlock(pos.west().up(), world, BlockAtliStem.block, facing);
									...
									PlaceBlock(pos.west().south(), world, BlockAtliStemNW.block, EnumFacing.WEST);
									PlaceBlock(pos.west().south().up(), world, BlockAtliStem.block, EnumFacing.WEST);
					...
					if (facing == EnumFacing.SOUTH) {
						if ((world.getBlockState(pos.north().up()).getBlockFaceShape(world, pos.north().up(), facing) == BlockFaceShape.SOLID)
							...
							PlaceBlock(pos, world, BlockAtliStem.block, facing);
							//if ((Math.random() > 0.82) && (counter >= 4) && (counter < (TrunkHeight - 1))) {
							//	PlaceBlock(pos.south(), world, BlockAtliShoot.block, facing);
							//}
							counter = counter + 1;
							if (counter >= TrunkHeight) {
								PlaceBlock(pos.up(), world, BlockAtliShoot.block, EnumFacing.UP);
								...
								if (world.rand.nextInt(4) == 0) {
									...
									if (world.getBlockState(pos.north().west()).getBlockFaceShape(world, pos.north().west(), EnumFacing.SOUTH) == BlockFaceShape.SOLID
										...
										PlaceBlock(pos.west().north(), world, BlockAtliStemNE.block, EnumFacing.WEST);
										PlaceBlock(pos.west().north().up(), world, BlockAtliStem.block, EnumFacing.WEST);
								...
								if (world.rand.nextInt(4) == 0) {
									...
									if (world.getBlockState(pos.north().east()).getBlockFaceShape(world, pos.north().east(), EnumFacing.SOUTH) == BlockFaceShape.SOLID
										...
										PlaceBlock(pos.east().north(), world, BlockAtliStemNW.block, EnumFacing.EAST);
										PlaceBlock(pos.east().north().up(), world, BlockAtliStem.block, EnumFacing.EAST);
						...
						if (facing == EnumFacing.EAST) {
							if ((world.getBlockState(pos.west().up()).getBlockFaceShape(world, pos.west().up(), facing) == BlockFaceShape.SOLID)
								...
								PlaceBlock(pos, world, BlockAtliStem.block, facing);
								//if ((Math.random() > 0.82) && (counter >= 4) && (counter < (TrunkHeight - 1))) {
								//		PlaceBlock(pos.east(), world, BlockAtliShoot.block, facing);
								//}
								counter = counter + 1;
								if (counter >= TrunkHeight) {
									PlaceBlock(pos.up(), world, BlockAtliShoot.block, EnumFacing.UP);
									...
									if (world.rand.nextInt(4) == 0) {
										...
										if (world.getBlockState(pos.west().south()).getBlockFaceShape(world, pos.west().south(), EnumFacing.EAST) == BlockFaceShape.SOLID
											...
											PlaceBlock(pos.south(), world, BlockAtliStemNE.block, facing);
											PlaceBlock(pos.south().up(), world, BlockAtliStem.block, facing);
											...
											PlaceBlock(pos.south().west(), world, BlockAtliStemNE.block, EnumFacing.SOUTH);
											PlaceBlock(pos.south().west().up(), world, BlockAtliStem.block, EnumFacing.SOUTH);
									...
									if (world.rand.nextInt(4) == 0) {
										...
										if (world.getBlockState(pos.west().north()).getBlockFaceShape(world, pos.west().north(), EnumFacing.EAST) == BlockFaceShape.SOLID
											...
											PlaceBlock(pos.north(), world, BlockAtliStemNW.block, facing);
											PlaceBlock(pos.north().up(), world, BlockAtliStem.block, facing);
											...
											PlaceBlock(pos.north().west(), world, BlockAtliStemNW.block, EnumFacing.NORTH);
											PlaceBlock(pos.north().west().up(), world, BlockAtliStem.block, EnumFacing.NORTH);
							...
							if ((world.getBlockState(pos.east().up()).getBlockFaceShape(world, pos.east().up(), facing) == BlockFaceShape.SOLID)
								...
								PlaceBlock(pos, world, BlockAtliStem.block, facing);
								//if ((Math.random() > 0.82) && (counter >= 4) && (counter < (TrunkHeight - 1))) {
								//		PlaceBlock(pos.west(), world, BlockAtliShoot.block, facing);
								//}
								counter = counter + 1;
								if (counter >= TrunkHeight) {
									PlaceBlock(pos.up(), world, BlockAtliShoot.block, EnumFacing.UP);
									...
									if (world.rand.nextInt(4) == 0) {
										...
										if (world.getBlockState(pos.east().north()).getBlockFaceShape(world, pos.east().north(), EnumFacing.WEST) == BlockFaceShape.SOLID
											...
											PlaceBlock(pos.north().east(), world, BlockAtliStemNE.block, EnumFacing.NORTH);
											PlaceBlock(pos.north().east().up(), world, BlockAtliStem.block, EnumFacing.NORTH);
									...
									if (world.rand.nextInt(4) == 0) {
										...
										if (world.getBlockState(pos.east().south()).getBlockFaceShape(world, pos.east().south(), EnumFacing.WEST) == BlockFaceShape.SOLID
											...
											PlaceBlock(pos.south().east(), world, BlockAtliStemNW.block, EnumFacing.SOUTH);
											PlaceBlock(pos.south().east().up(), world, BlockAtliStem.block, EnumFacing.SOUTH);
								...
								if (facing == EnumFacing.EAST) {
									if (rightSpace) {
										if (!changeRightAxis) {
											...
											//And swap to the new axis now:
											facing = EnumFacing.NORTH;
											PlaceBlock(pos.north().west(), world, BlockAtliStemNW.block, facing);
											//And apply recursion for the next block:
											counter = counter + 1;
