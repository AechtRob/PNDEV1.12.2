...
public class ProcedureWorldGenEncblue extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenEncblue(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}

	public static void executeProcedure(java.util.HashMap<String, Object> dependencies) {
		if (dependencies.get("x") == null) {
			System.err.println("Failed to load dependency x for procedure WorldGenEncblue!");
			return;
		}
		if (dependencies.get("y") == null) {
			System.err.println("Failed to load dependency y for procedure WorldGenEncblue!");
			return;
		}
		if (dependencies.get("z") == null) {
			System.err.println("Failed to load dependency z for procedure WorldGenEncblue!");
			return;
		}
		if (dependencies.get("world") == null) {
			System.err.println("Failed to load dependency world for procedure WorldGenEncblue!");
			return;
		}
		...
		if ((world.canSeeSky(new BlockPos((int) x, (int) y, (int) z)))
			...
			//Trunk:
			TrunkHeight = Math.round(Math.random() * 3) - 1;
			if (Math.random() > 0.4) TrunkHeight = Math.round(TrunkHeight * 0.5);
			if (TrunkHeight < 0) TrunkHeight = 0;

			counter = 0;
			while (counter <= TrunkHeight) {
				ProcedureTreeLog.executeProcedure((int) x, (int) (y + counter), (int) z, world, BlockEncblueLog.block, EnumFacing.DOWN);
				counter = counter + 1;
				}
