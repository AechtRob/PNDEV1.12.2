...
import net.lepidodendron.block.BlockMapleLeaves;
...
public class ProcedureWorldGenMaple extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenMaple(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    private static List<ProcedureWorldGenMaple.FoliageCoordinates> foliageCoords;
    ...
    static void generateLeafNodeList()
    {
        ...
        foliageCoords = Lists.<ProcedureWorldGenMaple.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenMaple.FoliageCoordinates(basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            foliageCoords.add(new ProcedureWorldGenMaple.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    {
        for (ProcedureWorldGenMaple.FoliageCoordinates procedureworldgenmaple$foliagecoordinates : foliageCoords)
        {
            generateLeafNode(procedureworldgenmaple$foliagecoordinates);
        }
    }
    ...
    static void generateLeafNode(BlockPos pos)
    {
        ...
        {
            //crosSection(pos.up(i), leafSize(i), BlockMapleLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
            crosSection(pos.up(i), leafSize(i), BlockMapleLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    static void generateTrunk()
    {
        ...
        Block block = BlockMapleLog.block;
    ...
    static void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            Functions.setBlockStateAndCheckForDoublePlant(world,blockpos1, BlockMapleLog.block.getDefaultState().withProperty(FACING,blocklog$enumaxis), 3);
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenMaple.FoliageCoordinates procedureworldgenmaple$foliagecoordinates : foliageCoords)
        {
            int i = procedureworldgenmaple$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(basePos.getX(), i, basePos.getZ());

			//System.err.println("Branch height? " + i);

            if (!blockpos.equals(procedureworldgenmaple$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
            	//System.err.println("limb called " + blockpos.getX() + " " + blockpos.getY() + " " + blockpos.getZ());
                limb(blockpos, procedureworldgenmaple$foliagecoordinates, BlockMapleLog.block);
            }
        }
    } 
