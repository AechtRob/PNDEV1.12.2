...
public class ProcedureWorldGenMightyOak extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenMightyOak(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    private static List<ProcedureWorldGenMightyOak.FoliageCoordinates> foliageCoords;
    ...
    static void generateLeafNodeList()
    {
        ...
        foliageCoords = Lists.<ProcedureWorldGenMightyOak.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenMightyOak.FoliageCoordinates(basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            foliageCoords.add(new ProcedureWorldGenMightyOak.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    {
        for (ProcedureWorldGenMightyOak.FoliageCoordinates procedureworldgenmightyoak$foliagecoordinates : foliageCoords)
        {
            generateLeafNode(procedureworldgenmightyoak$foliagecoordinates);
        }
    }
    ...
    static void generateLeafNode(BlockPos pos)
    {
        ...
        {
            //crosSection(pos.up(i), leafSize(i), BlockSycamoreLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
            crosSection(pos.up(i), leafSize(i), Blocks.LEAVES.getStateFromMeta(0).withProperty(CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    static void generateTrunk(int trunkSize)
    {
        ...
        Block block = Blocks.LOG.getStateFromMeta(0).getBlock();
    ...
    private static BlockLog.EnumAxis getLogAxis(BlockPos p_175938_1_, BlockPos p_175938_2_)
    {
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenMightyOak.FoliageCoordinates procedureworldgenmightyoak$foliagecoordinates : foliageCoords)
        {
            int i = procedureworldgenmightyoak$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(basePos.getX(), i, basePos.getZ());

			//System.err.println("Branch height? " + i);

            if (!blockpos.equals(procedureworldgenmightyoak$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
            	//System.err.println("limb called " + blockpos.getX() + " " + blockpos.getY() + " " + blockpos.getZ());
                limb(blockpos, procedureworldgenmightyoak$foliagecoordinates, Blocks.LOG.getStateFromMeta(0).getBlock());
            }
        }
    } 
