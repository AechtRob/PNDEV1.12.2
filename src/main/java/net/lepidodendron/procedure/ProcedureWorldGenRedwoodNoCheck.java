...
public class ProcedureWorldGenRedwoodNoCheck extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenRedwoodNoCheck(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
	...
	public static void executeProcedure(java.util.HashMap<String, Object> dependencies) {
		...
		if ((world.canSeeSky(new BlockPos((int) x, (int) y, (int) z)))
			...
			//Trunk:
			TrunkHeight = (double) (25);
			if (Math.random() >= 0.35) {
				TrunkHeight = TrunkHeight + Math.round((Math.random() * 100) / 3);
			}
			else {
			...
			while (counter <= TrunkHeight) {
				...
				ProcedureTreeLog.executeProcedure((int) (x + 1), (int) (y + counter), (int) (z - 1), world, BlockRedwoodLog.block, EnumFacing.NORTH);
				ProcedureTreeLog.executeProcedure((int) (x + 1), (int) (y + counter), (int) z, world, BlockRedwoodLog.block, EnumFacing.NORTH);
				ProcedureTreeLog.executeProcedure((int) x, (int) (y + counter), (int) (z - 1), world, BlockRedwoodLog.block, EnumFacing.NORTH);
				counter = counter + 1;
				}
			...
			while (counter <= TrunkHeight + 4 + Math.floor(4 * randomiser)) {
			...
			while (counter >= TrunkHeight / 7) {
				...
				if (BranchAxis == "east") {
					...
					if ((randomiser >= 0.8) && (counter/TrunkHeight < 0.6)) {
						ProcedureTreeLog.executeProcedure((int)xx, (int) (y + counter), (int) zz, world, BlockRedwoodLog.block, EnumFacing.UP);
						xx = xx + 1;
					}
