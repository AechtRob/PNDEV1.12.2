...
public class ProcedureWorldGenBisonia extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenBisonia(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    private static List<ProcedureWorldGenBisonia.FoliageCoordinates> foliageCoords;
    private static double branchSlope = 0.881D;
    ...
    static void generateLeafNodeList()
    {
        ...
        foliageCoords = Lists.<ProcedureWorldGenBisonia.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenBisonia.FoliageCoordinates(basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            foliageCoords.add(new ProcedureWorldGenBisonia.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    {
        for (ProcedureWorldGenBisonia.FoliageCoordinates procedureworldgenapple$foliagecoordinates : foliageCoords)
        {
    ...
    static void generateLeafNode(BlockPos pos)
    {
        ...
        {
            //crosSection(pos.up(i), leafSize(i), BlockBisoniaLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
            crosSection(pos.up(i), leafSize(i), BlockBisoniaLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    static void generateTrunk()
    {
        ...
        Block block = BlockBisoniaLog.block;
    ...
    static void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            Functions.setBlockStateAndCheckForDoublePlant(world,blockpos1, BlockBisoniaLog.block.getDefaultState().withProperty(FACING,blocklog$enumaxis), 3);
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenBisonia.FoliageCoordinates procedureworldgenapple$foliagecoordinates : foliageCoords)
        {
            ...
            if (!blockpos.equals(procedureworldgenapple$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
                ...
                limb(blockpos, procedureworldgenapple$foliagecoordinates, BlockBisoniaLog.block);
            }
        }
    } 
