...
public class ProcedureWorldGenRattan extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenRattan(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
	...
	public static void executeProcedure(int x, int y, int z, World world) {
		...
		//System.err.println("running WorldGenRattan!");
		//	return;
		//}
		//if (dependencies.get("y") == null) {
		//	System.err.println("Failed to load dependency y for procedure WorldGenRattan!");
		//	return;
		//}
		//if (dependencies.get("z") == null) {
		//	System.err.println("Failed to load dependency z for procedure WorldGenRattan!");
		//	return;
		//}
		//if (dependencies.get("world") == null) {
		//	System.err.println("Failed to load dependency world for procedure WorldGenRattan!");
		//	return;
		//}
		...
		{//Check each axis to try to work out where to grow:
			...
			//System.err.println("running WorldGenRattan: " + x + " " + y + " " + z );
		...
		if (FaceN || FaceS || FaceE || FaceW)
			...
			//Trunk:
			TrunkHeight = 12 + (int) Math.round(Math.random() * 28D);
			counter = 0;
					...
					if (facing == EnumFacing.NORTH) {
						if (
							...
							&& (canPlace(pos.west().up(3), world))
								...
								PlaceBlock(pos, world, BlockRattanStemSE.block, facing);
								PlaceBlock(pos.west(), world, BlockRattanStemNW.block, facing);
							...
							&& (canPlace(pos.west().up(3).south(), world))
								...
								PlaceBlock(pos.west().south(), world, BlockRattanStemNW.block, facing);
								//And apply recursion for the next block:
								counter = counter + 1;
								Bendplaced = true;
						...
						if (facing == EnumFacing.SOUTH) {
							if (
								...
								&& (canPlace(pos.east().up(3), world))
									...
									//We can place the bend:
									PlaceBlock(pos, world, BlockRattanStemSE.block, facing);
									PlaceBlock(pos.east(), world, BlockRattanStemNW.block, facing);
									//And apply recursion for the next block:
									counter = counter + 1;
									Bendplaced = true;
								...
								&& (canPlace(pos.east().up(3).north(), world))
									...
									PlaceBlock(pos.east().north(), world, BlockRattanStemNW.block, facing);
									//And apply recursion for the next block:
									counter = counter + 1;
									Bendplaced = true;
							...
							if (facing == EnumFacing.EAST) {
								if (
									...
									&& (canPlace(pos.south().up(3), world))
										...
										//We can place the bend:
										PlaceBlock(pos, world, BlockRattanStemSE.block, facing);
										PlaceBlock(pos.south(), world, BlockRattanStemNW.block, facing);
										//And apply recursion for the next block:
										counter = counter + 1;
										Bendplaced = true;
									...
									&& (canPlace(pos.south().up(3).west(), world))
										...
										//We can place the bend round the corner::
										PlaceBlock(pos, world, BlockRattanStemSW.block, facing);
										//And swap to the new axis now:
										facing = EnumFacing.SOUTH;
										PlaceBlock(pos.south().west(), world, BlockRattanStemNE.block, facing);
										//And apply recursion for the next block:
										counter = counter + 1;
										Bendplaced = true;
								...
								if (
									...
									&& (canPlace(pos.south().up(3).east(), world))
										...
										//And swap to the new axis now:
										facing = EnumFacing.SOUTH;
										PlaceBlock(pos.south().east(), world, BlockRattanStemNW.block, facing);
										//And apply recursion for the next block:
										counter = counter + 1;
										Bendplaced = true;
					...
					if (facing == EnumFacing.NORTH) {
						if (
							...
							&& (canPlace(pos.east().up(3), world))
								...
								//We can place the bend:									
								PlaceBlock(pos, world, BlockRattanStemSW.block, facing);
								PlaceBlock(pos.east(), world, BlockRattanStemNE.block, facing);
							...
							&& (canPlace(pos.east().up(3).south(), world))
								...
								//And swap to the new axis now:
								facing = EnumFacing.EAST;
								PlaceBlock(pos.east().south(), world, BlockRattanStemNE.block, facing);
								//And apply recurion for the next block:
								counter = counter + 1;
								Bendplaced = true;
						...
						if (facing == EnumFacing.SOUTH) {
							if (
								...
								&& (canPlace(pos.west().up(3), world))
									...
									//We can place the bend:									
									PlaceBlock(pos, world, BlockRattanStemSW.block, facing);
									PlaceBlock(pos.west(), world, BlockRattanStemNE.block, facing);
									//And apply recurion for the next block:
									counter = counter + 1;
									Bendplaced = true;
									PlaceSegment(facing, counter, TrunkHeight, world, pos.west().up());
								...
								&& (canPlace(pos.west().up(3).north(), world))
									...
									//And swap to the new axis now:
									facing = EnumFacing.WEST;
									PlaceBlock(pos.west().north(), world, BlockRattanStemNE.block, facing);
									//And apply recurion for the next block:
									counter = counter + 1;
									Bendplaced = true;
							...
							if (facing == EnumFacing.EAST) {
								if (
									...
									&& (canPlace(pos.south().up(3), world))
										...
										//We can place the bend:									
										PlaceBlock(pos, world, BlockRattanStemSW.block, facing);
										PlaceBlock(pos.south(), world, BlockRattanStemNE.block, facing);
										//And apply recurion for the next block:
										counter = counter + 1;
										Bendplaced = true;
										PlaceSegment(facing, counter, TrunkHeight, world, pos.south().up());
								...
								if (
									...
									&& (canPlace(pos.north().up(3), world))
										...
										//We can place the bend:									
										PlaceBlock(pos, world, BlockRattanStemSW.block, facing);
										PlaceBlock(pos.north(), world, BlockRattanStemNE.block, facing);
										//And apply recurion for the next block:
										counter = counter + 1;
										Bendplaced = true;
									...
									&& (canPlace(pos.north().up(3).east(), world))
										...
										PlaceBlock(pos.north().east(), world, BlockRattanStemNE.block, facing);
										//And apply recurion for the next block:
										counter = counter + 1;
										Bendplaced = true;
				...
				if (facing == EnumFacing.NORTH) {
					if ((world.getBlockState(pos.south().up()).getBlockFaceShape(world, pos.south().up(), facing) == BlockFaceShape.SOLID) 
						...
						&& (canPlace(pos.up(2), world))
							...
							//		PlaceBlock(pos.north(), world, BlockRattanShoot.block, facing);
							//}
							counter = counter + 1;
							if (counter >= TrunkHeight) {
					...
					if (facing == EnumFacing.SOUTH) {
						if ((world.getBlockState(pos.north().up()).getBlockFaceShape(world, pos.north().up(), facing) == BlockFaceShape.SOLID) 
							...
							&& (canPlace(pos.up(2), world))
								...
								PlaceBlock(pos, world, BlockRattanStem.block, facing);
								//if ((Math.random() > 0.82) && (counter >= 4) && (counter < (TrunkHeight - 1))) {
								//	PlaceBlock(pos.south(), world, BlockRattanShoot.block, facing);
								//}
								counter = counter + 1;
								if (counter >= TrunkHeight) {
									PlaceBlock(pos.up(), world, BlockRattanShoot.block, EnumFacing.UP);
						...
						if (facing == EnumFacing.EAST) {
							if ((world.getBlockState(pos.west().up()).getBlockFaceShape(world, pos.west().up(), facing) == BlockFaceShape.SOLID) 
								...
								&& (canPlace(pos.up(2), world))
									...
									PlaceBlock(pos, world, BlockRattanStem.block, facing);
									//if ((Math.random() > 0.82) && (counter >= 4) && (counter < (TrunkHeight - 1))) {
									//		PlaceBlock(pos.east(), world, BlockRattanShoot.block, facing);
									//}
									counter = counter + 1;
									if (counter >= TrunkHeight) {
										PlaceBlock(pos.up(), world, BlockRattanShoot.block, EnumFacing.UP);
							...
							if ((world.getBlockState(pos.east().up()).getBlockFaceShape(world, pos.east().up(), facing) == BlockFaceShape.SOLID) 
								...
								&& (canPlace(pos.up(2), world))
									...
									PlaceBlock(pos, world, BlockRattanStem.block, facing);
									//if ((Math.random() > 0.82) && (counter >= 4) && (counter < (TrunkHeight - 1))) {
									//		PlaceBlock(pos.west(), world, BlockRattanShoot.block, facing);
									//}
									counter = counter + 1;
									if (counter >= TrunkHeight) {
										PlaceBlock(pos.up(), world, BlockRattanShoot.block, EnumFacing.UP);
								...
								if (facing == EnumFacing.EAST) {
									if (rightSpace) {
										if (!changeRightAxis) {
											//place the bend:
											PlaceBlock(pos, world, BlockRattanStemSE.block, facing);
											PlaceBlock(pos.north(), world, BlockRattanStemNW.block, facing);
											//And apply recursion for the next block:
											counter = counter + 1;
											PlaceSegment(facing, counter, TrunkHeight, world, pos.north().up());
										}
											...
											//And swap to the new axis now:
											facing = EnumFacing.NORTH;
											PlaceBlock(pos.north().west(), world, BlockRattanStemNW.block, facing);
											//And apply recursion for the next block:
											counter = counter + 1;
