...
import net.lepidodendron.block.BlockCephalotaxusLeaves;
import net.lepidodendron.block.BlockCephalotaxusLeavesBerries;
...
public class ProcedureWorldGenCephalotaxus extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenCephalotaxus(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}

	public static void executeProcedure(java.util.HashMap<String, Object> dependencies) {
		if (dependencies.get("x") == null) {
			System.err.println("Failed to load dependency x for procedure WorldGenCephalotaxus!");
			return;
		}
		if (dependencies.get("y") == null) {
			System.err.println("Failed to load dependency y for procedure WorldGenCephalotaxus!");
			return;
		}
		if (dependencies.get("z") == null) {
			System.err.println("Failed to load dependency z for procedure WorldGenCephalotaxus!");
			return;
		}
		if (dependencies.get("world") == null) {
			System.err.println("Failed to load dependency world for procedure WorldGenCephalotaxus!");
			return;
		}
		...
		int TrunkHeight = 1 + (int) (Math.round(Math.random() * 6D));
		int counter = 0;
		int yy = y;
		int upright = 0;
		int yyy = 0;
		...
		if ((world.canSeeSky(new BlockPos((int) x, (int) y, (int) z)))
			...
			while (counter <= TrunkHeight) {
				yy = y + counter;
				ProcedureTreeLog.executeProcedure((int) x, (int) yy, (int) z, world, BlockCephalotaxusLog.block, EnumFacing.NORTH);
				counter = counter + 1;
			}
			...
			if (Math.random() > 0.85) {
				//North
				ProcedureTreeLog.executeProcedure(x, yy, z - 1, world, BlockCephalotaxusLog.block, EnumFacing.WEST);
			...
			if (Math.random() > 0.85) {
				//East
				ProcedureTreeLog.executeProcedure(x + 1, yy, z, world, BlockCephalotaxusLog.block, EnumFacing.UP);
			...
			while (yy > (Math.round((double)TrunkHeight * 0.38) + y)) {
				...
				if (Math.random() > 0.66) {
					...
					ProcedureTreeLog.executeProcedure(x, yy, z - 1, world, BlockCephalotaxusLog.block, EnumFacing.EAST);
					ProcedureLeavesAroundLog.executeProcedure(x, yy, z - 1, world, getLeaves(), 1, 0.3);
					ProcedureTreeLog.executeProcedure(x, yy, z - 2, world, BlockCephalotaxusLog.block, EnumFacing.EAST);
					upright = rand.nextInt(3);
					yyy = 1;
					while (yyy <= upright) {
						ProcedureTreeLog.executeProcedure(x, yy + yyy, z - 2, world, BlockCephalotaxusLog.block, EnumFacing.NORTH);
				...
				if (Math.random() > 0.66) {
					...
					ProcedureTreeLog.executeProcedure(x + 2, yy, z, world, BlockCephalotaxusLog.block, EnumFacing.UP);
					upright = rand.nextInt(3);
					yyy = 1;
					while (yyy <= upright) {
						ProcedureTreeLog.executeProcedure(x + 2, yy + yyy, z, world, BlockCephalotaxusLog.block, EnumFacing.NORTH);
	...
	{
		if (Math.random() > 0.8) {
			return BlockCephalotaxusLeavesBerries.block;
		}
		return BlockCephalotaxusLeaves.block;
	}
