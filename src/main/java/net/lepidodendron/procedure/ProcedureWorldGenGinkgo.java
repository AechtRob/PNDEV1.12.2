...
public class ProcedureWorldGenGinkgo extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenGinkgo(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
	...
	public static void executeProcedure(java.util.HashMap<String, Object> dependencies) {
		...
		if ((world.canSeeSky(new BlockPos((int) x, (int) y, (int) z)))
			...
			while (counter <= TrunkHeight) {
				ProcedureTreeLog.executeProcedure(x, (int) (y + counter), z, world, BlockGinkgoLog.block, EnumFacing.NORTH);
				counter = counter + 1;
				}
			...
			yy = y + (int) counter;
			zz = z;
			ProcedureTreeLeaf.executeProcedure(xx, yy, zz, world, BlockGinkgoLeaves.block);
			...
			while (counter <= TrunkHeight) {
				...
				while (counterext <= (1 + randomiser) * ((((((TrunkHeight - counter) / TrunkHeight) * (TrunkHeight / 6)) * ((counter / TrunkHeight) * (TrunkHeight / 6))) * 0.66))) {
					xx = x;
					yy = y + (int) counter;
					zz = z - (int) counterext;
					ProcedureTreeLog.executeProcedure(xx, yy, zz, world, BlockGinkgoLog.block, EnumFacing.WEST);
					ProcedureLeavesAroundLog.executeProcedure(xx, yy, zz, world, BlockGinkgoLeaves.block, 2, 0.2);
				...
				if ((counterext>1) && (Math.random() * (7/counter) >= 0.2)) {
					...
					ProcedureTreeLog.executeProcedure(xx, yy, zz, world, BlockGinkgoLog.block, EnumFacing.SOUTH);
					ProcedureLeavesAroundLog.executeProcedure(xx, yy, zz, world, BlockGinkgoLeaves.block, 1, 0.2);
					
					xx = x;
					yy = yy + 1;
					zz = zz;
					ProcedureTreeLog.executeProcedure(xx, yy, zz, world, BlockGinkgoLog.block, EnumFacing.SOUTH);
					ProcedureLeavesAroundLog.executeProcedure(xx, yy, zz, world, BlockGinkgoLeaves.block, 2, 0.45);
				...
				if (counterext > 3) {
					...
					while (randomiser2 <= (counterext * 0.7)) {
						...
						if (Math.random() > 0.5) {
							...
							ProcedureTreeLog.executeProcedure(xx, yy, zz, world, BlockGinkgoLog.block, EnumFacing.UP);
						}
						ProcedureLeavesAroundLog.executeProcedure(xx, yy, zz, world, BlockGinkgoLeaves.block, 2, 0.45);
