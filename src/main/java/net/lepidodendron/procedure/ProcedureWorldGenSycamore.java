...
import net.lepidodendron.block.BlockSycamoreLeaves;
...
public class ProcedureWorldGenSycamore extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenSycamore(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    private static double leafDensity = 0.6D;
    ...
    static void generateLeafNodeList()
    {
        ...
        foliageCoords = Lists.<ProcedureWorldGenSycamore.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenSycamore.FoliageCoordinates(basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            foliageCoords.add(new ProcedureWorldGenSycamore.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    {
        for (ProcedureWorldGenSycamore.FoliageCoordinates procedureworldgensycamore$foliagecoordinates : foliageCoords)
        {
            generateLeafNode(procedureworldgensycamore$foliagecoordinates);
        }
    }
    ...
    static void generateTrunk(int trunkSize)
    {
        ...
        Block block = BlockSycamoreLog.block;
    ...
    static void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            Functions.setBlockStateAndCheckForDoublePlant(world,blockpos1, BlockSycamoreLog.block.getDefaultState().withProperty(FACING,blocklog$enumaxis), 3);
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenSycamore.FoliageCoordinates procedureworldgensycamore$foliagecoordinates : foliageCoords)
        {
            int i = procedureworldgensycamore$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(basePos.getX(), i, basePos.getZ());

			//System.err.println("Branch height? " + i);

            if (!blockpos.equals(procedureworldgensycamore$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
            	//System.err.println("limb called " + blockpos.getX() + " " + blockpos.getY() + " " + blockpos.getZ());
                limb(blockpos, procedureworldgensycamore$foliagecoordinates, BlockSycamoreLog.block);
            }
        }
    } 
