...
import net.lepidodendron.block.BlockCinnamonLeaves;
...
public class ProcedureWorldGenCinnamon extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenCinnamon(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    private static int heightLimit;
   	private static int heightLimitLimit = 8;
    private static double heightAttenuation = 0.618D;
    private static double leafDensity = 2.5D;
    private static int leafDistanceLimit = 2;
    private static List<ProcedureWorldGenCinnamon.FoliageCoordinates> foliageCoords;
    private static double branchSlope = 0.681D;
    private static double scaleWidth = 1.1D;
    ...
    static void generateLeafNodeList()
    {
        ...
        int k = heightLimit - 2;
        foliageCoords = Lists.<ProcedureWorldGenCinnamon.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenCinnamon.FoliageCoordinates(basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            foliageCoords.add(new ProcedureWorldGenCinnamon.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    {
        for (ProcedureWorldGenCinnamon.FoliageCoordinates procedureworldgencinnamon$foliagecoordinates : foliageCoords)
        {
            generateLeafNode(procedureworldgencinnamon$foliagecoordinates);
        }
    }
    ...
    static void generateLeafNode(BlockPos pos)
    {
        ...
        {
            //crosSection(pos.up(i), leafSize(i), BlockCinnamonLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
            crosSection(pos.up(i), leafSize(i), BlockCinnamonLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    static void generateTrunk()
    {
        ...
        Block block = BlockCinnamonLog.block;
    ...
    static void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            Functions.setBlockStateAndCheckForDoublePlant(world,blockpos1, BlockCinnamonLog.block.getDefaultState().withProperty(FACING,blocklog$enumaxis), 3);
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenCinnamon.FoliageCoordinates procedureworldgencinnamon$foliagecoordinates : foliageCoords)
        {
            int i = procedureworldgencinnamon$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(basePos.getX(), i, basePos.getZ());

			//System.err.println("Branch height? " + i);

            if (!blockpos.equals(procedureworldgencinnamon$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
            	//System.err.println("limb called " + blockpos.getX() + " " + blockpos.getY() + " " + blockpos.getZ());
                limb(blockpos, procedureworldgencinnamon$foliagecoordinates, BlockCinnamonLog.block);
            }
        }
    } 
