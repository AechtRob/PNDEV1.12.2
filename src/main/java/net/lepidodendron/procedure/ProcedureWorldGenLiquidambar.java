...
public class ProcedureWorldGenLiquidambar extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenLiquidambar(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    static void generateLeafNodeList()
    {
        ...
        int j = basePos.getY() + height - rand.nextInt(5);
        int k = heightLimit - leafDistanceLimit;
        foliageCoords = Lists.<ProcedureWorldGenLiquidambar.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenLiquidambar.FoliageCoordinates(basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            foliageCoords.add(new ProcedureWorldGenLiquidambar.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    static void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            Functions.setBlockStateAndCheckForDoublePlant(world,blockpos1, BlockLiquidambarLog.block.getDefaultState().withProperty(FACING,blocklog$enumaxis), 3);
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenLiquidambar.FoliageCoordinates procedureworldgenplane$foliagecoordinates : foliageCoords)
        {
            ...
            if (!blockpos.equals(procedureworldgenplane$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
                ...
                limb(blockpos, procedureworldgenplane$foliagecoordinates, BlockLiquidambarLog.block);
            }
        }
    } 
