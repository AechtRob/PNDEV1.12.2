...
public class ProcedureWorldGenEmbothrium extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenEmbothrium(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    private static List<ProcedureWorldGenEmbothrium.FoliageCoordinates> foliageCoords;
...
//        }
        ...
        foliageCoords = Lists.<ProcedureWorldGenEmbothrium.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenEmbothrium.FoliageCoordinates(basePos.up(k), j));
    ...
    {
        for (ProcedureWorldGenEmbothrium.FoliageCoordinates procedureworldgenembothrium$foliagecoordinates : foliageCoords)
        {
            generateLeafNode(procedureworldgenembothrium$foliagecoordinates);
        }
    }
    ...
    static void generateLeafNode(BlockPos pos)
    {
        ...
        {
            //crosSection(pos.up(i), leafSize(i), BlockEmbothriumLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
            crosSection(pos.up(i), leafSize(i), BlockEmbothriumLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenEmbothrium.FoliageCoordinates procedureworldgenembothrium$foliagecoordinates : foliageCoords)
        {
            int i = procedureworldgenembothrium$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(basePos.getX(), i, basePos.getZ());

			//System.err.println("Branch height? " + i);

            if (!blockpos.equals(procedureworldgenembothrium$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
            	//System.err.println("limb called " + blockpos.getX() + " " + blockpos.getY() + " " + blockpos.getZ());
                limb(blockpos, procedureworldgenembothrium$foliagecoordinates, BlockEmbothriumLog.block);
            }
        }
    } 
