...
import net.lepidodendron.block.BlockLaurusLeaves;
import net.lepidodendron.block.BlockLaurusLog;
...
public class ProcedureWorldGenLaurus extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenLaurus(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    private static double leafDensity = 0.85D;
    private static int leafDistanceLimit = 3;
    private static List<ProcedureWorldGenLaurus.FoliageCoordinates> foliageCoords;
    private static double branchSlope = 0.681D;
    private static double scaleWidth = 1.7D;
    ...
    static void generateLeafNodeList()
    {
        ...
        foliageCoords = Lists.<ProcedureWorldGenLaurus.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenLaurus.FoliageCoordinates(basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            foliageCoords.add(new ProcedureWorldGenLaurus.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    {
        for (ProcedureWorldGenLaurus.FoliageCoordinates procedureworldgenapple$foliagecoordinates : foliageCoords)
        {
    ...
    static void generateLeafNode(BlockPos pos)
    {
        ...
        {
            //crosSection(pos.up(i), leafSize(i), BlockLaurusLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
            crosSection(pos.up(i), leafSize(i), BlockLaurusLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    static void generateTrunk()
    {
        ...
        Block block = BlockLaurusLog.block;
    ...
    static void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            Functions.setBlockStateAndCheckForDoublePlant(world,blockpos1, BlockLaurusLog.block.getDefaultState().withProperty(FACING,blocklog$enumaxis), 3);
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenLaurus.FoliageCoordinates procedureworldgenapple$foliagecoordinates : foliageCoords)
        {
            ...
            if (!blockpos.equals(procedureworldgenapple$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
                ...
                limb(blockpos, procedureworldgenapple$foliagecoordinates, BlockLaurusLog.block);
            }
        }
    } 
