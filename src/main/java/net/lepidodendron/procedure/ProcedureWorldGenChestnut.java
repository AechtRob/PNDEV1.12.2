...
import net.lepidodendron.block.BlockChestnutLeaves;
import net.lepidodendron.block.BlockChestnutLog;
...
public class ProcedureWorldGenChestnut extends ElementsLepidodendronMod.ModElement {
	public ProcedureWorldGenChestnut(ElementsLepidodendronMod instance) {
		super(instance, 42);
	}
    ...
    private static double leafDensity = 0.7D;
    private static int leafDistanceLimit = 3;
    private static List<ProcedureWorldGenChestnut.FoliageCoordinates> foliageCoords;
    private static double branchSlope = 0.895D;
    ...
    static void generateLeafNodeList()
    {
        ...
        foliageCoords = Lists.<ProcedureWorldGenChestnut.FoliageCoordinates>newArrayList();
        foliageCoords.add(new ProcedureWorldGenChestnut.FoliageCoordinates(basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            foliageCoords.add(new ProcedureWorldGenChestnut.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    {
        for (ProcedureWorldGenChestnut.FoliageCoordinates procedureworldgenchestnut$foliagecoordinates : foliageCoords)
        {
            generateLeafNode(procedureworldgenchestnut$foliagecoordinates);
        }
    }
    ...
    static void generateLeafNode(BlockPos pos)
    {
        ...
        {
            //crosSection(pos.up(i), leafSize(i), BlockChestnutLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
            crosSection(pos.up(i), leafSize(i), BlockChestnutLeaves.block.getDefaultState().withProperty(CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    static void generateTrunk(int trunkSize)
    {
        ...
        Block block = BlockChestnutLog.block;
    ...
    static void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_)
    {
        ...
        for (int j = 0; j <= i; ++j)
        {
            ...
            Functions.setBlockStateAndCheckForDoublePlant(world,blockpos1, BlockChestnutLog.block.getDefaultState().withProperty(FACING,blocklog$enumaxis), 3);
    ...
    static void generateLeafNodeBases()
    {
        for (ProcedureWorldGenChestnut.FoliageCoordinates procedureworldgenchestnut$foliagecoordinates : foliageCoords)
        {
            int i = procedureworldgenchestnut$foliagecoordinates.getBranchBase();
            BlockPos blockpos = new BlockPos(basePos.getX(), i, basePos.getZ());

			//System.err.println("Branch height? " + i);

            if (!blockpos.equals(procedureworldgenchestnut$foliagecoordinates) && leafNodeNeedsBase(i - basePos.getY()))
            {
            	//System.err.println("limb called " + blockpos.getX() + " " + blockpos.getY() + " " + blockpos.getZ());
                limb(blockpos, procedureworldgenchestnut$foliagecoordinates, BlockChestnutLog.block);
            }
        }
    } 
