...
public class BlockYam extends ElementsLepidodendronMod.ModElement {
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimYam))
...
//		}
		...
		if (!matchBiome(biome, LepidodendronConfigPlants.genYamBlacklistBiomes)) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genYamOverrideBiomes))
			biomeCriteria = true;
		if (!LepidodendronConfigPlants.genYam && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			biomeCriteria = false;
...
//		}
		...
		int GenChance = 30;
		double GenMultiplier = LepidodendronConfigPlants.multiplierYam;
