...
public class BlockOmprelostrobus extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:omprelostrobus")
	public static final Block block = null;
	public BlockOmprelostrobus(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.omprelostrobus);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:omprelostrobus", "inventory"));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:omprelostrobus", BlockOmprelostrobus.block);
		OreDictionary.registerOre("plantPrehistoric", BlockOmprelostrobus.block);
		OreDictionary.registerOre("plant", BlockOmprelostrobus.block);
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimOmprelostrobus))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genOmprelostrobus && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genOmprelostrobusBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genOmprelostrobusOverrideBiomes))
			biomeCriteria = true;
		...
		int GenChance = 6;
		double GenMultiplier = LepidodendronConfigPlants.multiplierOmprelostrobus;
		...
		int maxheight = LepidodendronConfigPlants.maxheightOmprelostrobus;
		int minheight = LepidodendronConfigPlants.minheightOmprelostrobus;
								...
								if (((BlockCustomFlower) block).canBlockStay(world, blockpos1)) {
									...
									if (k == j) {
										if (k >= height + random.nextInt(2) - random.nextInt(2)) {
											world.setBlockState(blockpos1.up(k), BlockOmprelostrobusShoot.block.getDefaultState(), 2);
										}
									}
								}
							}
	...
	public static class BlockCustomFlower extends BlockReed implements IAdvancementGranter {
		...
		public BlockCustomFlower() {
			...
			setResistance(0.2F);
			setLightLevel(0F);
			setTranslationKey("pf_omprelostrobus");
			setRegistryName("omprelostrobus");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_OMPRELOSTROBUS;
		}
		...
		public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos){
			...
			if (worldIn.getBlockState(pos.up()).getBlock() != this && worldIn.getBlockState(pos.up()).getBlock() != BlockOmprelostrobusShoot.block) {
				if (isBottom) {
			...
			if (world.getBlockState(pos.down()).getBlock() == block || this.checkForDrop(world, pos, state)) {
				if (world.isAirBlock(pos.up())) {
					...
					if (l >= height + random.nextInt(2) - random.nextInt(2) - 1) {
						world.setBlockState(pos.up(), BlockOmprelostrobusShoot.block.getDefaultState(), 2);
					}
				}
			}
		}
