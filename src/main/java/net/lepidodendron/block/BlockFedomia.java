...
public class BlockFedomia extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:fedomia")
	public static final Block block = null;
	public BlockFedomia(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.fedomia);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("fedomia"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:fedomia", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockFedomia.LEVEL).build());
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (biome.getRegistryName().toString().equalsIgnoreCase("lepidodendron:ediacaran_extreme_hills")
			|| biome.getRegistryName().toString().equalsIgnoreCase("lepidodendron:ediacaran_stromatolite_pavement")) {
			biomeCriteria = true;
		}
		if (!biomeCriteria)
			return;
			...
			(new WorldGenReed() {
				...
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < 24; ++i) {
						...
						if (!(world.getBiome(blockpos1).getRegistryName().toString().equalsIgnoreCase("lepidodendron:ediacaran_extreme_hills")
						...
						if (Functions.isWater(world, blockpos1)) {
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockFedomia.BlockCustom.FACING, enumfacing), 2);
									return true;
								} else {
									for (EnumFacing enumfacing1 : BlockFedomia.BlockCustom.FACING.getAllowedValues()) {
										pos1 = blockpos1;
										...
										if (enumfacing1 != EnumFacing.UP && enumfacing1 != EnumFacing.DOWN &&
											...
											world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockFedomia.BlockCustom.FACING, enumfacing1), 2);
											return true;
										}
									}
								}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_FEDOMIA;
		}
