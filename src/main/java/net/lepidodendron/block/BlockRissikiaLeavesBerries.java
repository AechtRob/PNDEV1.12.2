...
public class BlockRissikiaLeavesBerries extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:rissikia_leaves_berries")
	public static final Block block = null;
	public BlockRissikiaLeavesBerries(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.rissikia_leaves_berries);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("rissikia_leaves_berries"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:rissikia_leaves_berries", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:rissikia_sapling", BlockRissikiaLeavesBerries.block);
		OreDictionary.registerOre("plantPrehistoric", BlockRissikiaLeavesBerries.block);
		OreDictionary.registerOre("plant", BlockRissikiaLeavesBerries.block);
		OreDictionary.registerOre("treeLeaves", BlockRissikiaLeavesBerries.block);
	}
	...
	public static class BlockCustom extends BlockLeavesPF {
		public BlockCustom() {
			super();
			setTranslationKey("pf_rissikia_leaves_berries");
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation) {
				...
				if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(ItemRissikiaBerries.block, (int) (1));
					stackSeed.setCount(1);
					ItemHandlerHelper.giveItemToPlayer(player, stackSeed);
					world.setBlockState(pos, BlockRissikiaLeavesBerries.block.getDefaultState().withProperty(DECAYABLE, state.getValue(DECAYABLE)).withProperty(CHECK_DECAY, state.getValue(CHECK_DECAY)).withProperty(FRUITS, false), 3);
					return true;
				}
				return true;
			}
