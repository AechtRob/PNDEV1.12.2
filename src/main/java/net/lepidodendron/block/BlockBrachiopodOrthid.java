...
public class BlockBrachiopodOrthid extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:brachiopod_orthid")
	public static final Block block = null;
	public BlockBrachiopodOrthid(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.brachiopod_orthid);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("brachiopod_orthid"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:brachiopod_orthid", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockBrachiopodOrthid.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("dnaPNBrachiopodOrthid", BlockBrachiopodOrthid.block);
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		int multiplier = 1;
		if ((dimID == LepidodendronConfig.dimDevonian)
				|| (dimID == LepidodendronConfig.dimCambrian) || (dimID == LepidodendronConfig.dimSilurian)
				|| (dimID == LepidodendronConfig.dimPermian) || (dimID == LepidodendronConfig.dimCarboniferous)
		) {
			multiplier = 2;
		}
		if ((dimID == LepidodendronConfig.dimOrdovician)
					...
					for (int i = 0; i < 40; ++i) {
						...
						if (world.getBlockState(blockpos1).getBlock() == Blocks.WATER || world.getBlockState(blockpos1).getBlock() == Blocks.FLOWING_WATER) {
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								...
								for (EnumFacing enumfacing1 : BlockBrachiopodOrthid.BlockCustom.FACING.getAllowedValues()) {
									pos1 = blockpos1;
									...
									if (enumfacing1 != EnumFacing.UP && enumfacing1 != EnumFacing.DOWN &&
										...
										world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockBrachiopodOrthid.BlockCustom.FACING, enumfacing1), 2);
										return true;
									}
								}
							}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable  {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_brachiopod_orthid");
    	...
    	public BlockRenderLayer getRenderLayer()
    {
		...
		public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
			if (LepidodendronConfig.showTooltips) {
				...
				tooltip.add("Periods: Cambrian - Ordovician - Silurian - Devonian - Carboniferous - Permian");}
