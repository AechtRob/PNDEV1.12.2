...
public class BlockWoodHorsetail extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:wood_horsetail")
	public static final Block block = null;
	public BlockWoodHorsetail(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.wood_horsetail);
	}
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:wood_horsetail", BlockWoodHorsetail.block);
    ...
    public BlockRenderLayer getRenderLayer()
    {
	        ...
	        if (!world.isRemote)
	        {
				...
				if (!this.canSurviveAt(world, pos))
		        {
					...
					if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock)) && (canSurviveAt(world, targetBlock))) {
						world.setBlockState(targetBlock, BlockWoodHorsetail.block.getDefaultState(), 3);	
					}
		...
		public boolean canSurviveAt(World worldIn, BlockPos pos) {
			...
			//System.err.println("Light:" + getLight);
		...
		public Block planted() {
			return BlockWoodHorsetail.block;
		}
