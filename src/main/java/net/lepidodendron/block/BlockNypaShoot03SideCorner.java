...
public class BlockNypaShoot03SideCorner extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:nypa_shoot_03_side_corner")
	public static final Block block = null;
	public BlockNypaShoot03SideCorner(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.nypa_shoot);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("nypa_shoot_03_side_corner"));
	...
	//			new ModelResourceLocation("lepidodendron:nypa_shoot_03_side_corner", "inventory"));
	//}
	...
	public static class BlockCustom extends BlockLeaves implements IAdvancementGranter {
		...
		public BlockCustom() {
			setTranslationKey("pf_nypa_shoot_03_side_corner");
    	...
    	public BlockRenderLayer getRenderLayer()
    {
		...
		public void neighborChanged(IBlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (((world.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.NORTH) 
				&& (world.getBlockState(pos.south().west()).getBlock() != BlockNypaShoot03.block))
				{
			...
			if (((world.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.SOUTH) 
				&& (world.getBlockState(pos.north().east()).getBlock() != BlockNypaShoot03.block))
				{
				world.destroyBlock(pos, false);
				return;
			}
			...
			if (((world.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.EAST) 
				&& (world.getBlockState(pos.west().north()).getBlock() != BlockNypaShoot03.block))
				{
				world.destroyBlock(pos, false);
				return;
			}
			...
			if (((world.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.WEST) 
				&& (world.getBlockState(pos.east().south()).getBlock() != BlockNypaShoot03.block))
				{
				world.destroyBlock(pos, false);
				return;
			}
		...
		public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
			...
			if (((worldIn.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.NORTH) 
				&& (worldIn.getBlockState(pos.south().west()).getBlock() != BlockNypaShoot03.block))
				{
			...
			if (((worldIn.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.SOUTH) 
				&& (worldIn.getBlockState(pos.north().east()).getBlock() != BlockNypaShoot03.block))
				{
				worldIn.destroyBlock(pos, false);
				return;
			}
			...
			if (((worldIn.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.EAST) 
				&& (worldIn.getBlockState(pos.west().north()).getBlock() != BlockNypaShoot03.block))
				{
				worldIn.destroyBlock(pos, false);
				return;
			}
			...
			if (((worldIn.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.WEST) 
				&& (worldIn.getBlockState(pos.east().south()).getBlock() != BlockNypaShoot03.block))
				{
				worldIn.destroyBlock(pos, false);
				return;
			}
