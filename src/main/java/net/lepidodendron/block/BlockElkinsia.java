...
import net.lepidodendron.item.ItemElkinsiaSeeds;
...
public class BlockElkinsia extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:elkinsia")
	public static final Block block = null;
	public BlockElkinsia(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.elkinsia);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:elkinsia", "inventory"));
	}
        ...
        if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation)
	        {
	        	...
	        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					if (Math.random() > 0.5) {
						ItemStack stackSeed = new ItemStack(ItemElkinsiaSeeds.block, (int) (1));
