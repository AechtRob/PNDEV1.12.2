...
public class BlockLesleyaCone extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:lesleya_shoot_cone")
	public static final Block block = null;
	public BlockLesleyaCone(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.lesleya_cone);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("lesleya_shoot_cone"));
    	...
    	public BlockRenderLayer getRenderLayer()
    {
		...
		public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
			...
			if (Math.random()>0.8 && Math.random()>0.8 && (!(((Boolean)state.getValue(CHECK_DECAY)).booleanValue() && ((Boolean)state.getValue(DECAYABLE)).booleanValue()))) {
				worldIn.setBlockState(pos, BlockLesleya.block.getDefaultState(),3);
			}
		}
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
		{
			if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation)
			{
				...
				if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(ItemLesleyaCone.block, (int) (1));
					...
					if (Math.random() > 0.95) {
						...
						world.setBlockState(pos, BlockLesleya.block.getDefaultState());
					}
					return true;
				}
				return true;
