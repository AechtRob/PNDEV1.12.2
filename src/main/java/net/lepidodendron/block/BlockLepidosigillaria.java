...
public class BlockLepidosigillaria extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:lepidosigillaria")
	public static final Block block = null;
	public BlockLepidosigillaria(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.lepidosigillaria);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:lepidosigillaria", "inventory"));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:lepidosigillaria", BlockLepidosigillaria.block);
		OreDictionary.registerOre("plantPrehistoric", BlockLepidosigillaria.block);
		OreDictionary.registerOre("plant", BlockLepidosigillaria.block);
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimLepidosigillaria))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genLepidosigillaria && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genLepidosigillariaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genLepidosigillariaOverrideBiomes))
			biomeCriteria = true;
		...
		int GenChance = 6;
		double GenMultiplier = LepidodendronConfigPlants.multiplierLepidosigillaria;
		...
		int maxheight = LepidodendronConfigPlants.maxheightLepidosigillaria;
		int minheight = LepidodendronConfigPlants.minheightLepidosigillaria;
								...
								if (((BlockCustomFlower) block).canBlockStay(world, blockpos1)) {
									...
									if (k == j) {
										if (k >= height + random.nextInt(2) - random.nextInt(2)) {
											world.setBlockState(blockpos1.up(k), BlockLepidosigillariaShoot.block.getDefaultState(), 2);
										}
									}
								}
							}
	...
	public static class BlockCustomFlower extends BlockReed {
		...
		public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos){
			...
			if (worldIn.getBlockState(pos.up()).getBlock() != this && worldIn.getBlockState(pos.up()).getBlock() != BlockLepidosigillariaShoot.block) {
				if (isBottom) {
			...
			if (world.getBlockState(pos.down()).getBlock() == block || this.checkForDrop(world, pos, state)) {
				if (world.isAirBlock(pos.up())) {
					...
					if (l >= height + random.nextInt(2) - random.nextInt(2) - 1) {
						world.setBlockState(pos.up(), BlockLepidosigillariaShoot.block.getDefaultState(), 2);
					}
				}
			}
		}
