...
import net.lepidodendron.item.ItemAnomozamitesSeeds;
...
public class BlockAnomozamitesLeaves extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:anomozamites_leaves_worldgen")
	public static final Block block = null;
	public BlockAnomozamitesLeaves(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.anomozamites_leaves_worldgen);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("anomozamites_leaves_worldgen"));
	...
	public void registerModels(ModelRegistryEvent event) {
		...
		//		new ModelResourceLocation("lepidodendron:anomozamites_leaves_worldgen", "inventory"));
	...
	public static class BlockCustom extends BlockLeaves {
		...
		public BlockCustom() {
			super();
			setTranslationKey("pf_anomozamites_leaves_worldgen");
    ...
    public BlockRenderLayer getRenderLayer()
    {
        ...
        public ItemStack getSilkTouchDrop(IBlockState state)  {
            return new ItemStack(BlockAnomozamitesLeavesPlaceable.block, (int) (1));
        }
		...
		public NonNullList<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
			return NonNullList.withSize(1, new ItemStack(BlockAnomozamitesLeavesPlaceable.block, (int) (1)));
		}
		...
		public void neighborChanged(IBlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			if (((Boolean)state.getValue(CHECK_DECAY)).booleanValue() && ((Boolean)state.getValue(DECAYABLE)).booleanValue())
				...
				if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.NORTH) {
					...
					if (block != BlockAnomozamitesLog.block)
						{
							world.setBlockToAir(pos);
							if ((Math.random() >= 0.5) && (!LepidodendronConfig.doPropagation)) {
									//Spawn another sapling:
									if (!world.isRemote) {
										EntityItem entityToSpawn = new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockAnomozamitesSapling.block, (int) (1)));
					...
					if (block != BlockAnomozamitesLog.block)
						{
							...
							if ((Math.random() >= 0.5) && (!LepidodendronConfig.doPropagation)) {
									//Spawn another sapling:
									if (!worldIn.isRemote) {
										EntityItem entityToSpawn = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockAnomozamitesSapling.block, (int) (1)));
										entityToSpawn.setPickupDelay(10);
        ...
        if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation)
	        {
	        	...
	        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					if (Math.random() > 0.5) {
						ItemStack stackSeed = new ItemStack(ItemAnomozamitesSeeds.block, (int) (1));
