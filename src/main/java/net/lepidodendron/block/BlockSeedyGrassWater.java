...
import net.lepidodendron.item.ItemSeedyGrassSeeds;
...
public class BlockSeedyGrassWater extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:seedy_grass_water")
	public static final Block block = null;
	public BlockSeedyGrassWater(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.seedy_grass);
	}
	...
	public static class BlockCustom extends BlockLilyPad implements IGrowable, net.minecraftforge.common.IShearable, IAdvancementGranter {
		public BlockCustom() {
			...
			setLightOpacity(0);
			setCreativeTab(null);
			if (LepidodendronConfigPlants.spreadSeedyGrass && LepidodendronConfig.spreadPlantsAtAll) {
				setTickRandomly(true);
			}
			else {
				setTickRandomly(false);
			}
			setTranslationKey("pf_seedy_grass_water");
			setRegistryName("seedy_grass_water");
		}

		@Nullable
		@Override
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_SEEDY_GRASS;
		}
		...
		public void onEntityCollision(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
			...
			if (entityIn instanceof EntityBoat)
			{
				...
				if (!LepidodendronConfig.doPropagation) {
					EntityItem entityToSpawn = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ItemSeedyGrassItem.block, (int) (1)));
		...
		public NonNullList<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
			return NonNullList.withSize(1, new ItemStack(ItemSeedyGrassItem.block, (int) (1)));
		}
		...
		public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player) {
			return new ItemStack(ItemSeedyGrassItem.block, (int) (1));
		}
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				drops.add(new ItemStack(ItemSeedyGrassItem.block, (int) (1)));
			}
		}
				...
				if (!this.canSurviveAt(world, pos))
		        {
					...
					if (canSurviveAt(world, targetBlock)) {
						if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock))) {
							world.setBlockState(targetBlock, BlockSeedyGrassWater.block.getDefaultState(), 3);
						}
					}
					else if (BlockSeedyGrassLand.BlockCustom.canSurviveAt(world, targetBlock)) {
						if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock))) {
							world.setBlockState(targetBlock, BlockSeedyGrassLand.block.getDefaultState(), 3);
						}
					}
	    ...
	    {
	    	if (!LepidodendronConfigPlants.spreadSeedyGrass) {return true;}
	        return false;
	    }
...
//	    	}
	    ...
	    {
        if ((!player.capabilities.allowEdit) || (player.getHeldItemMainhand().getItem() != new ItemStack(ItemSeedyGrassItem.block, (int) (1)).getItem()) || !LepidodendronConfig.doPropagation)
	        {
	            return true;
	        }
	        else {
	        	...
	        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					if (Math.random() > 0.35) {
						ItemStack stackSeed = new ItemStack(ItemSeedyGrassSeeds.block, (int) (1));
