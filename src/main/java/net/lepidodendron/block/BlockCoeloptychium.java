...
public class BlockCoeloptychium extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:coeloptychium")
	public static final Block block = null;
	public BlockCoeloptychium(ElementsLepidodendronMod instance) { super(instance, LepidodendronSorter.coeloptychium); }

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("coeloptychium"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:coeloptychium", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockCoeloptychium.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("staticdnaPNlepidodendron:coeloptychium", BlockCoeloptychium.block);
	}
				...
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < 64; ++i) {
						...
						if (biome instanceof BiomeCambrian) {
							era = biome.getRegistryName().toString().equalsIgnoreCase("lepidodendron:cretaceous");
						}
						if (!era) {
							continue;
						}
						if (Functions.isWater(world, blockpos1)) {
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockCoeloptychium.BlockCustom.FACING, enumfacing), 2);
									return true;
								} else {
									for (EnumFacing enumfacing1 : BlockCoeloptychium.BlockCustom.FACING.getAllowedValues()) {
										pos1 = blockpos1;
										...
										if (enumfacing1 != EnumFacing.UP && enumfacing1 != EnumFacing.DOWN &&
											...
											world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockCoeloptychium.BlockCustom.FACING, enumfacing1), 2);
											return true;
										}
									}
								}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_coeloptychium");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_COELOPTYCHIUM;
		}
