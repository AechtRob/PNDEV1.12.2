...
public class BlockErnietta extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:ernietta")
	public static final Block block = null;
	public BlockErnietta(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.ernietta);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("ernietta"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:ernietta", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockErnietta.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("staticdnaPNlepidodendron:ernietta", BlockErnietta.block);
	}
			...
			(new WorldGenReed() {
				...
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < 12; ++i) {
						...
						if (!(biomeName.equalsIgnoreCase("lepidodendron:ediacaran_shallow_reef"))
						) {
							continue;
						}
						if (world.getBlockState(blockpos1).getBlock() == Blocks.WATER) {
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockErnietta.BlockCustom.FACING, enumfacing), 2);
									return true;
								}
							}
						}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_ERNIETTA;
		}
		...
		public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
			if (LepidodendronConfig.showTooltips) {
				tooltip.add("Type: Frondose bag-like organism");
