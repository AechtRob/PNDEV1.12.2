...
import net.lepidodendron.item.ItemCycasCone;
...
public class BlockCycasShootCone extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:cycas_shoot_cone")
	public static final Block block = null;
	public BlockCycasShootCone(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.cycas_shoot_cone);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("cycas_shoot_cone"));
	...
	public void registerModels(ModelRegistryEvent event) {
		...
		//		new ModelResourceLocation("lepidodendron:cycas_shoot_worldgen", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockLeaves.DECAYABLE, BlockLeaves.CHECK_DECAY).build());
	}
	public static class BlockCustom extends BlockLeaves implements IAdvancementGranter {
		public BlockCustom() {
			super();
			setTranslationKey("pf_cycas_shoot_cone");
		...
		public NonNullList<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
			return NonNullList.withSize(1, new ItemStack(BlockCycasShootPlaceable.block, (int) (1)));
		}
		...
		public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
			...
			if (((Boolean)state.getValue(CHECK_DECAY)).booleanValue() && ((Boolean)state.getValue(DECAYABLE)).booleanValue())
			{
				...
				if (block != BlockCycasLog.block) {
					...
					worldIn.setBlockState(pos, BlockCycasShoot.block.getDefaultState(), 3);
				}
			}
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
		{
			if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation)
			{
				...
				if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(ItemCycasCone.block, (int) (1));
					...
					if (Math.random() > 0.95) {
						...
						world.setBlockState(pos, BlockCycasShoot.block.getDefaultState());
					}
					return true;
				}
				return true;
