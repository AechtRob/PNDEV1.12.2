...
public class BlockUtrechtiaLeavesPlaceable extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:utrechtia_leaves")
	public static final Block block = null;
	public BlockUtrechtiaLeavesPlaceable(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.utrechtia_leaves);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("utrechtia_leaves"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:utrechtia_leaves", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:utrechtia_sapling", BlockUtrechtiaLeavesPlaceable.block);
		OreDictionary.registerOre("plantPrehistoric", BlockUtrechtiaLeavesPlaceable.block);
	    ...
	    public void onBlockAdded(World world, BlockPos pos, IBlockState state) 
	    {
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.NORTH) {
				if (world.isAirBlock(pos.north())) {
					world.setBlockState(pos.north(), BlockUtrechtiaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.NORTH), 3);
				}
				else {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.SOUTH) {
				if (world.isAirBlock(pos.south())) {
					world.setBlockState(pos.south(), BlockUtrechtiaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.SOUTH), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.EAST) {
				if (world.isAirBlock(pos.east())) {
					world.setBlockState(pos.east(), BlockUtrechtiaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.EAST), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.WEST) {
				if (world.isAirBlock(pos.west())) {
					world.setBlockState(pos.west(), BlockUtrechtiaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.WEST), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.UP) {
				if (world.isAirBlock(pos.up())) {
					world.setBlockState(pos.up(), BlockUtrechtiaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.UP), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.DOWN) {
				if (world.isAirBlock(pos.down())) {
					world.setBlockState(pos.down(), BlockUtrechtiaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.DOWN), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
				...
				if (block == BlockUtrechtiaLeavesTop.block)
				{
				...
				}.getEnumFacing(pos.north()) != EnumFacing.NORTH)
					{
						...
						if ((Math.random() >= 0.8) &&  (!world.isRemote)) {
							if (!LepidodendronConfig.doPropagation) {
								EntityItem entityToSpawn = new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockUtrechtiaSapling.block, (int) (1)));
								entityToSpawn.setPickupDelay(10);
								world.spawnEntity(entityToSpawn);
							}
							else {
								EntityItem entityToSpawn = new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ItemUtrechtiaFruit.block, (int) (1)));
								entityToSpawn.setPickupDelay(10);
								world.spawnEntity(entityToSpawn);
							}
						}
