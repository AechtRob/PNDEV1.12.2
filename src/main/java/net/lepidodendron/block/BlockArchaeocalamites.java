...
public class BlockArchaeocalamites extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:archaeocalamites")
	public static final Block block = null;
	public BlockArchaeocalamites(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.archaeocalamites);
	}
	...
	//public void registerModels(ModelRegistryEvent event) {
		...
		//		new ModelResourceLocation("lepidodendron:archaeocalamites", "inventory"));
	//}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimArchaeocalamites))
		...
		int GenChance = 3;
		double GenMultiplier = LepidodendronConfigPlants.multiplierArchaeocalamites;
		...
		int maxheight = LepidodendronConfigPlants.maxheightArchaeocalamites;
		int minheight = LepidodendronConfigPlants.minheightArchaeocalamites;
						...
						if (world.isAirBlock(blockpos1) && world.isAirBlock(blockpos1.up()) && world.isAirBlock(blockpos1.up(2)) && blockpos1.getY() >= minH && (blockpos1.getY() <= maxH || maxH == 0) ) {
					    	...
					    	if ((colWater < 6) && (colWater > 1)) {
					    		...
					    		while (counter <= colWater) {
									if (world.getBlockState(blockpos1.down(counter)).getMaterial() == Material.WATER) {
										world.setBlockState(blockpos1.down(counter), BlockArchaeocalamitesStem.block.getDefaultState(), 2);
									}
					    			counter = counter + 1;
					    		}
					    	}
							...
							for (int k = 0; k <= j; ++k) {
								if (((BlockCustomFlower) block).canBlockStay(world, blockpos1)) {
									...
									if (k == (j - 1)) {
										world.setBlockState(blockpos1.up(k), BlockArchaeocalamites2.block.getDefaultState(), 2);
									}
									if (k == j) {
										world.setBlockState(blockpos1.up(k), BlockArchaeocalamites3.block.getDefaultState(), 2);
									}
								}
							}
						}
	...
	public static class BlockCustomFlower extends BlockReed implements IAdvancementGranter {
		public BlockCustomFlower() {
			...
			setResistance(0.2F);
			setLightLevel(0F);
			setTranslationKey("pf_archaeocalamites");
			setRegistryName("archaeocalamites");
		}

		@Nullable
		@Override
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_ARCHAEOCALAMITES;
		}
		...
		public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, @Nullable TileEntity te, ItemStack stack) {
			if (stack.getItem() == Items.SHEARS && LepidodendronConfig.doPropagation
					...
					(worldIn.getBlockState(pos.down()).getMaterial() == Material.GROUND
							...
							&& worldIn.getBlockState(pos.down()).getBlock() != BlockArchaeocalamitesStem.block
							&& (
	    ...
	    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
			...
			if (worldIn.getBlockState(pos.down(2)).getBlock() == BlockArchaeocalamitesStem.block) {
				worldIn.destroyBlock(pos.down(2), true);
			}
			if ((worldIn.getBlockState(pos.up()).getBlock() == this)
				|| (worldIn.getBlockState(pos.up()).getBlock() == BlockArchaeocalamites2.block)
				|| (worldIn.getBlockState(pos.up()).getBlock() == BlockArchaeocalamites3.block))
			{
		...
		public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player) {
			return new ItemStack(ItemArchaeocalamitesItem.block, (int) (1));
		}
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				drops.add(new ItemStack(ItemArchaeocalamitesItem.block, (int) (1)));
			}
		}
		...
		public boolean canPlaceBlockAt(World world, BlockPos pos) {
			...
			if ((canSustainPlantRevised(world.getBlockState(pos.down(2)), world, pos.down(2), EnumFacing.UP, this)
				&& (isWaterBlock(world, pos.down()))) || block3 == BlockArchaeocalamitesStem.block) {
					canPlace2 = true;
				}
		...
		public void updateTick(World world, BlockPos pos, IBlockState state, Random random) {
			if (world.getBlockState(pos.down()).getBlock() == block || checkForDropRevised(world, pos, state)) {
				if (world.isAirBlock(pos.up())) {
					world.setBlockState(pos.up(), BlockArchaeocalamites2.block.getDefaultState(), 2);
				}
				if ((world.isAirBlock(pos.up(2))) && (world.getBlockState(pos.up()).getBlock() == BlockArchaeocalamites2.block)) {
					world.setBlockState(pos.up(2), BlockArchaeocalamites3.block.getDefaultState(), 2);
				}
				else {
					if ((world.isAirBlock(pos.up(3))) && ((world.getBlockState(pos.up())).getBlock() == BlockArchaeocalamites2.block)
						&& ((world.getBlockState(pos.up(2))).getBlock() == BlockArchaeocalamites3.block)) {
						int l;
						...
						if (l < 9) {
							...
							if (i1 == 15) {
								...
								//if (world.isAirBlock(pos.up(2))) {
									world.setBlockState(pos.up(2), BlockArchaeocalamites2.block.getDefaultState(), 2);
								//}
								//if (world.isAirBlock(pos.up(3))) {
									world.setBlockState(pos.up(3), BlockArchaeocalamites3.block.getDefaultState(), 2);
								//}
							} else {
