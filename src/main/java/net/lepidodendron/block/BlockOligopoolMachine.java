...
public class BlockOligopoolMachine extends ElementsLepidodendronMod.ModElement {
	...
	public static class TileEntityOligopoolMachine extends TileEntityLockableLoot implements ITickable, ISidedInventory, IEnergyStorage {
			...
			if (LepidodendronConfig.machinesRF) {
				...
				if (tileEntity instanceof BlockOligopoolMachine.TileEntityOligopoolMachine) {
					...
					if (te.getEnergyStored() < te.getMaxEnergyStored()) {
						...
						EnumFacing facing = this.getWorld().getBlockState(this.getPos()).getValue(BlockOligopoolMachine.BlockCustom.FACING);
