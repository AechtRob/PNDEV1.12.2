...
import net.lepidodendron.item.ItemAzollaItem;
...
public class BlockAzolla extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:azolla")
	public static final Block block = null;
	public BlockAzolla(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.azolla);
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!matchBiome(biome, LepidodendronConfigPlants.genAzollaBlacklistBiomes)) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genAzollaOverrideBiomes))
			biomeCriteria = true;
...
//		double GenMultiplier = LepidodendronConfigPlants.weightAzolla;
...
//		}
		...
		for (int i = 0; i < (int) GenChance; i++) {
			...
			(new SalviniaGenerator((Block) this.block)).generate(world, random, new BlockPos(l6, i11, l14));
		}
	}
	...
	public static class BlockCustom extends SeedSporeLilyPadBase implements IGrowable, net.minecraftforge.common.IShearable, IAdvancementGranter {
		public BlockCustom() {
			...
			setLightOpacity(0);
			setCreativeTab(null);
			if (LepidodendronConfigPlants.spreadAzolla && LepidodendronConfig.spreadPlantsAtAll) {
				setTickRandomly(true);
			}
			else {
				setTickRandomly(false);
			}
			setTranslationKey("pf_azolla");
			setRegistryName("azolla");
		}

		@Nullable
		@Override
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_AZOLLA;
		}
		...
		public void onEntityCollision(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
			...
			if (entityIn instanceof EntityBoat)
			{
				...
				if (!LepidodendronConfig.doPropagation) {
					EntityItem entityToSpawn = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ItemAzollaItem.block, (int) (1)));
		...
		public NonNullList<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
			return NonNullList.withSize(1, new ItemStack(ItemAzollaItem.block, (int) (1)));
		}
		...
		public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player) {
			return new ItemStack(ItemAzollaItem.block, (int) (1));
		}
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				drops.add(new ItemStack(ItemAzollaItem.block, (int) (1)));
			}
		}
			...
			if (!world.isRemote)
			{
				...
				if (!this.canBlockStay(world, pos, state))
				{
					...
					if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock)) && (canSurviveAt(world, targetBlock))) {
						world.setBlockState(targetBlock, BlockAzolla.block.getDefaultState(), 3);
					}
	    ...
	    {
	    	if (!LepidodendronConfigPlants.spreadAzolla) {return true;}
	        return false;
	    }
