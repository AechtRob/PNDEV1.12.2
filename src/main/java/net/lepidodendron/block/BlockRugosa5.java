...
public class BlockRugosa5 extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:rugosa_5")
	public static final Block block = null;
	public BlockRugosa5(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.rugosa_5);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("rugosa_5"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:rugosa_5", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockRugosa5.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("staticdnaPNrugosa_5", BlockRugosa5.block);
	}
						...
						{	boolean waterDepthCheckMax = false;
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockRugosa5.BlockCustom.FACING, enumfacing), 2);
									return true;
								} else {
									for (EnumFacing enumfacing1 : BlockRugosa5.BlockCustom.FACING.getAllowedValues()) {
										...
										if (enumfacing1 != EnumFacing.UP && enumfacing1 != EnumFacing.DOWN &&
											...
											world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockRugosa5.BlockCustom.FACING, enumfacing1), 2);
											return true;
										}
									}
								}
