...
public class BlockNypaShoot05Side extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:nypa_shoot_05_side")
	public static final Block block = null;
	public BlockNypaShoot05Side(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.nypa_shoot);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("nypa_shoot_05_side"));
	...
	//			new ModelResourceLocation("lepidodendron:nypa_shoot_05_side", "inventory"));
	//}
	...
	public static class BlockCustom extends BlockLeaves {
		...
		public BlockCustom() {
			setTranslationKey("pf_nypa_shoot_05_side");
    ...
    public BlockRenderLayer getRenderLayer()
    {
		...
		public void neighborChanged(IBlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (((world.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.NORTH) 
				&& (world.getBlockState(pos.south()).getBlock() != BlockNypaShoot05.block))
				{
			...
			if (((world.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.SOUTH) 
				&& (world.getBlockState(pos.north()).getBlock() != BlockNypaShoot05.block))
				{
				world.destroyBlock(pos, false);
				return;
			}
			...
			if (((world.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.EAST) 
				&& (world.getBlockState(pos.west()).getBlock() != BlockNypaShoot05.block))
				{
				world.destroyBlock(pos, false);
				return;
			}
			...
			if (((world.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.WEST) 
				&& (world.getBlockState(pos.east()).getBlock() != BlockNypaShoot05.block))
				{
				world.destroyBlock(pos, false);
				return;
			}
		...
		public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
			...
			if (((worldIn.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.NORTH) 
				&& (worldIn.getBlockState(pos.south()).getBlock() != BlockNypaShoot05.block))
				{
			...
			if (((worldIn.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.SOUTH) 
				&& (worldIn.getBlockState(pos.north()).getBlock() != BlockNypaShoot05.block))
				{
				worldIn.destroyBlock(pos, false);
				return;
			}
			...
			if (((worldIn.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.EAST) 
				&& (worldIn.getBlockState(pos.west()).getBlock() != BlockNypaShoot05.block))
				{
				worldIn.destroyBlock(pos, false);
				return;
			}
			...
			if (((worldIn.getBlockState(pos)).getValue(BlockHorizontal.FACING) == EnumFacing.WEST) 
				&& (worldIn.getBlockState(pos.east()).getBlock() != BlockNypaShoot05.block))
				{
				worldIn.destroyBlock(pos, false);
				return;
			}
