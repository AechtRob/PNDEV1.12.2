...
public class BlockXenocladiaStemSE extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:xenocladia_stem_se")
	public static final Block block = null;
	public BlockXenocladiaStemSE(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.xenocladia_stem_se);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("xenocladia_stem_se"));
	...
	//			new ModelResourceLocation("lepidodendron:xenocladia_stem_se", "inventory"));
	//}
	public static class BlockCustom extends Block implements IAdvancementGranter {
		...
		public BlockCustom() {
			super(Material.WOOD);
			setTranslationKey("pf_xenocladia_stem_se");
        ...
        if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()))
	        {
	        	...
	        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					world.setBlockState(pos, BlockXenocladiaStemSW.block.getDefaultState().withProperty(FACING, world.getBlockState(pos).getValue(FACING)), 3);
