...
public class BlockDesmiophyllumLeavesBerries extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:desmiophyllum_leaves_berries")
	public static final Block block = null;
	public BlockDesmiophyllumLeavesBerries(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.desmiophyllum_leaves_berries);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("desmiophyllum_leaves_berries"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:desmiophyllum_leaves_berries", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:desmiophyllum_sapling", BlockDesmiophyllumLeavesBerries.block);
		OreDictionary.registerOre("plantPrehistoric", BlockDesmiophyllumLeavesBerries.block);
		OreDictionary.registerOre("plant", BlockDesmiophyllumLeavesBerries.block);
		OreDictionary.registerOre("treeLeaves", BlockDesmiophyllumLeavesBerries.block);
	}
	...
	public static class BlockCustom extends BlockLeavesPF {
		public BlockCustom() {
			super();
			setTranslationKey("pf_desmiophyllum_leaves_berries");
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation) {
				...
				if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(ItemDesmiophyllumBerries.block, (int) (1));
					stackSeed.setCount(1);
					ItemHandlerHelper.giveItemToPlayer(player, stackSeed);
					world.setBlockState(pos, BlockDesmiophyllumLeavesBerries.block.getDefaultState().withProperty(DECAYABLE, state.getValue(DECAYABLE)).withProperty(CHECK_DECAY, state.getValue(CHECK_DECAY)).withProperty(FRUITS, false), 3);
					return true;
				}
				return true;
			}
