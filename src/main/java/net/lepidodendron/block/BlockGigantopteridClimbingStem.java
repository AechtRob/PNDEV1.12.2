...
public class BlockGigantopteridClimbingStem extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:gigantopterid_climbing_stem")
	public static final Block block = null;
	public BlockGigantopteridClimbingStem(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.gigantopterid_climbing_stem);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("gigantopterid_climbing_stem"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:gigantopterid_climbing_stem", "inventory"));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:gigantopterid_climbing_sapling", BlockGigantopteridClimbingStem.block);
		OreDictionary.registerOre("logWood", BlockGigantopteridClimbingStem.block);
		OreDictionary.registerOre("itemRope", BlockGigantopteridClimbingStem.block);
	}
	...
	public static class BlockCustom extends Block implements IAdvancementGranter {
		...
		public BlockCustom() {
			super(Material.WOOD);
			setTranslationKey("pf_gigantopterid_climbing_stem");
		...
		public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos){
	        ...
	        if ((block == BlockGigantopteridClimbingShoot.block) || (block == BlockGigantopteridClimbingShootPlaceable.block) ) {
			...
			if (state.getValue(FACING) == EnumFacing.EAST) {
				...
				if (worldIn.getBlockState(pos.east()).getBlock() == BlockGigantopteridClimbingStemNE.block) {
					state = state.withProperty(EAST, worldIn.getBlockState(pos.east()).getValue(FACING) == EnumFacing.NORTH);
				}
				if (worldIn.getBlockState(pos.west()).getBlock() == BlockGigantopteridClimbingStemNW.block) {
					state = state.withProperty(WEST, worldIn.getBlockState(pos.west()).getValue(FACING) == EnumFacing.NORTH);
				}
				if (worldIn.getBlockState(pos.east().south()).getBlock() == BlockGigantopteridClimbingStemNE.block) {
					state = state.withProperty(EAST, worldIn.getBlockState(pos.east().south()).getValue(FACING) == EnumFacing.EAST);
				}
				if (worldIn.getBlockState(pos.west().south()).getBlock() == BlockGigantopteridClimbingStemNW.block) {
