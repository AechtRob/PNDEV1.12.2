...
public class BlockLyginopterisShoot extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:lyginopteris_shoot_worldgen")
	public static final Block block = null;
	public BlockLyginopterisShoot(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.lyginopteris_shoot_worldgen);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("lyginopteris_shoot_worldgen"));
	...
	public void registerModels(ModelRegistryEvent event) {
		...
		//		new ModelResourceLocation("lepidodendron:lyginopteris_shoot_worldgen", "inventory"));
	...
	public static class BlockCustom extends SeedSporeLeavesBase implements IAdvancementGranter {
		...
		public BlockCustom() {
			super();
			setTranslationKey("pf_lyginopteris_shoot_worldgen");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_LYGINOPTERIS;
		}
    ...
    public BlockRenderLayer getRenderLayer()
    {
		...
		public void neighborChanged(IBlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			if (((Boolean)state.getValue(CHECK_DECAY)).booleanValue() && ((Boolean)state.getValue(DECAYABLE)).booleanValue())
				...
				if (
					...
					&& (!(state.getValue(FACING) == EnumFacing.UP && world.getBlockState(pos.down()).getBlock() == BlockLyginopterisStem.block))
					&& (!(state.getValue(FACING) == EnumFacing.DOWN && world.getBlockState(pos.up()).getBlock() == BlockLyginopterisStem.block))
					)
					{
						...
						if ((Math.random() >= 0.5) && (!LepidodendronConfig.doPropagation)) {
								...
								if (!world.isRemote) {
									EntityItem entityToSpawn = new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockLyginopterisSapling.block, (int) (1)));
		...
		public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) 
		{
			...
			if (
				...
				&& (!(state.getValue(FACING) == EnumFacing.UP && worldIn.getBlockState(pos.down()).getBlock() == BlockLyginopterisStem.block))
				&& (!(state.getValue(FACING) == EnumFacing.DOWN && worldIn.getBlockState(pos.up()).getBlock() == BlockLyginopterisStem.block))
				)
				{
					...
					if ((Math.random() >= 0.5) && (!LepidodendronConfig.doPropagation)) {
							//Spawn another sapling:
							if (!worldIn.isRemote) {
								EntityItem entityToSpawn = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockLyginopterisSapling.block, (int) (1)));
								entityToSpawn.setPickupDelay(10);
		...
		public Block planted() {
			return BlockLyginopterisSapling.block;
		}
