...
public class BlockPrehistoricGroundBasic extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:prehistoric_ground_cover")
	public static final Block block = null;
	public BlockPrehistoricGroundBasic(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.prehistoric_ground_cover);
	}
	            ...
	            else
	            {
	                if (worldIn.getLightFromNeighbors(pos.up()) >= 9)
	                {
	                    for (int i = 0; i < 4; ++i)
	                    {
							...
							{
	                            worldIn.setBlockState(blockpos, BlockPrehistoricGroundBasic.block.getDefaultState());
	                        }
	                    }
	                }
	            }
