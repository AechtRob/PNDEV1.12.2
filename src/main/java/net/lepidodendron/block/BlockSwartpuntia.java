...
public class BlockSwartpuntia extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:swartpuntia")
	public static final Block block = null;
	public BlockSwartpuntia(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.swartpuntia);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("swartpuntia"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:swartpuntia", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockSwartpuntia.LEVEL).build());
	}
		...
		for (int i = 0; i < 16; i++) {
			...
			(new WorldGenReed() {
				...
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < 48; ++i) {
						BlockPos blockpos1 = pos.add(random.nextInt(4) - random.nextInt(4), random.nextInt(4) - random.nextInt(4), random.nextInt(4) - random.nextInt(4));
						...
						if (!(biomeName.equalsIgnoreCase("lepidodendron:ediacaran_stromatolite_pavement")
								|| biomeName.equalsIgnoreCase("lepidodendron:precambrian_sea")
						...
						if (world.getBlockState(blockpos1).getBlock() == Blocks.WATER) {
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockSwartpuntia.BlockCustom.FACING, enumfacing), 2);
									return true;
								}
							}
						}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_swartpuntia");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_SWARTPUNTIA;
		}
