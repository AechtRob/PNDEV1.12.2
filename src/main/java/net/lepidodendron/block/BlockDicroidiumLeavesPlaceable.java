...
public class BlockDicroidiumLeavesPlaceable extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:dicroidium_leaves")
	public static final Block block = null;
	public BlockDicroidiumLeavesPlaceable(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.dicroidium_leaves);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("dicroidium_leaves"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:dicroidium_leaves", "inventory"));
	...
	public static class BlockCustom extends SeedSporeLeavesBase implements IAdvancementGranter {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_dicroidium_leaves");
	    ...
	    public void onBlockAdded(World world, BlockPos pos, IBlockState state) 
	    {
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.NORTH) {
				if (world.isAirBlock(pos.north())) {
					world.setBlockState(pos.north(), BlockDicroidiumLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.NORTH), 3);
				}
				else {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.SOUTH) {
				if (world.isAirBlock(pos.south())) {
					world.setBlockState(pos.south(), BlockDicroidiumLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.SOUTH), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.EAST) {
				if (world.isAirBlock(pos.east())) {
					world.setBlockState(pos.east(), BlockDicroidiumLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.EAST), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.WEST) {
				if (world.isAirBlock(pos.west())) {
					world.setBlockState(pos.west(), BlockDicroidiumLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.WEST), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.UP) {
				if (world.isAirBlock(pos.up())) {
					world.setBlockState(pos.up(), BlockDicroidiumLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.UP), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.DOWN) {
				if (world.isAirBlock(pos.down())) {
					world.setBlockState(pos.down(), BlockDicroidiumLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.DOWN), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
