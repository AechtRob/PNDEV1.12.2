...
public class BlockXianguangia extends ElementsLepidodendronMod.ModElement {
	...
	public BlockXianguangia(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.xianguangia);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("xianguangia"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:xianguangia", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockXianguangia.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("staticdnaPNxianguangia", BlockXianguangia.block);
	}
						...
						{	boolean waterDepthCheckMax = false;
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockXianguangia.BlockCustom.FACING, enumfacing), 2);
									return true;
								} else {
									for (EnumFacing enumfacing1 : BlockXianguangia.BlockCustom.FACING.getAllowedValues()) {
										pos1 = blockpos1;
										...
										if (enumfacing1 != EnumFacing.UP && enumfacing1 != EnumFacing.DOWN &&
											...
											world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockXianguangia.BlockCustom.FACING, enumfacing1), 2);
											return true;
										}
									}
								}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable  {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_xianguangia");
