...
public class BlockBirdsnestFern extends ElementsLepidodendronMod.ModElement {
	...
	public static class BlockCustom extends SeedSporeLeavesBase implements IAdvancementGranter {
		...
		public NonNullList<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
			return NonNullList.withSize(1, new ItemStack(BlockBirdsnestFern.block, (int) (1)));
		}
    	...
    	public BlockRenderLayer getRenderLayer()
    {
		...
		public Item getItemDropped(IBlockState state, java.util.Random rand, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				return Item.getItemFromBlock(BlockBirdsnestFern.block);
			}
		}
		...
		public void neighborChanged(IBlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (world.isAirBlock(pos.down())) {
				...
				if ((Math.random() > 0.66) && (!LepidodendronConfig.doPropagation)) {
					...
					if (!world.isRemote) {
						EntityItem entityToSpawn = new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockBirdsnestFern.block, (int) (1)));
						entityToSpawn.setPickupDelay(10);
