...
public class BlockAlethopterisLeaves extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:alethopteris_leaves")
	public static final Block block = null;
	public BlockAlethopterisLeaves(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.alethopteris_leaves);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("alethopteris_leaves"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:alethopteris_sapling", BlockAlethopterisLeaves.block);
	...
	public static class BlockCustom extends BlockLeavesPF {
		...
		public Item getItemDropped(IBlockState state, java.util.Random rand, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				return Item.getItemFromBlock(BlockAlethopterisSapling.block);
			}
		}
		...
		public void breakBlock(World worldIn, BlockPos pos, IBlockState state)  {
			...
			if (state.getBlock() != BlockAlethopterisLeaves1.block) {
		            ...
		            for (int j1 = -2; j1 <= 2; ++j1)
		            {
		                for (int k1 = -2; k1 <= 2; ++k1)
		                {
		                    for (int l1 = -2; l1 <= 2; ++l1)
		                    {
		                        ...
		                        if (iblockstate.getBlock() == BlockAlethopterisLeaves1.block)
		                        {
		                        	ProcedureTreeLeaf.executeProcedure((int) blockpos.getX(), (int) blockpos.getY(), (int) blockpos.getZ(), worldIn, BlockAlethopterisLeaves.block);
		                        }
		                    }
		                }
		            }
