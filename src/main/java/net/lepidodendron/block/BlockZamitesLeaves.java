...
import net.lepidodendron.item.ItemZamitesSeeds;
...
public class BlockZamitesLeaves extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:zamites_leaves_worldgen")
	public static final Block block = null;
	public BlockZamitesLeaves(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.zamites_leaves_worldgen);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("zamites_leaves_worldgen"));
	...
	public void registerModels(ModelRegistryEvent event) {
		...
		//		new ModelResourceLocation("lepidodendron:zamites_leaves_worldgen", "inventory"));
	...
	public static class BlockCustom extends BlockLeaves {
		...
		public BlockCustom() {
			super();
			setTranslationKey("pf_zamites_leaves_worldgen");
    ...
    public BlockRenderLayer getRenderLayer()
    {
		...
		public Item getItemDropped(IBlockState state, java.util.Random rand, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				return Item.getItemFromBlock(BlockZamitesSapling.block);
			}
		}
        ...
        public ItemStack getSilkTouchDrop(IBlockState state)  {
            return new ItemStack(BlockZamitesLeavesPlaceable.block, (int) (1));
        }
		...
		public NonNullList<ItemStack> onSheared(ItemStack item, net.minecraft.world.IBlockAccess world, BlockPos pos, int fortune) {
			return NonNullList.withSize(1, new ItemStack(BlockZamitesLeavesPlaceable.block, (int) (1)));
		}
		...
		public void neighborChanged(IBlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			if (((Boolean)state.getValue(CHECK_DECAY)).booleanValue() && ((Boolean)state.getValue(DECAYABLE)).booleanValue())
				...
				if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.NORTH) {
					...
					if (block != BlockZamitesLog.block)
						{
							world.setBlockToAir(pos);
							if ((Math.random() >= 0.5) && (!LepidodendronConfig.doPropagation)) {
									//Spawn another sapling:
									if (!world.isRemote) {
										EntityItem entityToSpawn = new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockZamitesSapling.block, (int) (1)));
					...
					if (block != BlockZamitesLog.block)
						{
							...
							if ((Math.random() >= 0.5) && (!LepidodendronConfig.doPropagation)) {
									//Spawn another sapling:
									if (!worldIn.isRemote) {
										EntityItem entityToSpawn = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockZamitesSapling.block, (int) (1)));
										entityToSpawn.setPickupDelay(10);
        ...
        if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation)
	        {
	        	...
	        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					if (Math.random() > 0.5) {
						ItemStack stackSeed = new ItemStack(ItemZamitesSeeds.block, (int) (1));
