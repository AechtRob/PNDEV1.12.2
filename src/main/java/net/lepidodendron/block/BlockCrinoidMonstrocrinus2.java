...
public class BlockCrinoidMonstrocrinus2 extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:crinoid_monstrocrinus_2")
	public static final Block block = null;
	public BlockCrinoidMonstrocrinus2(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.crinoid_monstrocrinus);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:crinoid_monstrocrinus_2", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("staticdnaPNlepidodendron:crinoid_monstrocrinus_2", BlockCrinoidMonstrocrinus2.block);
		OreDictionary.registerOre("pndietCrinoid", BlockCrinoidMonstrocrinus2.block);
		OreDictionary.registerOre("holdfastDrops", BlockCrinoidMonstrocrinus2.block);
	}
			...
			(new WorldGenReed() {
				...
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < 1; ++i) {
						...
						int clump = random.nextInt(6) + 1;
						for (int ii = 1; ii < clump; ++ii) {
							int xVar = Math.max(world.rand.nextInt(4) - world.rand.nextInt(4), 3);
							int zVar = Math.max(world.rand.nextInt(4) - world.rand.nextInt(4), 3);
							if (world.rand.nextBoolean() && xVar > 0) {
								xVar = -xVar;
							}
							if (world.rand.nextBoolean() && zVar > 0) {
								zVar = -zVar;
							}
							pos = pos.add(xVar, 0, zVar);
	...
	private static int crinoidHeight(World worldIn, BlockPos pos, Random random) {
		if (worldIn.getBiome(pos).getRegistryName().toString().equalsIgnoreCase("lepidodendron:silurian_sea_garden")) {
			if (random.nextInt(8) != 0) {
				return BlockCrinoidMonstrocrinus2.BlockCustom.crinoidheight;
			}
		}
		return 1 + random.nextInt(random.nextInt(random.nextInt(BlockCrinoidMonstrocrinus2.BlockCustom.crinoidheight) + 1) + 1);
	}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_crinoid_monstrocrinus_2");
			setRegistryName("crinoid_monstrocrinus_2");
		}

		@Nullable
		@Override
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_CRINOID_MONSTROCRINUS_2;
		}
