...
public class BlockNathorstiana extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:nathorstiana")
	public static final Block block = null;
	public BlockNathorstiana(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.nathorstiana);
	}
	...
	public static class BlockCustom extends SeedSporeLilyPadBase implements IGrowable, IAdvancementGranter {
		public BlockCustom() {
			...
			setLightOpacity(3);
			setCreativeTab(null);
			if (LepidodendronConfigPlants.spreadNathorstiana) {
				setTickRandomly(true);
			}
			else {
				setTickRandomly(false);
			}
			setTranslationKey("pf_nathorstiana");
			setRegistryName("nathorstiana");
		}

		@Nullable
		@Override
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_NATHORSTIANA;
		}
		...
		public void onEntityCollision(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
			...
			if (entityIn instanceof EntityBoat)
			{
				...
				if (!LepidodendronConfig.doPropagation) {
					EntityItem entityToSpawn = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ItemNathorstianaItem.block, (int) (1)));
    ...
    public BlockRenderLayer getRenderLayer()
    {
	        ...
	        if (!world.isRemote)
	        {
				...
				if (!this.canBlockStay(world, pos, state))
		        {
					...
					if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock)) && (canSurviveAt(world, targetBlock))) {
						world.setBlockState(targetBlock, BlockNathorstiana.block.getDefaultState(), 3);	
					}
	    ...
	    {
	    	if (!LepidodendronConfigPlants.spreadNathorstiana) {return true;}
	        return false;
	    }
