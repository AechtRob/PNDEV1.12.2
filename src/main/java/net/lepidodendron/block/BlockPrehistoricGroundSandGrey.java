...
public class BlockPrehistoricGroundSandGrey extends ElementsLepidodendronMod.ModElement {
	...
	public BlockPrehistoricGroundSandGrey(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.sandy_prehistoric_ground_cover_grey);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("sandy_prehistoric_ground_cover_grey"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("sand", BlockPrehistoricGroundSandGrey.block);
		OreDictionary.registerOre("blockSand", BlockPrehistoricGroundSandGrey.block);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:sandy_prehistoric_ground_cover_grey", "inventory"));
	}
	...
	public static class BlockCustom extends BlockFalling implements IGrowable {
		public BlockCustom() {
			super(Material.SAND);
			setTranslationKey("pf_sandy_prehistoric_ground_cover_grey");
	    ...
	    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
	    {
	        ...
	        if (!worldIn.isRemote)
	        {
	            ...
	            {
	                worldIn.setBlockState(pos, BlockSandGrey.block.getDefaultState());
	            }
	            else
	            {
					...
					else
					{
		                ...
		                if (worldIn.getLightFromNeighbors(pos.up()) >= 9)
		                {
		                    for (int i = 0; i < 4; ++i)
		                    {
								...
								if (
									(
										((iblockstate1.getBlock() == BlockSandGrey.block)
										)
								...
								)
		                        {
									...
									if (waterCriteria)
		                            	worldIn.setBlockState(blockpos, BlockPrehistoricGroundSandGrey.block.getDefaultState());
		                        }
		                    }
	                    }
	                }
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			drops.add(new ItemStack(BlockSandGrey.block, (int) (1), 0));
			if (Math.random() > 0.95) {
