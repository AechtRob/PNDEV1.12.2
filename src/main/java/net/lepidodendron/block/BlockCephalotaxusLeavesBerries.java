...
import net.lepidodendron.item.ItemCephalotaxusBerries;
...
public class BlockCephalotaxusLeavesBerries extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:cephalotaxus_leaves_berries")
	public static final Block block = null;
	public BlockCephalotaxusLeavesBerries(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.cephalotaxus_leaves_berries);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("cephalotaxus_leaves_berries"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:cephalotaxus_leaves_berries", "inventory"));
	...
	public static class BlockCustom extends BlockLeavesPF {
		public BlockCustom() {
			super();
			setTranslationKey("pf_cephalotaxus_leaves_berries");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_CEPHALOTAXUS;
		}
		...
		public Item getItemDropped(IBlockState state, java.util.Random rand, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				if (state.getValue(FRUITS)) {
					return new ItemStack(ItemCephalotaxusBerries.block, (int) (1)).getItem();
				}
				return new ItemStack(Items.AIR, (int) (1)).getItem();
			}
			else {
				return Item.getItemFromBlock(BlockCephalotaxusSapling.block);
			}
		}
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation) {
				...
				if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(ItemCephalotaxusBerries.block, (int) (1));
					stackSeed.setCount(1);
					ItemHandlerHelper.giveItemToPlayer(player, stackSeed);
					world.setBlockState(pos, BlockCephalotaxusLeavesBerries.block.getDefaultState().withProperty(DECAYABLE, state.getValue(DECAYABLE)).withProperty(CHECK_DECAY, state.getValue(CHECK_DECAY)).withProperty(FRUITS, false), 3);
					return true;
				}
				return true;
			}
