...
public class BlockSkeletonBungartius extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:skeleton_bungartius")
	public static final Block block = null;
	public BlockSkeletonBungartius(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.skeleton_bungartius);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("skeleton_bungartius"));
		elements.items.add(() -> new ItemBlock(block).setRegistryName(block.getRegistryName()).setMaxStackSize(64));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		GameRegistry.registerTileEntity(TileEntitySkeletonBungartius.class, "lepidodendron:tileentityskeleton_bungartius");
		OreDictionary.registerOre("bone", BlockSkeletonBungartius.block);
		OreDictionary.registerOre("mobdnaPNlepidodendron:prehistoric_flora_bungartius", BlockSkeletonBungartius.block);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:entities/skeleton_bungartius", "inventory"));
	...
	public static class BlockCustom extends BlockSkeletonBase {
		...
		public TileEntity createTileEntity(World world, IBlockState state) {
			return new TileEntitySkeletonBungartius();
		}

		public TileEntitySkeletonBungartius createNewTileEntity(World worldIn, int meta) {
			return new TileEntitySkeletonBungartius();
		}
		...
		public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			...
			if (!worldIn.isRemote) {
				if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
					...
					if (tileEntity != null) {
						if (tileEntity instanceof TileEntitySkeletonBungartius) {
							if (((TileEntitySkeletonBungartius) tileEntity).getStages() >= newStage) {
