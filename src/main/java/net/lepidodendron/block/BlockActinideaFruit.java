...
import net.lepidodendron.item.ItemActinideaFruit;
...
public class BlockActinideaFruit extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:actinidea_fruit")
	public static final Block block = null;
	public BlockActinideaFruit(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.actinidea_fruit);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("actinidea_fruit"));
	}
	...
	public static class BlockCustom extends BlockVine implements IAdvancementGranter {
		public BlockCustom() {
			...
			setTranslationKey("pf_actinidea_fruit");
		...
		public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, @Nullable TileEntity te, ItemStack stack)
		{
			if (!worldIn.isRemote) {
				if (!LepidodendronConfig.doPropagation && stack.getItem() == Items.SHEARS) {
					...
					spawnAsEntity(worldIn, pos, new ItemStack(BlockActinidea.block, 1, 0));
				} else {
					player.addStat(StatList.getBlockStats(this));
					spawnAsEntity(worldIn, pos, new ItemStack(ItemActinideaFruit.block, 1, 0));
				}
			}
	    ...
	    public boolean canAttachTo(World p_193395_1_, BlockPos p_193395_2_, EnumFacing p_193395_3_)
	    {
	        ...
	        return this.isAcceptableNeighbor(p_193395_1_, p_193395_2_.offset(p_193395_3_.getOpposite()), p_193395_3_) && (block == Blocks.AIR || block == this || block == BlockActinidea.block || this.isAcceptableNeighbor(p_193395_1_, p_193395_2_.up(), EnumFacing.UP));
	    }
	        ...
	        if (!worldIn.isRemote)
	        {
	            if (worldIn.rand.nextInt(4) == 0 && worldIn.isAreaLoaded(pos, 4)) // Forge: check area to prevent loading unloaded chunks
	            {
	                    ...
	                    for (int l = -4; l <= 4; ++l)
	                    {
	                        for (int i1 = -1; i1 <= 1; ++i1)
	                        {
								if (
										...
										|| (worldIn.getBlockState(pos.add(k, i1, l)).getBlock() == BlockActinidea.block)
									)
	                            {
	                                --j;
	                ...
	                if (enumfacing1 == EnumFacing.UP && pos.getY() < 255 && worldIn.isAirBlock(blockpos2))
	                {
						...
						IBlockState state1 = BlockActinidea.block.getDefaultState()
	                ...
	                else if (enumfacing1.getAxis().isHorizontal() && !((Boolean)state.getValue(getPropertyFor(enumfacing1))).booleanValue())
	                {
	                    if (!flag)
	                    {
	                        ...
	                        {
								IBlockState state2 = BlockActinidea.block.getDefaultState()
										.withProperty(UP, state.getValue(UP))
										.withProperty(NORTH, state.getValue(NORTH))
										.withProperty(EAST, state.getValue(EAST))
										.withProperty(SOUTH, state.getValue(SOUTH))
	                ...
	                else
	                {
	                    if (pos.getY() > 1)
	                    {
	                        ...
	                        if ((worldIn.getBlockState(blockpos3)).getMaterial() == Material.AIR)
	                        {
								...
								IBlockState state3 = BlockActinidea.block.getDefaultState()
	                        ...
	                        else if (block == this || block == BlockActinidea.block)
	                        {
		...
		public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			if ((!playerIn.capabilities.allowEdit) || (!playerIn.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation)
			{
				...
				if (!((hand != playerIn.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(ItemActinideaFruit.block, (int) (1));
					stackSeed.setCount(1);
					ItemHandlerHelper.giveItemToPlayer(playerIn, stackSeed);
					worldIn.setBlockState(pos, BlockActinidea.block.getDefaultState()
							.withProperty(UP, state.getValue(UP))
							.withProperty(NORTH, state.getValue(NORTH))
							.withProperty(SOUTH, state.getValue(SOUTH))
							.withProperty(EAST, state.getValue(EAST))
		...
		private boolean recheckGrownSides(World worldIn, BlockPos pos, IBlockState state)
		{
			...
			for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
			{
				...
				if (((Boolean)state.getValue(propertybool)).booleanValue() && !this.canAttachTo(worldIn, pos, enumfacing.getOpposite()))
				{
					...
					if ((iblockstate1.getBlock() != this && iblockstate1.getBlock() != BlockActinidea.block)
