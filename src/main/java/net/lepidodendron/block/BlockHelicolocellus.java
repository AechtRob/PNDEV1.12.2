...
public class BlockHelicolocellus extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:helicolocellus")
	public static final Block block = null;
	public BlockHelicolocellus(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.helicolocellus);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("helicolocellus"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:helicolocellus", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockHelicolocellus.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("staticdnaPNlepidodendron:helicolocellus", BlockHelicolocellus.block);
		OreDictionary.registerOre("pndietCrinoid", BlockHelicolocellus.block);
		OreDictionary.registerOre("holdfastDrops", BlockHelicolocellus.block);
	}
			...
			(new WorldGenReed() {
				...
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < 24; ++i) {
						...
						if (!(world.getBiome(blockpos1).getRegistryName().toString().equalsIgnoreCase("lepidodendron:ediacaran_shallow_reef")
								|| world.getBiome(blockpos1).getRegistryName().toString().equalsIgnoreCase("lepidodendron:precambrian_sea")
								|| world.getBiome(blockpos1).getRegistryName().toString().equalsIgnoreCase("lepidodendron:ediacaran_beach"))) {
							continue;
						}
						if (blockpos1.getY() < 63 + random.nextInt(3)) {
							continue;
						}
						if (Functions.isWater(world, blockpos1)) {
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockHelicolocellus.BlockCustom.FACING, enumfacing), 2);
									return true;
								} else {
									for (EnumFacing enumfacing1 : BlockHelicolocellus.BlockCustom.FACING.getAllowedValues()) {
										pos1 = blockpos1;
										...
										if (enumfacing1 != EnumFacing.UP && enumfacing1 != EnumFacing.DOWN &&
											...
											world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockHelicolocellus.BlockCustom.FACING, enumfacing1), 2);
											return true;
										}
									}
								}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_helicolocellus");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_HELICOLOCELLUS;
		}
