...
public class BlockRedLeafyAlgae extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:red_leafy_algae")
	public static final Block block = null;
	public BlockRedLeafyAlgae(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.red_leafy_algae);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("red_leafy_algae"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:red_leafy_algae", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockRedLeafyAlgae.LEVEL).build());
	}

	@Override
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {		
		
		int weight = LepidodendronConfigPlants.weightRedLeafyAlgae;
		...
		if (!matchBiome(biome, LepidodendronConfigPlants.genRedLeafyAlgaeBlacklistBiomes)) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genRedLeafyAlgaeOverrideBiomes))
			biomeCriteria = true;
