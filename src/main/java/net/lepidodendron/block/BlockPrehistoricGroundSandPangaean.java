...
public class BlockPrehistoricGroundSandPangaean extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:sandy_prehistoric_ground_cover_pangaean")
	public static final Block block = null;
	public BlockPrehistoricGroundSandPangaean(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.sandy_prehistoric_ground_cover_pangaean);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("sandy_prehistoric_ground_cover_pangaean"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:sandy_prehistoric_ground_cover_pangaean", "inventory"));
	}
	...
	public static class BlockCustom extends BlockFalling implements IGrowable, ISustainsPlantType {
	    ...
	    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
	    {
	        ...
	        if (!worldIn.isRemote)
	        {
	            ...
	            {
	                worldIn.setBlockState(pos, BlockSandPangaean.block.getDefaultState());
	            }
	            else
	            {
					...
					else
					{
		                ...
		                if (worldIn.getLightFromNeighbors(pos.up()) >= 9)
		                {
		                    for (int i = 0; i < 4; ++i)
		                    {
								...
								if (
									(
										((iblockstate1.getBlock() == BlockSandPangaean.block)
										)
								...
								)
		                        {
									...
									waterDown = LepidodendronConfig.waterPangaeanVertical;
									if (waterDown > 6) waterDown = 6;
									if (waterDown < 0) waterDown = 0;
					
									waterCriteria = false;
									waterDist = LepidodendronConfig.waterPangaeanHorizontal;
									if (waterDist > 16) waterDist = 16;
									if (waterDist < 0) waterDist = 0;
									//Is there water nearby?
									xct = -waterDist;
									...
									if (waterCriteria)
		                            	worldIn.setBlockState(blockpos, BlockPrehistoricGroundSandPangaean.block.getDefaultState());
		                        }
		                    }
	                    }
	                }
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			...
			if (Math.random() > 0.95) {
				drops.add(new ItemStack(BlockPrehistoricGroundCoverPlantsPangaean.block, (int) (1), 0));
	        }
		}
	            ...
	            while (true)
	            {
	                if (j >= i / 16)
	                {
	                    if (worldIn.isAirBlock(blockpos1))
	                    {
                            ...
                            IBlockState iblockstate1 = BlockPrehistoricGroundCoverPlantsPangaean.block.getDefaultState();
