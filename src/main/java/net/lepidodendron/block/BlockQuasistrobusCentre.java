...
import net.lepidodendron.item.ItemQuasistrobusSeeds;
...
public class BlockQuasistrobusCentre extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:quasistrobus_centre")
	public static final Block block = null;
	public BlockQuasistrobusCentre(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.quasistrobus_centre);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("quasistrobus_centre"));
	...
	public static class BlockCustom extends BlockLeaves {
		public BlockCustom() {
			super();
			setTranslationKey("pf_quasistrobus_centre");
    ...
    public BlockRenderLayer getRenderLayer()
    {
		...
		public void neighborChanged(IBlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (block != BlockQuasistrobus.block)
				world.setBlockToAir(pos);

			block = world.getBlockState(pos.up()).getBlock();
			if (block != BlockQuasistrobusTop.block) {
				world.setBlockToAir(pos);
        ...
        if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation)
	        {
	        	...
	        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					if (Math.random() > 0.5) {
						ItemStack stackSeed = new ItemStack(ItemQuasistrobusSeeds.block, (int) (1));
