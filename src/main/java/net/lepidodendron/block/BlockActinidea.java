...
public class BlockActinidea extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:actinidea")
	public static final Block block = null;
	public BlockActinidea(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.actinidea);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("actinidea"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:actinidea", "inventory"));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:actinidea", BlockActinidea.block);
		OreDictionary.registerOre("plantPrehistoric", BlockActinidea.block);
		OreDictionary.registerOre("plant", BlockActinidea.block);
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimActinidea))
...
//		}
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genActinideaOverrideBiomes))
			biomeCriteria = true;
		if (!LepidodendronConfigPlants.genActinidea && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			biomeCriteria = false;
...
//		}
		...
		int GenChance = 30;
		double GenMultiplier = LepidodendronConfigPlants.multiplierActinidea;
	...
	public static class BlockCustom extends BlockVine implements IAdvancementGranter {
		public BlockCustom() {
			...
			setTranslationKey("pf_actinidea");
	    ...
	    public boolean canAttachTo(World p_193395_1_, BlockPos p_193395_2_, EnumFacing p_193395_3_)
	    {
	        ...
	        return this.isAcceptableNeighbor(p_193395_1_, p_193395_2_.offset(p_193395_3_.getOpposite()), p_193395_3_) && (block == Blocks.AIR || block == this || block == BlockActinideaFruit.block || this.isAcceptableNeighbor(p_193395_1_, p_193395_2_.up(), EnumFacing.UP));
	    }
	        ...
	        if (!worldIn.isRemote)
	        {
	            if (worldIn.rand.nextInt(4) == 0 && worldIn.isAreaLoaded(pos, 4)) // Forge: check area to prevent loading unloaded chunks
	            {
	                    ...
	                    for (int l = -4; l <= 4; ++l)
	                    {
	                        for (int i1 = -1; i1 <= 1; ++i1)
	                        {
	                            if (
									...
									|| (worldIn.getBlockState(pos.add(k, i1, l)).getBlock() == BlockActinideaFruit.block)
										)
	                            {
	                                --j;
	                ...
	                if (enumfacing1 == EnumFacing.UP && pos.getY() < 255 && worldIn.isAirBlock(blockpos2))
	                {
						...
						if (rand.nextInt(3) == 0) {
							state1 = BlockActinideaFruit.block.getDefaultState()
	                ...
	                else if (enumfacing1.getAxis().isHorizontal() && !((Boolean)state.getValue(getPropertyFor(enumfacing1))).booleanValue())
	                {
	                    if (!flag)
	                    {
	                        ...
	                        else if (iblockstate3.getBlockFaceShape(worldIn, blockpos4, enumfacing1) == BlockFaceShape.SOLID)
	                        {
								...
								if (rand.nextInt(3) == 0) {
									state2 = BlockActinideaFruit.block.getDefaultState()
											.withProperty(UP, state.getValue(UP))
											.withProperty(NORTH, state.getValue(NORTH))
											.withProperty(EAST, state.getValue(EAST))
											.withProperty(SOUTH, state.getValue(SOUTH))
	                ...
	                else
	                {
	                    if (pos.getY() > 1)
	                    {
	                        ...
	                        if ((worldIn.getBlockState(blockpos3)).getMaterial() == Material.AIR)
	                        {
								...
								if (rand.nextInt(3) == 0) {
									state3 = BlockActinideaFruit.block.getDefaultState()
	                        ...
	                        else if (block == this || block == BlockActinideaFruit.block)
	                        {
	    ...
	    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
	        if (LepidodendronConfig.showTooltips) {
	        	...
	        	tooltip.add("Periods: Palaeogene (Eocene) - Neogene - Pleistocene");
		...
		private boolean recheckGrownSides(World worldIn, BlockPos pos, IBlockState state)
		{
			...
			for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
			{
				...
				if (((Boolean)state.getValue(propertybool)).booleanValue() && !this.canAttachTo(worldIn, pos, enumfacing.getOpposite()))
				{
					...
					if ((iblockstate1.getBlock() != this && iblockstate1.getBlock() != BlockActinideaFruit.block)
