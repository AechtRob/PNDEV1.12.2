...
public class BlockCrinoidScyphocrinus extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:crinoid_scyphocrinus")
	public static final Block block = null;
	public BlockCrinoidScyphocrinus(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.crinoid_scyphocrinus);
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		for (int i = 0; i < 3 * multiplier; i++) {
			...
			int iii = random.nextInt(6) + 1;
			(new WorldGenReed() {
				@Override
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < iii; ++i) {
						BlockPos blockpos1 = pos.add(random.nextInt(4) - random.nextInt(4), 0, random.nextInt(4) - random.nextInt(4));
						if (((BlockCustom) block).isWaterBlock(world, blockpos1) && ((BlockCustom) block).isWaterBlock(world, blockpos1.down()) && ((BlockCustom) block).isWaterBlock(world, blockpos1.down(2))) {
							if (world.getBlockState(blockpos1.up()).getMaterial() == Material.WATER && world.isAirBlock(blockpos1.up(2))) {
								world.setBlockState(blockpos1.up(2), BlockCrinoidScyphocrinusFloat.block.getDefaultState());
								...
								for (int k = 0; k < j; ++k)
									if (((BlockCustom) block).canBlockStay(world, blockpos1)
											&& ((BlockCustom) block).canBlockStay(world, blockpos1.down(k)))
										world.setBlockState(blockpos1.down(k), block.getDefaultState(), 2);
							}
						}
					}
					return true;
	...
	private static int crinoidHeight(World worldIn, BlockPos pos, Random random) {
		if (worldIn.getBiome(pos).getRegistryName().toString().equalsIgnoreCase("lepidodendron:silurian_sea_garden")) {
			if (random.nextInt(8) != 0) {
				return BlockCrinoidScyphocrinus.BlockCustom.crinoidheight;
			}
		}
		return 1 + random.nextInt(random.nextInt(random.nextInt(BlockCrinoidScyphocrinus.BlockCustom.crinoidheight) + 1) + 1);
	}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable  {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_crinoid_scyphocrinus");
			setRegistryName("crinoid_scyphocrinus");
		}
		...
		public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos){
			...
			return state.withProperty(TOPSHOOT, (worldIn.getBlockState(pos.down()).getBlock() != this));
		}
		...
		public NonNullList<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
			return NonNullList.withSize(1, new ItemStack(ItemCrinoidScyphocrinusItem.block, (int) (1)));
		}
    	...
    	public BlockRenderLayer getRenderLayer()
    {
		...
		public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
			...
			if (worldIn.getBlockState(pos.up(2)).getBlock() == BlockCrinoidScyphocrinusFloat.block) {
		...
		public boolean canPlaceBlockAt(World world, BlockPos pos) {
			...
			if (world.getBlockState(pos.up(2)).getBlock() == BlockCrinoidScyphocrinusFloat.block) {
				water = true;
			}

			if (world.getBlockState(pos.up()).getBlock() != this && world.getBlockState(pos.up(2)).getBlock() != BlockCrinoidScyphocrinusFloat.block) {
				water = false;
			}
