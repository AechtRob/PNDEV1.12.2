...
public class BlockScarburgiaLeavesBerries extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:scarburgia_leaves_berries")
	public static final Block block = null;
	public BlockScarburgiaLeavesBerries(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.scarburgia_leaves_berries);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("scarburgia_leaves_berries"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:scarburgia_leaves_berries", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:scarburgia_sapling", BlockScarburgiaLeavesBerries.block);
		OreDictionary.registerOre("plantPrehistoric", BlockScarburgiaLeavesBerries.block);
		OreDictionary.registerOre("plant", BlockScarburgiaLeavesBerries.block);
		OreDictionary.registerOre("treeLeaves", BlockScarburgiaLeavesBerries.block);
	}
	...
	public static class BlockCustom extends BlockLeavesPF {
		public BlockCustom() {
			super();
			setTranslationKey("pf_scarburgia_leaves_berries");
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation) {
				...
				if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(ItemScarburgiaBerries.block, (int) (1));
					stackSeed.setCount(1);
					ItemHandlerHelper.giveItemToPlayer(player, stackSeed);
					world.setBlockState(pos, BlockScarburgiaLeavesBerries.block.getDefaultState().withProperty(DECAYABLE, state.getValue(DECAYABLE)).withProperty(CHECK_DECAY, state.getValue(CHECK_DECAY)).withProperty(FRUITS, false), 3);
					return true;
				}
				return true;
			}
