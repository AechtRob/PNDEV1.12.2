...
public class BlockSwampHorsetailWaterNoSpore extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:swamp_horsetail_water_nospore")
	public static final Block block = null;
	public BlockSwampHorsetailWaterNoSpore(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.swamp_horsetail_water_nospore);
	}
	...
	public static class BlockCustom extends BlockLilyPad implements IGrowable, net.minecraftforge.common.IShearable {
		public BlockCustom() {
			...
			setTranslationKey("pf_swamp_horsetail_water_nospore");
			setRegistryName("swamp_horsetail_water_nospore");
		}
				...
				if (!this.canSurviveAt(world, pos))
		        {
					...
					if (canSurviveAt(world, targetBlock)) {
						if (Math.random() > (1 - (spread / 100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock))) {
							world.setBlockState(targetBlock, BlockSwampHorsetailWaterNoSpore.block.getDefaultState(), 3);
						}
					...
					if (Math.random() > 0.9) {
						if (Math.random() > 0.6) {
							world.setBlockState(pos, BlockSwampHorsetailWater.block.getDefaultState());
						}
					}
				}
			}
