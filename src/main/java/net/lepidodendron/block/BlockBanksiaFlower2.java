...
public class BlockBanksiaFlower2 extends ElementsLepidodendronMod.ModElement {
	...
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("banksia_2_flower"));
		elements.items.add(() -> new ItemBlock(block).setRegistryName(block.getRegistryName()));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		GameRegistry.registerTileEntity(TileEntityCustom.class, "lepidodendron:tileentitybanksia_2_flower");
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:banksia_2_flower", "inventory"));
	}
        ...
        if ((!player.capabilities.allowEdit) || (player.getHeldItemMainhand().getItem() != Item.getItemFromBlock(this)) || !LepidodendronConfig.doPropagation)
	        {
	        	...
	        	if ((new Object() {
		        	...
		        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
						if (Math.random() > 0.4) {
							ItemStack stackSeed = new ItemStack(ItemBanksia2Seeds.block, (int) (1));
