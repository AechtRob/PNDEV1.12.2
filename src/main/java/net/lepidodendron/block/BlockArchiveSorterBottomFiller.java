...
public class BlockArchiveSorterBottomFiller extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:archive_sorter_bottom_filler")
	public static final Block block = null;

	public BlockArchiveSorterBottomFiller(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.archive_sorter_top);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("archive_sorter_bottom_filler"));
...
//				new ModelResourceLocation("lepidodendron:archive_sorter_bottom_filler", "inventory"));
	}
	...
	public static class BlockCustom extends Block {
		...
		protected net.minecraft.block.state.BlockStateContainer createBlockState() {
			return new net.minecraft.block.state.BlockStateContainer(this, new IProperty[]{FACING, OPEN});
		}
		...
		public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
			...
			if (!worldIn.isRemote) {
				...
				boolean flag = false;
				if (worldIn.getBlockState(pos.down()).getBlock() != BlockArchiveSorterBottom.block) {
					flag = true;
				}
				switch (((BlockArchiveSorterBottom.TileEntityArchiveBottom) te).filingCategory) {
					case 0:
					default:
						if (worldIn.getBlockState(pos.up()).getBlock() != BlockArchiveSorterTop.block) {
							flag = true;
						}
					...
					case 2:
					case 3:
						if (worldIn.getBlockState(pos.up()).getBlock() != BlockArchiveSorterTopFiller.block) {
							flag = true;
						}
						if (worldIn.getBlockState(pos.offset(state.getValue(FACING).rotateY())).getBlock() != this) {
							flag = true;
