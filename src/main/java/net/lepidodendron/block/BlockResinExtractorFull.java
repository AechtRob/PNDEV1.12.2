...
public class BlockResinExtractorFull extends ElementsLepidodendronMod.ModElement {
	...
	public void init(FMLInitializationEvent event) {
		GameRegistry.registerTileEntity(BlockResinExtractorFull.TileEntityCustom.class, "lepidodendron:tileentityresin_extractor_full");
	}
	...
	public static class BlockCustom extends Block {
		...
		public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
			if (worldIn.getTileEntity(pos) != null) {
				if (worldIn.getTileEntity(pos) instanceof BlockResinExtractorFull.TileEntityCustom) {
					Boolean extractable = isBlockActive((World) worldIn, pos, state.getValue(BlockResinExtractorFull.BlockCustom.FACING));
					if (extractable) {
		...
		public TileEntity createTileEntity(World world, IBlockState state) {
			return new BlockResinExtractorFull.TileEntityCustom();
		}

		public BlockResinExtractorFull.TileEntityCustom createNewTileEntity(World worldIn, int meta) {
			return new BlockResinExtractorFull.TileEntityCustom();
		}
	...
	public static class TileEntityCustom extends TileEntity implements ITickable {
		...
		public void update() {
			...
			Boolean extractable = BlockResinExtractorFull.BlockCustom.isBlockActive(world, pos, state.getValue(BlockResinExtractorFull.BlockCustom.FACING));
			if (state.getValue(BlockResinExtractorFull.BlockCustom.EXTRACTING) > 0 && !extractable) {
				if (!world.isRemote) {
					world.setBlockState(this.getPos(), BlockResinExtractorFull.block.getDefaultState().withProperty(BlockResinExtractorFull.BlockCustom.FACING, state.getValue(BlockResinExtractorFull.BlockCustom.FACING)).withProperty(BlockResinExtractorFull.BlockCustom.EXTRACTING, 0));
				}
			} else if (state.getValue(BlockResinExtractorFull.BlockCustom.EXTRACTING) == 0 && extractable) {
				if (!world.isRemote) {
					world.setBlockState(this.getPos(), BlockResinExtractorFull.block.getDefaultState().withProperty(BlockResinExtractorFull.BlockCustom.FACING, state.getValue(BlockResinExtractorFull.BlockCustom.FACING)).withProperty(BlockResinExtractorFull.BlockCustom.EXTRACTING, 1));
				}
			}
		}
	}
