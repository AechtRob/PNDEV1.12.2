...
import net.lepidodendron.item.ItemTylerianthusFlower;
import net.lepidodendron.item.ItemTylerianthusSeeds;
...
public class BlockTylerianthus extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:tylerianthus")
	public static final Block block = null;
	public BlockTylerianthus(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.tylerianthus);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("tylerianthus"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:tylerianthus", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:tylerianthus", BlockTylerianthus.block);
		OreDictionary.registerOre("plantPrehistoric", BlockTylerianthus.block);
		OreDictionary.registerOre("plant", BlockTylerianthus.block);
	}
	...
	public static class BlockCustom extends BlockBush implements IAdvancementGranter {
		...
		public BlockCustom() {
			super();
			setTranslationKey("pf_tylerianthus");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_TYLERIANTHUS;
		}
		...
		public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			...
			if ((!(worldIn.isRemote)) && worldIn.getBlockState(pos).getValue(FLOWERS)) {
						...
						&& hand == EnumHand.MAIN_HAND)
				{
					...
					Block.spawnAsEntity(worldIn, pos, new ItemStack(ItemTylerianthusFlower.block, 1));
					stack.damageItem(1, playerIn);
					return true;
				}

				if ((!playerIn.capabilities.allowEdit) || (playerIn.getHeldItemMainhand().getItem() != ItemTylerianthusFlower.block) || (!LepidodendronConfig.doPropagation) || (!worldIn.getBlockState(pos).getValue(FLOWERS)))
				{
					return true;
				}
				else {
					...
					if (!((hand != playerIn.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
						if (Math.random() > 0.5) {
							ItemStack stackSeed = new ItemStack(ItemTylerianthusSeeds.block, (int) (1));
				...
				&& (world.getBlockState(pos.down()).getBlock() != Blocks.GRAVEL.getDefaultState().getBlock()))
			{
				...
				if (Math.random() > 0.66) {
					if (!LepidodendronConfig.doPropagation) {
						...
						if (!world.isRemote) {
							EntityItem entityToSpawn = new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockTylerianthus.block, (int) (1)));
					...
					if ((Math.random() > 0.7)) {
						if ((Math.random() > 0.7)) {
							if (world.getBlockState(pos).getValue(FLOWERS)) {
								world.setBlockState(pos, BlockTylerianthus.block.getDefaultState().withProperty(FLOWERS, false), 3);
							}
							else {
								world.setBlockState(pos, BlockTylerianthus.block.getDefaultState().withProperty(FLOWERS, true), 3);
							}
