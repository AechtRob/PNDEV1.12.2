...
import net.lepidodendron.item.ItemOliveBerries;
...
public class BlockOliveLeavesBerries extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:olive_leaves_berries")
	public static final Block block = null;
	public BlockOliveLeavesBerries(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.olive_leaves);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("olive_leaves_berries"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:olive_leaves_berries", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:olive_sapling", BlockOliveLeavesBerries.block);
		OreDictionary.registerOre("plantPrehistoric", BlockOliveLeavesBerries.block);
		OreDictionary.registerOre("plant", BlockOliveLeavesBerries.block);
		OreDictionary.registerOre("treeLeaves", BlockOliveLeavesBerries.block);
	}
	...
	public static class BlockCustom extends BlockLeavesPF {
		public BlockCustom() {
			super();
			setTranslationKey("pf_olive_leaves_berries");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_OLIVE;
		}
		...
		public Item getItemDropped(IBlockState state, java.util.Random rand, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				if (state.getValue(FRUITS)) {
					return new ItemStack(ItemOliveBerries.block, (int) (1)).getItem();
				}
				return new ItemStack(Items.AIR, (int) (1)).getItem();
			}
			else {
				return Item.getItemFromBlock(BlockOliveSapling.block);
			}
		}
		...
		public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation) {
				...
				if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					ItemStack stackSeed = new ItemStack(ItemOliveBerries.block, (int) (1));
					stackSeed.setCount(1);
					ItemHandlerHelper.giveItemToPlayer(player, stackSeed);
					world.setBlockState(pos, BlockOliveLeavesBerries.block.getDefaultState().withProperty(DECAYABLE, state.getValue(DECAYABLE)).withProperty(CHECK_DECAY, state.getValue(CHECK_DECAY)).withProperty(FRUITS, false), 3);
					return true;
				}
				return true;
			}
