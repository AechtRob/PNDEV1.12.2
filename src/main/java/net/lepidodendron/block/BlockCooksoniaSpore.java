...
public class BlockCooksoniaSpore extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:cooksonia_spore")
	public static final Block block = null;
	public BlockCooksoniaSpore(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.cooksonia_spore);
	}
    ...
    public BlockRenderLayer getRenderLayer()
    {
		...
		public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player) {
			return new ItemStack(BlockCooksonia.block, (int) (1));
		}
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				drops.add(new ItemStack(BlockCooksonia.block, (int) (1)));
			}
		}
	        ...
	        if (!world.isRemote)
	        {
				...
				if (!canSurviveAt(world, pos))
		        {
					...
					if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock)) && (canSurviveAt(world, targetBlock))) {
						world.setBlockState(targetBlock, BlockCooksonia.block.getDefaultState(), 3);	
					}
					//Chance of collapsing:
		        	if ((Math.random() > 0.3)) {
						world.setBlockState(pos, BlockCooksonia.block.getDefaultState(), 3);
		        	}
				}
			}
		}
