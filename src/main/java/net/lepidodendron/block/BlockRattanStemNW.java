...
public class BlockRattanStemNW extends ElementsLepidodendronMod.ModElement {
	...
	public BlockRattanStemNW(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.rattan_stem_nw);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("rattan_stem_nw"));
	...
	//			new ModelResourceLocation("lepidodendron:rattan_stem_nw", "inventory"));
	//}
	public static class BlockCustom extends Block implements IAdvancementGranter {
		...
		public BlockCustom() {
			super(Material.WOOD);
			setTranslationKey("pf_rattan_stem_nw");
		...
		public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos){
			...
			if (state.getValue(FACING) == EnumFacing.NORTH) {
				...
				if (block == BlockRattanStemSE.block || block == BlockRattanStemNE.block 
        ...
        if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()))
	        {
	        	...
	        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					world.setBlockState(pos, BlockRattanStemNE.block.getDefaultState().withProperty(FACING, world.getBlockState(pos).getValue(FACING)), 3);
