...
public class BlockTieteaLeavesPlaceable extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:tietea_leaves")
	public static final Block block = null;
	public BlockTieteaLeavesPlaceable(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.tietea_leaves);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("tietea_leaves"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:tietea_leaves", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("dnaPNTietea", BlockTieteaLeavesPlaceable.block);
	...
	public static class BlockCustom extends SeedSporeLeavesBase {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_tietea_leaves");
		...
		public NonNullList<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
			return NonNullList.withSize(1, new ItemStack(BlockTieteaLeavesPlaceable.block, (int) (1)));
		}
    ...
    public BlockRenderLayer getRenderLayer()
    {
		...
		public void neighborChanged(IBlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.NORTH) {
				...
				if (block == BlockTieteaLeavesTop.block)
				{
					if (new Object() {
				...
				}.getEnumFacing(pos.north()) != EnumFacing.NORTH)
					{
						...
						if ((Math.random() >= 0.9) && (!LepidodendronConfig.doPropagation)) {
								...
								if (!world.isRemote) {
									EntityItem entityToSpawn = new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockTieteaSapling.block, (int) (1)));
	    ...
	    public void onBlockAdded(World world, BlockPos pos, IBlockState state) 
	    {
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.NORTH) {
				if (world.isAirBlock(pos.north())) {
					world.setBlockState(pos.north(), BlockTieteaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.NORTH), 3);
				}
				else {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.SOUTH) {
				if (world.isAirBlock(pos.south())) {
					world.setBlockState(pos.south(), BlockTieteaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.SOUTH), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.EAST) {
				if (world.isAirBlock(pos.east())) {
					world.setBlockState(pos.east(), BlockTieteaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.EAST), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.WEST) {
				if (world.isAirBlock(pos.west())) {
					world.setBlockState(pos.west(), BlockTieteaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.WEST), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.UP) {
				if (world.isAirBlock(pos.up())) {
					world.setBlockState(pos.up(), BlockTieteaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.UP), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
			...
			if ((EnumFacing) state.getValue(BlockDirectional.FACING) == EnumFacing.DOWN) {
				if (world.isAirBlock(pos.down())) {
					world.setBlockState(pos.down(), BlockTieteaLeavesTop.block.getDefaultState().withProperty(FACING, EnumFacing.DOWN), 3);
				}
				else {
					world.destroyBlock(pos, false);
					if (!world.isRemote) {
