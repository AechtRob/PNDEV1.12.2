...
import net.lepidodendron.item.ItemIsopogonYellowFlower;
import net.lepidodendron.item.ItemIsopogonYellowSeeds;
...
public class BlockIsopogonYellow extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:isopogon_yellow")
	public static final Block block = null;
	public BlockIsopogonYellow(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.isopogon_yellow);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("isopogon_yellow"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:isopogon_yellow", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:isopogon_yellow", BlockIsopogonYellow.block);
		OreDictionary.registerOre("plantPrehistoric", BlockIsopogonYellow.block);
		OreDictionary.registerOre("plant", BlockIsopogonYellow.block);
	}
	...
	public static class BlockCustom extends BlockBush implements IAdvancementGranter {
		...
		public BlockCustom() {
			super();
			setTranslationKey("pf_isopogon_yellow");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_ISOPOGON;
		}
		...
		public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			...
			if ((!(worldIn.isRemote)) && worldIn.getBlockState(pos).getValue(FLOWERS)) {
						...
						&& hand == EnumHand.MAIN_HAND)
				{
					...
					Block.spawnAsEntity(worldIn, pos, new ItemStack(ItemIsopogonYellowFlower.block, 1));
					stack.damageItem(1, playerIn);
					return true;
				}

				if ((!playerIn.capabilities.allowEdit) || (playerIn.getHeldItemMainhand().getItem() != ItemIsopogonYellowFlower.block) || (!LepidodendronConfig.doPropagation) || (!worldIn.getBlockState(pos).getValue(FLOWERS)))
				{
					return true;
				}
				else {
					...
					if (!((hand != playerIn.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
						if (Math.random() > 0.5) {
							ItemStack stackSeed = new ItemStack(ItemIsopogonYellowSeeds.block, (int) (1));
					...
					&& (world.getBlockState(pos.down()).getBlock() != Blocks.GRAVEL.getDefaultState().getBlock()))
			{
				...
				if (Math.random() > 0.66) {
					if (!LepidodendronConfig.doPropagation) {
						...
						if (!world.isRemote) {
							EntityItem entityToSpawn = new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockIsopogonYellow.block, (int) (1)));
					...
					if ((Math.random() > 0.7)) {
						if ((Math.random() > 0.7)) {
							if (world.getBlockState(pos).getValue(FLOWERS)) {
								world.setBlockState(pos, BlockIsopogonYellow.block.getDefaultState().withProperty(FLOWERS, false), 3);
							}
							else {
								world.setBlockState(pos, BlockIsopogonYellow.block.getDefaultState().withProperty(FLOWERS, true), 3);
							}
