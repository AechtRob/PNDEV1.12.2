...
public class BlockAmphibianSpawnMegalocephalusPlaceable extends ElementsLepidodendronMod.ModElement {
	...
	public static class BlockCustom extends BlockMobSpawn  {
		...
		public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) 
		{
			...
			if (!(worldIn.isRemote)) {
				EntityPrehistoricFloraAgeableBase.summon(worldIn, EntityList.getKey(EntityPrehistoricFloraMegalocephalus.class).toString(), "{AgeTicks:0}", (double)pos.getX() + 0.5D, (double)pos.getY() + 0.5D, (double)pos.getZ() + 0.5D);
			}
