...
public class BlockPiyuania extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:piyuania")
	public static final Block block = null;
	public BlockPiyuania(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.piyuania);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("piyuania"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:piyuania", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockPiyuania.LEVEL).build());
	}
	...
	public void init(FMLInitializationEvent event) {
		...
		OreDictionary.registerOre("pndietSponge", BlockPiyuania.block);
	}
			...
			(new WorldGenReed() {
				...
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < 12; ++i) {
						...
						if (!(biomeName.equalsIgnoreCase("lepidodendron:ediacaran_sparse_sea"))
						) {
							continue;
						}
						...
						if (world.getBlockState(blockpos1).getBlock() == Blocks.WATER) {
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockPiyuania.BlockCustom.FACING, enumfacing), 2);
									return true;
								}
							}
						}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_PIYUANIA;
		}
