...
public class BlockRhyniaLand extends ElementsLepidodendronMod.ModElement {
	...
	public BlockRhyniaLand(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.rhynia_land);
	}
		...
		public void updateTick(World world, BlockPos pos, IBlockState state, Random random) {
	        ...
	        if (!world.isRemote)
	        {
				...
				if (!canSurviveAt(world, pos))
		        {
					...
					else if (BlockRhyniaWater.BlockCustom.canSurviveAt(world, targetBlock)) {
						if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock))) {
							world.setBlockState(targetBlock, BlockRhyniaWater.block.getDefaultState(), 3);
						}
					}
					...
					if (YouAreNotAloneNooneIsAlone && Math.random() > 0.9) {
						...
						if ((Math.random() > 0.7)) {
							if ((Math.random() > 0.7)) {
								world.setBlockState(pos, BlockRhyniaLandSpore.block.getDefaultState(), 3);
							}
						}
					}
				}
