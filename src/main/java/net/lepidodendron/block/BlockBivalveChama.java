...
public class BlockBivalveChama extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:bivalve_chama")
	public static final Block block = null;
	public BlockBivalveChama(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.bivalve_chama);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("bivalve_chama"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:bivalve_chama", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockBivalveChama.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("staticdnaPNlepidodendron:bivalve_chama", BlockBivalveChama.block);
		OreDictionary.registerOre("itemShellfish", BlockBivalveChama.block);
		OreDictionary.registerOre("pnfurnaceSeafood", BlockBivalveChama.block);
		OreDictionary.registerOre("pndietShellfish", BlockBivalveChama.block);
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (biome instanceof BiomeCretaceousEarly)
		{
			...
			if (biomeCretaceousEarly.getBiomeType() == EnumBiomeTypeCretaceousEarly.Early_Cretaceous_Ocean_Shore_Southern
					...
					|| biomeCretaceousEarly.getBiomeType() == EnumBiomeTypeCretaceousEarly.Early_Cretaceous_Ocean) {
				biomeCriteria = true;
			}
			else {
				biomeCriteria = false;
						...
						if (Functions.isWater(world, blockpos1)) {
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockBivalveChama.BlockCustom.FACING, enumfacing), 2);
									return true;
								}
							}
						}
	...
	public static class BlockCustom extends BlockBivalve  {
		...
		public BlockCustom() {
			super();
			setTranslationKey("pf_bivalve_chama");
		}

		@Nullable
		@Override
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_BIVALVE_CHAMA;
		}
