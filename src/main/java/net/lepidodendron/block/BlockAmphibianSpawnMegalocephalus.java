...
public class BlockAmphibianSpawnMegalocephalus extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:amphibian_spawn_megalocephalus_worldgen")
	public static final Block block = null;
	public BlockAmphibianSpawnMegalocephalus(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.amphibian_spawn_megalocephalus);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("amphibian_spawn_megalocephalus_worldgen"));
	...
	public void registerModels(ModelRegistryEvent event) {
		...
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockAmphibianSpawnMegalocephalus.LEVEL).build());
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		for (int i = 0; i < (int) 1; i++) {
			...
			if (SpawnLocations1.spawnsHere("lepidodendron:prehistoric_flora_megalocephalus", biome.getRegistryName().toString())) {
