...
public class BlockNautiloidShellTropaeum extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:shell_tropaeum")
	public static final Block block = null;
	public BlockNautiloidShellTropaeum(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.shell_tropaeum);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("shell_tropaeum"));
		elements.items.add(() -> new ItemBlock(block).setRegistryName(block.getRegistryName()).setMaxStackSize(64));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		GameRegistry.registerTileEntity(BlockNautiloidShellTropaeum.TileEntityCustom.class, "lepidodendron:tileentityshell_tropaeum");
		OreDictionary.registerOre("mobdnaPNlepidodendron:prehistoric_flora_tropaeum", BlockNautiloidShellTropaeum.block);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:shell_tropaeum", "inventory"));
	}
		...
		for (int i = 0; i < 10; i++) {
			...
			if (!(
					biome.getRegistryName().toString().equalsIgnoreCase("lepidodendron:cretaceous_early_beach_australia_antarctica")
							...
							|| biome.getRegistryName().toString().equalsIgnoreCase("lepidodendron:cretaceous_early_beach_australia_antarctica_muddy")
			)
			) {
				break;
			}
			...
			if (world.isAirBlock(pos) && pos.getY() < Functions.getAdjustedSeaLevel(world, pos) + 3
				...
				if (tileEntity != null) {
					BlockNautiloidShellTropaeum.TileEntityCustom te = (BlockNautiloidShellTropaeum.TileEntityCustom) tileEntity;
	...
	public static class BlockCustom extends BlockPNTaxidermyItem {
		...
		public BlockCustom() {
			//super(Material.ROCK);
			setTranslationKey("pf_shell_tropaeum");
		...
		public TileEntity createTileEntity(World world, IBlockState state) {
			return new BlockNautiloidShellTropaeum.TileEntityCustom();
		}

		public BlockNautiloidShellTropaeum.TileEntityCustom createNewTileEntity(World worldIn, int meta) {
			return new BlockNautiloidShellTropaeum.TileEntityCustom();
		}
