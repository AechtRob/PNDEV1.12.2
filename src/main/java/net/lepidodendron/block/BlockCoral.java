...
import net.lepidodendron.world.gen.ReefGenerator;
...
public class BlockCoral extends ElementsLepidodendronMod.ModElement {
	...
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("coral"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("pndietCoral", BlockCoral.block);
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimReef))
		...
		if (!matchBiome(biome, LepidodendronConfigPlants.genReefBlacklistBiomes)) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genReefOverrideBiomes))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		...
		for (int i = 0; i < (int) 10 * multiplier; i++) {
			...
			(new ReefGenerator((Block) block)).generate(world, random, new BlockPos(l6, i11, l14));
		}
	}
