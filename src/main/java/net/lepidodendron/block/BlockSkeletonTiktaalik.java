...
public class BlockSkeletonTiktaalik extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:skeleton_tiktaalik")
	public static final Block block = null;
	public BlockSkeletonTiktaalik(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.skeleton_tiktaalik);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("skeleton_tiktaalik"));
		elements.items.add(() -> new ItemBlock(block).setRegistryName(block.getRegistryName()).setMaxStackSize(64));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		GameRegistry.registerTileEntity(TileEntitySkeletonTiktaalik.class, "lepidodendron:tileentityskeleton_tiktaalik");
		OreDictionary.registerOre("bone", BlockSkeletonTiktaalik.block);
		OreDictionary.registerOre("mobdnaPNlepidodendron:prehistoric_flora_tiktaalik", BlockSkeletonTiktaalik.block);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:entities/skeleton_tiktaalik", "inventory"));
	}
	public static class BlockCustom extends BlockSkeletonBase {
		...
		public TileEntity createTileEntity(World world, IBlockState state) {
			return new TileEntitySkeletonTiktaalik();
		}

		public TileEntitySkeletonTiktaalik createNewTileEntity(World worldIn, int meta) {
			return new TileEntitySkeletonTiktaalik();
		}
		...
		public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			...
			if (!worldIn.isRemote) {
				if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
					...
					if (tileEntity != null) {
						if (tileEntity instanceof TileEntitySkeletonTiktaalik) {
							if (((TileEntitySkeletonTiktaalik) tileEntity).getStages() >= newStage) {
