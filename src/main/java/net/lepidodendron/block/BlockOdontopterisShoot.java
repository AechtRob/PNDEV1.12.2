...
import net.lepidodendron.item.ItemOdontopterisSeeds;
...
public class BlockOdontopterisShoot extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:odontopteris_shoot_worldgen")
	public static final Block block = null;
	public BlockOdontopterisShoot(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.odontopteris_shoot_worldgen);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("odontopteris_shoot_worldgen"));
	...
	public void registerModels(ModelRegistryEvent event) {
		...
		//		new ModelResourceLocation("lepidodendron:odontopteris_shoot_worldgen", "inventory"));
    ...
    public BlockRenderLayer getRenderLayer()
    {
		...
		public void neighborChanged(IBlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			if (((Boolean)state.getValue(CHECK_DECAY)).booleanValue() && ((Boolean)state.getValue(DECAYABLE)).booleanValue())
				...
				if (
					...
					&& (!(state.getValue(FACING) == EnumFacing.UP && world.getBlockState(pos.down()).getBlock() == BlockOdontopterisStem.block))
					&& (!(state.getValue(FACING) == EnumFacing.DOWN && world.getBlockState(pos.up()).getBlock() == BlockOdontopterisStem.block))
					)
					{
						...
						if ((Math.random() >= 0.5) && (!LepidodendronConfig.doPropagation)) {
								...
								if (!world.isRemote) {
									EntityItem entityToSpawn = new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockOdontopterisSapling.block, (int) (1)));
		...
		public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) 
		{
			...
			if (
				...
				&& (!(state.getValue(FACING) == EnumFacing.UP && worldIn.getBlockState(pos.down()).getBlock() == BlockOdontopterisStem.block))
				&& (!(state.getValue(FACING) == EnumFacing.DOWN && worldIn.getBlockState(pos.up()).getBlock() == BlockOdontopterisStem.block))
				)
				{
					...
					if ((Math.random() >= 0.5) && (!LepidodendronConfig.doPropagation)) {
							//Spawn another sapling:
							if (!worldIn.isRemote) {
								EntityItem entityToSpawn = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockOdontopterisSapling.block, (int) (1)));
								entityToSpawn.setPickupDelay(10);
        ...
        if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation)
	        {
	        	...
	        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					if (Math.random() > 0.5) {
						ItemStack stackSeed = new ItemStack(ItemOdontopterisSeeds.block, (int) (1));
