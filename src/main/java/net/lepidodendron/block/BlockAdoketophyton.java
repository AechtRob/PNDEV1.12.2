...
public class BlockAdoketophyton extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:adoketophyton")
	public static final Block block = null;
	public BlockAdoketophyton(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.adoketophyton);
	}
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:adoketophyton", BlockAdoketophyton.block);
	...
	public static class BlockCustom extends BlockBush implements IGrowable, IAdvancementGranter, IPottable {
		...
		public IBlockState getPotState() {
			return BlockAdoketophytonSpore.block.getDefaultState();
		}
			...
			if (!world.isRemote)
	        {
				...
				if (!canSurviveAt(world, pos))
		        {
					...
					if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock)) && (canSurviveAt(world, targetBlock))) {
						world.setBlockState(targetBlock, BlockAdoketophyton.block.getDefaultState(), 3);	
					}
					...
					//Chance of spores:
		        		...
		        		if ((Math.random() > 0.7)) {
							if ((Math.random() > 0.7)) {
								world.setBlockState(pos, BlockAdoketophytonSpore.block.getDefaultState(), 3);
							}
			        	}
		        	}
				}
