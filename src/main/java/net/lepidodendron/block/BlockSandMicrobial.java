...
public class BlockSandMicrobial extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:sand_microbial")
	public static final Block block = null;
	public BlockSandMicrobial(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.sand_microbial);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("sand_microbial"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:sand_microbial", "inventory"));
	}
	...
	public static class BlockCustom extends BlockFalling implements IGrowable {
		public BlockCustom() {
			super(Material.SAND);
			setTranslationKey("pf_sandy_microbial");
		...
		public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
		{
			...
			if (!worldIn.isRemote)
			{
				...
				{
					worldIn.setBlockState(pos, Blocks.SAND.getStateFromMeta(0));
				}
				else {
					if (worldIn.getLightFromNeighbors(pos.up()) >= 9)
					{
						for (int i = 0; i < 4; ++i)
						{
							...
							if ((
									(iblockstate1.getBlock() == Blocks.SAND.getStateFromMeta(0).getBlock())
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			...
			if (Math.random() > 0.95) {
				drops.add(new ItemStack(BlockBacterialCrust.block, (int) (1), 0));
			}
		}
				...
				while (true)
				{
					if (j >= i / 16)
					{
						if (worldIn.isAirBlock(blockpos1))
						{
							...
							IBlockState iblockstate1 = BlockBacterialCrust.block.getDefaultState();
