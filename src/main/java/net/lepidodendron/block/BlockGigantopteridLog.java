...
public class BlockGigantopteridLog extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:gigantopterid_log")
	public static final Block block = null;
	public BlockGigantopteridLog(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.gigantopterid_log);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("gigantopterid_log"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0, new ModelResourceLocation("lepidodendron:gigantopterid_log", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("dnaPNGigantopterid", BlockGigantopteridLog.block);
    ...
    public static final PropertyBool ROOTS = PropertyBool.create("roots");
		...
		public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos){
			...
			if ((block1 == BlockGigantopteridShoot.block) || (block1 == BlockGigantopteridShootPlaceable.block)) {
				attachmentblock1 = true;
	    	}
			
	        return state.withProperty(ROOTS, Boolean.valueOf(attachmentblock)).withProperty(DOWN, Boolean.valueOf(block == this || attachmentblock || block == BlockGigantopteridShoot.block || block == BlockGigantopteridShootPlaceable.block)).withProperty(UP, Boolean.valueOf(block1 == this || attachmentblock1)).withProperty(NORTH, Boolean.valueOf(block2 == this)).withProperty(EAST, Boolean.valueOf(block3 == this)).withProperty(SOUTH, Boolean.valueOf(block4 == this)).withProperty(WEST, Boolean.valueOf(block5 == this)).withProperty(ATTACHMENT, attachmentblock);
	    }
    ...
    public BlockRenderLayer getRenderLayer()
    {
	    ...
	    {
	        return new BlockStateContainer(this, new IProperty[] {NORTH, EAST, SOUTH, WEST, UP, DOWN, ATTACHMENT, ROOTS});
	    }
		...
		public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
	    {
	        ...
	        if (Math.random() > 0.66) {
				...
				if (!worldIn.isRemote) {
					EntityItem entityToSpawn = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockGigantopteridSapling.block, (int) (1)));
