...
public class BlockSawdonia extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:sawdonia")
	public static final Block block = null;
	public BlockSawdonia(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.sawdonia);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:sawdonia", "inventory"));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:sawdonia", BlockSawdonia.block);
		OreDictionary.registerOre("plantPrehistoric", BlockSawdonia.block);
		OreDictionary.registerOre("plant", BlockSawdonia.block);
	}
	...
	public static class BlockCustom extends BlockBush implements IGrowable, IAdvancementGranter, IPottable {
		public BlockCustom() {
			...
			if (LepidodendronConfigPlants.spreadSawdonia && LepidodendronConfig.spreadPlantsAtAll) {
				setTickRandomly(true);
			}
			else {
				setTickRandomly(false);
			}
			setTranslationKey("pf_sawdonia");
			setRegistryName("sawdonia");
		}

		@Override
		public IBlockState getPotState() {
			return BlockSawdoniaSpore.block.getDefaultState();
		}

		@Nullable
		@Override
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_SAWDONIA;
		}
    ...
    public BlockRenderLayer getRenderLayer()
    {
	        ...
	        if (!world.isRemote)
	        {
				...
				if (!canSurviveAt(world, pos))
		        {
					...
					if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock)) && (canSurviveAt(world, targetBlock))) {
						world.setBlockState(targetBlock, BlockSawdonia.block.getDefaultState(), 3);	
					}
					...
					if (YouAreNotAloneNooneIsAlone && Math.random() > 0.9 && (world.getBlockState(pos.down()).getBlock() != BlockSawdoniaStem.block)) {
						if (Math.random() > 0.7) {
							//world.destroyBlock(pos, false);
						...
						if ((world.isAirBlock(pos.up())) && (world.getBlockState(pos.down()).getBlock() != BlockSawdoniaStem.block) && (Math.random() > 0.6)) {
							if ((Math.random() > 0.6)) {
								world.setBlockState(pos, BlockSawdoniaStem.block.getDefaultState(), 3);
								world.setBlockState(pos.up(), BlockSawdonia.block.getDefaultState(), 3);
							}
			        	}
			        	else {
							//Chance of spores:
				        	if ((Math.random() > 0.7)) {
								if ((Math.random() > 0.7)) {
									world.setBlockState(pos, BlockSawdoniaSpore.block.getDefaultState(), 3);
								}
				        	}
			        	}
					}
	    ...
	    {
	    	if (!LepidodendronConfigPlants.spreadSawdonia) {return true;}
	        return false;
	    }
		...
		public boolean canSurviveAt(World worldIn, BlockPos pos) {
			...
			if (worldIn.getBlockState(pos.down()).getBlock() == BlockSawdoniaStem.block)
				return true;
...
//	    	}
			...
			int distH = (int) LepidodendronConfigPlants.waterSawdoniaHorizontal;
			if (distH < 1) distH = 1;
			if (distH > 16) distH = 16;
			int distV = (int) LepidodendronConfigPlants.waterSawdoniaVertical;
			if (distV < 1) distV = 1;
			if (distV > 6) distV = 6;
	    ...
	    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
	        if (LepidodendronConfig.showTooltips) {
	        	...
	        	tooltip.add("Periods: [Late Silurian (?) - ] Devonian - Early Carboniferous");
