...
public class BlockMacrocystisKelp extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:macrocystis_kelp")
	...
	public void init(FMLInitializationEvent event) {
		...
		OreDictionary.registerOre("holdfastDrops", BlockMacrocystisKelp.block);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:macrocystis_kelp", "inventory"));
				...
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < 40; ++i) {
						...
						if (((BlockCustom) block).isWaterBlock(world, blockpos1) && ((BlockCustom) block).isWaterBlock(world, blockpos1.up())) {
							...
							int j = 1 + random.nextInt(random.nextInt(random.nextInt(40) + 1) + 1);
							j = Math.min(40, j);
							for (int k = 0; k < j; ++k)
