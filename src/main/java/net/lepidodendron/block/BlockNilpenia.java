...
public class BlockNilpenia extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:nilpenia")
	public static final Block block = null;
	public BlockNilpenia(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.nilpenia);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("nilpenia"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:nilpenia", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockNilpenia.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("staticdnaPNlepidodendron:nilpenia", BlockNilpenia.block);
	}
			...
			(new WorldGenReed() {
				...
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < 1; ++i) {
						...
						if (!(biomeName.equalsIgnoreCase("lepidodendron:ediacaran_frondose_forest"))
						) {
							continue;
						}
						...
						if (world.getBlockState(blockpos1).getBlock() == Blocks.WATER) {
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockNilpenia.BlockCustom.FACING, enumfacing), 2);
									return true;
								}
							}
						}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_NILPENIA;
		}
