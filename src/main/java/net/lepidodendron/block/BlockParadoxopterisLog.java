...
public class BlockParadoxopterisLog extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:paradoxopteris_log")
	public static final Block block = null;
	public BlockParadoxopterisLog(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.paradoxopteris_log);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("paradoxopteris_log"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0, new ModelResourceLocation("lepidodendron:paradoxopteris_log", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:paradoxopteris_sapling", BlockParadoxopterisLog.block);
		OreDictionary.registerOre("logWood", BlockParadoxopterisLog.block);
	}
	...
	public static final PropertyBool ROOTS_BELOW = PropertyBool.create("roots_below");
	public static final PropertyBool WATER = PropertyBool.create("water");

    	public BlockCustom() {
			super(Material.WOOD);
			setTranslationKey("pf_paradoxopteris_log");
		...
		public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos)
	    {
			...
			if (worldIn.getBlockState(pos.down(2)).getBlockFaceShape(worldIn, pos.down(), EnumFacing.UP)
					== BlockFaceShape.SOLID
	        ...
	        return state.withProperty(WATER, roots).withProperty(ROOTS_BELOW, rootsdown).withProperty(ROOTS, roots).withProperty(DOWN, Boolean.valueOf(block == this || leavesblock || block != Blocks.AIR)).withProperty(UP, Boolean.valueOf(block1 == this || leavesblock1)).withProperty(NORTH, Boolean.valueOf(block2 == this || leavesblock2)).withProperty(EAST, Boolean.valueOf(block3 == this || leavesblock3)).withProperty(SOUTH, Boolean.valueOf(block4 == this || leavesblock4)).withProperty(WEST, Boolean.valueOf(block5 == this || leavesblock5)).withProperty(SHOOT, leavesblock1);
	    }
    	...
    	public BlockRenderLayer getRenderLayer()
    {
	    ...
	    {
	        return new BlockStateContainer(this, new IProperty[] {NORTH, EAST, SOUTH, WEST, UP, DOWN, SHOOT, ROOTS, ROOTS_BELOW, WATER});
	    }
		...
		public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta,
			...
			if (worldIn.getBlockState(pos).getMaterial() == Material.WATER) {
				return BlockParadoxopterisLogWater.block.getDefaultState();
			}
		...
		public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state) {
			...
			if (shouldBeWater(worldIn, pos)) {
				worldIn.setBlockState(pos, BlockParadoxopterisLogWater.block.getDefaultState());
			}
		}
		...
		public static boolean shouldBeWater(World world, BlockPos pos) {
