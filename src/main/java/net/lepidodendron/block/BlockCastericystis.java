...
public class BlockCastericystis extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:castericystis")
	public static final Block block = null;
	public BlockCastericystis(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.castericystis);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("castericystis"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:castericystis", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockCastericystis.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("dnaPNCastericystis", BlockCastericystis.block);
	}
						...
						if (world.getBlockState(blockpos1).getBlock() == Blocks.WATER) {
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockCastericystis.BlockCustom.FACING, enumfacing), 2);
									return true;
								}
							}
						}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_castericystis");
		...
		public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
			if (LepidodendronConfig.showTooltips) {
				tooltip.add("Type: Cystoid-like sessile animal");
