...
import net.lepidodendron.item.ItemNotocyamusFlower;
...
public class BlockNotocyamus extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:notocyamus")
	public static final Block block = null;
	public BlockNotocyamus(ElementsLepidodendronMod instance) {
	...
	public static class BlockCustom extends BlockLilyPad implements IGrowable, IAdvancementGranter {
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_NOTOCYAMUS;
		}
		...
		public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player) {
			return new ItemStack(ItemNotocyamusItem.block, (int) (1));
		}
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				drops.add(new ItemStack(ItemNotocyamusItem.block, (int) (1)));
			}
		}
					...
					if ((Math.random() > 0.7)) {
						if ((Math.random() > 0.7)) {
							if (world.getBlockState(pos).getValue(FLOWERS)) {
								world.setBlockState(pos, BlockNotocyamus.block.getDefaultState().withProperty(FLOWERS, false), 3);
								world.notifyBlockUpdate(pos, world.getBlockState(pos), world.getBlockState(pos), 3);
							}
							else {
								world.setBlockState(pos, BlockNotocyamus.block.getDefaultState().withProperty(FLOWERS, true), 3);
								world.notifyBlockUpdate(pos, world.getBlockState(pos), world.getBlockState(pos), 3);
							}
						}
					}
		...
		public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			...
			if (!(worldIn.isRemote)) {
						...
						&& hand == EnumHand.MAIN_HAND)
				{
					...
					Block.spawnAsEntity(worldIn, pos, new ItemStack(ItemNotocyamusFlower.block, 1));
					stack.damageItem(1, playerIn);
					return true;
				}

				if ((!playerIn.capabilities.allowEdit) || (playerIn.getHeldItemMainhand().getItem() != ItemNotocyamusFlower.block) || !LepidodendronConfig.doPropagation)
				{
					return true;
				}
				else {
					worldIn.setBlockState(pos, BlockNotocyamusSeeds.block.getDefaultState(), 3);
