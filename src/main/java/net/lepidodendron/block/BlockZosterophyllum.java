...
public class BlockZosterophyllum extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:zosterophyllum")
	public static final Block block = null;
	public BlockZosterophyllum(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.zosterophyllum);
	}
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:zosterophyllum", BlockZosterophyllum.block);
	...
	public static class BlockCustom extends BlockBush implements IGrowable, IAdvancementGranter, IPottable {
		...
		public IBlockState getPotState() {
			return BlockZosterophyllumSpore.block.getDefaultState();
		}
	        ...
	        if (!world.isRemote)
	        {
				...
				if (!canSurviveAt(world, pos))
		        {
					...
					//Chance of spores:
		        		...
		        		if ((Math.random() > 0.7)) {
							if ((Math.random() > 0.7)) {
								world.setBlockState(pos, BlockZosterophyllumSpore.block.getDefaultState(), 3);
							}
			        	}
		        	}
				}
