...
public class BlockSalvinia extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:salvinia")
	public static final Block block = null;
	public BlockSalvinia(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.salvinia);
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!matchBiome(biome, LepidodendronConfigPlants.genSalviniaBlacklistBiomes)) {
...
//		GenChance = Math.min(100, (int) Math.round((double) GenChance * GenMultiplier));
		...
		for (int i = 0; i < (int) GenChance; i++) {
			...
			(new SalviniaGenerator((Block) block)).generate(world, random, new BlockPos(l6, i11, l14));
		}
	}
	...
	public static class BlockCustom extends SeedSporeLilyPadBase implements IGrowable, net.minecraftforge.common.IShearable, IAdvancementGranter {
		public BlockCustom() {
			...
			setTranslationKey("pf_salvinia");
			setRegistryName("salvinia");
		}

		@Nullable
		@Override
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_SALVINIA;
		}
		...
		public void onEntityCollision(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
			...
			if (entityIn instanceof EntityBoat)
			{
				...
				if (!LepidodendronConfig.doPropagation) {
					EntityItem entityToSpawn = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ItemSalviniaItem.block, (int) (1)));
		...
		public NonNullList<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
			return NonNullList.withSize(1, new ItemStack(ItemSalviniaItem.block, (int) (1)));
		}
		...
		public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player) {
			return new ItemStack(ItemSalviniaItem.block, (int) (1));
		}
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				drops.add(new ItemStack(ItemSalviniaItem.block, (int) (1)));
			}
		}
			...
			if (!world.isRemote)
			{
				...
				if (!this.canBlockStay(world, pos, state))
				{
					...
					if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock)) && (canSurviveAt(world, targetBlock))) {
						world.setBlockState(targetBlock, BlockSalvinia.block.getDefaultState(), 3);
					}
	    ...
	    {
	    	if (!LepidodendronConfigPlants.spreadSalvinia) {return true;}
	        return false;
	    }
