...
public class BlockHops extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:hops")
	public static final Block block = null;
	public BlockHops(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.hops);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("hops"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:hops", "inventory"));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:hops", BlockHops.block);
		OreDictionary.registerOre("plantPrehistoric", BlockHops.block);
		OreDictionary.registerOre("plant", BlockHops.block);
		OreDictionary.registerOre("itemMossForStone", BlockHops.block);
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimHops))
...
//		}
		...
		if (!matchBiome(biome, LepidodendronConfigPlants.genHopsBlacklistBiomes)) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genHopsOverrideBiomes))
			biomeCriteria = true;
		if (!LepidodendronConfigPlants.genHops && (!LepidodendronConfig.genAllPlants) && (!LepidodendronConfig.genAllPlantsModern))
			biomeCriteria = false;
...
//		}
		...
		int GenChance = 30;
		double GenMultiplier = LepidodendronConfigPlants.multiplierHops;
	...
	public static class BlockCustom extends BlockVine implements IAdvancementGranter {
	    ...
	    public boolean canAttachTo(World p_193395_1_, BlockPos p_193395_2_, EnumFacing p_193395_3_)
	    {
	        ...
	        return this.isAcceptableNeighbor(p_193395_1_, p_193395_2_.offset(p_193395_3_.getOpposite()), p_193395_3_) && (block == Blocks.AIR || block == this || block == BlockHopsFruit.block || this.isAcceptableNeighbor(p_193395_1_, p_193395_2_.up(), EnumFacing.UP));
	    }
	        ...
	        if (!worldIn.isRemote)
	        {
	            if (worldIn.rand.nextInt(4) == 0 && worldIn.isAreaLoaded(pos, 4)) // Forge: check area to prevent loading unloaded chunks
	            {
	                    ...
	                    for (int l = -4; l <= 4; ++l)
	                    {
	                        for (int i1 = -1; i1 <= 1; ++i1)
	                        {
	                            if (
									...
									|| (worldIn.getBlockState(pos.add(k, i1, l)).getBlock() == BlockHopsFruit.block)
										)
	                            {
	                                --j;
	                ...
	                if (enumfacing1 == EnumFacing.UP && pos.getY() < 255 && worldIn.isAirBlock(blockpos2))
	                {
						...
						if (rand.nextInt(3) == 0) {
							state1 = BlockHopsFruit.block.getDefaultState()
	                ...
	                else if (enumfacing1.getAxis().isHorizontal() && !((Boolean)state.getValue(getPropertyFor(enumfacing1))).booleanValue())
	                {
	                    if (!flag)
	                    {
	                        ...
	                        else if (iblockstate3.getBlockFaceShape(worldIn, blockpos4, enumfacing1) == BlockFaceShape.SOLID)
	                        {
								...
								if (rand.nextInt(3) == 0) {
									state2 = BlockHopsFruit.block.getDefaultState()
											.withProperty(UP, state.getValue(UP))
											.withProperty(NORTH, state.getValue(NORTH))
											.withProperty(EAST, state.getValue(EAST))
											.withProperty(SOUTH, state.getValue(SOUTH))
	                ...
	                else
	                {
	                    if (pos.getY() > 1)
	                    {
	                        ...
	                        if ((worldIn.getBlockState(blockpos3)).getMaterial() == Material.AIR)
	                        {
								...
								if (rand.nextInt(3) == 0) {
									state3 = BlockHopsFruit.block.getDefaultState()
	                        ...
	                        else if (block == this || block == BlockHopsFruit.block)
	                        {
		...
		private boolean recheckGrownSides(World worldIn, BlockPos pos, IBlockState state)
		{
			...
			for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
			{
				...
				if (((Boolean)state.getValue(propertybool)).booleanValue() && !this.canAttachTo(worldIn, pos, enumfacing.getOpposite()))
				{
					...
					if ((iblockstate1.getBlock() != this && iblockstate1.getBlock() != BlockHopsFruit.block)
