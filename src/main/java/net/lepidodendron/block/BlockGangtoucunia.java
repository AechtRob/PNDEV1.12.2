...
public class BlockGangtoucunia extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:gangtoucunia")
	public static final Block block = null;
	public BlockGangtoucunia(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.gangtoucunia);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("gangtoucunia"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		GameRegistry.registerTileEntity(TileEntityGangtoucunia.class, "lepidodendron:tileentitygangtoucunia");
		OreDictionary.registerOre("staticdnaPNlepidodendron:gangtoucunia", BlockGangtoucunia.block);
		OreDictionary.registerOre("pndietEncruster", BlockGangtoucunia.block);
	}
				...
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < 40; ++i) {
						...
						{	boolean waterDepthCheckMax = false;
							...
							IBlockState state = BlockGangtoucunia.block.getDefaultState();
							int ii = random.nextInt(4);
							if (ii == 0) {
								state = BlockGangtoucunia.block.getDefaultState().withProperty(FACING, EnumFacing.NORTH);
							}
							if (ii == 1) {
								state = BlockGangtoucunia.block.getDefaultState().withProperty(FACING, EnumFacing.SOUTH);
							}
							if (ii == 2) {
								state = BlockGangtoucunia.block.getDefaultState().withProperty(FACING, EnumFacing.EAST);
							}
							if (ii == 3) {
								state = BlockGangtoucunia.block.getDefaultState().withProperty(FACING, EnumFacing.WEST);
							}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_gangtoucunia");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_GANGTOUCUNIA;
		}
		...
		public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
			...
			if (te != null) {
				if (te instanceof TileEntityGangtoucunia) {
					TileEntityGangtoucunia Gangtoucunia = (TileEntityGangtoucunia) te;
					Gangtoucunia.setHidden(0);
					Gangtoucunia.setCluster(worldIn.rand.nextInt(3) + 1);
					Gangtoucunia.setOffset1(Math.max(worldIn.rand.nextFloat() * 0.45F, 0.2F) * (worldIn.rand.nextBoolean() ? 1F : -1F ));
					Gangtoucunia.setOffset2(Math.max(worldIn.rand.nextFloat() * 0.45F, 0.2F) * (worldIn.rand.nextBoolean() ? 1F : -1F ));
				}
			}
		...
		public TileEntity createTileEntity(World world, IBlockState state) {
			return new TileEntityGangtoucunia();
		}
		...
		public void hideGangtoucunia(World world, BlockPos pos) {
			TileEntity te = world.getTileEntity(pos);
			int intHidden = 0;
			if (te != null) {
				if (te instanceof TileEntityGangtoucunia) {
					TileEntityGangtoucunia Gangtoucunia = (TileEntityGangtoucunia) te;
					intHidden = Gangtoucunia.getHidden();
					...
					Gangtoucunia.setHidden(intHidden);
