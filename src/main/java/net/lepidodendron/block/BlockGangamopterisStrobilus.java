...
public class BlockGangamopterisStrobilus extends ElementsLepidodendronMod.ModElement {
	...
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("gangamopteris_strobilus"));
		elements.items.add(() -> new ItemBlock(block).setRegistryName(block.getRegistryName()));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		GameRegistry.registerTileEntity(TileEntityCustom.class, "lepidodendron:tileentitygangamopteris_strobilus");
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:gangamopteris_strobilus", "inventory"));
	}
	public static class BlockCustom extends Block implements ITileEntityProvider, net.minecraftforge.common.IShearable {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_gangamopteris_strobilus");
    ...
    public BlockRenderLayer getRenderLayer()
    {
		...
		public void neighborChanged(IBlockState state, World world, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			int z = pos.getZ();
			{
				...
				ProcedureGangamopterisStrobilusNeighbourBlockChanges.executeProcedure($_dependencies);
			}
		}
        ...
        if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()) || !LepidodendronConfig.doPropagation)
	        {
	        	...
	        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					if (Math.random() > 0.5) {
						ItemStack stackSeed = new ItemStack(ItemGangamopterisSeeds.block, (int) (1));
