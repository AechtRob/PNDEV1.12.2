...
public class BlockRugosa2 extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:rugosa_2")
	public static final Block block = null;
	public BlockRugosa2(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.rugosa_2);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("rugosa_2"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:rugosa_2", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockRugosa2.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("staticdnaPNlepidodendron:rugosa_2", BlockRugosa2.block);
		OreDictionary.registerOre("pndietCoral", BlockRugosa2.block);
	}
						...
						{	boolean waterDepthCheckMax = false;
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockRugosa2.BlockCustom.FACING, enumfacing), 2);
									return true;
								} else {
									for (EnumFacing enumfacing1 : BlockRugosa2.BlockCustom.FACING.getAllowedValues()) {
										pos1 = blockpos1;
										...
										if (enumfacing1 != EnumFacing.UP && enumfacing1 != EnumFacing.DOWN &&
											...
											world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockRugosa2.BlockCustom.FACING, enumfacing1), 2);
											return true;
										}
									}
								}
