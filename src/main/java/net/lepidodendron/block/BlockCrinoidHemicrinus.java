...
public class BlockCrinoidHemicrinus extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:crinoid_hemicrinus")
	public static final Block block = null;
	public BlockCrinoidHemicrinus(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.crinoid_hemicrinus);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("crinoid_hemicrinus"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:crinoid_hemicrinus", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockCrinoidHemicrinus.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("staticdnaPNlepidodendron:crinoid_hemicrinus", BlockCrinoidHemicrinus.block);
		OreDictionary.registerOre("pndietCrinoid", BlockCrinoidHemicrinus.block);
		OreDictionary.registerOre("holdfastDrops", BlockCrinoidHemicrinus.block);
	}
...
//		{
//			BiomeCretaceousEarly biomexyz = (BiomeCretaceousEarly) biome;
//			if (biomexyz.getBiomeType() == EnumBiomeTypeCretaceousEarly.Ocean) {
//				biomeCriteria = true;
//			}
//			else {
//				biomeCriteria = false;
//			}
//			if (biome.getRegistryName().toString().equalsIgnoreCase("lepidodendron:early_cretaceous_ocean_shore_atlantic")) {
//				biomeCriteria = false;
//			}
//		}
		if (!biomeCriteria)
		...
		int multiplier = 1;
		if (biome.getRegistryName().toString().equalsIgnoreCase("lepidodendron:jurassic_ocean_shore")) {
			multiplier = 5;
		}
						...
						{	boolean waterDepthCheckMax = false;
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockCrinoidHemicrinus.BlockCustom.FACING, enumfacing), 2);
									return true;
								} else {
									for (EnumFacing enumfacing1 : BlockCrinoidHemicrinus.BlockCustom.FACING.getAllowedValues()) {
										pos1 = blockpos1;
										...
										if (enumfacing1 != EnumFacing.UP && enumfacing1 != EnumFacing.DOWN &&
											...
											world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockCrinoidHemicrinus.BlockCustom.FACING, enumfacing1), 2);
											return true;
										}
									}
								}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_crinoid_hemicrinus");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_CRINOID_HEMICRINUS;
		}
