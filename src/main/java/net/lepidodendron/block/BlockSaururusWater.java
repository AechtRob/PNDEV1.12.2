...
import net.lepidodendron.item.ItemSaururusSeeds;
import net.lepidodendron.item.ItemSaururusItem;
...
public class BlockSaururusWater extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:saururus_water")
	public static final Block block = null;
	public BlockSaururusWater(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.saururus);
	}
	...
	public static class BlockCustom extends BlockLilyPad implements IGrowable, net.minecraftforge.common.IShearable, IAdvancementGranter {
		public BlockCustom() {
			...
			setLightOpacity(0);
			setCreativeTab(null);
			if (LepidodendronConfigPlants.spreadSaururus && LepidodendronConfig.spreadPlantsAtAll) {
				setTickRandomly(true);
			}
			else {
				setTickRandomly(false);
			}
			setTranslationKey("pf_saururus_water");
			setRegistryName("saururus_water");
		}

		@Nullable
		@Override
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_SAURURUS;
		}
		...
		public void onEntityCollision(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
			...
			if (entityIn instanceof EntityBoat)
			{
				...
				if (!LepidodendronConfig.doPropagation) {
					EntityItem entityToSpawn = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(ItemSaururusItem.block, (int) (1)));
		...
		public NonNullList<ItemStack> onSheared(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
			return NonNullList.withSize(1, new ItemStack(ItemSaururusItem.block, (int) (1)));
		}
		...
		public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player) {
			return new ItemStack(ItemSaururusItem.block, (int) (1));
		}
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				drops.add(new ItemStack(ItemSaururusItem.block, (int) (1)));
			}
		}
				...
				if (!this.canSurviveAt(world, pos))
		        {
					...
					if (canSurviveAt(world, targetBlock)) {
						if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock))) {
							world.setBlockState(targetBlock, BlockSaururusWater.block.getDefaultState(), 3);
						}
					}
					else if (BlockSaururusLand.BlockCustom.canSurviveAt(world, targetBlock)) {
						if (Math.random() > (1-(spread/100)) && (targetBlock != pos) && (world.isAirBlock(targetBlock))) {
							world.setBlockState(targetBlock, BlockSaururusLand.block.getDefaultState(), 3);
						}
					}
	    ...
	    {
	    	if (!LepidodendronConfigPlants.spreadSaururus) {return true;}
	        return false;
	    }
...
//	    	}
	    ...
	    {
        if ((!player.capabilities.allowEdit) || (player.getHeldItemMainhand().getItem() != new ItemStack(ItemSaururusItem.block, (int) (1)).getItem()) || !LepidodendronConfig.doPropagation)
	        {
	            return true;
	        }
	        else {
	        	...
	        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					if (Math.random() > 0.35) {
						ItemStack stackSeed = new ItemStack(ItemSaururusSeeds.block, (int) (1));
