...
public class BlockSphenoecium extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:sphenoecium")
	public static final Block block = null;
	public BlockSphenoecium(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.sphenoecium);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("sphenoecium"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:sphenoecium", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockSphenoecium.LEVEL).build());
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (biome instanceof BiomeCambrian) {
			BiomeCambrian biomeEnumBiomeTypeCambrian = (BiomeCambrian) biome;
			if (biomeEnumBiomeTypeCambrian.getBiomeType() == EnumBiomeTypeCambrian.Reef) {
				biomeCriteria = true;
			}
			else {
				biomeCriteria = false;
						...
						{	boolean waterDepthCheckMax = false;
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockSphenoecium.BlockCustom.FACING, enumfacing), 2);
									return true;
								} else {
									for (EnumFacing enumfacing1 : BlockSphenoecium.BlockCustom.FACING.getAllowedValues()) {
										pos1 = blockpos1;
										...
										if (enumfacing1 != EnumFacing.UP && enumfacing1 != EnumFacing.DOWN &&
											...
											world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockSphenoecium.BlockCustom.FACING, enumfacing1), 2);
											return true;
										}
									}
								}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public BlockCustom() {
			...
			setTranslationKey("pf_sphenoecium");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_SPHENOECIUM;
		}
