...
public class BlockOtavia extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:otavia")
	public static final Block block = null;
	public BlockOtavia(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.otavia);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("otavia"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:otavia", "inventory"));
		ModelLoader.setCustomStateMapper(block, (new StateMap.Builder()).ignore(BlockOtavia.LEVEL).build());
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("staticdnaPNlepidodendron:otavia", BlockOtavia.block);
		OreDictionary.registerOre("itemSponge", BlockOtavia.block);
		OreDictionary.registerOre("pndietSponge", BlockOtavia.block);
	}
						...
						if (world.getBlockState(blockpos1).getBlock() == Blocks.WATER) {
							...
							if (waterDepthCheckMin & waterDepthCheckMax) {
								if (((world.getBlockState(pos1).getMaterial() == Material.SAND)
									...
									world.setBlockState(blockpos1, block.getDefaultState().withProperty(BlockOtavia.BlockCustom.FACING, enumfacing), 2);
									return true;
								}
							}
						}
	...
	public static class BlockCustom extends Block implements net.minecraftforge.common.IShearable, IAdvancementGranter {
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_OTAVIA;
		}
		...
		public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
			if (LepidodendronConfig.showTooltips) {
				...
				tooltip.add("Periods: Neoproterozoic (Tonian - Cryogenian - Ediacaran)");
			}
