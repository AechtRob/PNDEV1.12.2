...
public class BlockOdontopterisStemNE extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:odontopteris_stem_ne")
	public static final Block block = null;
	public BlockOdontopterisStemNE(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.odontopteris_stem_ne);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("odontopteris_stem_ne"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:odontopteris_stem_ne", "inventory"));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:odontopteris_sapling", BlockOdontopterisStemNE.block);
        ...
        if ((!player.capabilities.allowEdit) || (!player.getHeldItemMainhand().isEmpty()))
	        {
	        	...
	        	if (!((hand != player.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
					world.setBlockState(pos, BlockOdontopterisStemSE.block.getDefaultState().withProperty(FACING, world.getBlockState(pos).getValue(FACING)), 3);
