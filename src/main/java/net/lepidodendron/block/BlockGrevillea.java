...
import net.lepidodendron.item.ItemGrevilleaFlower;
import net.lepidodendron.item.ItemGrevilleaSeeds;
...
public class BlockGrevillea extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:grevillea")
	public static final Block block = null;
	public BlockGrevillea(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.grevillea);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("grevillea"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:grevillea", "inventory"));
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:grevillea", BlockGrevillea.block);
		OreDictionary.registerOre("plantPrehistoric", BlockGrevillea.block);
		OreDictionary.registerOre("plant", BlockGrevillea.block);
	}
	...
	public static class BlockCustom extends BlockBush implements IAdvancementGranter {
		...
		public BlockCustom() {
			super();
			setTranslationKey("pf_grevillea");
		...
		public CustomTrigger getModTrigger() {
			return ModTriggers.CLICK_GREVILLEA;
		}
		...
		public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			...
			if ((!(worldIn.isRemote)) && worldIn.getBlockState(pos).getValue(FLOWERS)) {
						...
						&& hand == EnumHand.MAIN_HAND)
				{
					...
					Block.spawnAsEntity(worldIn, pos, new ItemStack(ItemGrevilleaFlower.block, 1));
					stack.damageItem(1, playerIn);
					return true;
				}

				if ((!playerIn.capabilities.allowEdit) || (playerIn.getHeldItemMainhand().getItem() != ItemGrevilleaFlower.block) || (!LepidodendronConfig.doPropagation) || (!worldIn.getBlockState(pos).getValue(FLOWERS)))
				{
					return true;
				}
				else {
					...
					if (!((hand != playerIn.getActiveHand()) && (hand == EnumHand.MAIN_HAND))) {
						if (Math.random() > 0.5) {
							ItemStack stackSeed = new ItemStack(ItemGrevilleaSeeds.block, (int) (1));
					...
					&& (world.getBlockState(pos.down()).getBlock() != Blocks.GRAVEL.getDefaultState().getBlock()))
			{
				...
				if (Math.random() > 0.66) {
					if (!LepidodendronConfig.doPropagation) {
						...
						if (!world.isRemote) {
							EntityItem entityToSpawn = new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(BlockGrevillea.block, (int) (1)));
					...
					if ((Math.random() > 0.7)) {
						if ((Math.random() > 0.7)) {
							if (world.getBlockState(pos).getValue(FLOWERS)) {
								world.setBlockState(pos, BlockGrevillea.block.getDefaultState().withProperty(FLOWERS, false), 3);
							}
							else {
								world.setBlockState(pos, BlockGrevillea.block.getDefaultState().withProperty(FLOWERS, true), 3);
							}
