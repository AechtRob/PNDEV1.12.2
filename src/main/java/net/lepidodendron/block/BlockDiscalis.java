...
public class BlockDiscalis extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:discalis")
	public static final Block block = null;
	public BlockDiscalis(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.discalis);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("lepidodendron:discalis", "inventory"));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:discalis", BlockDiscalis.block);
		OreDictionary.registerOre("plantPrehistoric", BlockDiscalis.block);
		OreDictionary.registerOre("plant", BlockDiscalis.block);
	}
	...
	public static class BlockCustom extends BlockBush implements IGrowable, IAdvancementGranter, IPottable {
		public BlockCustom() {
			...
			setTranslationKey("pf_discalis");
			setRegistryName("discalis");
		}

		@Override
		public IBlockState getPotState() {
			return BlockDiscalisSpore.block.getDefaultState();
		}
	        ...
	        if (!world.isRemote)
	        {
				...
				if (!canSurviveAt(world, pos))
		        {
					...
					//Chance of spores:
		        		...
		        		if ((Math.random() > 0.7)) {
							if ((Math.random() > 0.7)) {
								world.setBlockState(pos, BlockDiscalisSpore.block.getDefaultState(), 3);
							}
			        	}
		        	}
				}
