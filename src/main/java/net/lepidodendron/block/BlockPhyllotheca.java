...
public class BlockPhyllotheca extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:phyllotheca")
	public static final Block block = null;
	public BlockPhyllotheca(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.phyllotheca);
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (shouldGenerateInDimension(dimID, LepidodendronConfigPlants.dimPhyllotheca))
			dimensionCriteria = true;
		if (!LepidodendronConfigPlants.genPhyllotheca && !LepidodendronConfig.genAllPlants)
		...
		if ((!matchBiome(biome, LepidodendronConfig.genGlobalBlacklist)) && (!matchBiome(biome, LepidodendronConfigPlants.genPhyllothecaBlacklistBiomes))) {
		...
		if (matchBiome(biome, LepidodendronConfigPlants.genPhyllothecaOverrideBiomes))
			biomeCriteria = true;
		...
		int GenChance = 3;
		double GenMultiplier = LepidodendronConfigPlants.multiplierPhyllotheca;
		...
		int minheight = LepidodendronConfigPlants.minheightPhyllotheca;
				...
				public boolean generate(World world, Random random, BlockPos pos) {
					for (int i = 0; i < 10; ++i) {
						...
						if (world.isAirBlock(blockpos1) && world.isAirBlock(blockpos1.up()) && blockpos1.getY() >= minH && (blockpos1.getY() <= maxH || maxH == 0) ) {
							...
							int colWater = 1;
							while (colWater <= 3 && isWaterBlock(world, blockpos1.down(colWater))) {
					    		colWater = colWater + 1;
					    	}
					    	if ((colWater < 3) && (colWater > 1)) {
					    		//Build the stem:
					    		int counter = 2;
							...
							for (int k = 0; k <= j; ++k) {
								if (((BlockCustomFlower) block).canBlockStay(world, blockpos1)) {
									...
									if (k == j) {
										world.setBlockState(blockpos1.up(k), BlockPhyllothecaTop.block.getDefaultState(), 2);
									}
								}
							}
						}
	...
	public static class BlockCustomFlower extends BlockReed {
		...
		public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, @Nullable TileEntity te, ItemStack stack) {
			if (stack.getItem() == Items.SHEARS && LepidodendronConfig.doPropagation
					...
					(worldIn.getBlockState(pos.down()).getMaterial() == Material.GROUND
							...
							&& worldIn.getBlockState(pos.down()).getBlock() != BlockPhyllothecaStem.block
							&& (
	    ...
	    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
			...
			if (worldIn.getBlockState(pos.down(2)).getBlock() == BlockPhyllothecaStem.block) {
				worldIn.destroyBlock(pos.down(2), true);
			}
			if ((worldIn.getBlockState(pos.up()).getBlock() == this)
				|| (worldIn.getBlockState(pos.up()).getBlock() == BlockPhyllothecaTop.block))
			{
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			if (LepidodendronConfig.doPropagation) {
				...
				drops.add(new ItemStack(ItemPhyllothecaItem.block, (int) (1)));
			}
		}
		...
		public boolean canPlaceBlockAt(World world, BlockPos pos) {
			...
			if ((canSustainPlantRevised(world.getBlockState(pos.down(2)), world, pos.down(2), EnumFacing.UP, this)
				&& (isWaterBlock(world, pos.down()))) || block3 == BlockPhyllothecaStem.block) {
					canPlace2 = true;
				}
		...
		public void updateTick(World world, BlockPos pos, IBlockState state, Random random) {
			if (world.getBlockState(pos.down()).getBlock() == block || checkForDropRevised(world, pos, state)) {
				if (world.isAirBlock(pos.up())) {
					world.setBlockState(pos.up(), BlockPhyllothecaTop.block.getDefaultState(), 2);
				}
				else {
					if ((world.isAirBlock(pos.up(2))) && ((world.getBlockState(pos.up())).getBlock() == BlockPhyllothecaTop.block)
						) {
						int l;
						...
						if (l < 4) {
							...
							if (i1 == 15) {
								...
								world.setBlockState(pos.up(2), BlockPhyllothecaTop.block.getDefaultState(), 2);
