...
public class EntityPrehistoricFloraPhuwiangosaurus extends EntityPrehistoricFloraLandWadingBase implements IAdvancementGranter, ITrappableLand {
	...
	public EntityPrehistoricFloraPhuwiangosaurus(World world) {
		super(world);
		setSize(3.0F, 5F);
		extraStepHeight = 1F;
		minWidth = 0.1F;
	...
	public Animation[] getAnimations() {
		return new Animation[]{GRAZE_ANIMATION, HURT_ANIMATION, ATTACK_ANIMATION, NOISE_ANIMATION, DRINK_ANIMATION, ROAR_ANIMATION, MAKE_NEST_ANIMATION, LAY_ANIMATION, EAT_ANIMATION, LOOK_ANIMATION};
	}
	...
	public boolean isGrazing()
	{
		...
		boolean test = (this.getPFGrazing() <= 0
				...
				&&
				(
					(isBlockGrazable(this.world.getBlockState(entityPos.north(4).up(12)))
						&& isGrazable(this.world, entityPos, EnumFacing.NORTH))

						|| (isBlockGrazable(this.world.getBlockState(entityPos.south(4).up(12)))
						&& isGrazable(this.world, entityPos, EnumFacing.SOUTH))

						|| (isBlockGrazable(this.world.getBlockState(entityPos.east(4).up(12)))
						&& isGrazable(this.world, entityPos, EnumFacing.EAST))

						|| (isBlockGrazable(this.world.getBlockState(entityPos.west(4).up(12)))
		...
		if (test) {
			...
			if (!test2 && isBlockGrazable(this.world.getBlockState(entityPos.north(4).up(12)))) {
				facing = EnumFacing.NORTH;
				if (Functions.getEntityCentre(this).z - Functions.getEntityBlockPos(this).getZ() <= 0.5D) {
					test2 = true;
				}
			}
			else if (!test2 && isBlockGrazable(this.world.getBlockState(entityPos.south(4).up(12)))) {
				facing = EnumFacing.SOUTH;
				if (Functions.getEntityCentre(this).z - Functions.getEntityBlockPos(this).getZ() >= 0.5D) {
					test2 = true;
				}
			}
			else if (!test2 && isBlockGrazable(this.world.getBlockState(entityPos.east(4).up(12)))) {
				facing = EnumFacing.EAST;
				if (Functions.getEntityCentre(this).z - Functions.getEntityBlockPos(this).getX() >= 0.5D) {
					test2 = true;
				}
			}
			else if (!test2 && isBlockGrazable(this.world.getBlockState(entityPos.west(4).up(12)))) {
	...
	public AxisAlignedBB getRenderBoundingBox() {
		if (LepidodendronConfig.renderBigMobsProperly && (this.maxWidth * this.getAgeScale()) > 1F) {
			return this.getEntityBoundingBox().grow(20.0, 15.00, 20.0);
		}
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_PHUWIANGOSAURUS;
	}

	@Nullable
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.PHUWIANGOSAURUS_LOOT_YOUNG;
		}
		return LepidodendronMod.PHUWIANGOSAURUS_LOOT;
	}
