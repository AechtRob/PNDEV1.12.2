...
public class EntityPrehistoricFloraEusarcana extends EntityPrehistoricFloraSwimmingBottomWalkingWaterBase implements IAdvancementGranter, ITrappableWater {
	...
	private static final DataParameter<Boolean> SWIMMINGPN = EntityDataManager.createKey(EntityPrehistoricFloraEusarcana.class, DataSerializers.BOOLEAN);
	//Needs to be here because it is not loaded in time to be accessed by the client if it's on the parent class!

	public EntityPrehistoricFloraEusarcana(World world) {
		super(world);
		setSize(0.4F, 0.2F);
		minWidth = 0.1F;
		maxWidth = 0.5F;
...
//		this.targetTasks.addTask(3, new HuntAI(this, EntityPrehistoricFloraTrilobiteSwimBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.EUSARCANA_LOOT_YOUNG;
		}
		return LepidodendronMod.EUSARCANA_LOOT;
	}
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_EUSARCANA;
	}
