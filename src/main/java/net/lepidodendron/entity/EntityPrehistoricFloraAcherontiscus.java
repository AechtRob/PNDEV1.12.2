...
import net.lepidodendron.block.BlockAmphibianSpawnAcherontiscus;
...
public class EntityPrehistoricFloraAcherontiscus extends EntityPrehistoricFloraSwimmingAmphibianBase implements ITrappableWater, ITrappableLand, IAdvancementGranter {
	...
	public int WaterDist() {
		int i = (int) LepidodendronConfig.waterPhlegethontia;
		if (i > 16) {i = 16;}
		if (i < 1) {i = 1;}
		return i;
	}
...
//		this.targetTasks.addTask(2, new HuntSmallerThanMeAIAgeable(this, EntityPrehistoricFloraAmphibianBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase, 0.15));
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				&& (BlockAmphibianSpawnAcherontiscus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockAmphibianSpawnAcherontiscus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockAmphibianSpawnAcherontiscus.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockAmphibianSpawnAcherontiscus.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockAmphibianSpawnAcherontiscus.block.getDefaultState();
			if (BlockAmphibianSpawnAcherontiscus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockAmphibianSpawnAcherontiscus.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockAmphibianSpawnAcherontiscus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockAmphibianSpawnAcherontiscus.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_ACHERONTISCUS;
	}
