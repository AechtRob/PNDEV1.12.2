...
public class EntityPrehistoricFloraScaphognathus extends EntityPrehistoricFloraLandClimbingFlyingWalkingBase implements IAdvancementGranter, IScreamerFlier, IGuano, ITrappableLand {

	private boolean screaming;
	public int screamAlarmCooldown;

	public EntityPrehistoricFloraScaphognathus(World world) {
		super(world);
		setSize(0.65F, 0.45F);
		minWidth = 0.10F;
		maxWidth = 0.65F;
	...
	public boolean attackEntityFrom(DamageSource ds, float i) {
		...
		if (e instanceof EntityLivingBase && this.hasAlarm() && !this.world.isRemote) {
			...
			List<EntityPrehistoricFloraScaphognathus> pterodactylus = this.world.getEntitiesWithinAABB(EntityPrehistoricFloraScaphognathus.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
			for (EntityPrehistoricFloraScaphognathus currentPterodactylus : pterodactylus) {
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_SCAPHOGNATHUS;
	}

	@Nullable
	protected ResourceLocation getLootTable() {
		return LepidodendronMod.SCAPHOGNATHUS_LOOT;
	}
