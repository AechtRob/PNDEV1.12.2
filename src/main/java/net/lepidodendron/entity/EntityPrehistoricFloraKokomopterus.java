...
import net.lepidodendron.block.BlockEurypteridEggsKokomopterus;
...
public class EntityPrehistoricFloraKokomopterus extends EntityPrehistoricFloraEurypteridBase implements ITrappableWater {

	public BlockPos currentTarget;
	@SideOnly(Side.CLIENT)
	public ChainBuffer chainBuffer;

	public EntityPrehistoricFloraKokomopterus(World world) {
		super(world);
		setSize(0.6F, 0.6F);
		minWidth = 0.1F;
		maxWidth = 0.6F;
...
//		this.targetTasks.addTask(2, new HuntAI(this, EntityPrehistoricFloraTrilobiteSwimBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
	...
	protected float getAISpeedEurypterid() {
		float AIspeed = (float) Math.min(1F, (this.getAgeScale() * 2F)) * 0.18F;
		if (this.getIsFast()) {
	...
	public void onLivingUpdate() {
		...
		if (this.getAnimation() == ATTACK_ANIMATION && this.getAnimationTick() == 8 && this.getAttackTarget() != null) {
			this.getNavigator().tryMoveToXYZ(this.getAttackTarget().posX, this.getAttackTarget().posY, this.getAttackTarget().posZ, getAISpeedEurypterid() * 10);
			launchAttack();
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && this.isInWater() && this.isPFAdult() && this.getCanBreed() && this.getLaying() && this.getTicks() > 0
				&& (BlockEurypteridEggsKokomopterus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockEurypteridEggsKokomopterus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockEurypteridEggsKokomopterus.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockEurypteridEggsKokomopterus.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockEurypteridEggsKokomopterus.block.getDefaultState();
			if (BlockEurypteridEggsKokomopterus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockEurypteridEggsKokomopterus.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockEurypteridEggsKokomopterus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockEurypteridEggsKokomopterus.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	protected ResourceLocation getLootTable() {
		 		if (!this.isPFAdult()) {
			return LepidodendronMod.KOKOMOPTERUS_LOOT_YOUNG;
		}
		return LepidodendronMod.KOKOMOPTERUS_LOOT;
	}
