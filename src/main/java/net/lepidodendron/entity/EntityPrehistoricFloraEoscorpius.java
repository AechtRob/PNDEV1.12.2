...
import net.lepidodendron.entity.render.entity.RenderEoscorpius;
...
public class EntityPrehistoricFloraEoscorpius extends EntityPrehistoricFloraScorpion implements ITrappableAir, IAdvancementGranter {
	...
	public EntityPrehistoricFloraEoscorpius(World world) {
		super(world);
		setSize(0.3F, 0.3F);
		minWidth = 0.10F;
		maxWidth = 0.30F;
	...
	protected ResourceLocation getLootTable() {
		if (this.getBabies() && (!this.getIsBaby())) {
			return LepidodendronMod.EOSCORPIUS_LOOT;
		}
	...
	public void onDeath(DamageSource cause) {
		if (!world.isRemote && this.getBabies() && (!this.getIsBaby()) && cause != BlockGlassJar.BlockCustom.FREEZE) {
			...
			for (int i = 0; i <= ii; i++) {
				//Spawn babies:
				Entity entity = null;
				entity = ItemMonsterPlacer.spawnCreature(this.world, EntityList.getKey(EntityPrehistoricFloraEoscorpius.class), (double)this.getPosition().getX() + 0.5D, (double)this.getPosition().getY() + 0.5D, (double)this.getPosition().getZ() + 0.5D);
				EntityPrehistoricFloraEoscorpius baby = (EntityPrehistoricFloraEoscorpius) entity;
				baby.setAgeTicks(1);
				baby.setIsBaby(true);
				if (Math.random() >= 0.8) {
					baby.setBabies(true);
	...
	protected void dropLoot(boolean wasRecentlyHit, int lootingModifier, DamageSource source)
	{
		if (source == BlockGlassJar.BlockCustom.FREEZE) {
			...
			ResourceLocation resourcelocation = LepidodendronMod.EOSCORPIUS_LOOT_JAR;
	...
	public static double offsetCaseMagnified(@Nullable String variant) { return 0.9; }
	...
	@SideOnly(Side.CLIENT)
	public static ResourceLocation textureDisplay(@Nullable String variant) {return RenderEoscorpius.TEXTURE;}
	@SideOnly(Side.CLIENT)
	public static ModelBase modelDisplay(@Nullable String variant) {return RenderDisplays.modelEoscorpius;}
	public static float getScaler(@Nullable String variant) {
		return RenderEoscorpius.getScaler();
	}
	...
	public static float getScalerMagnified(@Nullable String variant) {
		return RenderEoscorpius.getScaler() * 2.5F;
	}
