...
import net.lepidodendron.entity.render.entity.RenderSinosauropteryx;
...
public class EntityPrehistoricFloraSinosauropteryx extends EntityPrehistoricFloraLandBase implements IAdvancementGranter, IScreamer, ICurious, INervous, ITrappableLand {
	...
	public EntityPrehistoricFloraSinosauropteryx(World world) {
		super(world);
		setSize(0.375F, 0.5F);
		minWidth = 0.12F;
		maxWidth = 0.375F;
	...
	public Animation[] getAnimations() {
		return new Animation[]{GRAPPLE_ANIMATION, GRAZE_ANIMATION, ATTACK_ANIMATION, ROAR_ANIMATION, LAY_ANIMATION, EAT_ANIMATION, MAKE_NEST_ANIMATION,};
	}
	...
	public boolean attackEntityFrom(DamageSource ds, float i) {
		...
		if (e instanceof EntityLivingBase) {
			...
			List<EntityPrehistoricFloraSinosauropteryx> Sinosauropteryx = this.world.getEntitiesWithinAABB(EntityPrehistoricFloraSinosauropteryx.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
			for (EntityPrehistoricFloraSinosauropteryx currentSinosauropteryx : Sinosauropteryx) {
				currentSinosauropteryx.setAnimation(NO_ANIMATION);
				currentSinosauropteryx.setRevengeTarget(ee);
				currentSinosauropteryx.setAlarmTarget(ee);
				currentSinosauropteryx.alarmCooldown = rand.nextInt(20);
			}
		}
	...
	public boolean findGrappleTarget() {
		...
		List<EntityPrehistoricFloraSinosauropteryx> Sinosauropteryx = world.getEntitiesWithinAABB(EntityPrehistoricFloraSinosauropteryx.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
		for (EntityPrehistoricFloraSinosauropteryx currentSinosauropteryx : Sinosauropteryx) {
			if ((!currentSinosauropteryx.getIsCuriousWalking()) && currentSinosauropteryx.isPFAdult() && this.isPFAdult() && currentSinosauropteryx != this && (!currentSinosauropteryx.willGrapple) && this.canEntityBeSeen(currentSinosauropteryx)) {
				this.setGrappleTarget(currentSinosauropteryx);
				currentSinosauropteryx.willGrapple = true;
				this.willGrapple = true;
				currentSinosauropteryx.setGrappleTarget(this);
				return true;
			}
		}
		return false;
	...
	protected void initEntityAI() {
		...
		tasks.addTask(8, new LandWanderFollowHuman(this, 1.0D, 5));
		tasks.addTask(9, new LandWanderFollowParent(this, 1.05D));
		tasks.addTask(10, new LandWanderHerd(this, 1.00D, Math.max(1F, this.width) * this.getNavigator().getPathSearchRange() * 0.75F, 5));
		tasks.addTask(11, new LandWanderAvoidWaterAndHumanAI(this, 1.0D));
		tasks.addTask(12, new LandWanderAvoidWaterAI(this, 1.0D));
		tasks.addTask(13, new EntityWatchClosestAI(this, EntityPlayer.class, 6.0F, 1.0F));
		tasks.addTask(14, new EntityLookIdleAI(this));
	...
	public void onEntityUpdate() {
		...
		if ((!this.world.isRemote) && this.getAnimation() == IDLE3
				&& (this.getAnimationTick() == IDLE3.getDuration() - 1) || this.isReallyInWater()) {
			this.standCooldown = 3000;
			this.setAnimation(NO_ANIMATION);
		}
		if ((!this.world.isRemote) && this.getAnimation() == IDLE4
				&& (this.getAnimationTick() == IDLE4.getDuration() - 1) || this.isReallyInWater()) {
			this.standCooldown = 3000;
			this.setAnimation(NO_ANIMATION);
		}
	}
	...
	protected ResourceLocation getLootTable() {
		...
		return LepidodendronMod.SINOSAUROPTERYX_LOOT;
	}
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_SINOSAUROPTERYX;
	}
	...
	@SideOnly(Side.CLIENT)
	public static ResourceLocation textureDisplay(@Nullable String variant) { return RenderSinosauropteryx.TEXTURE;
	}
	@SideOnly(Side.CLIENT)
	public static ModelBase modelDisplay(@Nullable String variant) {
		return RenderDisplays.modelSinosauropteryx;
	}
	public static float getScaler(@Nullable String variant) {
		return RenderSinosauropteryx.getScaler();
	}
