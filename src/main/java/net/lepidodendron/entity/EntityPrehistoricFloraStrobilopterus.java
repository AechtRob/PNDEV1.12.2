...
import net.lepidodendron.block.BlockEurypteridEggsStrobilopterus;
...
public class EntityPrehistoricFloraStrobilopterus extends EntityPrehistoricFloraSwimmingBottomWalkingWaterBase implements ITrappableWater, IAdvancementGranter {
	...
	private static final DataParameter<Boolean> SWIMMINGPN = EntityDataManager.createKey(EntityPrehistoricFloraStrobilopterus.class, DataSerializers.BOOLEAN);
	//Needs to be here because it is not loaded in time to be accessed by the client if it's on the parent class!

	public EntityPrehistoricFloraStrobilopterus(World world) {
		super(world);
		setSize(0.2F, 0.2F);
		minWidth = 0.1F;
		maxWidth = 0.2F;
...
//Lay eggs perhaps:
			if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
					&& (BlockEurypteridEggsStrobilopterus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
					|| BlockEurypteridEggsStrobilopterus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
					&& (BlockEurypteridEggsStrobilopterus.block.canPlaceBlockAt(world, this.getPosition())
					|| BlockEurypteridEggsStrobilopterus.block.canPlaceBlockAt(world, this.getPosition().down()))
			) {
				//if (Math.random() > 0.5) {
			...
			if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
				...
				IBlockState eggs = BlockEurypteridEggsStrobilopterus.block.getDefaultState();
				if (BlockEurypteridEggsStrobilopterus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockEurypteridEggsStrobilopterus.block.canPlaceBlockAt(world, this.getPosition())) {
					world.setBlockState(this.getPosition(), eggs);
					this.setLaying(false);
					this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
				}
				if (BlockEurypteridEggsStrobilopterus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockEurypteridEggsStrobilopterus.block.canPlaceBlockAt(world, this.getPosition().down())) {
...
//		this.targetTasks.addTask(3, new HuntAI(this, EntityPrehistoricFloraTrilobiteSwimBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
    ...
    protected ResourceLocation getLootTable() {
        return LepidodendronMod.STROBILOPTERUS_LOOT;
    }
	//Rendering taxidermy:
	//--------------------
	...
	public static ModelBase modelDisplay(@Nullable String variant) {
		return RenderDisplays.modelStrobilopterus;
	}
	public static float getScaler(@Nullable String variant) {
		return RenderStrobilopterus.getScaler();
	}
	public static float widthSupport(@Nullable String variant) {return 0.04F;}
    @Nullable
    @Override
    public CustomTrigger getModTrigger() {
        return ModTriggers.CLICK_STROBILOPTERUS;
    }
}