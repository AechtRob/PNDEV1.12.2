...
public class PathNavigateGroundNoDeepWater extends PathNavigate
{
    private boolean shouldAvoidSun;

    public PathNavigateGroundNoDeepWater(EntityLiving entity, World worldIn)
    {
    ...
    {
        this.nodeProcessor = new WalkNodeProcessorNoDeepWater();
...
//                        }
    ...
    public static boolean isUnacceptableWater(Entity entity) {
        if (!entity.world.isBlockLoaded(entity.getPosition())) {
            return false;
        }
        boolean isInWater = (entity.world.getBlockState(entity.getPosition()).getMaterial() == Material.WATER || entity.isInsideOfMaterial(Material.WATER) || entity.isInsideOfMaterial(Material.CORAL)
                || (entity.world.getBlockState(entity.getPosition().down()).getMaterial() == Material.WATER
                && !entity.onGround)
        );
        if (!isInWater) {
            return false;
    ...
    public static boolean isTooeepIBA(IBlockAccess worldIn, BlockPos pos) {
        //get depth of water column here:
        //First get bottom block:
        boolean isWaterBottom = false;
        int yy = pos.getY();
        int yyy = 0;
        while ((!isWaterBottom) && yy > 0) {
            if (worldIn.getBlockState(pos.down(yyy)).getMaterial() != Material.WATER) {
                if (yyy > 1) {
                    return true;
                }
                isWaterBottom = true;
        ...
        //Now simply check if there is more than one block of water here:
        if (worldIn.getBlockState(pos.down(yyy - 2)).getMaterial() == Material.WATER) {
            return true;
        }
        return false;
    ...
    public static boolean isTooDeep(World world, BlockPos pos) {
        //get depth of water column here:
        //First get bottom block:
        boolean isWaterBottom = false;
        int yy = pos.getY();
        int yyy = 0;
        while ((!isWaterBottom) && yy > 0) {
            if (world.getBlockState(pos.down(yyy)).getMaterial() != Material.WATER) {
                if (yyy > 1) {
                    return true;
                }
                isWaterBottom = true;
        ...
        //Now simply check if there is more than one block of water here:
        if (world.getBlockState(pos.down(yyy - 2)).getMaterial() == Material.WATER) {
            return true;
        }
        return false;
