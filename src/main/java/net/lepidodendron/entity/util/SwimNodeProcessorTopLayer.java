...
public class SwimNodeProcessorTopLayer extends NodeProcessor
{
        ...
        for (int i = p_186327_1_; i < p_186327_1_ + this.entitySizeX; ++i)
        {
            for (int j = p_186327_2_; j < p_186327_2_ + this.entitySizeY; ++j)
            {
                for (int k = p_186327_3_; k < p_186327_3_ + this.entitySizeZ; ++k)
                {
                    ...
                    IBlockState iblockstate1 = this.blockaccess.getBlockState(blockpos$mutableblockpos.setPos(i, j + 1, k));

                    if (iblockstate.getMaterial() != Material.WATER || (iblockstate.getMaterial() == Material.WATER && iblockstate1.getMaterial() == Material.WATER))
                    {
