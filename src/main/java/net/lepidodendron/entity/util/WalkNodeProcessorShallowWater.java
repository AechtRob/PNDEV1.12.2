...
public class WalkNodeProcessorShallowWater extends NodeProcessor
{
    ...
    public PathNodeType checkNeighborBlocks(IBlockAccess worldIn, int p_193578_2_, int p_193578_3_, int p_193578_4_, PathNodeType p_193578_5_)
    {
        ...
        if (p_193578_5_ == PathNodeType.WALKABLE)
        {
            for (int i = -1; i <= 1; ++i)
            {
                for (int j = -1; j <= 1; ++j)
                {
                    if (i != 0 || j != 0)
                    {
                        IBlockState state = worldIn.getBlockState(blockpos$pooledmutableblockpos.setPos(i + p_193578_2_, p_193578_3_, j + p_193578_4_));
                        Block block = state.getBlock();
                        PathNodeType type = block.getAiPathNodeType(state, worldIn, blockpos$pooledmutableblockpos, this.currentEntity);
    ...
    protected PathNodeType getPathNodeTypeRaw(IBlockAccess worldIn, int p_189553_2_, int p_189553_3_, int p_189553_4_)
    {
        ...
        if (worldIn.getBlockState(blockpos1).getBlock().isPassable(worldIn, blockpos1)
                && worldIn.getBlockState(blockpos1.down()).getBlock().isPassable(worldIn, blockpos1.down())) {
        ...
        else if (block != Blocks.TRAPDOOR && block != Blocks.IRON_TRAPDOOR && block != Blocks.WATERLILY)
        {
            ...
            else if (!(block instanceof BlockFence) && !(block instanceof BlockWall) && (!(block instanceof BlockFenceGate) || ((Boolean)iblockstate.getValue(BlockFenceGate.OPEN)).booleanValue()))
            {
                ...
                {
                    return block.isPassable(worldIn, blockpos) ? PathNodeType.OPEN : PathNodeType.BLOCKED;
                }
            }
            else
            {
