...
import net.lepidodendron.entity.render.entity.RenderNedcolbertia;
...
public class EntityPrehistoricFloraNedcolbertia extends EntityPrehistoricFloraLandBase implements IAdvancementGranter, IScreamer, ITrappableLand {
	...
	public EntityPrehistoricFloraNedcolbertia(World world) {
		super(world);
		setSize(0.5F, 0.9F);
		minWidth = 0.18F;
		maxWidth = 0.5F;
	...
	public boolean attackEntityFrom(DamageSource ds, float i) {
		...
		if (e instanceof EntityLivingBase) {
			...
			List<EntityPrehistoricFloraNedcolbertia> Nedcolbertia = Functions.getEntitiesWithinAABBPN(this.world, EntityPrehistoricFloraNedcolbertia.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)), EntitySelectors.NOT_SPECTATING);
			for (EntityPrehistoricFloraNedcolbertia currentNedcolbertia : Nedcolbertia) {
				currentNedcolbertia.setAnimation(NO_ANIMATION);
				currentNedcolbertia.setRevengeTarget(ee);
				currentNedcolbertia.setAlarmTarget(ee);
				currentNedcolbertia.alarmCooldown = rand.nextInt(20);
			}
		}
...
//TODO make override so noise cant play at the same time as idles
	...
	protected ResourceLocation getLootTable() {
        if (!this.isPFAdult()) {
            return LepidodendronMod.NEDCOLBERTIA_LOOT_YOUNG;
        }
		return LepidodendronMod.NEDCOLBERTIA_LOOT;
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_NEDCOLBERTIA;
	}
    //Rendering taxidermy:
    //--------------------
    ...
    @SideOnly(Side.CLIENT)
    public static ResourceLocation textureDisplay(@Nullable String variant) { return RenderNedcolbertia.TEXTURE;
    }
    @SideOnly(Side.CLIENT)
    public static ModelBase modelDisplay(@Nullable String variant) {
        return RenderDisplays.modelNedcolbertia;
    }
    public static float getScaler(@Nullable String variant) {
        return RenderNedcolbertia.getScaler();
    }
}