...
public class EntityPrehistoricFloraGalleonosaurus extends EntityPrehistoricFloraLandBase implements IScreamer, ITrappableLand, IAdvancementGranter {
	...
	public EntityPrehistoricFloraGalleonosaurus(World world) {
		super(world);
		setSize(0.5F, 0.7F);
		minWidth = 0.05F;
		maxWidth = 0.5F;
	...
	public boolean attackEntityFrom(DamageSource ds, float i) {
		...
		if (e instanceof EntityLivingBase && this.hasAlarm()) {
			...
			List<EntityPrehistoricFloraGalleonosaurus> Galleonosaurus = this.world.getEntitiesWithinAABB(EntityPrehistoricFloraGalleonosaurus.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
			for (EntityPrehistoricFloraGalleonosaurus currentGalleonosaurus : Galleonosaurus) {
				currentGalleonosaurus.setAnimation(NO_ANIMATION);
				currentGalleonosaurus.setRevengeTarget(ee);
				currentGalleonosaurus.setAlarmTarget(ee);
				currentGalleonosaurus.alarmCooldown = rand.nextInt(20);
			}
		}
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.GALLEONOSAURUS_LOOT_YOUNG;
		}
		return LepidodendronMod.GALLEONOSAURUS_LOOT;
	}

	@Nullable
	@Override
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_GALLEONOSAURUS;
	}
