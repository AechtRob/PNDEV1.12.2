...
import net.lepidodendron.block.BlockEggsDiplodoselache;
...
public class EntityPrehistoricFloraDiplodoselache extends EntityPrehistoricFloraAgeableFishBase implements IAdvancementGranter, ITrappableWater {
...
//		this.targetTasks.addTask(1, new HuntAI(this, EntitySquid.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				&& (BlockEggsDiplodoselache.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockEggsDiplodoselache.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockEggsDiplodoselache.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockEggsDiplodoselache.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockEggsDiplodoselache.block.getDefaultState();
			if (BlockEggsDiplodoselache.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockEggsDiplodoselache.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockEggsDiplodoselache.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockEggsDiplodoselache.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_DIPLODOSELACHE;
	}
	@Nullable
	protected ResourceLocation getLootTable() {
		return LepidodendronMod.DIPLODOSELACHE_LOOT;
	}
	//Rendering taxidermy:
	//--------------------
