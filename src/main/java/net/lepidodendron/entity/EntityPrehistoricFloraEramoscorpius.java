...
public class EntityPrehistoricFloraEramoscorpius extends EntityPrehistoricFloraWalkingAmphibianBase implements ITrappableWater, IAdvancementGranter, IBrood {
	...
	protected ResourceLocation getLootTable() {
		if (this.getBabies() && (!this.getIsBaby())) {
			return LepidodendronMod.ERAMOSCORPIUS_LOOT;
		}
	...
	public void onDeath(DamageSource cause) {
		if (!world.isRemote && this.getBabies() && (!this.getIsBaby())) {
			...
			for (int i = 0; i <= ii; i++) {
				//Spawn babies:
				Entity entity = null;
				entity = ItemMonsterPlacer.spawnCreature(this.world, EntityList.getKey(EntityPrehistoricFloraEramoscorpius.class), (double)this.getPosition().getX() + 0.5D, (double)this.getPosition().getY() + 0.5D, (double)this.getPosition().getZ() + 0.5D);
				EntityPrehistoricFloraEramoscorpius baby = (EntityPrehistoricFloraEramoscorpius) entity;
				baby.setAgeTicks(1);
				baby.setIsBaby(true);
				if (Math.random() >= 0.8) {
					baby.setBabies(true);
	...
	protected void dropLoot(boolean wasRecentlyHit, int lootingModifier, DamageSource source)
	{
		if (source == BlockGlassJar.BlockCustom.FREEZE) {
			...
			ResourceLocation resourcelocation = LepidodendronMod.ERAMOSCORPIUS_LOOT_JAR;
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_ERAMOSCORPIUS;
	}
}