...
import net.lepidodendron.entity.render.entity.RenderStruthiomimus;
...
public class EntityPrehistoricFloraStruthiomimus extends EntityPrehistoricFloraLandBase implements IAdvancementGranter, ITrappableLand {
	...
	public EntityPrehistoricFloraStruthiomimus(World world) {
		super(world);
		setSize(0.49F, 0.52F);
		minWidth = 0.05F;
		maxWidth = 0.49F;
	...
	public Animation[] getAnimations() {
		return new Animation[]{DRINK_ANIMATION, ATTACK_ANIMATION, ROAR_ANIMATION, LAY_ANIMATION, EAT_ANIMATION, MAKE_NEST_ANIMATION, GRAPPLE_ANIMATION, STAND_ANIMATION};
	}
	...
	public int getDrinkLength() {
		return 170;  //grazes, does not drink
	}
	...
	public boolean isAnimationDirectionLocked(Animation animation) {
		return animation == GRAPPLE_ANIMATION || animation == STAND_ANIMATION
	...
	protected void initEntityAI() {
		...
		tasks.addTask(4, new GrappleAI(this, 1.0D, false, this.getGrappleLength(), this.getGrappleAnimation(), 0.25));
	...
	public boolean findGrappleTarget() {
		...
		List<EntityPrehistoricFloraStruthiomimus> Struthiomimus = Functions.getEntitiesWithinAABBPN(world, EntityPrehistoricFloraStruthiomimus.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)), EntitySelectors.NOT_SPECTATING);
		for (EntityPrehistoricFloraStruthiomimus currentStruthiomimus : Struthiomimus) {
			if (currentStruthiomimus.isPFAdult() && this.isPFAdult() && currentStruthiomimus != this && (!currentStruthiomimus.willGrapple) && this.canEntityBeSeen(currentStruthiomimus)) {
				this.setGrappleTarget(currentStruthiomimus);
				currentStruthiomimus.willGrapple = true;
				this.willGrapple = true;
				currentStruthiomimus.setGrappleTarget(this);
				return true;
			}
		}
		return false;
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_STRUTHIOMIMUS;
	}

	@Nullable
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.STRUTHIOMIMUS_LOOT_YOUNG;
		}
		return LepidodendronMod.STRUTHIOMIMUS_LOOT;
	}
	//Rendering taxidermy:
	//--------------------
    ...
    @SideOnly(Side.CLIENT)
    public static ResourceLocation textureDisplay(@Nullable String variant) { return RenderStruthiomimus.TEXTURE;
    }
    @SideOnly(Side.CLIENT)
    public static ModelBase modelDisplay(@Nullable String variant) {
        return RenderDisplays.modelStruthiomimus;
    }
    public static float getScaler(@Nullable String variant) {
        return RenderStruthiomimus.getScaler();
    }
