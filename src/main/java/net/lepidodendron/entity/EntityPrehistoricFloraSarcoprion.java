...
import net.lepidodendron.block.BlockEggsSarcoprion;
...
public class EntityPrehistoricFloraSarcoprion extends EntityPrehistoricFloraAgeableFishBase implements IAdvancementGranter, ITrappableWater {
	...
	private static final DataParameter<Integer> SARCOPRION_TYPE = EntityDataManager.<Integer>createKey(EntityPrehistoricFloraSarcoprion.class, DataSerializers.VARINT);

	public EntityPrehistoricFloraSarcoprion(World world) {
		super(world);
		setSize(0.9F, 0.9F);
		minWidth = 0.3F;
		maxWidth = 0.9F;
	...
	protected void entityInit() {
		super.entityInit();
		this.dataManager.register(SARCOPRION_TYPE, 0);
	}
	...
	public boolean canMateWith(EntityAnimal otherAnimal)
	{
		...
		else if (otherAnimal.getClass() != this.getClass())
		{
			...
			switch (this.breedPNVariantsMatch()) {
				...
				case -1:
					if (((EntityPrehistoricFloraSarcoprion)otherAnimal).getPNType() == this.getPNType()) {
						return false;
					}
					break;
	...
	public enum Type
	{
		...
		public static EntityPrehistoricFloraSarcoprion.Type byId(int id)
		{
		...
		public static EntityPrehistoricFloraSarcoprion.Type getTypeFromString(String nameIn)
		{
	...
	public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
		...
		this.setPNType(EntityPrehistoricFloraSarcoprion.Type.byId(rand.nextInt(EntityPrehistoricFloraSarcoprion.Type.values().length) + 1));
		return livingdata;
	}

	public void setPNType(EntityPrehistoricFloraSarcoprion.Type type)
	{
		this.dataManager.set(SARCOPRION_TYPE, Integer.valueOf(type.ordinal()));
	}

	public EntityPrehistoricFloraSarcoprion.Type getPNType()
	{
		return EntityPrehistoricFloraSarcoprion.Type.byId(((Integer)this.dataManager.get(SARCOPRION_TYPE)).intValue());
	}
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		{
			this.setPNType(EntityPrehistoricFloraSarcoprion.Type.getTypeFromString(compound.getString("PNType")));
		}
	}
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.SARCOPRION_LOOT_YOUNG;
		}
		return LepidodendronMod.SARCOPRION_LOOT;
	}
...
//		//this.targetTasks.addTask(6, new HuntSmallerThanMeAIAgeable(this, EntityLivingBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase, 0.2));
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				&& (BlockEggsSarcoprion.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockEggsSarcoprion.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockEggsSarcoprion.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockEggsSarcoprion.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockEggsSarcoprion.block.getDefaultState();
			if (BlockEggsSarcoprion.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockEggsSarcoprion.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockEggsSarcoprion.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockEggsSarcoprion.block.canPlaceBlockAt(world, this.getPosition().down())) {
...
//	}
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_SARCOPRION;
	}
