...
import net.lepidodendron.block.BlockAmphibianSpawnNigerpeton;
...
public class EntityPrehistoricFloraNigerpeton extends EntityPrehistoricFloraSwimmingAmphibianBase implements IAdvancementGranter, ITrappableWater, ITrappableLand {

	public BlockPos currentTarget;
	@SideOnly(Side.CLIENT)
	public ChainBuffer chainBuffer;

	public EntityPrehistoricFloraNigerpeton(World world) {
		super(world);
		setSize(0.9F, 0.40F);
		minWidth = 0.2F;
		maxWidth = 0.9F;
...
//		this.targetTasks.addTask(1, new HuntAI(this, EntityPrehistoricFloraLandClimbingBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && this.isInWater() && this.isPFAdult() && this.getCanBreed() && this.getLaying() && this.getTicks() > 0
				&& (BlockAmphibianSpawnNigerpeton.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockAmphibianSpawnNigerpeton.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockAmphibianSpawnNigerpeton.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockAmphibianSpawnNigerpeton.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockAmphibianSpawnNigerpeton.block.getDefaultState();
			if (BlockAmphibianSpawnNigerpeton.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockAmphibianSpawnNigerpeton.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockAmphibianSpawnNigerpeton.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockAmphibianSpawnNigerpeton.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_NIGERPETON;
	}
	@Nullable
	protected ResourceLocation getLootTable() {
		 		if (!this.isPFAdult()) {
			return LepidodendronMod.NIGERPETON_LOOT_YOUNG;
		}
		return LepidodendronMod.NIGERPETON_LOOT;
	}
	//Rendering taxidermy:
	//--------------------
