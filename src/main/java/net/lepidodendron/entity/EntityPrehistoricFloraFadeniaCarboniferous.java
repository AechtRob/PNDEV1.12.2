...
import net.lepidodendron.block.BlockEggsFadeniaCarboniferous;
...
public class EntityPrehistoricFloraFadeniaCarboniferous extends EntityPrehistoricFloraAgeableFishBase implements IAdvancementGranter, ITrappableWater {
...
//		//this.targetTasks.addTask(6, new HuntSmallerThanMeAIAgeable(this, EntityLivingBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase, 0.2));
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				&& (BlockEggsFadeniaCarboniferous.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockEggsFadeniaCarboniferous.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockEggsFadeniaCarboniferous.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockEggsFadeniaCarboniferous.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockEggsFadeniaCarboniferous.block.getDefaultState();
			if (BlockEggsFadeniaCarboniferous.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockEggsFadeniaCarboniferous.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockEggsFadeniaCarboniferous.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockEggsFadeniaCarboniferous.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	protected ResourceLocation getLootTable() {
		 		if (!this.isPFAdult()) {
			return LepidodendronMod.FADENIA_CARBONIFEROUS_LOOT_YOUNG;
		}
		return LepidodendronMod.FADENIA_CARBONIFEROUS_LOOT;
	}
	...
	public static float getScaler(@Nullable String variant) {
		return RenderFadeniaCarboniferous.getScaler();
	}

	@Nullable
	@Override
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_FADENIA_CARBONIFEROUS;
	}
