...
public class EntityPrehistoricFloraScorpion_Gigantoscorpio extends EntityPrehistoricFloraScorpion implements IAdvancementGranter, ITrappableAir {

	public BlockPos currentTarget;
	@SideOnly(Side.CLIENT)
	public ChainBuffer chainBuffer;

	public EntityPrehistoricFloraScorpion_Gigantoscorpio(World world) {
		super(world);
		setSize(0.3F, 0.3F);
		minWidth = 0.10F;
		maxWidth = 0.30F;
	...
	protected ResourceLocation getLootTable() {
		if (this.getBabies() && (!this.getIsBaby())) {
			return LepidodendronMod.GIGANTOSCORPIO_LOOT;
		}
	...
	public void onDeath(DamageSource cause) {
		if (!world.isRemote && this.getBabies() && (!this.getIsBaby()) && cause != BlockGlassJar.BlockCustom.FREEZE) {
			...
			for (int i = 0; i <= ii; i++) {
				//Spawn babies:
				Entity entity = null;
				entity = ItemMonsterPlacer.spawnCreature(this.world, EntityList.getKey(EntityPrehistoricFloraScorpion_Gigantoscorpio.class), (double)this.getPosition().getX() + 0.5D, (double)this.getPosition().getY() + 0.5D, (double)this.getPosition().getZ() + 0.5D);
				EntityPrehistoricFloraScorpion_Gigantoscorpio baby = (EntityPrehistoricFloraScorpion_Gigantoscorpio) entity;
				baby.setAgeTicks(1);
				baby.setIsBaby(true);
				if (Math.random() >= 0.8) {
					baby.setBabies(true);
	...
	protected void dropLoot(boolean wasRecentlyHit, int lootingModifier, DamageSource source)
	{
		if (source == BlockGlassJar.BlockCustom.FREEZE) {
			...
			ResourceLocation resourcelocation = LepidodendronMod.GIGANTOSCORPIO_LOOT_JAR;
	...
	public static float getScaler(@Nullable String variant) {
		return RenderScorpion_Gigantoscorpio.getScaler();
	}
	@SideOnly(Side.CLIENT)
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_SCORPION_GIGANTOSCORPIO;
	}
	//-------------------
