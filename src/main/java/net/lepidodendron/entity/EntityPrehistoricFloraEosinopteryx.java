...
public class EntityPrehistoricFloraEosinopteryx extends EntityPrehistoricFloraLandBase implements IAdvancementGranter, ITrappableLand {
	...
	public EntityPrehistoricFloraEosinopteryx(World world) {
		super(world);
		setSize(0.5F, 0.9F);
		minWidth = 0.20F;
		maxWidth = 0.5F;
	...
	public boolean findGrappleTarget() {
		...
		List<EntityPrehistoricFloraEosinopteryx> Fukuivenator = world.getEntitiesWithinAABB(EntityPrehistoricFloraEosinopteryx.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
		for (EntityPrehistoricFloraEosinopteryx currentFukuivenator : Fukuivenator) {
			if ((!currentFukuivenator.getIsCuriousWalking()) && currentFukuivenator.isPFAdult() && this.isPFAdult() && currentFukuivenator != this && (!currentFukuivenator.willGrapple) && this.canEntityBeSeen(currentFukuivenator)) {
				this.setGrappleTarget(currentFukuivenator);
				currentFukuivenator.willGrapple = true;
				this.willGrapple = true;
				currentFukuivenator.setGrappleTarget(this);
				return true;
			}
		}
		return false;
	...
	public SoundEvent getHurtSound(DamageSource ds) {
	    return (SoundEvent) SoundEvent.REGISTRY
	            .getObject(new ResourceLocation("lepidodendron:fukuivenator_hurt"));
	}

	@Override
	public SoundEvent getDeathSound() {
	    return (SoundEvent) SoundEvent.REGISTRY
	            .getObject(new ResourceLocation("lepidodendron:fukuivenator_death"));
	}
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_EOSINOPTERYX;
	}

	@Nullable
	protected ResourceLocation getLootTable() {
		return LepidodendronMod.EOSINOPTERYX_LOOT;
	}
