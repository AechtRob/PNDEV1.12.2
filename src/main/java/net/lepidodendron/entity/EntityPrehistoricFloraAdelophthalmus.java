...
import net.lepidodendron.item.entities.ItemBucketAdelophthalmus;
...
public class EntityPrehistoricFloraAdelophthalmus extends EntityPrehistoricFloraEurypteridBase {

	public BlockPos currentTarget;
	@SideOnly(Side.CLIENT)
	public ChainBuffer chainBuffer;

	public EntityPrehistoricFloraAdelophthalmus(World world) {
		super(world);
		setSize(0.4F, 0.25F);
		experienceValue = 0;
	...
	protected float getAISpeedEurypterid() {
		...
		return (float) Math.min(0.9F, (this.getAgeScale() * 2F)) * 0.35F;
	}
	...
	//            .getObject(new ResourceLocation("lepidodendron:Adelophthalmus_idle"));
	//}
	...
	//    return (net.minecraft.util.SoundEvent) net.minecraft.util.SoundEvent.REGISTRY
	//            .getObject(new ResourceLocation("lepidodendron:Adelophthalmus_hurt"));
	//}
	...
	//    return (net.minecraft.util.SoundEvent) net.minecraft.util.SoundEvent.REGISTRY
	//            .getObject(new ResourceLocation("lepidodendron:Adelophthalmus_death"));
	//}
	@Override
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && LepidodendronConfig.doMultiplyMobs && this.getTicks() > 0
				&& (BlockEurypteridEggsAdelophthalmus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockEurypteridEggsAdelophthalmus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockEurypteridEggsAdelophthalmus.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockEurypteridEggsAdelophthalmus.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -30 && this.getTicks() < 0 && LepidodendronConfig.doMultiplyMobs) {
			...
			if (BlockEurypteridEggsAdelophthalmus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockEurypteridEggsAdelophthalmus.block.canPlaceBlockAt(world, this.getPosition().down())) {
		...
		if (this.getAgeScale() < 0.5F) { //Only catch babies
			...
			if (!itemstack.isEmpty()) {
				if (itemstack.getItem() == Items.WATER_BUCKET) {
					...
					ItemStack itemstack1 = new ItemStack(ItemBucketAdelophthalmus.block, (int) (1));
