...
import net.lepidodendron.block.BlockAmphibianSpawnSclerocephalus;
...
public class EntityPrehistoricFloraSclerocephalus extends EntityPrehistoricFloraSwimmingAmphibianBase implements IAdvancementGranter, ITrappableWater, ITrappableLand {
...
//		this.targetTasks.addTask(3, new HuntSmallerThanMeAIAgeable(this, EntityPrehistoricFloraAmphibianBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase, 0.1));
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				...
				|| BlockAmphibianSpawnSclerocephalus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockAmphibianSpawnSclerocephalus.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockAmphibianSpawnSclerocephalus.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockAmphibianSpawnSclerocephalus.block.getDefaultState();
			if (BlockAmphibianSpawnSclerocephalus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockAmphibianSpawnSclerocephalus.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockAmphibianSpawnSclerocephalus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockAmphibianSpawnSclerocephalus.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_SCLEROCEPHALUS;
	}
	@Nullable
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.SCLEROCEPHALUS_LOOT_YOUNG;
		}
		return LepidodendronMod.SCLEROCEPHALUS_LOOT;
	}
	//Rendering taxidermy:
	//--------------------
