...
public class EntityPrehistoricFloraCaiuajara extends EntityPrehistoricFloraLandClimbingFlyingWalkingBase implements IAdvancementGranter, IGuano, IScreamerFlier, ITrappableLand {

	private boolean screaming;
	public int screamAlarmCooldown;

	public EntityPrehistoricFloraCaiuajara(World world) {
		super(world);
		setSize(0.85F, 0.5F);
		minWidth = 0.10F;
		maxWidth = 0.85F;
	...
	public boolean attackEntityFrom(DamageSource ds, float i) {
		...
		if (e instanceof EntityLivingBase && this.hasAlarm() && !this.world.isRemote) {
			...
			List<EntityPrehistoricFloraCaiuajara> tapejara = this.world.getEntitiesWithinAABB(EntityPrehistoricFloraCaiuajara.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
			for (EntityPrehistoricFloraCaiuajara currentTapejara : tapejara) {
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_CAIUAJARA;
	}

	@Nullable
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.CAIUAJARA_LOOT_YOUNG;
		}
		return LepidodendronMod.CAIUAJARA_LOOT;
	}
