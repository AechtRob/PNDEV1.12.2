...
public class EntityPrehistoricFloraPsittacosaurus extends EntityPrehistoricFloraLandBase implements IScreamer, ITrappableLand, IAdvancementGranter {
	...
	public boolean findGrappleTarget() {
		...
		List<EntityPrehistoricFloraPsittacosaurus> Psittacosaurus = world.getEntitiesWithinAABB(EntityPrehistoricFloraPsittacosaurus.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
		for (EntityPrehistoricFloraPsittacosaurus currentPsittacosaurus : Psittacosaurus) {
			if ((!currentPsittacosaurus.getIsCuriousWalking()) && currentPsittacosaurus.isPFAdult() && this.isPFAdult() && currentPsittacosaurus != this && (!currentPsittacosaurus.willGrapple) && this.canEntityBeSeen(currentPsittacosaurus)) {
				this.setGrappleTarget(currentPsittacosaurus);
				currentPsittacosaurus.willGrapple = true;
				this.willGrapple = true;
				currentPsittacosaurus.setGrappleTarget(this);
				return true;
			}
		}
		return false;
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.PSITTACOSAURUS_LOOT_YOUNG;
		}
		return LepidodendronMod.PSITTACOSAURUS_LOOT;
	}
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_PSITTACOSAURUS;
	}
}