...
public class EntityPrehistoricFloraEusaurosphargis extends EntityPrehistoricFloraLandCarnivoreBase implements IAdvancementGranter, IBluffer, ITrappableLand {
	...
	public EntityPrehistoricFloraEusaurosphargis(World world) {
		...
		IDLE1 = Animation.create(15);
		IDLE2 = Animation.create(760); //Burrow
	...
	@Override
	public int getEatTick() {return 8;}
	...
	public Animation[] getAnimations() {
		return new Animation[]{ATTACK_ANIMATION, ROAR_ANIMATION, LAY_ANIMATION, EAT_ANIMATION, NOISE_ANIMATION, IDLE1, IDLE2};
	}
	...
	public float getAISpeedLand() {
		...
		if (this.getAnimation() == IDLE2 || this.getAnimation() == GRAZE_ANIMATION || this.getAnimation() == MAKE_NEST_ANIMATION) {
			return 0.0F;
		}
		if (this.getIsFast()) {
	...
	protected void initEntityAI() {
		...
		tasks.addTask(3, new AgeableWarnEntity<>(this, EntityPlayer.class, 4, IDLE2));
	...
	public void onLivingUpdate() {
		...
		if ((!this.world.isRemote) && this.getAnimation() == ROAR_ANIMATION && this.getWarnTarget() != null
			...
			List<EntityPrehistoricFloraEusaurosphargis> Camptosaurus = this.world.getEntitiesWithinAABB(EntityPrehistoricFloraEusaurosphargis.class, new AxisAlignedBB(this.getPosition().add(-12, -6, -12), this.getPosition().add(12, 6, 12)));
			for (EntityPrehistoricFloraEusaurosphargis currentCamptosaurus : Camptosaurus) {
				currentCamptosaurus.setRevengeTarget(this.getWarnTarget());
			}
		}

		boolean isPanickingNow = this.getRevengeTarget() != null || this.isBurning();

		if (this.wasPanicking && !isPanickingNow) {
			BlockPos entityPos = Functions.getEntityBlockPos(this);
			IBlockState state = this.world.getBlockState(entityPos.down());

			if (this.getAnimation() == NO_ANIMATION && (state.getMaterial() == Material.GROUND || state.getMaterial() == Material.SAND || state.getMaterial() == Material.GRASS)) {
				System.out.println("[DEBUG] PLAYING IDLE");
				this.setAnimation(IDLE2);
			}
		}

		this.wasPanicking = isPanickingNow;
	...
	protected ResourceLocation getLootTable() {
		return LepidodendronMod.EUSAUROSPHARGIS_LOOT;
	}

	@Nullable
	@Override
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_EUSAUROSPHARGIS;
	}
