...
public class EntityPrehistoricFloraGegepterus extends EntityPrehistoricFloraLandClimbingFlyingWalkingBase implements IAdvancementGranter, IGuano, IScreamerFlier, ITrappableLand {

	private boolean screaming;
	public int screamAlarmCooldown;

	public EntityPrehistoricFloraGegepterus(World world) {
		super(world);
		setSize(0.4F, 0.3F);
		minWidth = 0.10F;
		maxWidth = 0.4F;
	...
	public boolean attackEntityFrom(DamageSource ds, float i) {
		...
		if (e instanceof EntityLivingBase && this.hasAlarm() && !this.world.isRemote) {
			...
			List<EntityPrehistoricFloraGegepterus> tapejara = this.world.getEntitiesWithinAABB(EntityPrehistoricFloraGegepterus.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
			for (EntityPrehistoricFloraGegepterus currentTapejara : tapejara) {
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_GEGEPTERUS;
	}

	@Nullable
	protected ResourceLocation getLootTable() {
		return LepidodendronMod.GEGEPTERUS_LOOT;
	}
