...
public class EntityPrehistoricFloraTenontosaurus extends EntityPrehistoricFloraBiQuadLandBase implements IScreamer, IAdvancementGranter, IBluffer, ITrappableLand {
	...
	public EntityPrehistoricFloraTenontosaurus(World world) {
		super(world);
		setSize(1F, 1.75F);
		minWidth = 0.20F;
		maxWidth = 1F;
	...
	public boolean attackEntityFrom(DamageSource ds, float i) {
		...
		if (e instanceof EntityLivingBase) {
			...
			this.setBipedal();
			List<EntityPrehistoricFloraTenontosaurus> Tenontosaurus = this.world.getEntitiesWithinAABB(EntityPrehistoricFloraTenontosaurus.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
			for (EntityPrehistoricFloraTenontosaurus currentTenontosaurus : Tenontosaurus) {
				currentTenontosaurus.setAnimation(NO_ANIMATION);
				currentTenontosaurus.setRevengeTarget(ee);
				currentTenontosaurus.setAlarmTarget(ee);
				currentTenontosaurus.setBipedal();
				currentTenontosaurus.alarmCooldown = rand.nextInt(20);
			}
		}
...
//	}
	...
	public SoundEvent getAmbientSound() {
		return (SoundEvent) SoundEvent.REGISTRY
				.getObject(new ResourceLocation("lepidodendron:tenontosaurus_idle"));
	}
	...
	public SoundEvent getDeathSound() {
	    return (SoundEvent) SoundEvent.REGISTRY
	            .getObject(new ResourceLocation("lepidodendron:tenontosaurus_death"));
	}

	public SoundEvent getAlarmSound() {
		return (SoundEvent) SoundEvent.REGISTRY
				.getObject(new ResourceLocation("lepidodendron:tenontosaurus_alarm"));
	}
	...
	public void onLivingUpdate() {
		...
		if ((!this.world.isRemote) && this.getAnimation() == ROAR_ANIMATION && this.getWarnTarget() != null
			...
			List<EntityPrehistoricFloraTenontosaurus> Camptosaurus = this.world.getEntitiesWithinAABB(EntityPrehistoricFloraTenontosaurus.class, new AxisAlignedBB(this.getPosition().add(-12, -6, -12), this.getPosition().add(12, 6, 12)));
			for (EntityPrehistoricFloraTenontosaurus currentCamptosaurus : Camptosaurus) {
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.TENONTOSAURUS_LOOT_YOUNG;
		}
		return LepidodendronMod.TENONTOSAURUS_LOOT;
	}

	@Nullable
	@Override
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_TENONTOSAURUS;
	}
