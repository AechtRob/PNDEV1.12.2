...
import net.lepidodendron.block.BlockEurypteridEggsOrcanopterus;
...
public class EntityPrehistoricFloraPentecopterus extends EntityPrehistoricFloraSwimmingBottomWalkingWaterBase implements ITrappableWater, IAdvancementGranter {
	...
	private static final DataParameter<Boolean> SWIMMINGPN = EntityDataManager.createKey(EntityPrehistoricFloraPentecopterus.class, DataSerializers.BOOLEAN);
	//Needs to be here because it is not loaded in time to be accessed by the client if it's on the parent class!

	public EntityPrehistoricFloraPentecopterus(World world) {
		super(world);
		setSize(0.9F, 0.5F);
		minWidth = 0.1F;
		maxWidth = 0.9F;
...
//Lay eggs perhaps:
			if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
					&& (BlockEurypteridEggsOrcanopterus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
					|| BlockEurypteridEggsOrcanopterus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
					&& (BlockEurypteridEggsOrcanopterus.block.canPlaceBlockAt(world, this.getPosition())
					|| BlockEurypteridEggsOrcanopterus.block.canPlaceBlockAt(world, this.getPosition().down()))
			) {
				//if (Math.random() > 0.5) {
			...
			if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
				...
				IBlockState eggs = BlockEurypteridEggsOrcanopterus.block.getDefaultState();
				if (BlockEurypteridEggsOrcanopterus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockEurypteridEggsOrcanopterus.block.canPlaceBlockAt(world, this.getPosition())) {
					world.setBlockState(this.getPosition(), eggs);
					this.setLaying(false);
					this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
				}
				if (BlockEurypteridEggsOrcanopterus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockEurypteridEggsOrcanopterus.block.canPlaceBlockAt(world, this.getPosition().down())) {
...
//		this.targetTasks.addTask(3, new HuntAI(this, EntityPrehistoricFloraTrilobiteSwimBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
    ...
    protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.PENTECOPTERUS_LOOT_YOUNG;
		}
    ...
    public CustomTrigger getModTrigger() {
        return ModTriggers.CLICK_PENTECOPTERUS;
    }
}