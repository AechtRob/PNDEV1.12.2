...
public class EntityPrehistoricFloraTawa extends EntityPrehistoricFloraLandBase implements ITrappableLand, IAdvancementGranter {
	...
	public EntityPrehistoricFloraTawa(World world) {
		super(world);
		setSize(0.5F, 0.4F);
		minWidth = 0.20F;
		maxWidth = 0.5F;
	...
	public boolean findGrappleTarget() {
		...
		List<EntityPrehistoricFloraTawa> Tawa = world.getEntitiesWithinAABB(EntityPrehistoricFloraTawa.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
		for (EntityPrehistoricFloraTawa currentTawa : Tawa) {
			if ((!currentTawa.getIsCuriousWalking()) && currentTawa.isPFAdult() && this.isPFAdult() && currentTawa != this && (!currentTawa.willGrapple) && this.canEntityBeSeen(currentTawa)) {
				this.setGrappleTarget(currentTawa);
				currentTawa.willGrapple = true;
				this.willGrapple = true;
				currentTawa.setGrappleTarget(this);
				return true;
			}
		}
		return false;
...
//		this.targetTasks.addTask(4, new HuntSmallerThanMeAIAgeable(this, EntityLivingBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase, 0.2));
	...
	public SoundEvent getAmbientSound() {
		return (SoundEvent) SoundEvent.REGISTRY
				.getObject(new ResourceLocation("lepidodendron:tawa_idle"));
	...
	public SoundEvent getHurtSound(DamageSource ds) {
	    return (SoundEvent) SoundEvent.REGISTRY
	            .getObject(new ResourceLocation("lepidodendron:tawa_hurt"));
	}
	...
	public SoundEvent getChatterSound() {
		return (SoundEvent) SoundEvent.REGISTRY
				.getObject(new ResourceLocation("lepidodendron:tawa_alarm"));
	}
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.TAWA_LOOT_YOUNG;
		}
		return LepidodendronMod.TAWA_LOOT;
	}

	@Nullable
	@Override
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_TAWA;
	}
