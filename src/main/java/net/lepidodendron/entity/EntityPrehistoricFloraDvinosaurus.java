...
import net.lepidodendron.block.BlockAmphibianSpawnDvinosaurus;
...
public class EntityPrehistoricFloraDvinosaurus extends EntityPrehistoricFloraSwimmingAmphibianBase implements ITrappableWater, ITrappableLand {

	public BlockPos currentTarget;
	@SideOnly(Side.CLIENT)
	public ChainBuffer tailBuffer;
	//public static final SoundEvent dvinosaurus_ROAR = create("dvinosaurus_roar");

	public EntityPrehistoricFloraDvinosaurus(World world) {
		super(world);
		setSize(0.65F, 0.40F);
		minWidth = 0.1F;
		maxWidth = 0.65F;
	...
	public int WaterDist() {
		int i = (int) LepidodendronConfig.waterDvinosaurus;
		if (i > 16) {i = 16;}
		if (i < 1) {i = 1;}
		return i;
	}
...
//		this.targetTasks.addTask(4, new HuntSmallerThanMeAIAgeable(this, EntityLivingBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase, 0.15));
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				&& (BlockAmphibianSpawnDvinosaurus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockAmphibianSpawnDvinosaurus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockAmphibianSpawnDvinosaurus.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockAmphibianSpawnDvinosaurus.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockAmphibianSpawnDvinosaurus.block.getDefaultState();
			if (BlockAmphibianSpawnDvinosaurus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockAmphibianSpawnDvinosaurus.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockAmphibianSpawnDvinosaurus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockAmphibianSpawnDvinosaurus.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.DVINOSAURUS_LOOT_YOUNG;
		}
		return LepidodendronMod.DVINOSAURUS_LOOT;
	}
