package net.lepidodendron.entity.ai;

public class DietString {

    public static final String[] NULL = new String[]{};
    public static final String[] FISHFOOD = new String[]{"itemFishFood"};
    public static final String[] BONE = new String[]{"pndietBone"};
    public static final String[] FISH = new String[]{"pndietFish","listAllfishraw","listAllfishcooked","listAllfishfresh"};
    public static final String[] MEAT = new String[]{"pndietMeat","listAllmeatraw","listAllmeatcooked"};
    public static final String[] BUG = new String[]{"pndietBug"};
    public static final String[] SHELLFISH = new String[]{"pndietShellfish","foodClamraw","foodClamcooked","foodScallopraw","foodScallopcooked","foodMusselraw","foodMusselcooked","foodOysterraw","foodOystercooked"};
    public static final String[] EGGS = new String[]{"listAllegg"};
    public static final String[] ALGAE = new String[]{"itemAlgae","cropSeaweed"};
    public static final String[] PLANTS = new String[]{"plant","listAllgreenveggie"};
    public static final String[] ROTTEN = new String[]{"pndietRotten"};
    public static final String[] MOSS = new String[]{"itemMoss"};
    public static final String[] SPONGE = new String[]{"pndietSponge"};
    public static final String[] CORAL = new String[]{"pndietCoral"};
    public static final String[] FRUIT = new String[]{"pndietFruit","listAllfruit"};
    public static final String[] SEED = new String[]{"pndietSeed","listAllseed","listAllgrain"};
    public static final String[] ROOTS = new String[]{"pndietTuber","listAllrootveggie"};
    public static final String[] MICROBIAL = new String[]{"pndietMicrobial"};
    public static final String[] NAUTILOID = new String[]{"pndietNautiloid","foodCalamariraw","foodCalamaricooked","foodOctopusraw","foodOctopuscooked"};
    public static final String[] CRUSTACEAN = new String[]{"pndietCrustacean","foodCrabraw","foodCrabcooked","foodCrayfishraw","foodCrayfishcooked","foodShrimpraw","foodShrimpcooked"};
    public static final String[] FUNGI = new String[]{"pndietFungus","listAllmushroom"};
    public static final String[] CRINOID = new String[]{"pndietCrinoid"}; //Cystoids, Crinoids, Blastoids, Echinoderms
    public static final String[] ENCRUSTER = new String[]{"pndietEncruster"};
    public static final String[] SUGAR = new String[]{"pndietSugar"};


}
