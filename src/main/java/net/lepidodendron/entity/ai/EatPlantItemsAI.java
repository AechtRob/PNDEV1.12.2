...
public class EatPlantItemsAI extends EntityAIBase {
    private final EntityPrehistoricFloraAgeableBase entity;
    private final ItemsSorter targetSorter;
    private EntityItem targetItem;
    private double speed;

    public EatPlantItemsAI(EntityPrehistoricFloraAgeableBase entity, double speedIn) {
        super();
        this.entity = entity;
        this.speed = speedIn;
    ...
    private EntityItem getNearestItem(int range) {
        ...
        for (EntityItem currentItem : Items) {
            if (!currentItem.getItem().isEmpty()) {
                if (OreDictionary.containsMatch(false, OreDictionary.getOres("plant"), currentItem.getItem())
