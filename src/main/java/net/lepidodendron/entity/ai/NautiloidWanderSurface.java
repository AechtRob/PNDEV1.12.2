    ...
    public BlockPos findWaterTarget() {
        ...
        if (this.PrehistoricFloraNautiloidBase.getAttackTarget() == null) {
            for (int i = 0; i < 10; i++) {
                ...
                if (this.PrehistoricFloraNautiloidBase.world.getBlockState(randPos).getMaterial() == Material.WATER && !isAtTop(randPos) && Math.random() < 0.90) {
                    int ii = 0;
                    while ((randPos.up(ii).getY() > 1) && this.PrehistoricFloraNautiloidBase.world.getBlockState(randPos.up(ii)).getMaterial() == Material.WATER) {
                        randPosVar = randPos.up(ii);
                        ii = ii + 1;
                    }
