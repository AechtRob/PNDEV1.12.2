...
public class EntityPrehistoricFloraIyuku extends EntityPrehistoricFloraLandBase implements IScreamer, IAdvancementGranter, ITrappableLand {
	...
	public EntityPrehistoricFloraIyuku(World world) {
		super(world);
		setSize(0.7F, 1.3F);
		minWidth = 0.12F;
		maxWidth = 0.7F;
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_IYUKU;
	}
	...
	public boolean attackEntityFrom(DamageSource ds, float i) {
		...
		if (e instanceof EntityLivingBase && (!(this instanceof EntityPrehistoricFloraIyuku))) {
	...
	public boolean findGrappleTarget() {
		...
		List<EntityPrehistoricFloraIyuku> Dryosaurus = world.getEntitiesWithinAABB(EntityPrehistoricFloraIyuku.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
		for (EntityPrehistoricFloraIyuku currentDryosaurus : Dryosaurus) {
			if (!(currentDryosaurus instanceof EntityPrehistoricFloraIyuku)) {
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.IYUKU_LOOT_YOUNG;
		}
		return LepidodendronMod.IYUKU_LOOT;
	}
