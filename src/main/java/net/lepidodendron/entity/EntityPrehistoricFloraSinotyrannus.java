...
public class EntityPrehistoricFloraSinotyrannus extends EntityPrehistoricFloraLandCarnivoreBase implements IAdvancementGranter, ITrappableLand {
	...
	public EntityPrehistoricFloraSinotyrannus(World world) {
		super(world);
		setSize(1.75F, 1.8F);
		minWidth = 0.20F;
		maxWidth = 1.75F;
	...
	public boolean findGrappleTarget() {
		...
		List<EntityPrehistoricFloraSinotyrannus> Sinotyrannus = world.getEntitiesWithinAABB(EntityPrehistoricFloraSinotyrannus.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
		for (EntityPrehistoricFloraSinotyrannus currentSinotyrannus : Sinotyrannus) {
			if ((!currentSinotyrannus.getIsCuriousWalking()) && currentSinotyrannus.isPFAdult() && this.isPFAdult() && currentSinotyrannus != this && (!currentSinotyrannus.willGrapple) && this.canEntityBeSeen(currentSinotyrannus)) {
				this.setGrappleTarget(currentSinotyrannus);
				currentSinotyrannus.willGrapple = true;
				this.willGrapple = true;
				currentSinotyrannus.setGrappleTarget(this);
				return true;
			}
		}
		return false;
...
//		this.targetTasks.addTask(4, new HuntSmallerThanMeAIAgeable(this, EntityLivingBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase, 0.2));
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.SINOTYRANNUS_LOOT_YOUNG;
		}
		return LepidodendronMod.SINOTYRANNUS_LOOT;
	}

	@Nullable
	@Override
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_SINOTYRANNUS;
	}
