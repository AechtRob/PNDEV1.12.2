...
public class EntityPrehistoricFloraRhinocarcinosoma extends EntityPrehistoricFloraSwimmingBottomWalkingWaterBase implements IAdvancementGranter, ITrappableWater {
	...
	private static final DataParameter<Boolean> SWIMMINGPN = EntityDataManager.createKey(EntityPrehistoricFloraRhinocarcinosoma.class, DataSerializers.BOOLEAN);
	//Needs to be here because it is not loaded in time to be accessed by the client if it's on the parent class!

	public EntityPrehistoricFloraRhinocarcinosoma(World world) {
		super(world);
		setSize(0.5F, 0.2F);
		minWidth = 0.1F;
		maxWidth = 0.5F;
...
//		this.targetTasks.addTask(3, new HuntAI(this, EntityPrehistoricFloraTrilobiteSwimBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.RHINOCARCINOSOMA_LOOT_YOUNG;
		}
		return LepidodendronMod.RHINOCARCINOSOMA_LOOT;
	}
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_RHINOCARCINOSOMA;
	}
