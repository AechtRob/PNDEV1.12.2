...
import net.lepidodendron.entity.render.entity.RenderUteodon;
...
public class EntityPrehistoricFloraUteodon extends EntityPrehistoricFloraLandCarnivoreBase implements IAdvancementGranter, IBluffer, ITrappableLand {
	...
	public boolean isGrazing()
	{
		...
		if (test) {
			...
			if (!test2 && isBlockGrazable(this.world.getBlockState(entityPos.north()))) {
				facing = EnumFacing.NORTH;
				if (Functions.getEntityCentre(this).z - Functions.getEntityBlockPos(this).getZ() >= 0.1D
						&& Functions.getEntityCentre(this).z - Functions.getEntityBlockPos(this).getZ() <= 0.3D) {
					test2 = true;
				}
			}
				...
				facing = EnumFacing.EAST;
				if (Functions.getEntityCentre(this).x - Functions.getEntityBlockPos(this).getX() >= 0.1D
						&& Functions.getEntityCentre(this).x - Functions.getEntityBlockPos(this).getX() <= 0.3D) {
					test2 = true;
				}
			}
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.UTEODON_LOOT_YOUNG;
		}
		return LepidodendronMod.UTEODON_LOOT;
	}

	@Nullable
	@Override
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_UTEODON;
	}
	...
	public static ModelBase modelDisplay(@Nullable String variant) {
		return RenderDisplays.modelUteodon;
	}
	public static float getScaler(@Nullable String variant) {
		return RenderUteodon.getScaler();
	}
