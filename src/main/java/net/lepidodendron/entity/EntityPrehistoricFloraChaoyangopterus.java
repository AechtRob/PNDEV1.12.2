...
public class EntityPrehistoricFloraChaoyangopterus extends EntityPrehistoricFloraLandClimbingFlyingWalkingBase implements IAdvancementGranter, IGuano, IScreamerFlier, ITrappableLand {

	private boolean screaming;
	public int screamAlarmCooldown;

	public EntityPrehistoricFloraChaoyangopterus(World world) {
		super(world);
		setSize(0.85F, 0.5F);
		minWidth = 0.10F;
		maxWidth = 0.85F;
	...
	public boolean attackEntityFrom(DamageSource ds, float i) {
		...
		if (e instanceof EntityLivingBase && this.hasAlarm() && !this.world.isRemote) {
			...
			List<EntityPrehistoricFloraChaoyangopterus> tapejara = this.world.getEntitiesWithinAABB(EntityPrehistoricFloraChaoyangopterus.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
			for (EntityPrehistoricFloraChaoyangopterus currentTapejara : tapejara) {
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_CHAOYANGOPTERUS;
	}

	@Nullable
	protected ResourceLocation getLootTable() {
		return LepidodendronMod.CHAOYANGOPTERUS_LOOT;
	}
	...
	public SoundEvent getAmbientSound() {
		return (SoundEvent) SoundEvent.REGISTRY.getObject(new ResourceLocation("lepidodendron:chaoyangopterus_idle"));
	}

	@Override
	public SoundEvent getHurtSound(DamageSource ds) {
		return (SoundEvent) SoundEvent.REGISTRY.getObject(new ResourceLocation("lepidodendron:chaoyangopterus_hurt"));
	}
