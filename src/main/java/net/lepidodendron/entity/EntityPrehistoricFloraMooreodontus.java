...
import net.lepidodendron.block.BlockEggsMooreodontus;
import net.lepidodendron.block.base.IAdvancementGranter;
import net.lepidodendron.entity.ai.*;
import net.lepidodendron.entity.base.EntityPrehistoricFloraAgeableFishBase;
import net.lepidodendron.entity.render.entity.RenderMooreodontus;
...
public class EntityPrehistoricFloraMooreodontus extends EntityPrehistoricFloraAgeableFishBase implements IAdvancementGranter, ITrappableWater {

	public BlockPos currentTarget;
	@SideOnly(Side.CLIENT)
	public ChainBuffer tailBuffer;

	public EntityPrehistoricFloraMooreodontus(World world) {
		super(world);
		setSize(0.68F, 0.6F);
		minWidth = 0.1F;
		maxWidth = 0.68F;
...
//		this.targetTasks.addTask(2, new HuntSmallerThanMeAIAgeable(this, EntityPrehistoricFloraAgeableFishBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase, 0.1));
//		this.targetTasks.addTask(2, new HuntSmallerThanMeAIAgeable(this, EntityPrehistoricFloraAmphibianBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase, 0.1));
//		this.targetTasks.addTask(1, new HuntAI(this, EntitySquid.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				&& (BlockEggsMooreodontus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockEggsMooreodontus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockEggsMooreodontus.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockEggsMooreodontus.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockEggsMooreodontus.block.getDefaultState();
			if (BlockEggsMooreodontus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockEggsMooreodontus.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockEggsMooreodontus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockEggsMooreodontus.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_MOOREODONTUS;
	}
	@Nullable
	protected ResourceLocation getLootTable() {
		return LepidodendronMod.MOOREODONTUS_LOOT;
	}
	//Rendering taxidermy:
	//--------------------
	...
	public static double lowerfrontverticallinedepth(@Nullable String variant) {return 0.7;}
	public static double lowerbackverticallinedepth(@Nullable String variant) {return 0.5;}
	public static double lowerfrontlineoffset(@Nullable String variant) {return -0.015;}
	...
	public static double lowerbacklineoffsetperpendiular(@Nullable String variant) {return -0.4;}
	@SideOnly(Side.CLIENT)
	public static ResourceLocation textureDisplay(@Nullable String variant) {return RenderMooreodontus.TEXTURE;}
	@SideOnly(Side.CLIENT)
	public static ModelBase modelDisplay(@Nullable String variant) {return RenderDisplays.modelMooreodontus;}
	public static float getScaler(@Nullable String variant) {return RenderMooreodontus.getScaler();}
