...
import net.lepidodendron.entity.render.entity.RenderPhuwiangvenator;
...
public class EntityPrehistoricFloraPhuwiangvenator extends EntityPrehistoricFloraLandBase implements IAdvancementGranter, ITrappableLand {
	...
	public EntityPrehistoricFloraPhuwiangvenator(World world) {
		super(world);
		setSize(1.5F, 1.8F);
		minWidth = 0.20F;
		maxWidth = 0.8F;
	...
	public boolean findGrappleTarget() {
		...
		List<EntityPrehistoricFloraPhuwiangvenator> Phuwiangvenator = world.getEntitiesWithinAABB(EntityPrehistoricFloraPhuwiangvenator.class, new AxisAlignedBB(this.getPosition().add(-6, -4, -6), this.getPosition().add(6, 4, 6)));
		for (EntityPrehistoricFloraPhuwiangvenator currentPhuwiangvenator : Phuwiangvenator) {
			if ((!currentPhuwiangvenator.getIsCuriousWalking()) && currentPhuwiangvenator.isPFAdult() && this.isPFAdult() && currentPhuwiangvenator != this && (!currentPhuwiangvenator.willGrapple) && this.canEntityBeSeen(currentPhuwiangvenator)) {
				this.setGrappleTarget(currentPhuwiangvenator);
				currentPhuwiangvenator.willGrapple = true;
				this.willGrapple = true;
				currentPhuwiangvenator.setGrappleTarget(this);
				return true;
			}
		}
		return false;
	...
	public SoundEvent getHurtSound(DamageSource ds) {
	    return (SoundEvent) SoundEvent.REGISTRY
	            .getObject(new ResourceLocation("lepidodendron:phuwiangvenator_hurt"));
	}

	@Override
	public SoundEvent getDeathSound() {
	    return (SoundEvent) SoundEvent.REGISTRY
	            .getObject(new ResourceLocation("lepidodendron:phuwiangvenator_death"));
	}
	...
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_PHUWIANGVENATOR;
	}
	@Nullable
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.PHUWIANGVENATOR_LOOT_YOUNG;
		}
		return LepidodendronMod.PHUWIANGVENATOR_LOOT;
	}
    ...
    @SideOnly(Side.CLIENT)
    public static ResourceLocation textureDisplay(@Nullable String variant) { return RenderPhuwiangvenator.TEXTURE;
    }
    @SideOnly(Side.CLIENT)
    public static ModelBase modelDisplay(@Nullable String variant) {
        return RenderDisplays.modelPhuwiangvenator;
    }
    public static float getScaler(@Nullable String variant) {
        return RenderPhuwiangvenator.getScaler();
    }
