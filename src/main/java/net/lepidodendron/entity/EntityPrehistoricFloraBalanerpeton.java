...
import net.lepidodendron.block.BlockAmphibianSpawnBalanerpeton;
...
public class EntityPrehistoricFloraBalanerpeton extends EntityPrehistoricFloraSwimmingAmphibianBase {
	...
	public EntityPrehistoricFloraBalanerpeton(World world) {
		super(world);
		setSize(0.3F, 0.20F);
		minWidth = 0.1F;
		maxWidth = 0.3F;
...
//		this.targetTasks.addTask(1, new HuntAI(this, EntityPrehistoricFloraLandClimbingBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				&& (BlockAmphibianSpawnBalanerpeton.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockAmphibianSpawnBalanerpeton.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockAmphibianSpawnBalanerpeton.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockAmphibianSpawnBalanerpeton.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -30 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockAmphibianSpawnBalanerpeton.block.getDefaultState();
			if (BlockAmphibianSpawnBalanerpeton.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockAmphibianSpawnBalanerpeton.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockAmphibianSpawnBalanerpeton.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockAmphibianSpawnBalanerpeton.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	protected ResourceLocation getLootTable() {
		 		if (!this.isPFAdult()) {
			return LepidodendronMod.BALANERPETON_LOOT_YOUNG;
		}
