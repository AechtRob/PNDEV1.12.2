...
public class RenderSaperion extends RenderLiving<EntityPrehistoricFloraSaperion> {
    ...
    public RenderSaperion(RenderManager mgr) {
        super(mgr, new ModelSaperion(), 0.0f);
    }
    @Override
    public void doRender(EntityPrehistoricFloraSaperion entity, double x, double y, double z, float entityYaw, float partialTicks) {
        try {
            ...
            if (callerClass.equalsIgnoreCase("vazkii.patchouli.client.book.page.PageEntity")) {
                ...
                RenderDisplays.modelSaperionBook.renderStaticBook(this.prepareScale(entity, partialTicks));
                if (flag)
                {
