...
public class RenderTorvosaurus extends RenderLiving<EntityPrehistoricFloraTorvosaurus> {
    ...
    @Override
    public void doRender(EntityPrehistoricFloraTorvosaurus entity, double x, double y, double z, float entityYaw, float partialTicks) {
        try {
        ...
        if (clazz.getName().equalsIgnoreCase("vazkii.patchouli.client.book.page.PageEntity")) {
            ...
            RenderDisplays.modelTorvosaurusBook.renderStaticBook(this.prepareScale(entity, partialTicks));
            if (flag)
            {
