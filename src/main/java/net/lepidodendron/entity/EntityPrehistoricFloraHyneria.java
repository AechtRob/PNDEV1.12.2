...
import net.lepidodendron.block.BlockEggsHyneria;
...
public class EntityPrehistoricFloraHyneria extends EntityPrehistoricFloraAgeableFishBase implements IAdvancementGranter, ITrappableWater {
...
//		this.targetTasks.addTask(3, new HuntAI(this, EntityVillager.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
	...
	public void onLivingUpdate() {
		...
		if (this.getAnimation() == ATTACK_ANIMATION && this.getAnimationTick() == 5 && this.getAttackTarget() != null) {
			//System.err.println("launchAttack");
			launchAttack();
		}
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				&& (BlockEggsHyneria.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockEggsHyneria.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -30 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockEggsHyneria.block.getDefaultState();
			if (BlockEggsHyneria.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockEggsHyneria.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockEggsHyneria.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockEggsHyneria.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	public boolean attackEntityAsMob(Entity entity) {
		//System.err.println("attack test");
	...
	protected ResourceLocation getLootTable() {
		 		if (!this.isPFAdult()) {
			return LepidodendronMod.HYNERIA_LOOT_YOUNG;
		}
		return LepidodendronMod.HYNERIA_LOOT;
	}
	...
	public static ModelBase modelDisplay(@Nullable String variant) {
		return RenderDisplays.modelHyneria;
	}
	public static float getScaler(@Nullable String variant) {
		return RenderHyneria.getScaler();
	}

	@Nullable
	@Override
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_HYNERIA;
	}
