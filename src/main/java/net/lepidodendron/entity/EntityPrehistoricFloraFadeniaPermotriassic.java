...
import net.lepidodendron.block.BlockEggsFadeniaPermotriassic;
...
public class EntityPrehistoricFloraFadeniaPermotriassic extends EntityPrehistoricFloraAgeableFishBase implements IAdvancementGranter, ITrappableWater {
	...
	protected float getAISpeedFish() {
		float AIspeed = 0.189f;
		if (this.getIsFast()) {
			AIspeed = AIspeed * 2.6F;
		}
		if (this.getTicks() < 0) {
...
//	}
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				&& (BlockEggsFadeniaPermotriassic.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockEggsFadeniaPermotriassic.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockEggsFadeniaPermotriassic.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockEggsFadeniaPermotriassic.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -30 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockEggsFadeniaPermotriassic.block.getDefaultState();
			if (BlockEggsFadeniaPermotriassic.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockEggsFadeniaPermotriassic.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockEggsFadeniaPermotriassic.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockEggsFadeniaPermotriassic.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	protected ResourceLocation getLootTable() {
		 		if (!this.isPFAdult()) {
			return LepidodendronMod.FADENIA_PERMOTRIASSIC_LOOT_YOUNG;
		}
		return LepidodendronMod.FADENIA_PERMOTRIASSIC_LOOT;
	}
	...
	public static ModelBase modelDisplay(@Nullable String variant) {
		return RenderDisplays.modelFadenia;
	}
	public static float getScaler(@Nullable String variant) {
		return RenderFadeniaPermotriassic.getScaler();
	}

	@Nullable
	@Override
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_FADENIA_PERMOTRIASSIC;
	}
