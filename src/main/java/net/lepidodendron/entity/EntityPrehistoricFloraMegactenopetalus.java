...
import net.lepidodendron.block.BlockEggsMegactenopetalus;
...
public class EntityPrehistoricFloraMegactenopetalus extends EntityPrehistoricFloraAgeableFishBase implements IAdvancementGranter {
...
//		this.targetTasks.addTask(3, new HuntAI(this, EntitySquid. class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				&& (BlockEggsMegactenopetalus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockEggsMegactenopetalus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockEggsMegactenopetalus.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockEggsMegactenopetalus.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -30 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockEggsMegactenopetalus.block.getDefaultState();
			if (BlockEggsMegactenopetalus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockEggsMegactenopetalus.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockEggsMegactenopetalus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockEggsMegactenopetalus.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	public static ModelBase modelDisplay(@Nullable String variant) {
		return RenderDisplays.modelMegactenopetalus;
	}
	public static float getScaler(@Nullable String variant) {
		return RenderMegactenopetalus.getScaler();
	}
