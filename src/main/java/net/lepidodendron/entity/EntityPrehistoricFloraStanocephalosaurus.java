...
import net.lepidodendron.block.BlockAmphibianSpawnStanocephalosaurus;
...
public class EntityPrehistoricFloraStanocephalosaurus extends EntityPrehistoricFloraSwimmingAmphibianBase implements ITrappableWater, ITrappableLand {
	...
	protected void initEntityAI() {
		...
		tasks.addTask(3, new AmphibianWander(this, NO_ANIMATION, 0.025, 20, 3));
...
//		this.targetTasks.addTask(1, new HuntAI(this, EntityPrehistoricFloraSwimmingAmphibianBase.class, true, (Predicate<Entity>) entity -> entity instanceof EntityLivingBase));
	...
	public void onEntityUpdate() {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getCanBreed() && (LepidodendronConfig.doMultiplyMobs || this.getLaying()) && this.getTicks() > 0
				&& (BlockAmphibianSpawnStanocephalosaurus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP)
				|| BlockAmphibianSpawnStanocephalosaurus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP))
				&& (BlockAmphibianSpawnStanocephalosaurus.block.canPlaceBlockAt(world, this.getPosition())
				|| BlockAmphibianSpawnStanocephalosaurus.block.canPlaceBlockAt(world, this.getPosition().down()))
		){
			//if (Math.random() > 0.5) {
		...
		if (!world.isRemote && spaceCheckEggs() && this.isInWater() && this.isPFAdult() && this.getTicks() > -47 && this.getTicks() < 0) {
			...
			IBlockState eggs = BlockAmphibianSpawnStanocephalosaurus.block.getDefaultState();
			if (BlockAmphibianSpawnStanocephalosaurus.block.canPlaceBlockOnSide(world, this.getPosition(), EnumFacing.UP) && BlockAmphibianSpawnStanocephalosaurus.block.canPlaceBlockAt(world, this.getPosition())) {
				world.setBlockState(this.getPosition(), eggs);
				this.setLaying(false);
				this.playSound(SoundEvents.ENTITY_CHICKEN_EGG, 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
			}
			if (BlockAmphibianSpawnStanocephalosaurus.block.canPlaceBlockOnSide(world, this.getPosition().down(), EnumFacing.UP) && BlockAmphibianSpawnStanocephalosaurus.block.canPlaceBlockAt(world, this.getPosition().down())) {
	...
	protected ResourceLocation getLootTable() {
		return LepidodendronMod.STANOCEPHALOSAURUS_LOOT;
	}
