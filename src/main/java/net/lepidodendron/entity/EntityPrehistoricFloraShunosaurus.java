...
public class EntityPrehistoricFloraShunosaurus extends EntityPrehistoricFloraLandCarnivoreBase implements IAdvancementGranter, ITrappableLand {
	...
	public Animation[] getAnimations() {
		return new Animation[]{DRINK_ANIMATION, HURT_ANIMATION, GRAZE_ANIMATION, ATTACK_ANIMATION, ROAR_ANIMATION, NOISE_ANIMATION, LAY_ANIMATION, EAT_ANIMATION, MAKE_NEST_ANIMATION, IDLE_1, IDLE_2};
	}
	...
	public boolean isGrazing()
	{
		...
		boolean test = (this.getPFGrazing() <= 0
				...
				&&
				(
						(isBlockGrazable(this.world.getBlockState(entityPos.north(3).up(5)))
								&& isGrazable(this.world, entityPos, EnumFacing.NORTH))

								|| (isBlockGrazable(this.world.getBlockState(entityPos.south(3).up(5)))
								&& isGrazable(this.world, entityPos, EnumFacing.SOUTH))

								|| (isBlockGrazable(this.world.getBlockState(entityPos.east(3).up(5)))
								&& isGrazable(this.world, entityPos, EnumFacing.EAST))

								|| (isBlockGrazable(this.world.getBlockState(entityPos.west(3).up(5)))
		...
		if (test) {
			...
			EnumFacing facing = null;
			if (!test2 && isBlockGrazable(this.world.getBlockState(entityPos.north(3).up(5)))) {
				facing = EnumFacing.NORTH;
			...
			else if (!test2 && isBlockGrazable(this.world.getBlockState(entityPos.south(3).up(5)))) {
				facing = EnumFacing.SOUTH;
				if (Functions.getEntityCentre(this).z - Functions.getEntityBlockPos(this).getZ() >= 0.2D
					&& Functions.getEntityCentre(this).z - Functions.getEntityBlockPos(this).getZ() <= 0.4D) {
					test2 = true;
			...
			else if (!test2 && isBlockGrazable(this.world.getBlockState(entityPos.east(3).up(5)))) {
				facing = EnumFacing.EAST;
			...
			else if (!test2 && isBlockGrazable(this.world.getBlockState(entityPos.west(3).up(5)))) {
				facing = EnumFacing.WEST;
				if (Functions.getEntityCentre(this).x - Functions.getEntityBlockPos(this).getX() >= 0.2D
					&& Functions.getEntityCentre(this).x - Functions.getEntityBlockPos(this).getX() <= 0.4D) {
					test2 = true;
			...
			if (facing != null && test && test2) {
				this.setGrazingFrom(entityPos.up(5).offset(facing, 3));
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.SHUNOSAURUS_LOOT_YOUNG;
		}
		return LepidodendronMod.SHUNOSAURUS_LOOT;
	}

	@Nullable
	@Override
	public CustomTrigger getModTrigger() {
		 return ModTriggers.CLICK_SHUNOSAURUS;
	}
	...
	public static ModelBase modelDisplay(@Nullable String variant) {
		return RenderDisplays.modelShunosaurus;
	}
	public static float getScaler(@Nullable String variant) {
		return RenderShunosaurus.getScaler();
	}
