...
public class EntityPrehistoricFloraMantellisaurus extends EntityPrehistoricFloraBiQuadLandBase implements IScreamer, IAdvancementGranter, IBluffer, ITrappableLand {
	...
	public EntityPrehistoricFloraMantellisaurus(World world) {
		super(world);
		setSize(1F, 1.75F);
		minWidth = 0.20F;
		maxWidth = 1F;
	...
	public boolean attackEntityFrom(DamageSource ds, float i) {
		...
		if (e instanceof EntityLivingBase) {
			...
			this.setBipedal();
			List<EntityPrehistoricFloraMantellisaurus> Mantellisaurus = this.world.getEntitiesWithinAABB(EntityPrehistoricFloraMantellisaurus.class, new AxisAlignedBB(this.getPosition().add(-8, -4, -8), this.getPosition().add(8, 4, 8)));
			for (EntityPrehistoricFloraMantellisaurus currentMantellisaurus : Mantellisaurus) {
				currentMantellisaurus.setAnimation(NO_ANIMATION);
				currentMantellisaurus.setRevengeTarget(ee);
				currentMantellisaurus.setAlarmTarget(ee);
				currentMantellisaurus.setBipedal();
				currentMantellisaurus.alarmCooldown = rand.nextInt(20);
			}
		}
...
//	}
	...
	public void onLivingUpdate() {
		...
		if ((!this.world.isRemote) && this.getAnimation() == ROAR_ANIMATION && this.getWarnTarget() != null
			...
			List<EntityPrehistoricFloraMantellisaurus> Camptosaurus = this.world.getEntitiesWithinAABB(EntityPrehistoricFloraMantellisaurus.class, new AxisAlignedBB(this.getPosition().add(-12, -6, -12), this.getPosition().add(12, 6, 12)));
			for (EntityPrehistoricFloraMantellisaurus currentCamptosaurus : Camptosaurus) {
	...
	protected ResourceLocation getLootTable() {
		if (!this.isPFAdult()) {
			return LepidodendronMod.MANTELLISAURUS_LOOT_YOUNG;
		}
		return LepidodendronMod.MANTELLISAURUS_LOOT;
	}

	@Nullable
	@Override
	public CustomTrigger getModTrigger() {
		return ModTriggers.CLICK_MANTELLISAURUS;
	}
