...
public class ItemBottleOfResin extends ElementsLepidodendronMod.ModElement {
	...
	public static class ItemCustom extends Item {
		...
		public static boolean applyResin(ItemStack stack, World worldIn, BlockPos target, EntityPlayer player, @javax.annotation.Nullable EnumHand hand)
		{
			...
			else if (blockTarget == BlockSandGrey.block) {
				if (!worldIn.isRemote) {worldIn.setBlockState(target, BlockSandGreySticky.block.getDefaultState(), 2);}
				resined = true;
			}
			...
			else if (blockTarget == BlockSandGreyWavy.block) {
				if (!worldIn.isRemote) {worldIn.setBlockState(target, BlockSandGreyWavySticky.block.getDefaultState(), 2);}
				resined = true;
			}
			...
			else if (blockTarget == BlockAlgalReef.block) {
				if (!worldIn.isRemote) {worldIn.setBlockState(target, BlockAlgalReefSticky.block.getDefaultState().withProperty(BlockAlgalReefSticky.FACING, iblockstate.getValue(BlockAlgalReef.FACING)), 2);}
				resined = true;
			}
			...
			else if (blockTarget == BlockRudistReef.block) {
				if (!worldIn.isRemote) {worldIn.setBlockState(target, BlockRudistReefSticky.block.getDefaultState().withProperty(BlockRudistReefSticky.FACING, iblockstate.getValue(BlockRudistReef.FACING)), 2);}
				resined = true;
			}
