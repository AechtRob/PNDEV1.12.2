...
import net.lepidodendron.block.BlockAzolla;
...
public class ItemAzollaItem extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:azolla_item")
	public static final Item block = null;
	public ItemAzollaItem(ElementsLepidodendronMod instance) {
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:azolla_item", "inventory"));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:azolla_item", ItemAzollaItem.block);
		OreDictionary.registerOre("plantPrehistoric", ItemAzollaItem.block);
		OreDictionary.registerOre("plant", ItemAzollaItem.block);
	}
	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_azolla_item");
			setRegistryName("azolla_item");
	            ...
	            if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK)
	            {
	                ...
	                if (iblockstate.getMaterial() == Material.WATER && ((Integer)iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0 && worldIn.isAirBlock(blockpos1)) {
						...
						if (canSurviveAt(worldIn, blockpos1)) {
							worldIn.setBlockState(blockpos1, BlockAzolla.block.getDefaultState());
							if (net.minecraftforge.event.ForgeEventFactory.onPlayerBlockPlace(playerIn, blocksnapshot, net.minecraft.util.EnumFacing.UP, handIn).isCanceled()) {
								blocksnapshot.restore(true, false);
								return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
							}

							worldIn.setBlockState(blockpos1, BlockAzolla.block.getDefaultState(), 11);
