...
import net.lepidodendron.block.BlockWireGridDoor;
...
public class ItemWireGridDoorItem extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:wire_grid_door_item")
	public static final Item block = null;
	public ItemWireGridDoorItem(ElementsLepidodendronMod instance) {
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("doorWood", ItemWireGridDoorItem.block);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:wire_grid_door_item", "inventory"));
	}
	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_wire_grid_door_item");
			setRegistryName("wire_grid_door_item");
		...
		public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	    {
	        ...
	        else
	        {
	            ...
	            if (player.canPlayerEdit(pos, facing, itemstack) && BlockWireGridDoor.block.canPlaceBlockAt(worldIn, pos))
	            {
	                EnumFacing enumfacing = EnumFacing.fromAngle((double)player.rotationYaw);
	                int i = enumfacing.getXOffset();
	                int j = enumfacing.getZOffset();
	                boolean flag = i < 0 && hitZ < 0.5F || i > 0 && hitZ > 0.5F || j < 0 && hitX > 0.5F || j > 0 && hitX < 0.5F;
	                placeDoor(worldIn, pos, enumfacing, BlockWireGridDoor.block, flag);
