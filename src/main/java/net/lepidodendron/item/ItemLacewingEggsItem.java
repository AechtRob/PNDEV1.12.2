...
public class ItemLacewingEggsItem extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:insect_eggs_lacewing_item")
	public static final Item block = null;
	public ItemLacewingEggsItem(ElementsLepidodendronMod instance) {
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:insect_eggs_lacewing_item", "inventory"));
	}
	...
	public static class ItemCustom extends Item {
		...
		public String getItemStackDisplayName(ItemStack stack)
		{
			if (stack.hasTagCompound()) {
				if (stack.getTagCompound().hasKey("PNType")) {
					...
					return I18n.translateToLocal("item.pf_insect_eggs_lacewing_item_full.name").trim()
								+ ": "
								+ I18n.translateToLocal("entity.prehistoric_flora_lacewing_" + variant + ".name").trim();
					}
				else {
		...
		public String getTranslationKey(ItemStack stack)
		{
			if (isBlockFromItemStack(stack)) {
				if (stack.getTagCompound().hasKey("PNType")) {
					return "item.insect_eggs_lacewing_item_" + stack.getTagCompound().getString("PNType");
				}
			}
					...
					if (isBlockFromItemStack(itemstack)) {
						...
						if (raytraceresult == null)
						{
							...
							if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK) {
								...
								if (BlockInsectEggsLacewing.block.canPlaceBlockOnSide(worldIn, blockpos.offset(raytraceresult.sideHit), raytraceresult.sideHit)) {
									worldIn.setBlockState(blockpos.offset(raytraceresult.sideHit), BlockInsectEggsLacewing.block.getDefaultState().withProperty(BlockInsectEggsLacewing.BlockCustom.FACING, raytraceresult.sideHit));
