...
import net.lepidodendron.block.BlockSycamoreDoor;
...
public class ItemSycamoreDoorItem extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:sycamore_door_item")
	public static final Item block = null;
	public ItemSycamoreDoorItem(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.sycamore_door_item);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:sycamore_door_item", "inventory"));
	}
	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_sycamore_door_item");
			setRegistryName("sycamore_door_item");
		...
		public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	    {
	        ...
	        else
	        {
	            ...
	            if (player.canPlayerEdit(pos, facing, itemstack) && BlockSycamoreDoor.block.canPlaceBlockAt(worldIn, pos))
	            {
	                EnumFacing enumfacing = EnumFacing.fromAngle((double)player.rotationYaw);
	                int i = enumfacing.getXOffset();
	                int j = enumfacing.getZOffset();
	                boolean flag = i < 0 && hitZ < 0.5F || i > 0 && hitZ > 0.5F || j < 0 && hitX > 0.5F || j > 0 && hitX < 0.5F;
	                placeDoor(worldIn, pos, enumfacing, BlockSycamoreDoor.block, flag);
