...
public class ItemArchaefructusItem extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:archaefructus_item")
	public static final Item block = null;
	public ItemArchaefructusItem(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.archaefructus_item);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:archaefructus_item", "inventory"));
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("plantdnaPNlepidodendron:archaefructus_item", ItemArchaefructusItem.block);
	        ...
	        else
	        {
	            if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK)
	            {
	                ...
	                if (canSurviveAt(worldIn, blockpos1) && (iblockstate.getMaterial() == Material.WATER && ((Integer)iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0 && worldIn.isAirBlock(blockpos1)))
	                {
	                    ...
	                    worldIn.setBlockState(blockpos1, BlockArchaefructus.block.getDefaultState());
	                    ...
	                    worldIn.setBlockState(blockpos1, BlockArchaefructus.block.getDefaultState(), 11);
