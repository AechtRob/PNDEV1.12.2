...
import net.lepidodendron.block.BlockLiquidambarDoor;
...
public class ItemLiquidambarDoorItem extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:liquidambar_door_item")
	public static final Item block = null;
	public ItemLiquidambarDoorItem(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.liquidambar_door_item);
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("doorWood", ItemLiquidambarDoorItem.block);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:liquidambar_door_item", "inventory"));
	}
	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_liquidambar_door_item");
			setRegistryName("liquidambar_door_item");
		...
		public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	    {
	        ...
	        else
	        {
	            ...
	            if (player.canPlayerEdit(pos, facing, itemstack) && BlockLiquidambarDoor.block.canPlaceBlockAt(worldIn, pos))
	            {
	                EnumFacing enumfacing = EnumFacing.fromAngle((double)player.rotationYaw);
	                int i = enumfacing.getXOffset();
	                int j = enumfacing.getZOffset();
	                boolean flag = i < 0 && hitZ < 0.5F || i > 0 && hitZ > 0.5F || j < 0 && hitX > 0.5F || j > 0 && hitX < 0.5F;
	                placeDoor(worldIn, pos, enumfacing, BlockLiquidambarDoor.block, flag);
