...
public class ItemNautiloidEggsDactylioceras extends ElementsLepidodendronMod.ModElement {
	...
	public ItemNautiloidEggsDactylioceras(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.eggs_dactylioceras);
	}

	@Override
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("pnfurnaceSeafood", ItemNautiloidEggsDactylioceras.block);
	}
	...
	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_eggs_dactylioceras");
	        ...
	        else
	        {
	            if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK)
	            {
					...
					if (iblockstate.getMaterial() == Material.WATER) {
						if (!(worldIn.isRemote)) {
							EntityPrehistoricFloraAmmonite_Dactylioceras.summon(worldIn, EntityList.getKey(EntityPrehistoricFloraAmmonite_Dactylioceras.class).toString(), "{AgeTicks:0}", blockpos.getX() + 0.5, blockpos.getY() + 0.5, blockpos.getZ() + 0.5);
						}
