...
public class ItemNautiloidEggsPachydiscus extends ElementsLepidodendronMod.ModElement {
	...
	public void init(FMLInitializationEvent event) {
		super.init(event);
		OreDictionary.registerOre("pnfurnaceSeafood", ItemNautiloidEggsPachydiscus.block);
	}
	        ...
	        else
	        {
	            if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK)
	            {
					...
					if (iblockstate.getMaterial() == Material.WATER) {
						if (!(worldIn.isRemote)) {
							EntityPrehistoricFloraAmmonite_Pachydiscus.summon(worldIn, EntityList.getKey(EntityPrehistoricFloraAmmonite_Pachydiscus.class).toString(), "{AgeTicks:0}", blockpos.getX() + 0.5, blockpos.getY() + 0.5, blockpos.getZ() + 0.5);
						}
