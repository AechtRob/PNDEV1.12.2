...
public class ItemBucketHaikouichthys extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:haikouichthys_bucket")
	public static final Item block = null;

	public ItemBucketHaikouichthys(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.haikouichthys_bucket);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:entities/haikouichthys_bucket", "inventory"));
	}

	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_haikouichthys_bucket");
			setRegistryName("haikouichthys_bucket");
			maxStackSize = 1;
				...
				else
				{
					...
					else
					{
						...
						if (MobSpawn == null && !(worldIn.isRemote)) {
							EntityPrehistoricFloraAgeableBase.summon(worldIn, EntityList.getKey(EntityPrehistoricFloraHaikouichthys.class).toString(), "", (double) posIn.getX() + 0.5D, (double) posIn.getY(), (double) posIn.getZ() + 0.5D);
						}
					}
		...
		public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
			if (LepidodendronConfig.showTooltips) {
				tooltip.add("Type: Uncertain swimming animal");
