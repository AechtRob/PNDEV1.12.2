...
public class ItemBucketOdontogriphus extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:odontogriphus_bucket")
	public static final Item block = null;

	public ItemBucketOdontogriphus(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.odontogriphus_bucket);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:entities/odontogriphus_bucket", "inventory"));
	}

	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_odontogriphus_bucket");
			setRegistryName("odontogriphus_bucket");
			maxStackSize = 1;
				...
				else
				{
					...
					else
					{
						...
						if (MobSpawn == null && !(worldIn.isRemote)) {
							EntityPrehistoricFloraAgeableBase.summon(worldIn, EntityList.getKey(EntityPrehistoricFloraOdontogriphus.class).toString(), "", (double) posIn.getX() + 0.5D, (double) posIn.getY(), (double) posIn.getZ() + 0.5D);
						}
					}
		...
		public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
			if (LepidodendronConfig.showTooltips) {
				tooltip.add("Type: Sea-slug-like animal");
