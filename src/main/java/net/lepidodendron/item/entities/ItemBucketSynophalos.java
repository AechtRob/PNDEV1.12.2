...
public class ItemBucketSynophalos extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:synophalos_bucket")
	public static final Item block = null;

	public ItemBucketSynophalos(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.synophalos_bucket);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:entities/synophalos_bucket", "inventory"));
	}

	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_synophalos_bucket");
			setRegistryName("synophalos_bucket");
			maxStackSize = 1;
		...
		public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn)
		{
			...
			//Get nbt from the bucket:
			int chain = ((itemstack).hasTagCompound() ? (itemstack).getTagCompound().getInteger("chain") : 1);
			...
			else
			{
...

				{
					...
					else if (tryPlaceContainedLiquid(playerIn, worldIn, blockpos1, chain))
					{
		...
		public boolean tryPlaceContainedLiquid(@Nullable EntityPlayer player, World worldIn, BlockPos posIn, int chain)
		{
				...
				else
				{
					...
					else
					{
						...
						if (MobSpawn == null && !(worldIn.isRemote)) {
							EntityPrehistoricFloraAgeableBase.summon(worldIn, EntityList.getKey(EntityPrehistoricFloraSynophalos.class).toString(), "{chain:" + chain + "}", (double) posIn.getX() + 0.5D, (double) posIn.getY(), (double) posIn.getZ() + 0.5D);
						}
					}
