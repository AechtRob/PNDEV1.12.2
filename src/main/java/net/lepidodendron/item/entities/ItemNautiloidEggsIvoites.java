...
public class ItemNautiloidEggsIvoites extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:eggs_ivoites")
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:entities/eggs_ivoites", "inventory"));
	}
	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_eggs_ivoites");
			setRegistryName("eggs_ivoites");
	        ...
	        else
	        {
	            if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK)
	            {
					...
					if (iblockstate.getMaterial() == Material.WATER) {
						if (!(worldIn.isRemote)) {
							EntityPrehistoricFloraIvoites.summon(worldIn, EntityList.getKey(EntityPrehistoricFloraIvoites.class).toString(), "{AgeTicks:0}", blockpos.getX() + 0.5, blockpos.getY() + 0.5, blockpos.getZ() + 0.5);
						}
