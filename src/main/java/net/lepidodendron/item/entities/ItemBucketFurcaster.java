...
public class ItemBucketFurcaster extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:furcaster_bucket")
	public static final Item block = null;

	public ItemBucketFurcaster(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.furcaster_bucket);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:entities/furcaster_bucket", "inventory"));
	}

	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_furcaster_bucket");
			setRegistryName("furcaster_bucket");
			maxStackSize = 1;
				...
				else
				{
					...
					else
					{
						...
						if (MobSpawn == null && !(worldIn.isRemote)) {
							EntityPrehistoricFloraAgeableBase.summon(worldIn, EntityList.getKey(EntityPrehistoricFloraFurcaster.class).toString(), "", (double) posIn.getX() + 0.5D, (double) posIn.getY(), (double) posIn.getZ() + 0.5D);
						}
					}
