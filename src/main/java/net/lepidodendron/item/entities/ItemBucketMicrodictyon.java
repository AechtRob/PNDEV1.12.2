...
public class ItemBucketMicrodictyon extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:microdictyon_bucket")
	public static final Item block = null;

	public ItemBucketMicrodictyon(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.microdictyon_bucket);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:entities/microdictyon_bucket", "inventory"));
	}

	public static class ItemCustom extends Item {
		public ItemCustom() {
			setTranslationKey("pf_microdictyon_bucket");
			setRegistryName("microdictyon_bucket");
			maxStackSize = 1;
				...
				else
				{
					...
					else
					{
						...
						if (MobSpawn == null && !(worldIn.isRemote)) {
							EntityPrehistoricFloraAgeableBase.summon(worldIn, EntityList.getKey(EntityPrehistoricFloraMicrodictyon.class).toString(), "", (double) posIn.getX() + 0.5D, (double) posIn.getY(), (double) posIn.getZ() + 0.5D);
						}
					}
