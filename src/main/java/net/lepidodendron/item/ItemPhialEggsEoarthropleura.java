...
public class ItemPhialEggsEoarthropleura extends ElementsLepidodendronMod.ModElement {
	...
	public ItemPhialEggsEoarthropleura(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.phial_eggs_eoarthropleura);
	}
		...
		public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			{
				...
				if (!player.canPlayerEdit(pos.offset(facing), facing, itemstack)) {
					...
					if (BlockInsectEggsEoarthropleura.block.canPlaceBlockOnSide(worldIn, pos.offset(facing), facing)) {
						worldIn.setBlockState(pos.offset(facing), BlockInsectEggsEoarthropleura.block.getStateForPlacement(worldIn, pos.offset(facing), facing, hitX, hitY, hitZ, 0, (EntityLivingBase) player));
