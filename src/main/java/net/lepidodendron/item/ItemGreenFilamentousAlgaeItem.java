...
public class ItemGreenFilamentousAlgaeItem extends ElementsLepidodendronMod.ModElement {
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:green_filamentous_algae_item", "inventory"));
	}
	public static class ItemCustom extends Item {
	            ...
	            if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK)
	            {
	                ...
	                if (iblockstate.getMaterial() == Material.WATER && ((Integer)iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0 && worldIn.isAirBlock(blockpos1)) {
						...
						if (canSurviveAt(worldIn, blockpos1)) {
							worldIn.setBlockState(blockpos1, BlockGreenFilamentousAlgae.block.getDefaultState());
							if (net.minecraftforge.event.ForgeEventFactory.onPlayerBlockPlace(playerIn, blocksnapshot, net.minecraft.util.EnumFacing.UP, handIn).isCanceled()) {
								blocksnapshot.restore(true, false);
								return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
							}

							worldIn.setBlockState(blockpos1, BlockGreenFilamentousAlgae.block.getDefaultState(), 11);
	    ...
	    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
	        if (LepidodendronConfig.showTooltips) {
				...
				tooltip.add("Periods: Ordovician - Silurian - Devonian - Carboniferous - Permian - Triassic - Jurassic - Cretaceous - Palaeogene - Neogene - Pleistocene - present");
