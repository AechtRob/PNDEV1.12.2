...
public class ItemIsoetesItem extends ElementsLepidodendronMod.ModElement {
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:isoetes_item", "inventory"));
	}
	...
	public static class ItemCustom extends Item {
	        ...
	        else
	        {
	            if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK)
	            {
	                ...
	                if (canSurviveAt(worldIn, blockpos1) && (iblockstate.getMaterial() == Material.WATER && ((Integer)iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0 && worldIn.isAirBlock(blockpos1)))
	                {
	                    ...
	                    worldIn.setBlockState(blockpos1, BlockIsoetes.block.getDefaultState());
	                    ...
	                    worldIn.setBlockState(blockpos1, BlockIsoetes.block.getDefaultState(), 11);
	    ...
	    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
	        if (LepidodendronConfig.showTooltips) {
				...
				tooltip.add("Periods: Permian - Triassic - Jurassic - Cretaceous - Palaeogene - Neogene - Pleistocene [- present]");
