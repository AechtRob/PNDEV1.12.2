...
public class ItemPhialEggsLimnoscelis extends ElementsLepidodendronMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:limnoscelis_spawn_bucket")
	public static final Item block = null;
	public ItemPhialEggsLimnoscelis(ElementsLepidodendronMod instance) {
		super(instance, LepidodendronSorter.phial_eggs_limnoscelis);
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("lepidodendron:phial_eggs_limnoscelis", "inventory"));
	}
	public static class ItemCustom extends Item {
		public ItemCustom() {
			setMaxDamage(0);
			maxStackSize = 8;
			setTranslationKey("pf_phial_eggs_limnoscelis");
		...
		public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
			{
				...
				if (!player.canPlayerEdit(pos.offset(facing), facing, itemstack)) {
					...
					if (BlockAmphibianSpawnLimnoscelisPlaceable.block.canPlaceBlockOnSide(worldIn, pos.offset(facing), facing)) {
						worldIn.setBlockState(pos.offset(facing), BlockAmphibianSpawnLimnoscelisPlaceable.block.getStateForPlacement(worldIn, pos.offset(facing), facing, hitX, hitY, hitZ, 0, (EntityLivingBase) player));
